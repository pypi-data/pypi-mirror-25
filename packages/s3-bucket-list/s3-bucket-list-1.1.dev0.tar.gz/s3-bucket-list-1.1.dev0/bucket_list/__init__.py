        ...
        try:
            for metric in client.list_metrics(metric_name='BucketSizeBytes'):
                ...
                try:
                    ...
                    status.update_status(bucket + " => " + str(size))
                    yield details
                except Exception, e:
                    print(TBVaccine().format_exc())
                    yield {
...
@click.option('--prefix')
@click.option('--filter', help="Only print buckets matching filter")
@click.option('--format', default="pretty")
def main(lifecycle, policies, prefix, filter, format):
    columns = [
    ...
    for bucket in query_buckets(lifecycle, policies, prefix, table):
        if filter is not None and _eval(filter, bucket) is False:
            continue
        table.append(bucket)
    table.end()
...
def _eval(filter, bucket):
    try:
        result = eval(filter, bucket)
        return result
    except Exception, e:
        return False
