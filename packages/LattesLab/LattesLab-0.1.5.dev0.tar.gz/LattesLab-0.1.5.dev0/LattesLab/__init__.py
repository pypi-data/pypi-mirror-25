...
def word_list_to_cloud(folder, topwords):
...
def get_node_connections(edgelist, edgenetwork, depth=1):
...
#list all the edges connecting to node intlist[i]
            dummie2 = edgenetwork.edges(edgelist[i])
            for y in dummie2:
...
#o primeiro ano de publicacao e the current year minus nworks
...
#        FUNCTION OPTIMIZATION BETWEEN 241 AND 246.
# IF IT DOESNT WORK RETURN TO COMMENTED TEXT ABOVE       
        if i==nworks-1:
            pubdata = pubyeardata.copy()
            strindex = ['year']*pubdata.shape[1]
            for i in range(1,pubdata.shape[1]):
...
def first_nonzero_rev(frame, frameindex, option='year'):
...
#option=1: first production value is first non-zero value of the production
#   vector. the last vector indexes are substituted per zeros
#option=2: first production value is first year of pibic scholarship. Doesn't
#   make sense to use it for non pibic based dataframes
...
def first_nonzero2(frame, frameindex, nrow):
    ...
    count = 0
#    nshift = frameindex[nrow] - 2004
    nshift = frameindex.iloc[nrow] - 2004
        ...
        if nquant==1:
            if x.attrib["ANO-DE-CONCLUSAO"]=="":
                x.attrib["ANO-DE-CONCLUSAO"] = \
                    str(int(x.attrib["ANO-DE-INICIO"]) + xpectd_years(gradkind))
...
def get_lattes_desc_folder(folder):
    import zipfile
...
def get_dataframe_from_folder(folder):
    ...
    #ESSA PARTE DO CODIGO MERECE UMA FUNCAO, TA BAGUNCADO	
        if x != []:
            for i in range(0, len(x[0].getchildren())):
                ...
                if x[0][i].tag == "MESTRADO":
                    [ano1master, nmaster] = \
                        get_grad_years(x[0][i],x[0][i].tag)
                if x[0][i].tag == "DOUTORADO":
                    [ano1phd, nphd] = \
                        get_grad_years(x[0][i],x[0][i].tag)
                if x[0][i].tag == "POS-DOUTORADO":
                    [ano1postdoc, nposdoc] = \
                        get_grad_years(x[0][i],x[0][i].tag)
...
#drop the old index
    ...
    return lattesframeame.to_csv(folder + 'dataframe.csv', index=False)
