...
def first_nonzero(frame, frameindex, option=0):
    ...
    Args:
        ...
        option=2: first production value is first year of pibic scholarship.
           Doesn't make sense to use it for non PIBIC based dataframes.
    """
    import pandas as pd
    from datetime import datetime
...
def get_graph(filename, opt='all'):
...
def lattes_classes_from_folder(folder, imin=2, imax=10):
    """
...
def filter_by_lattes_age(lattesframe, agedays):
    ...
    mask = [(datetime.today() - datetime.strptime(i, '%d%m%Y')).days < 365
