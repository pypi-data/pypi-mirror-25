...
def get_graph_from_folder(folder):
    """
    ...
    for file in filelist:
        dummiegraph = ll.get_graph_from_file(folder + file)
        vecgraph.append(dummiegraph)
        namelist.append(ll.lattes_owner(folder + file))
    ...
    plt.savefig(folder + 'graph'+ datetime.now().strftime('%Y%m%d%H%M%S') + \
                '.png')

    textfile = open(folder + "Labels" + datetime.now().strftime('%Y%m%d%H%M%S') + \
                    ".txt", "w")

    for x in network.nodes(data = True):
#        textfile.write(str(x[0])+ '\t'+ x[1]['name'] +'\n')
#        x[0].decode('iso-8859-1').encode('utf8')
        textfile.write(str(x[0])+ '\t'+ x[1]['name'])
    ...
    connections.to_csv(folder + "DataFrame" + \
...
def lattes_classes_from_folder(folder, imin=2, imax=10, option=0):
    """
