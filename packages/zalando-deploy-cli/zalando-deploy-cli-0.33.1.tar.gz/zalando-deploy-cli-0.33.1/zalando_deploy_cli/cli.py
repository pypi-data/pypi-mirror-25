...
def kms_encrypt(config, kms_keyid, region, plain_text_blob):
    if not kms_keyid:
        try:
...
@click.pass_obj
def encrypt(config, target, kube, strip, use_kms, kms_keyid, region):
    ...
    if legacy_mode:
        if use_kms:
            kms_encrypt(config, kms_keyid, region, plain_text.encode())
        else:
        ...
        if target == "cdp":
            kms_encrypt(config, kms_keyid, region, data)
        elif target == "jenkins":
