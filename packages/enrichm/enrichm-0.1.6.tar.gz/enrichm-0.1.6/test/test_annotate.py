...
class Tests(unittest.TestCase):
    ...
    def test_hello_world_nucleic(self):
        with tempdir.TempDir() as tmp:
            ...
            cmd = '%s annotate --threads 10 --ko --pfam --tigrfam --genome_directory %s --output %s --force' % (path_to_script, bin, tmp)
            subprocess.call(cmd, shell=True)

    def test_hello_world_protein(self):
        with tempdir.TempDir() as tmp:
            bin = os.path.join(path_to_data, 'test_protein_bin')
            cmd = '%s annotate --threads 10 --ko --pfam --tigrfam --protein_directory %s --output %s --force' % (path_to_script, bin, tmp)
            subprocess.call(cmd, shell=True)
