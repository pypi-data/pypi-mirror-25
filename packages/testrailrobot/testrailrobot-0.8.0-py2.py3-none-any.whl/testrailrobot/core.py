...
def update_progress(message):
    ...
    def print_progress_decorator(func):
        ...
        def function_wrapper(*args):
            TestRailRobot.print_progress(0, message)
            func(*args)
            TestRailRobot.print_progress(100, message)
...
class TestRailRobot:
    ...
    def __init__(self, server, user, password):
        ...
        # results that will be uploaded
        self.results_to_upload = []
        ...
        self.options = {
            ...
            # test results
            "update_existing_cases": False,

            # set this to False if you do not want to add new tests to
            # a existing TestRail run (when using use_run() method) and only
            # want to upload results for tests already in the run
        ...
        self._TestRailRobot__project = None
        self._TestRailRobot__suite = None
        self._TestRailRobot__run = None
        self._TestRailRobot__milestone = None
    ...
    def run(self, value):
        ...
        All test results will be uploaded to the specified run.
    ...
    def parse_robot_results(self, path):
        ...
        try:
            ...
            self.results = []
            robot_results = RobotResults(path).results

            for i, r in enumerate(robot_results):
                result = self._parse_result(r)
                self.results.append(result)

                TestRailRobot.print_progress((i + 1) / len(robot_results) * 100, "parsing robot results...")
            else:
    ...
    def upload_results(self):
        """ Uploads results from Robot Framework to TestRailRobot.
    ...
    @update_progress('connecting to TestRailRobot...')
    def _connect(self):
    ...
    def _get_testrail_project(self, value):
        """ Retrieves a project from TestRailRobot.
    ...
    def _get_testrail_suite(self, value, project_id):
        """ Retrieves a test suite from TestRailRobot.
    ...
    def _get_testrail_run(self, value, project_id):
        """ Retrieves a test run from TestRailRobot.
    ...
    def _get_testrail_milestone(self, value, project_id):
        """ Retrieves a milestone from TestRailRobot.
    ...
    def _get_testrail_tests(self):
        """ Retrieves automated tests from TestRailRobot.
    ...
    def _get_testrail_test_by_name(self, name, testrail_tests = None):
        ...
        to retrieve the tests in TestRailRobot.
    ...
    def _parse_result(self, robot_result):
        ...
        result['title'] = robot_result['title']
        result['status'] = robot_result['status']
        result['comment'] = robot_result['message']
        result[self.options['custom_is_autotest_field_name']] = True
        result['status_id'] = self._get_testrail_status_id(robot_result['status'])

        result['custom_steps'] = self._parse_steps_as_text(robot_result['steps'])
        result['custom_steps_separated'] = self._parse_steps_as_separated(robot_result['steps'])
    ...
    def _get_section(self):
        """ Retrieves the test case section for automated tests in TestRailRobot.
        ...
        Return:
            ...
            tests.
        """
        self.logger.debug('Trying to retrieve test section for autometed tests from TestRailRobot...')
    ...
    def _create_section(self, name):
        """ Creates a new section for automated tests in TestRailRobot.
    ...
    def _insert_cases(self):
        ...
        if cases_to_insert:
            for i, c in enumerate(cases_to_insert):
                ...
                TestRailRobot.print_progress((i + 1) / len(cases_to_insert) * 100, "inserting new cases...")
            else:
                self.logger.info("Inserted {} new test case(s).".format(i + 1))
        else:
            self.logger.info('No new cases to insert.')

    @update_progress('update existing cases...')
    def _update_cases(self):
        """ Updates test step information of test cases in TestRailRobot. """

        self.logger.info('Trying to update existing cases in TestRailRobot...')
        ...
        if cases_to_update:
            for i, c in enumerate(cases_to_update):
                ...
                TestRailRobot.print_progress((i + 1) / len(cases_to_update) * 100, "updating existing cases...")
            else:
                self.logger.info("Updated {} existing case(s) in TestRailRobot.".format(i + 1))
        else:
            self.logger.info('No cases to update.')

    @run_must_be_set
    @update_progress('upload results...')
    def _post_results_to_testrail(self):
        """ Posts results to TestRailRobot. """

        self.logger.info('Trying to post results to TestRailRobot...')
    ...
    def _print_statistics(self):
        """ Prints statistics of the results uploaded to terminal. """
        ...
        print('='*70)
        print("Log file:", os.path.abspath("TestRailRobot.log"))

    def _print_progress(self, progress, message):
        """ Prints the script's progress to terminal.
...
class RobotResults:
    ...
    def __init__(self, path):
        self.results = self._get_results_from_xml(path)
    ...
    def _get_results_from_xml(self, path):
        ...
        # find all tags named 'test'
        xml_results = ET.parse(path).findall('.//test')

        # for each 'test' tag parse and store test data
        for xmlt in xml_results:
            ...
            # for each 'kw' tag parse and store step data
