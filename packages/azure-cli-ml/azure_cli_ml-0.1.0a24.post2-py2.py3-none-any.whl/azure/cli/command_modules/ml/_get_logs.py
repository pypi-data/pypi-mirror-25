...
def get_logs_from_docker(container, requestId=None, verb=False):
    ...
    if requestId is not None and not isinstance(requestId, str):
        raise CLIError('requestId needs to be of type string')
    try:
        _print_util("Trying to get logs from container", verb)
        log_string = container.logs().decode('utf-8')
        _print_util("Processing Logs", verb)
        _process_logs(log_string, requestId)
    except Exception as ex:
        if verb:
            print(ex)
        raise CLIError('Failed to get logs')
...
def get_logs_from_kubernetes(service_id, kubeconfig, context, requestId=None, log_offset=5000, kubectl_location='kubectl', verb=False):
    ...
    '''
    try:
        _print_util("loading kubeconfig file", verb)
        config.load_kube_config(kubeconfig)
    except Exception as ex:
        if verb:
            print(ex)
        raise CLIError("Failed loading kubeconfig file")
    _print_util("Getting Replica sets from default namespace", verb)
    api_instance = client.ExtensionsV1beta1Api()
    replicas = api_instance.list_namespaced_replica_set("default")
    result_replica = None
    hash_id = _get_hash_service_id(service_id)
    _print_util("Got hash {0}".format(hash_id), verb)
    ...
    if result_replica is None:
        _print_util("No replicasets found that matches the service id", verb)
        raise CLIError("Service not found")
    _print_util("Found a replicaset matching the service id", verb)
    pod_hash = result_replica.metadata.labels.get('pod-template-hash', None)
    if pod_hash is None:
        _print_util("No pod_hash found that matches the service id", verb)
        raise CLIError("Service not found")
    _print_util("Using pod hash {0}".format(pod_hash), verb)
    pods = _get_pod_from_hash(pod_hash)
    if pods is None:
        _print_util("No pods found that matches the service id", verb)
        raise CLIError("Service not found")
    result_log = ''
    _print_util("Getting logs from  pods", verb)
    for pod in pods:
    ...
    try:
        _print_util("Processing logs from pods", verb)
        _process_logs(result_log, requestId)
    except Exception as ex:
        _print_util(traceback.format_exc(), verb)
        raise CLIError("Failed to parse logs")
