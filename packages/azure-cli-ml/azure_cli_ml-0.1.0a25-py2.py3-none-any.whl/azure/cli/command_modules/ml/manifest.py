...
def _manifest_create(manifest_name, driver_file, manifest_description, schema_file, dependencies, runtime, requirements,
    ...
    if os.sep in driver_file:
        raise CLIError('Unable to use a driver file not in current directory. Please navigate to the location of the driver file and try again.')
    ...
    if not runtime:
        raise CLIError('Missing runtime. Possible runtimes are: {}'.format('|'.join(SUPPORTED_RUNTIMES.keys())))
    elif runtime in SUPPORTED_RUNTIMES.keys():
        json_payload['targetRuntime']['runtimeType'] = SUPPORTED_RUNTIMES[runtime]
    else:
        raise CLIError('Provided runtime not supported. Possible runtimes are: {}'.format('|'.join(SUPPORTED_RUNTIMES.keys())))
