...
SUPPORTED_RUNTIMES = {'spark-py': 'SparkPython', 'python': 'Python'}
NINJA_RUNTIMES = ['mrs']
CREATE_CMD_SAMPLE = "az ml service create realtime -f <webservice file> -n <service name> [--model-file <model1> [--model-file <model2>] ...] [-p requirements.txt] [-d <dependency> [-d <dependency>] ...] [-s <schema>] [-r {0}] [-l] [-z <replicas>] [--collect-model-data]".format("|".join(SUPPORTED_RUNTIMES.keys()))  # pylint: disable=line-too-long
...
MLC_CLIENT_ENUMS_PATH = '{}.mlc_enums'.format(MLC_MODELS_PATH)

MLC_RESOURCE_ID_FMT = '/subscriptions/{}/resourcegroups/{}/providers/Microsoft.MachineLearningCompute/operationalizationClusters/{}'

TABLE_OUTPUT_TIME_FORMAT = '%Y-%m-%dT%H:%M:%S'
APP_INSIGHTS_URL = 'https://analytics.applicationinsights.io/subscriptions/{}/resourcegroups/{}/components/{}#/discover/home?apptype=Other%20(preview)'
