...
def _realtime_service_usage(service_id, verb, context):
    if context.in_local_mode():
        scoring_url, swagger_url = local_service_usage(service_id, verb, context)
    else:
        scoring_url, swagger_url = mms_service_usage(service_id, verb, context)
    ...
    print('    az ml service run realtime -i {} -d "{}"'.format(service_id, DEFAULT_INPUT_DATA))
    print()
    print('Sample CURL call:')
    if context.in_local_mode():
        print('    curl -X POST -H "Content-Type:application/json" --data "{}" {}'.format(DEFAULT_INPUT_DATA, scoring_url))
    else:
        print('    curl -X POST -H "Content-Type:application/json" -H "Authorization:Bearer <key>" --data "{}" {}'.format(DEFAULT_INPUT_DATA, scoring_url))
    return SUCCESS_RETURN_CODE


def realtime_service_logs(service_name, service_id, request_id, kube_config, verb, context=cli_context):
    _realtime_service_logs(service_name, service_id, request_id, kube_config, verb, context)


def _realtime_service_logs(service_name, service_id, request_id, kube_config, verb, context):
    if not service_name and not service_id:
        raise CLIError('Error, one of service name or service ID must be provided.')
    if context.in_local_mode():
        return local_service_logs(service_name, request_id, verb)
