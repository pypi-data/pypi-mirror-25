...
def local_service_create(service_name, image_id, app_insights_logging_enabled, model_data_collection_enabled, verb, context):
    ...
    try:
        ...
        raise CLIError('[Local mode] Failed to start container. Please report this to deployml@microsoft.com with your image id: {}'.format(image_location)) #pylint: disable=line-too-long
    ...
    while health_check_iteration < LOCAL_HEALTH_CHECK_POLLING_MAX_TRIES:
        ...
        if result.status_code == 200:
            ...
            raise CLIError('Error, container failed to initialize with : {}'.format(result.status_code))
    if health_check_iteration == LOCAL_HEALTH_CHECK_POLLING_MAX_TRIES:
        raise CLIError('Error, container initialization timed out')
    print('')
    print('[Local mode] Done')
    print('[Local mode] Service ID: {}'.format(service_name))
    print('[Local mode] Usage: az ml service run realtime -i {} -d \"{}\"'.format(service_name, DEFAULT_INPUT_DATA))
...
def local_service_run(service_name, input_data, verb, context):
    ...
    try:
        ...
        if input_data == '':
            ...
            input_data = get_sample_data(swagger_url, headers, verb)
            if not input_data:
                print(
...
def local_service_usage(service_name, verb, context):
    ...
    if len(containers) != 1:
        raise CLIError('[Local mode] Error retrieving container details.\n[Local mode] Label should match exactly '
                       'one container and instead matched {}.'.format(len(containers)))
    container = containers[0]
