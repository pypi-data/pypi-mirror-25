...
def mms_service_create(image_id, service_name, app_insights_logging_enabled, model_data_collection_enabled,
    ...
    print('Usage: az ml service run realtime -i {} -d \"{}\"'.format(service_id, DEFAULT_INPUT_DATA))
...
def mms_service_usage(service_id, verb, context):
    ...
    if resp.status_code == 200:
        ...
        try:
            ...
            swagger_url = '/'.join(scoring_url.split('/')[:-1]) + 'swagger.json'
        except KeyError:
            raise CLIError('Error, unable to retrieve ScoringUri from response payload: {}'.format(service))
    else:
...
def mms_service_logs(service_id, request_id, kube_config, verb, context):
    ...
    default_kube_path = '{0}/.kube/config'.format(os.path.expanduser('~'))
    if kube_config is None:
        if os.path.exists(default_kube_path):
            ...
            raise CLIError(
                'Kubeconfig needs to be provided to get logs in cluster mode. You can the following command to get it:'
