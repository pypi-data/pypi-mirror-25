...
def _image_usage(image_id, verb, context):
    ...
    if resp.status_code == 200:
        ...
        try:
            ...
            raise CLIError('Error, unable to retrieve ImageLocation from response payload: {}'.format(image))
        try:
            compute_id = image['ComputeResourceId'].split('/')
            env_rg = compute_id[-5]
            env_name = compute_id[-1]
        except KeyError:
            raise CLIError('Error, unable to retrieve ComputeResourceId from response payload: {}'.format(image))
        except IndexError:
            raise CLIError('Error, unable to parse ComputeResourceId: {}'.format(compute_id))
        try:
            image_type = image['ImageType']
        except KeyError:
            raise CLIError('Error, unable to retrieve ImageType from response payload: {}'.format(image))
        ...
        raise CLIError('Error occurred while attempting to retrieve image information.\n{}'.format(resp.content))
