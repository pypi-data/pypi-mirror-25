...
def bar_callback(data):
    ...
    sub_bar_data.set_data(bar.data)

    # todo 回测进行任务检测， 标记被触发的任务
    [task.state_analysis(bar['symbol'], bar['eob']) for task in context.inside_bar_tasks]
    # 3 执行被触发的任务
    [context.inside_file_module.on_bar(context, task.get_current_bars(bar['eob'])) for task in context.inside_bar_tasks if task.waiting]
    # 4 重置任务状态
...
def position_callback(data):
    ...
    position = protobuf_to_dict(position)
    symbol = position['symbol']
    account_id = position['account_id']
    accounts = context.accounts
    accounts[account_id].positions[symbol] = position
