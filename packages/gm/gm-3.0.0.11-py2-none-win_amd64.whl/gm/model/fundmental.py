...
from gm.pb.fundmental_pb2 import FundmentalServiceStub, GetFundmentalsReq, \
    ...
    GetContinuousContractsReq, GetFundmentalsRsp
...
class FundmentalApi(object):
    ...
    def _init_addr(self):
        # type: (Text) -> None
        new_addr = py_gmi_get_serv_addr(FUNDMENTAL_ADDR)
        if new_addr != self.addr:
            self.addr = new_addr
            channel = grpc.insecure_channel(self.addr)
            self.stub = FundmentalServiceStub(channel)

    def get_fundmentals(self, table, end_date, symbols=None, fields=None, filter=None, order_by=None, count=1000):
        # type: (Text, Text, Union[TextNone, List[Text]], TextNone, TextNone, TextNone, int) -> List[Dict[Text, Any]]
        """
        查询基本面财务数据
        """
        self._init_addr()
        ...
        req = GetFundmentalsReq(table=table, end_date=end_date, fields=fields, symbols=','.join(symbols), filter=filter,
                                order_by=order_by, count=count)
        resp = self.stub.GetFundmentals(req)
        result = []
        for item in resp.data:  # type: GetFundmentalsRsp.Fundmental
            r = {
                'symbol': item.symbol,
    ...
    def get_instruments(self, symbols=None, exchanges=None, sec_types=None, names=None, skip_suspended=True,
        ...
        resp = self.stub.GetInstruments(req)
        result = []
    ...
    def get_history_instruments(self, symbols, fields=None, start_date=None, end_date=None):
        ...
        resp = self.stub.GetHistoryInstruments(req)
        result = []
        for ins in resp.data:  # type: Instrument
            row = dict()
    ...
    def get_instrumentinfos(self, symbols=None, exchanges=None, sec_types=None, names=None, fields=None):
        ...
        req = GetInstrumentInfosReq(symbols=','.join(symbols), exchanges=','.join(exchanges),
                                    ...
                                    fields=','.join(filter_fields))
        resp = self.stub.GetInstrumentInfos(req)
        result = []
    ...
    def get_history_constituents(self, index, start_date=None, end_date=None):
        ...
        start_date = start_date.replace(day=1)  # start_date 取月初
        end_date = arrow.Arrow.fromdate(end_date).shift(months=1).replace(day=1).shift(days=-1).date()  # end_date取月末

        req = GetConstituentsReq(index=index, start_date=start_date.strftime('%Y-%m-%d'), end_date=end_date.strftime('%Y-%m-%d'))
        resp = self.stub.GetConstituents(req)
    ...
    def get_industry(self, code):
        ...
        resp = self.stub.GetIndustry(req)
    ...
    def get_trading_dates(self, exchange, start_date, end_date):
        ...
        resp = self.stub.GetTradingDates(req)
        if len(resp.dates) == 0:
            return []
        ds = []
    ...
    def get_previous_trading_date(self, exchange, date):
        ...
        resp = self.stub.GetPreviousTradingDate(req)
    ...
    def get_next_trading_date(self, exchange, date):
        ...
        resp = self.stub.GetNextTradingDate(req)
        rdate = resp.date  # type: Timestamp
        if not rdate.ListFields():  # 这个说明查询结果没有
            return None
        return rdate.ToDatetime().strftime('%Y-%m-%d')
    ...
    def get_dividend(self, symbol, start_date, end_date=None):
        ...
        resp = self.stub.GetDividends(req)
        result = []
    ...
    def get_continuous_contracts(self, csymbol, start_date=None, end_date=None):
        ...
        resp = self.stub.GetContinuousContracts(req)
