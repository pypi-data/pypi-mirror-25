...
from gm.constant import CSDK_OPERATE_SUCCESS, DATA_TYPE_TICK, \
    SUB_TAG, SCHEDULE_INFO, MODE_BACKTEST
...
from gm.csdk.c_sdk import py_gmi_current, py_gmi_schedule, \
    ...
    py_gmi_unsubscribe, \
    py_gmi_set_trade_serv_addr, py_gmi_set_message_serv_addr, \
    py_gmi_get_parameters, py_gmi_add_parameters, py_gmi_set_parameters, \
    py_gmi_log, py_gmi_strerror, py_gmi_run, py_gmi_stop, py_gmi_set_timer
...
def subscribe(symbols, frequency='1d', count=1, wait_group=False, wait_group_timeout='5s', unsubscribe_previous=False):
    """
...
def set_trade_serv_addr(addr):
    """
    自定义服务地址
    """
    py_gmi_set_trade_serv_addr(addr)


def set_message_serv_addr(addr):
    """
    自定义消息服务地址
    """
    py_gmi_set_message_serv_addr(addr)


def run(strategy_id='', filename='', mode=0, token='',
        backtest_start_time='',
        backtest_end_time='',
    ...
    for option, value in options:
        ...
        if option == '--backtest_initial_cash':
            backtest_initial_cash = value
            continue

        if option == '--backtest_transaction_ratio':
            backtest_transaction_ratio = value
            continue

        if option == '--backtest_commission_ratio':
            backtest_commission_ratio = value
            continue

        if option == '--backtest_slippage_ratio':
            backtest_slippage_ratio = value
            continue

        if option == '--backtest_adjust':
            backtest_adjust = value
            continue

        if option == '--backtest_check_cache':
            backtest_check_cache = value
            continue
    ...
    for name in enum.__all__:
        if name not in fmodule.__dict__:
            fmodule.__dict__[name] = getattr(enum, name)
