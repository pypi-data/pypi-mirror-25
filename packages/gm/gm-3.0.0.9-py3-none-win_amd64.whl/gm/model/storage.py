...
class Context(object):
    ...
    def __init__(self):
        self.runtime_configs = {}  # runtime configs, dictionary
    ...
    def _get_account_info(self, account_id):
        ...
        positions_infos = {position['symbol']: position for position in positions}
...
class BarTask(object):
    ...
    def state_analysis(self, symbol, eob):
        if symbol not in self.symbols:
            return

        # 执行过的任务不在执行
        if eob in self.performed_eobs:
            return

        # 重设任务时间
        self.time = eob
        # todo (这里需要用上timer, 暂时不处理)
        # 如果超时时间到了， 任务就必须执行， 直接设is_ready = True
        now = context.now
        if (now - self.time).seconds >= self.overtime:
            self.waiting = True
            self.performed_eobs.add(eob)
            return

        # 如果没到， 要判断一下是不是订阅的代码都到了啊。
        for symbol in self.symbols:
            sub_tag = SUB_TAG.format(symbol, self.frequency)
            from gm.model.storage_data import SubBarData
            if not SubBarData().data_is_ready(sub_tag, eob):
                return
    ...
    def get_current_bars(self, eob):
        from gm.model.storage_data import SubBarData
        bars = SubBarData().inside_get_data(self.symbols, self.frequency, eob)
