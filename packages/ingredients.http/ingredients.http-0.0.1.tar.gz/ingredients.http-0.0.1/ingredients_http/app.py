...
class HTTPApplication(object):
    ...
    def __setup_tools(self):
        cherrypy.tools.db_session = cherrypy.Tool('on_start_resource', self.db_session, priority=1)
        cherrypy.tools.model_params = cherrypy.Tool('before_request_body', model_params)
        cherrypy.tools.model_in = cherrypy.Tool('before_request_body', model_in)
