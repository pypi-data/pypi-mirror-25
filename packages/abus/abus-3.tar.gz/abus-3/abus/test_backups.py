...
class BackedUpHomedirTests(AbusTestBase):
   ...
   def test_error_prevents_closing_backup_run(self):
      ...
      hoemdirfile_path= self.homedir+"/unreadable"
      self.create_homedir_file(hoemdirfile_path,23)
      ...
      self.assertEqual(contents, "error 0 {}\n".format(hoemdirfile_path))
   ...
   def test_bad_include_entry_does_not_stop_other_backups(self):
      ...
      homedirfile_path= self.homedir+"/a_file"
      self.create_homedir_file(homedirfile_path,23)
      rc= main.main(["test", "-f", self.configfile, "--backup"])
      self.assertEqual(rc, 1)
