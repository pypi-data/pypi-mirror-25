...
@contextmanager
def connect(database_path, allow_create=False):
   db= Database(database_path, allow_create)
   yield db
   db.close()
...
class Database(object):
   ...
   def _get_connection(self, caller):
      ...
      :param caller:free text for debugging (to show who has the connection)
      :return:
      ...
      with self._get_connection("get_purgeable_backups") as conn:
         cur= conn.execute("""select CHECKSUM.checksum
               ...
               from (select checksum, min(timestamp) as birthtime, max(run_name) as last_alive
                  ...
                  join completed_run on completed_run.run_name > CHECKSUM.last_alive
   ...
   def remember_checksum(self, st_dev, st_ino, timestamp, checksum):
      with self._get_connection("remember_checksum") as conn:
         ...
         logging.debug("update checksum rowcount %d, expected: %d", cur.rowcount, dbg_existed)
         if cur.rowcount==0:
   ...
   def reconstruct_content(self, run_name, checksum_timestamp_path_rows):
      ...
      :type checksum_timestamp_path_rows: iterable of triplets
      :return: number of records changed, added, deleted
      :rtype: tuple
      """
      with self._get_connection("reconstruct_content") as conn:
         conn.execute("""create temp table if not exists reconstruct(
            path text not null primary key,
            timestamp float not null,
            checksum text not null)""")
         conn.execute("delete from reconstruct")
         conn.executemany("insert into reconstruct(checksum, timestamp, path) values(?,?,?)", checksum_timestamp_path_rows)
