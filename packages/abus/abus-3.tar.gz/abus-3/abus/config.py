...
class Configuration(object):
   ...
   def __init__(self, argv):
      ...
      self.is_location_reconstruction= args.reconstruct_location_index or args.reconstruct
      self.is_full_reconstruction= args.reconstruct
      ...
      if (self.list_all or self.cutoff) and not self.is_restore:
         raise ConfigurationError("conflicting command line options (-a/-d without -r)")
      actions= is_list, self.is_backup, self.is_restore, self.is_init, self.is_location_reconstruction
   ...
   def _get_command_line_options(self, argv):
      ...
      parser.add_argument('--init',
         action='store_true',
         help='Creates an empty cache database in the backup directory.')
      parser.add_argument('--reconstruct-location-index',
         action='store_true',
         help="""Reconstructs the location cache, which stores the location of each
            backup file within the backup directory.""")
      parser.add_argument('--reconstruct',
         action='store_true',
         help="""Reconstructs the backup contents cache, which stores the lists of files included in each backup run.
            Implies --reconstruct-location-index.""")
