...
class AbusTestBase(unittest.TestCase):
   ...
   @staticmethod
   def create_homedir_file(path, prime, timestamp=None):
   ...
   def setup_directories(self):
      ...
      with database.connect(self.databasepath, allow_create=True) as db:
         pass
   ...
   def setup_multiple_backups(self):
      ...
      for i in range(3):
         ...
         for path in change_paths:
            self.create_homedir_file(self.homedir+"/"+path, next(primes), timestamp)
         rc= main.main(["test", "-f", self.configfile, "--backup"])
         self.assertEqual(rc, 0)
         if i<2:
            with database.connect(self.databasepath) as db:
               self.change_latest_runname(db, offset+1000)

   @property
   def databasepath(self):
      return self.archivedir+"/index.sl3"

   def change_latest_runname(self, db, offset):
      """
      ...
      """
      with db._get_connection("testing_move_timestamps") as conn:
         latest= conn.execute("select max(run_name) from content").fetchall()[0][0]
         new_name= db._make_runname(time.mktime(time.strptime(latest, "%Y_%m_%d_%H%M"))+offset)
         for table in "content", "completed_run":
            conn.execute("update "+table+" set run_name= ? where run_name = ?", [new_name, latest])

      # renaming index file
      for direntry in self.find_files(self.archivedir):
         if direntry.name==latest+".lst":
            os.rename(direntry.path, direntry.path.replace(latest, new_name))
            break
