...
from multiprocessing import Lock, Process, JoinableQueue, cpu_count, Event, Value
    ...
    def run(self):
        ...
        while True:
            ...
            if new_result:
                ...
                dt = process_time() - t0
                logger.info("Run stage {2}, value {1}: elapsed time {0}".
                            format(dt, job.value, stage.name))
...
class ResultWriter(Process):
    def __init__(self, writer_queue: JoinableQueue, result_logger: ResultLog, lock: Lock, jobs: int):
        Process.__init__(self)
...
def run_experiments(values: Dict[str, Iterable], stages: Iterable[Stage], logger: ResultLog, predicate: RerunPredicate,
    ...
    value_list = list(product(*values.values()))
