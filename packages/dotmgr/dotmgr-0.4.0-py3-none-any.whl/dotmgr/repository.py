...
class Repository(object):
    ...
    def execute(self, args):
        ...
        """
        def _retry_without_color(error):
            if 'color' in error.stderr:
                del args[2]
                if self.verbose:
                    print('Executing `{}`'.format(' '.join(args)))
                print(_exec_raw(lambda: self._git().execute(args)))
                return
            _default_error_handler(error)
        args.insert(0, 'git')
        args.insert(2, '--color')
        if self.verbose:
            print('Executing `{}`'.format(' '.join(args)))
        output = _exec_raw(lambda: self._git().execute(args), _retry_without_color)
        if output:
            print(output)
