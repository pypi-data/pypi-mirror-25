...
class Wcc:
    ...
    def asmArtbooks(self,www_from,max_ctime,max_tidfrm,incmode,thread_count = 1,tk = 1):
        asmconfigArray = self.asmArtConfig() 
        if len(asmconfigArray) == 0 or asmconfigArray == None:
                ...
                if len(pagebook)>0:
                    for artinfo in pagebook:
                        ...
                        if (incmode == "both" or incmode == "time") and art_ctime < max_ctime:
                            print("严重错误, 发现了更小的art_ctime article ctime,请修改爬虫")
                            print(artinfo)
                            stop = True
                            break
                        elif (incmode == "both" or incmode == "tid") and  art_tidfrm < max_tidfrm:
                            print("严重错误,现了更小的tid_from article tidfrm,请修改爬虫")
                            print(artinfo)
                            stop = True
                            break
                        #检查每个文章是否超过时间或者tid
    ...
    #子类要实现这个函数
    def __asmArtbooksMt(self,www_from,max_ctime,max_tid_from,incmode,thread_count,tk):
        ...
        return
    #子类要实现这个函数
    def __asmArtbooksSt(self,www_from,max_ctime,max_tid_from,incmode,thread_count,tk):
        books = self.asmArtbooks(www_from,max_ctime,max_tid_from,incmode,thread_count,tk)
        self.__asmbooks.extend(books)
        return
        ...
        if books_info == None or len(books_info["books"]) == 0:
            ...
            if how == "all":
                ...
                if  self.isCtimeInc() == True and self.isTidInc() == True:
                    ...
                    incmode = "none"
                    print("本网站既不支持时间增序爬取,也不支持tid增序爬取,退出")
                    return[]
            ...
            if thread_count > 1:
                ...
                for tk in range(1,thread_count+1):
                    try:
                        thread_id = tk
                        t = threading.Thread(target = self.__asmArtbooksMt, args = (www_from,max_ctime,max_tid_from,incmode,thread_count,tk))
                        t.start()
                        threads.append(t)
                        print("Thread "+str(thread_id)+" spawn")
                    except Exception as err:
                ...
                self.__asmArtbooksSt(www_from,max_ctime,max_tid_from,incmode,1,1)
    ...
    def validate(self,www_from):
        resjson = Wcc.getjson("http://api.wikicivi.com/wcclist/"+www_from)
        #print(resjson)
        try:
            if resjson["status"] != 0:
                print("爬虫身份验证失败:"+www_from+" 请检查WCC服务器")
                return False
            wccinfo = resjson["result"]["data"]
    ...
    def make_fitem(farray):
        try:
            ...
            farray["md5"] = "unknown"
            if Wcc.existsFile('wikicivi-files',file_urlmd5_localpath) == True:
                ...
                if Wcc.downloadFile(file_urlmd5_localpath, file_url) == False:
                    ...
                    elif Wcc.existsFile('wikicivi-files',file_datmd5_localpath) == True:
                        print("hasmd5File:"+file_datmd5_localpath)
                    else:
                        if Wcc.uploadFile(file_urlmd5_localpath, file_datmd5_localpath,'wikicivi-files') == False:
    ...
    def uploadFile(file_localname, file_cloudname,oss_bucket='wikicivi-files'):
        ...
        for k in range(1,6):
            try:
                ...
                continue
        if success == True:
            print("uploadFile:"+file_cloudname+" Ok from "+file_localname)
        else:
