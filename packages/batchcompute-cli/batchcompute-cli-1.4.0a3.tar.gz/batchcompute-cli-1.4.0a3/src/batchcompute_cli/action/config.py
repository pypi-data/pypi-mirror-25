...
def all(region=None, osspath=None, locale=None, image=None, type=None, cluster=None, version=None, ssl=None, god=None):
    if not region and not osspath and not locale and not image and not type and not cluster and not version and not ssl and not god:
        show_config()
    else:
        if region:
