...
def extend_mount(desc, mount_m):
    if not desc.get('ClusterId') or desc.get('ClusterId') == '':
        ...
        if not desc['AutoCluster']['Configs']['Mounts'].get('Entries'):
            desc['AutoCluster']['Configs']['Mounts']['Entries'] = []

        # 去重
        i=0
        for n in desc['AutoCluster']['Configs']['Mounts']['Entries']:
            if mount_m.get(n['Source']):
                del desc['AutoCluster']['Configs']['Mounts']['Entries'][i]
                i = i - 1
            i+=1
