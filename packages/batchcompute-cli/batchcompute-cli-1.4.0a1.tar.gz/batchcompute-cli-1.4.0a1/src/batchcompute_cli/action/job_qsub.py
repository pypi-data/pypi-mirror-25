...
import re
import os
from ..util import client, config, formater, oss_client
import json
import uuid
import base64
import tarfile
...
def qsub(script_name, work_path=None, stderr_path=None, join='n',job_name=None, resource_list=None, stdout_path=None, env=False,
         show_json=False, show_opt=False):

    # 参数
    opt = {
        'job_name': job_name or 'qsub-job',
    ...
    #################################
    desc = to_job_desc(opt2['resource_list'])
    desc['Name'] = opt2['job_name']
...
def _gen_job_desc():
    return {
        ...
        "Description": "BatchCompute cli qsub job",
...
def parse_resource_list(s):
    ...
    for kv in kvp:
        ...
        if kv.startswith('select='):
            return parseSelect(s[7:])
        else:
            return parseKV(s)
...
def parseNodes(v):
    if '+' in v:
        arr = v.split('+').map(lambda n: int(n))
