    ...
    for (task_name,task_desc) in tasks.items():
        ...
        if not cluster_id or cluster_id=='':
            ...
            if opt.get('nas_access_group'):
                ...
                if not task_desc['AutoCluster']['Configs'].get('Networks'):
                    task_desc['AutoCluster']['Configs']['Networks'] = {}
                if not task_desc['AutoCluster']['Configs']['Networks'].get('NAS'):
                    task_desc['AutoCluster']['Configs']['Networks']['NAS'] = {}
        ...
        mount_m = get_mount_map(task_desc.get('Mounts'))
        if opt.get('mount'):
            extend(mount_m, trans_mount(opt['mount']))
        if opt.get('read_mount'):
            extend(mount_m, trans_mount(opt['read_mount']))
...
def extend_mount(desc, mount_m):
    ...
    for k,v in mount_m.items():
        if k.startswith('oss://') or k.startswith('nas://'):
            ...
            if not desc.get('ClusterId') or desc.get('ClusterId')=='':

                if not desc.get('AutoCluster'):
                    desc['AutoCluster'] = {}
                if not desc['AutoCluster'].get('Configs'):
                    desc['AutoCluster']['Configs'] = {}
                if not desc['AutoCluster']['Configs'].get('Mounts'):
                    desc['AutoCluster']['Configs']['Mounts'] = {}
    ...
    for sec in secs:
        ...
        if default_options:
            ...
            if cf.has_option(DEFAULT,'mount'):
                extend(mount_m , trans_mount(cf.get(DEFAULT,'mount')))
            if cf.has_option(DEFAULT,'read_mount'):
                extend(mount_m, trans_mount(cf.get(DEFAULT,'read_mount')))
            extend_mount(task_desc, mount_m)
        ...
        if not cluster_id or cluster_id=='':
            ...
            if cf.has_option(sec,'nas_access_group'):
                if not task_desc['AutoCluster'].get('Configs'):
                    task_desc['AutoCluster']['Configs'] = {}
                if not task_desc['AutoCluster']['Configs'].get('Mounts'):
                    task_desc['AutoCluster']['Configs']['Mounts'] = {}
                if not task_desc['AutoCluster']['Configs']['Mounts'].get('NAS'):
                    task_desc['AutoCluster']['Configs']['Mounts']['NAS'] = {}
        ...
        if cf.has_option(sec, 'mount'):
            extend(mount_m,trans_mount(cf.get(sec, 'mount')))
        if cf.has_option(sec,'read_mount'):
            extend(mount_m, trans_mount(cf.get(sec,'read_mount')))
        extend_mount(task_desc, mount_m)
