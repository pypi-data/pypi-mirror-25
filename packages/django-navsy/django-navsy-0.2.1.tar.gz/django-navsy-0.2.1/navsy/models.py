...
class Page(NodeModel):
    ...
    @staticmethod
    def create_first_object():
        if Page.objects.count() == 0:
            Page.objects.create(name='Home', home=True, menu=False)

    name = models.CharField(max_length=100)
    slug = AutoSlugField(
        populate_from='name', unique=True, always_update=True, blank=True, null=True)
    home = models.BooleanField(default=False)
    menu = models.BooleanField(default=True)
    published = models.BooleanField(default=True)

    def create_first_route(self):
        pattern_obj, pattern_obj_created = Pattern.objects.get_or_create(
            regex='')
        route_obj, route_obj_created = Route.objects.get_or_create(
            page=self, pattern=pattern_obj)
        return (route_obj, route_obj_created, )

    def enforce_unique_home(self):
        if self.home:
...
class Route(models.Model):
    ...
    def get_redirect_url(self):
        ...
        if self.redirect_to_first_child_page and page_obj and page_obj.children_count > 0:
            ...
            redirect_url = path_utils.fix_path(redirect_path, self.redirect_to_path)
    ...
    # TODO: impossible to save more routes without view_name, write signal on pre_save
