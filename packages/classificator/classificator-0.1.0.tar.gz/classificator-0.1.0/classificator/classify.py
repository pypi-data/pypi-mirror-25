    ...
    def _train_vectorizers(self, X):
        ...
        for col in range(len(self.config["data_specs"]["feature_methods"])):
            ...
            if method == "vectorize":
                ...
                tf.fit([str(x[col]) if (x[col] and ~pd.isnull(x[col]) and x[col] != np.nan) else "" for x in X])
                vec = Vec_(tf)
            else:
                vec = None
...
class MultiTextVectorizer(TransformerMixin, BaseEstimator):
    ...
    def _get_col(self, X, col, method):
        ...
        if method == "numeric":
            ...
            return [str(x[col]) if (x[col] and ~pd.isnull(x[col]) and x[col] != np.nan) else "" for x in X]
