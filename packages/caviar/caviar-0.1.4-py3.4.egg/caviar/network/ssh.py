...
class SSHSession:
	...
	def execute(self, cmd):
		...
		:raise caviar.engine.ssh.SSHUnavailableSessionError:
		   If it is not possible to use any physical session.
		:raise caviar.engine.ssh.SSHCommandError:
		...
		while should_continue:
			try:
				yield from self.__pool.get(self.__target).execute(cmd)
				should_continue = False
			except SSHUnavailableSessionError as e:
				available_attempts = self.__retry(available_attempts)
			except SSHInvalidSessionError as e:
...
class SSHSessionPool:
	...
	def logout_all(self):
		...
		for target, physical_session in self.__physical_sessions.iteritems():
