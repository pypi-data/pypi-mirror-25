...
def linfit(x, y, yerr=None):
    ...
    A = np.matrix(np.hstack((x.reshape(N,1),np.ones((N,1)))))
...
def nonlinfit(f, x, y, p, yerr=None, tol=1e-11, maxiter=50, plot=False):
    """
    ...
    for _ in range(maxiter):
        ...
        chi2_red_tmp = sum(np.power(y-f(x,p),2))/(N-num_param)
