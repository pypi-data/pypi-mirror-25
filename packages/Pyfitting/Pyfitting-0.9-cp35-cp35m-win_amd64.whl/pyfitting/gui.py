...
def cftool(x, y, yerr=None, num_points=1000):
    ...
    param_input.pack(side = Tk.RIGHT)
    ...
    def fitdata():
        ...
        p = nonlinfit(func, x, y, param, yerr=yerr, plot=True)
        for p_i in p:
            a.cla()
            b.cla()
            c.cla()
            ...
            a.errorbar(x, y, yerr=yerr, fmt='.r', label='data')
            a.plot(x_func,func(x_func,p_i), label=func_label%tuple(p_i))
            a.set_title('Fit')
            a.set_xlabel('x')     
            a.legend()

            residuals =  func(x,p_i)-y
            mean = np.mean(residuals)
            std = np.std(residuals)
            b.plot(x,residuals,'.')
            b.plot([0,max(x)],[mean, mean],'C1')
