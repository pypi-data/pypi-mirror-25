...
from testwa.webdriver.common.mobileby import MobileBy
...
class WebDriver(webdriver.Remote):
    ...
    # extend input keyboard value
    def webview_send_keys(self, value):
        """ input value from keyboard
        """
        data = {
            "value": value
        }
        self.execute(Command.KEYBOARD_INPUT, data)
        return self

    # extend find element by name for backward compatibility testwa
    def find_element_by_name(self, name):
        # body = {'using': By.ACCESSIBILITY_ID, 'value': name, 'mode': 'byName'}
        xpath = "//*[@content-desc='{0}']|//*[@text='{0}']".format(name)
        body = {'using': By.XPATH, 'value': xpath, 'mode': 'byName'}
        return self.execute(SeleniumCommand.FIND_ELEMENT, body)['value']

    def find_elements_by_name(self, name):
        # body = {'using': By.ACCESSIBILITY_ID, 'value': name, 'mode': 'byName'}
        xpath = "//*[@content-desc='{0}']|//*[@text='{0}']".format(name)
        body = {'using': By.XPATH, 'value': xpath, 'mode': 'byName'}
        return self.execute(SeleniumCommand.FIND_ELEMENTS, body)['value']

    # add checking points testwa
    def check_element_by_accessibility_id(self, id, note=None):
        """Checks an element by accessibility id.
        ...
        :Usage:
            driver.find_element_by_accessibility_id()
        """
        body = {'using': By.ACCESSIBILITY_ID, 'value': id, 'mode': 'check'}
        if note is not None:
            body['note'] = note

        try:
            self.execute(SeleniumCommand.FIND_ELEMENT, body)
            return True
        except:
            return False

    def check_element_by_class_name(self, class_name, note=None):
        """
        Checks an element by class name.
        ...
        """
        body = {'using': By.CLASS_NAME, 'value': class_name, 'mode': 'check'}
        if note is not None:
            body['note'] = note
        try:
            self.execute(SeleniumCommand.FIND_ELEMENT, body)
    ...
    def check_element_by_xpath(self, xpath, note=None):
        """
        ...
        """
        body = {'using': By.XPATH, 'value': xpath, 'mode': 'check'}
        if note is not None:
            body['note'] = note
        try:
            self.execute(SeleniumCommand.FIND_ELEMENT, body)
    ...
    def _addCommands(self):
        ...
        self.command_executor._commands[Command.KEYBOARD_INPUT] = \
            ('POST', '/session/$sessionId/value')
