...
from SkitterSchema.tweepy2schema import process_followees, process_favourites, assert_processed_outcome
...
class RandomWalkerTwitterCrawler(TwitterCrawler):
    ...
    def get_timeline(self):
        ...
        tl = self.api.user_timeline(include_rts=True, count=200, trim_user=False, exclude_replies=False,
                                    user_id=self.pointer, tweet_mode='extended')
        timeline_outcome += process_timeline(tl, self.pointer)
    ...
    def get_favs(self):
        ...
        favs = self.api.favorites(user_id=self.pointer, count=200, include_entities=True, tweet_mode='extended')
    ...
    def _run(self, identifier, result, idx, state, **kwargs):
        ...
        while self.stop.check() and self.excd['tl'] is False:
            ...
            try:
                assert_processed_outcome(outcome, self.pointer)
            except AssertionError as e:
                print(e, " (status)", self.pointer, "(current user)")
        ...
        result[idx] = (identifier, toutcome, self.mem, self.log)
