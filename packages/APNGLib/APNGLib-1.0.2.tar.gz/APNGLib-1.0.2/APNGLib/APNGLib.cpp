...
const unsigned int TransformNone           = 0x0;
const unsigned int TransformCrop           = 0x1;
const unsigned int TransformFlipHorizontal = 0x2;
const unsigned int TransformFlipVertical   = 0x4;
const unsigned int TransformNoGif1Frame    = 0x8;
...
void TransformPNG(APNGFrame &Frame, unsigned int TransformFlags, unsigned int xOffset, unsigned int yOffset, unsigned int xSize, unsigned int ySize){
	...
	if(xOffset>=0 && (xOffset+xSize)<=w && yOffset>=0 && (yOffset+ySize)<=h){
		unsigned char *Texels = new unsigned char[xSize*ySize*TexelSize];
	...
	if(TransformFlags&TransformFlipHorizontal){
	...
	if(TransformFlags&TransformFlipVertical){
...
bool ParseString(PyObject *Obj, char *r, size_t BufferLen);

bool ParseInt(PyObject *Obj, unsigned int *r){
	if(!PyNumber_Check(Obj)) return false;
	PyObject *LongType = PyNumber_Long(Obj);
	if(!LongType) return false;
	*r = PyLong_AsUnsignedLong(LongType);
	Py_DECREF(LongType);
	return true;
}

//We want to be able to process a variable number of arguments from python.
Py_ssize_t ParseArguments(PyObject *Args, const char *Fmt, ...){
	va_list lst;
	va_start(lst, Fmt);
	Py_ssize_t i = 0;
	
	for(const char *n = Fmt; *n; ++n, ++i){
		PyObject *Itm = PyTuple_GetItem(Args, i);
		if(!Itm) break;
		if(*n=='s'){
			char *result = va_arg(lst, char*);
			size_t BufferLen = va_arg(lst, unsigned int);
			if(!ParseString(Itm, result, BufferLen)) break;
		}else if(*n=='i'){
			if(!ParseInt(Itm, va_arg(lst, unsigned int*))) break;
		}
	}
	va_end(lst);
	return i;
...
static PyObject *APNGLib_Decompile(PyObject *Self, PyObject *Args){
	char szOut[256];
	char Outformat[256];
	char szInput[256];
	...
	if(ParseArguments(Args, "ssiiiii", szInput, sizeof(szInput), Outformat, sizeof(Outformat), &Transform, &xOffset, &yOffset, &xSize, &ySize)<3) return NULL;
	...
	for(unsigned int i=0;i<NbrFrames;i++){
		snprintf(szOut, sizeof(szOut), "%s%d.png", Outformat, i);
...
static PyObject *APNGLib_MakeGIF(PyObject *Self, PyObject *Args){
	char szOut[256];
	char szInput[256];
	unsigned int NbrLoops;
	unsigned int xOffset = -1;
	...
	if(ParseArguments(Args, "ssiiiii", szInput, sizeof(szInput), szOut, sizeof(szOut), &Transform, &xOffset, &yOffset, &xSize, &ySize)<3) return NULL;
	int res = load_apng(szInput, Frames, &NbrLoops);
	if(res<0){
		printf("load_apng failed: '%s'", szInput);
		return NULL;
	...
	if((NbrFrames==1 && (Transform&TransformNoGif1Frame)) || (!res && Frames.size()>1)){
		delete[] Frames[0].rows;
		delete[] Frames[0].p;
		Frames.erase(Frames.begin());
		if(NbrFrames==1 && (Transform&TransformNoGif1Frame)) return Py_BuildValue("i", NbrFrames);
	}
...
static PyMethodDef APNGLibMethods[] = {
	{"MakeGIF", APNGLib_MakeGIF, METH_VARARGS, "Loads and converts a animated png file into a gif, saving the resulting file.  make gif takes the following parameters: SourceFile, DestFile, TransformFlags, (Extra parameters if TransformCrop is set: xOffset, yOffset, xSize, ySize, bounds must be in frame size, otherwise no culling will occur)."},
...
bool ParseString(PyObject *Obj, char *r, size_t BufferLen){
	...
	strncpy(r, PyString_AsString(Obj), BufferLen);
	return true;
}
#else
...
bool ParseString(PyObject *Obj, char *r, size_t BufferLen){
	...
	PyObject *TempBytes = PyUnicode_AsEncodedString(Obj, "ASCII", "STRICT");
	if(!TempBytes) return false;
	strncpy(r, PyBytes_AS_STRING(TempBytes), BufferLen);
	Py_DECREF(TempBytes);
	return true;
}
#endif