...
def lookup_loc(loc: str) -> Either[LookupFailures, List[SpecLocation]]:
    ...
    return (
        (path_match // __.group('path')).lmap(Unsuitable('module path', loc, 'not a path')) // handle_path
        if path_match.present else
        file_match
        .flat_apzip(lambda a: (a.group('path') / Path).lmap(lambda b: Unsuitable('file path', loc, 'no path')))
