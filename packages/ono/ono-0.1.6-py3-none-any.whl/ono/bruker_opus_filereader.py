...
class Absorption:
    ...
    def baselineCorrection(self, wnMin, wnMax):
        condition = np.logical_and(
            self.wavenumber >= wnMin,
            self.wavenumber < wnMax
        )
    
        self.wnb = self.wavenumber[condition]
        AB_cond = self.AB[condition]
        
        self.baseline = fit.baselineConvexHull(self.wnb, AB_cond)    
        self.ABB = AB_cond - self.baseline
