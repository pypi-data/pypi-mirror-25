...
class ExtratorCNJ:
    ...
    def _valida_cnj(self, cnj):
        if self.FORMATO_CNJ.match(cnj) and self._confere_digito(cnj):
            return
    ...
    def _confere_digito(self, cnj):
        valor = re.sub('[- .]', '', cnj)
        orig_dv = valor[7:9]

        valor_sem_digito = int(valor[0:7] + valor[9:])

        check = 98 - ((valor_sem_digito * 100 % 97) % 97)
        if str(check).zfill(2) != orig_dv:
            raise CNJIncorrigivel
        return cnj
