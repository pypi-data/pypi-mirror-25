...
def get_request(url, by="MechanicalSoup", proxyUrl="", cookie="", delaySwitcher=1):
    ...
    if by == "seleniumPhantomJS":
        ...
        try:
            ...
            if (re.search(r"(页面不存在)|(未找到页面)|(page\s+not\s+found)|(404\s+not\s+found)", content, re.I)
...
def set_column_name_scan_module_unfinished(column_name,scan_way):
    ...
    sqlList.append(tmpSql)
    for each in sqlList:
        execute_sql_in_db(each, eval(get_key_value_from_config_file(
            configIniPath, 'default', 'db_name')))

    if scan_way in [1,3]:
        tmpSql="select table_name from information_schema.tables where table_schema='exp10itdb' and table_name regexp '.*_pang$'"
        result=execute_sql_in_db(tmpSql, eval(get_key_value_from_config_file(
            configIniPath, 'default', 'db_name')))
        if len(result)>0:
            for each in result:
                each_table=each[0]
...
get_pang_domains_finished varchar(50) not null default %s,\
get_sub_domains_finished varchar(50) not null default %s,\
...
final_scan_result mediumtext not null)" % (targets_table_name,default_get_pang_value,default_get_sub_value)
...
pang_domains_scan_finished varchar(50) not null default 0,\
sub_domains_scan_finished varchar(50) not null default 0,\
final_scan_result mediumtext not null)" % (first_targets_table_name,default_get_pang_value,default_get_sub_value)
...
http_domain varchar(70) not null)" % target_urls_table_name
                ...
                if choose == 'y' or choose == 'Y':
                    ...
                    execute_sql_in_db(sql3, eval(get_key_value_from_config_file(
                        configIniPath, 'default', 'db_name')))
                    set_scan_finished("pang_domains_sqli_scaned", eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')),
                          first_targets_table_name,"start_url", start_url)
                    set_scan_finished("sub_domains_sqli_scaned", eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')),
                          first_targets_table_name,"start_url", start_url)
                    ...
                    execute_sql_in_db(sql3, eval(get_key_value_from_config_file(
                        configIniPath, 'default', 'db_name')))
                    set_scan_finished("pang_domains_sqli_scaned", eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')),
                          targets_table_name,"start_url", start_url)
                    set_scan_finished("sub_domains_sqli_scaned", eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')),
                          targets_table_name,"start_url", start_url)
...
http_domain varchar(70) not null)" % each_pang_urls_table_name
        ...
        result = execute_sql_in_db("select actual_ip_from_cdn_scan from `%s` where start_url='%s'" %
                                   (get_main_target_table_name(target), start_url), eval(get_key_value_from_config_file(
                                       configIniPath, 'default', 'db_name')))
        if len(result) > 0:
            ip = result[0][0]
...
http_domain varchar(70) not null)" % each_pang_urls_table_name
    ...
    execute_sql_in_db(
        ...
        (main_target_table_name, start_url), eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')))
    ...
    if 1 == get_scan_finished(
        ...
        for each_table in table_name_list:
            execute_sql_in_db(
                ...
                (each_table, script_type_value,column_name, target), eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')))
...
def single_dirb_scan(target):
    ...
    if os.path.exists(log_file):
        ...
        result = execute_sql_in_db(
            ...
            (table_name_list[0],column_name, target), eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')))
        if len(result) > 0:
            ext = result[0][0]
        else:
...
def single_cms_scan(start_url):
    ...
    if 1 == get_scan_finished(
        ...
        result = execute_sql_in_db(
            ...
            (table_name_list[0],column_name, start_url), eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')))
        if len(result) > 0:
        ...
        for each_table in table_name_list:
            execute_sql_in_db(
                ...
                (each_table, cms_value,column_name, start_url), eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')))
            set_scan_finished(
                "cms_identify_scaned",
                eval(get_key_value_from_config_file(
                    configIniPath, 'default', 'db_name')),
...
def sqli_scan(start_url):
    ...
    http_domain_sqli_scaned = get_scan_finished(
        "sqli_scaned", eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')),
        main_target_table_name,"start_url",start_url)
        ...
        while(1):
            if checkvpn():
                if http_domain_sqli_scaned == 0 or pang_domains_sqli_scaned == 0 or sub_domains_sqli_scaned == 0:
                    ...
                    if http_domain_sqli_scaned == 0:
                        sqlmap_crawl(
                            start_url, bool_tor, post_or_not)
                            ...
                            for pang_http_domain in domain_pang_list:
                                ...
                                if pang_http_domain_value != http_domain:
                                    sqli_scaned = get_scan_finished(
                                        "sqli_scaned", eval(get_key_value_from_config_file(configIniPath,
                                            'default', 'db_name')), domain_pang_table_name, "http_domain",pang_http_domain_value)
                                    if sqli_scaned == 0:
                                        # 在子站表中查询是否当前旁站也是子站,如果是且在上次子站扫描中已经扫描过
                                        # 那么此次不再重复sqli扫描,并在旁站表中将sqli_scaned标记为1
                                        sqli_scaned = get_scan_finished(
                                            "sqli_scaned", eval(get_key_value_from_config_file(configIniPath,
                                                'default', 'db_name')), domain_sub_table_name,"http_domain", pang_http_domain_value)
                                        if sqli_scaned == 1:
                                            print(
                        ...
                        with open(domain_sub_file, "r+") as f:
                            ...
                            for sub_domain in domain_sub_list:
                                ...
                                if sub_http_domain_value != http_domain:
                                    sqli_scaned = get_scan_finished(
                                        "sqli_scaned", eval(get_key_value_from_config_file(configIniPath,
                                            'default', 'db_name')), domain_sub_table_name,"http_domain", sub_http_domain_value)
                                    if sqli_scaned == 0:
                                        # 在旁站表中查询是否当前子站也是旁站,如果是且在上次旁站扫描中已经扫描过
                                        # 那么此次不再重复sqli扫描,并在子站表中将sqli_scaned标记为1
                                        sqli_scaned = get_scan_finished(
                                            "sqli_scaned", eval(get_key_value_from_config_file(configIniPath,
                                                'default', 'db_name')), domain_pang_table_name,"http_domain", sub_http_domain_value)
                                        if sqli_scaned == 1:
                                            print(
    ...
    for each_table in table_name_list:
        if strings_to_write!="":
            execute_sql_in_db(
                ...
                (each_table, strings_to_write,column_name, start_url), eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')))
    ...
    if 1 == get_scan_finished(
        ...
        if target_is_pang_domain:
            ...
            for each_table in table_name_list:
                result = execute_sql_in_db("select actual_ip_from_cdn_scan from %s where %s='%s'" %
                                           (each_table,column_name, target), eval(get_key_value_from_config_file(configIniPath, 'default',
                                                                                                     'db_name')))
        ...
        for each_table in table_name_list:
            execute_sql_in_db(
                ...
                (each_table, strings_to_write,column_name, target), eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')))
                ...
                if len(result) > 0 and result[0][0] != "has cdn,but can not find actual ip":
                    ...
                    if target_is_sub_domain:
                        ...
                        main_target_http_domain = get_source_main_target_domain_of_sub_url(target)[
                            'http_domain']
                        result = execute_sql_in_db("select actual_ip_from_cdn_scan from %s where %s='%s'" %
                                                   (get_main_target_table_name(main_target_http_domain),column_name, main_target_http_domain), eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')))
                        if len(result) > 0 and result[0][0] != "has cdn,but can not find actual ip":
                            main_target_ip = result[0][0]
                            if ip != main_target_ip:
                                result = execute_sql_in_db("select port_scan_info from %s where %s='%s'" %
                                                           (each_table,column_name, target), eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')))
                                if len(result) > 0:
    ...
    if len(columns_result) > 0:
        for each in columns_result:
            ...
            column_scaned = get_scan_finished(
                column_name, eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')),
                main_target_table_name, "start_url",start_url)
            if column_scaned == 0:
                return
        set_scan_finished(
...
class MyScanner(object):
    ...
    def __init__(self, start_url, single_xxx_scan):
        ...
        http_domain_xxx_scaned = get_scan_finished(
            xxxValue + "_scaned", eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')),
            main_target_table_name, "start_url",start_url)
