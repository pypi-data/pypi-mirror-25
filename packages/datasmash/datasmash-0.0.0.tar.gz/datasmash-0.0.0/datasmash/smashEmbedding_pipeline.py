...
sec = SmashEmbedding(n_dim=num_dim, embed_class=mds_emb)
...
sec.set_training_data(X)

# return embedded coordinates using Sippl embedding (default) on distance matrix
print(sec.produce(nr=3, embedder='default'))

# return embedded coordinates using Sippl embedding (default) on distance matrix
print(sec.produce(nr=3, embedder='custom'))
