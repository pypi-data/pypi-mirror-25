...
project_setup_json = "ProjectSetup.json"

def get_project_setup_json():
    if os.environ.get("ConfigurationPath"):
        return os.path.join(os.environ.get("ConfigurationPath"), project_setup_json)
    return None
...
class Toolbox(object):
    def __init__(self):
        ...
        self.tools.append(ImportTreatmentAreaFeatureclass)
...
class CreateFolderStructure(object):
    ...
    def getParameterInfo(self):
        ...
        directory_workspace = arcpy.Parameter(name='directory_workspace',
                                             displayName='Directory sorkspace',
                                             direction='Input',
                                             datatype='DEFolder',
        ...
        # Parameter 1
        json_file = arcpy.Parameter(name='json_file',
                                   displayName='Projectsetup json file ',
                                   direction='Input',
                                   datatype='DEFile',
                                   parameterType='Required',
                                   enabled='True')
        json_file.value = get_project_setup_json()
...
# ------------------------------------------------------------
# Import TreatmentArea Featureclass
# ------------------------------------------------------------

class ImportTreatmentAreaFeatureclass(object):

    def __init__(self):
        """Define the tool"""
        self.label = "Import Treatment Area"
        self.description = "Import the treatment area featureclass"
        self.canRunInBackground = False
        self.category = "Project Setup"
    ...
    def getParameterInfo(self):
        ...
        local_gdb = arcpy.Parameter(name="local_gdb",
                                displayName="Projects Planning geodatabase",
                                   direction="Input",
                                   datatype="DEWorkspace",
                                   parameterType="Required")
        local_gdb.filter.list = ["Local Database"]
        local_gdb.value = global_flightline.planning_data_gdb_location
        local_gdb.value = global_flightline.planning_data_gdb_location
        params.append(local_gdb)

        # parameter 1
        treatment_area_featureclass = arcpy.Parameter(name="treatment_area_featureclass",
                                               displayName="Treatment Area featureclass",
                                              direction="Input",
                                              datatype="DEFeatureClass",
                                              parameterType="Required")
        params.append(treatment_area_featureclass)
        return params
    ...
    def execute(self, parameters, messages):
        ...
        directory_workspace = parameters[0].valueAsText
        treatment_area_featureclass = parameters[1].valueAsText
...
class EnterOrUpdateOperationMetadata(object):
    ...
    def getParameterInfo(self):
        ...
        # parameter 0
        local_gdb = arcpy.Parameter(name="local_gdb",
                                displayName="Projects Flightdata geodatabase",
                                   direction="Input",
                                   datatype="DEWorkspace",
                                   parameterType="Required")
        local_gdb.filter.list = ["Local Database"]
        ...
        operation_start_time = arcpy.Parameter(name="operation_start_time",
                                               displayName="Enter the time the operation went 'Toxic' i.e 0600",
                                              direction="Input",
                                              datatype="GPString",
                                              parameterType="Required")
...
class CreateNewFlightDataGdb(object):
    ...
    def execute(self, parameters, messages):
        ...
        project_directory = global_flightline.project_folder
        new_gdb_name = parameters[0].valueAsText + '.gdb'
