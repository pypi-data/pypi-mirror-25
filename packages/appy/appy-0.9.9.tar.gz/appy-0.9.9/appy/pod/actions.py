...
class Action:
    ...
    def evaluateBuffer(self, result, context, forceSource=None):
...
class MetaIf(Action):
    ...
    def __init__(self, buff, subExpr, elem, statements):
        Action.__init__(self, '', buff, subExpr, elem, None)
    ...
    def do(self, result, context, exprRes):
        if exprRes:
            ...
            note = self.reifyNote()
            # Inject the note in the buffer, after the first 'text:p' tag:
            # else, it might not be rendered.
            self.buffer.insertSubBuffer(note, after='text:p')
            self.evaluateBuffer(result, context, forceSource='buffer')
