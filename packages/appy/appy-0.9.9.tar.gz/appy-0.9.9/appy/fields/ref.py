...
class Initiator:
    ...
    def __init__(self, tool, req):
        self.tool = tool
        # Extract the initiator object and field from the "nav" request key
        self.navInfo = req['nav'].split('.')
        self.obj = tool.getObject(self.navInfo[1])
        self.field = self.obj.getField(self.navInfo[2])
        # We can also have information about the place to insert the newly
        # created object into the Ref.
    ...
    def link(self, newObj):
        '''Links p_newObj to the initiator object via self.field'''
        self.field.linkObject(self.obj, newObj, at=self.position)

    def getNavInfo(self, tied):
        '''Compute the correct nav info at the newly inserted p_tied object'''
        # At what position is p_tied among tied objects ?
        zobj = self.obj.o
        position = self.field.getIndexOf(zobj, tied.id) + 1
...
class Ref(Field):
    ...
    pxObjectActions = Px('''
     <div if="field.showActions" class="objectActions"
      ...
      <x if="not inPickList and (mayAdd == 'anywhere')">
       <img src=":url('addUp')" class="clickable" title=":_('object_add_up')"
            onclick=":'onAdd(%s,%s,%s)' % \
       ...
       <a if="not isinstance(create, basestring)" target="appyIFrame"
        ...
        <img src=":url('addUpFrom')" class="clickable"
             title=":_('object_add_from_up')"
    ...
    def getSelect(self, obj, forSearch=False):
        ...
        method = forSearch and self.sselect or self.select
    ...
    def linkObject(self, obj, value, back=False, noSecurity=True,
        ...
        if type(value) in sutils.sequenceTypes:
            for v in value:
                self.linkObject(obj, v, back, noSecurity, executeMethods, at)
            return
    ...
    def getPopupObjects(self, obj, name, rq, requestValue):
        ...
        if rq['semantics'] == 'checked':
            ...
            if self.link == 'popup':
                ...
                brains = tool.o.executeQuery(className, search=self.select,
