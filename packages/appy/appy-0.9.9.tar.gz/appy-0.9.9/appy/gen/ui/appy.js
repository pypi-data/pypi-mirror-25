...
function onLinkMany(action, id) {
  ...
  var statuses = node['_appy_' + elems[2] + '_cbs'];
  var uids = stringFromDict(statuses, true);
  // Get the array semantics
  var semantics = node['_appy_' + elems[2] + '_sem'];
...
function openPopup(popupId, msg, width, height, back) {
  ...
  var top = null;
  if (height) { top = (window.innerHeight - height) / 2 }
  else { top = window.innerHeight * 0.25 }
...
function askConfirm(actionType, action, msg, showComment, popupWidth) {
  ...
  openPopup("confirmActionPopup", msg, popupWidth);
}
...
function updateRowNumber(row, rowIndex, action) {
  ...
  var fields = row.getElementsByTagName('table'),
      ...
      newIndex = -1,
      id = old = elems = neww = val = w = oldIndex = null;
  // Patch fields
...
function deleteRow(tableId, deleteImg, ask, rowIndex) {
  ...
  for (var i=0; i < rows.length; i++) {
    if (i > rowIndex) updateRowNumber(rows[i], -1, 'add');
  }
...
function onSelectObjects(popupId, initiatorId, objectUrl, mode,
  ...
  var uids = stringFromDict(node['_appy_objs_cbs'], true);
