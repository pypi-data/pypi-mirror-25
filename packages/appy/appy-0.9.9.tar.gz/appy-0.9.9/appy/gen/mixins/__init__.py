...
class BaseMixin:
    ...
    def getInitiator(self):
        '''Gets information about a potential initiator object from the request
           and returns an instance of appy.fields.ref.Initiator.
        '''
        rq = getattr(self, 'REQUEST', None)
        if not rq or not rq.get('nav', '').startswith('ref.'): return
        return Initiator(self.getTool().appy(), rq)
    ...
    def createOrUpdate(self, created, values, initiator=None):
        ...
        if created and initiator:
            initiator.link(appyObject)
            zInitiator = initiator.obj.o
            # Record this change into the initiator's history when relevant
            if initiator.field.getAttribute(zInitiator, 'historized'):
                title = appyObject.o.getShownValue()
                cName = tool.getPortalType(appyObject.klass)
                zInitiator.addHistoryEvent('_dataadd_',
    ...
    def onCreate(self):
        ...
        if initiator:
            ...
            # a Ref field. The navigation string will allow Appy to detect the
            # initiator.
            urlParams['nav'] = '.'.join(initiator.navInfo)
            # Check that the user can add objects through this Ref field
            initiator.checkAdd()
            # Get potential information about where to insert the object
            if initiator.insertInfo:
                urlParams['insert'] = initiator.insertInfo
    ...
    def onUpdate(self):
        ...
        initiator = self.getInitiator()
        initiatorPage = initiator and initiator.field.pageName or None
        ...
        if (buttonClicked == 'save') or saveConfirmed:
            ...
            if isNew and initiator:
                if initiator.field.viewAdded: # Recompute the nav info
    ...
    def getListTitle(self, mode='link', nav='no', target=None, page='main',
        ...
        if mode == 'link':
            ...
            if hasattr(klass, 'uniqueLinks') and klass.uniqueLinks:
                ...
                url = self.getUrl(base=baseUrl, page=page, nav=nav,
                                  inPopup=inPopup)
            onClick = target.onClick and \
                      (' onclick="%s"' % target.getOnClick(self.id)) or ''
    ...
    def getUrl(self, base=None, mode='view', inPopup=False, relative=False,
        ...
        if not base:
            if not relative and useSso: # Get the base URL from the SSO
                ...
                if sso: base = sso.appUrl + self.absolute_url_path()
            if not base:
                base = relative and \
