...
class ExtendCommaDelimited(argparse._AppendAction):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        if isinstance(values, str):
            items.extend([_f for _f in values.split(',') if _f])
        else:
            for value in values:
                items.extend([_f for _f in value.split(',') if _f])
...
class ExtendCommaDelimitedToggle(argparse._AppendAction):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        if isinstance(values, str):
            values = [values]
        for value in values:
            neg, pos = split_negations([_f for _f in value.split(',') if _f])
            disabled.extend(neg)
            enabled.extend(pos)
...
class DelayedValue(object):
    ...
    def __init__(self, invokable, priority):
        ...
        if not isinstance(invokable, collections.Callable):
        ...
        if self._subparsers is not None:
            for x in self._subparsers._actions:
                if isinstance(x, argparse._SubParsersAction):
                    parsers.update(list(x._name_parser_map.keys()))
        return parsers
