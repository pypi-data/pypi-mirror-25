...
def buildingRegistryImage(inmemStore):
    """
    Are we building a registry image or not?
    """
    delivDict = inmemStore.getDict(DELIVERABLE_DICT)
    if delivDict.has_key('registry_url'):
        return delivDict.get('registry_url')
    return None
...
def calculateAndValidateDockerVersion(inMemStore):
    ...
    """
    # checking docker-py version.
    if docker.version.startswith('0.'):
        # For CentOS/RHEL 6.
        # Current version is 0.7.0 when install from epel
        dockerClient = docker.Client()
    else:
        # For CentOS/RHEL 7x.
        # This may be 1.10 if installed from epel or 2.x for pip installed.
        dockerClient = docker.from_env(version='1.19')

    dockerVersion = dockerClient.version()['Version']
    ...
    print ("ERROR: Detected docker version is %s" % dockerVersion)
    print ("ERROR: Workbench only supports Docker versions " + str(SUPPORTED_DOCKER_VERSIONS))
