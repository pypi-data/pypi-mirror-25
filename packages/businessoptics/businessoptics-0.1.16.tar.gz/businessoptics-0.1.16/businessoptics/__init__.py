...
class HasTuples(Client):
    ...
    def to_df(self):
        ...
        type_map = dict(STRING=str, DATETIME=pandas.Timestamp, INTEGER=numpy.int64, REAL=numpy.float64, BOOLEAN=bool)
        pandas_types = {k: type_map[v.upper()] for k, v in dimension_types.items()}
        date_columns = [k for k, v in dimension_types.items()
                        if v.upper() == 'DATETIME']
        return pandas.read_csv(csv_path, dtype=pandas_types, parse_dates=date_columns)
