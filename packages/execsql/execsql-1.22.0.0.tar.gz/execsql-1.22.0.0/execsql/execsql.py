...
class WriteSpec(object):
	...
	def __init__(self, message, dest=None, tee=None):
		# Inputs
		#	message: Required. Text to write.  May contain substitution variable references.
		#	dest: Optional. The to which the text should be written.  If omitted, the message
		#			is written to the console.
		#	tee: Optional.  Write to the console as well as to the specified file.  The argument
	...
	def write(self):
		...
		msg = self.msg
		while subscan:
			msg, subscan = subvars.substitute(msg)
		if self.outfile:
...
class ScriptCommands(object):
	...
	def append_substitution(self, template_str, sub_str):
		self.subvars.append_substitution(template_str, sub_str)
	def sub_exists(self, template_str):
...
metacommands.append(MetaCommand(ins_fn_rxs(r'^\s*WRITE\s+SCRIPT\s+(?P<script_id>\w+)(?:\s+(?P<append>APPEND\s+)?TO\s+', r')?\s*$'), x_writescript))
