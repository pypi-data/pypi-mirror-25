...
class WriteHooks(object):
	...
	def write(self, strval):
		...
		if conf:
			...
			enc = 'utf8'
		#if strval and len(strval) > 0 and strval[-1] != '\n':
		#	strval = u"%s\n" % strval
		if self.write_func:
	...
	def write_err(self, strval):
		...
		msg = u"%s\n" % strval
		if self.err_func:
			self.err_func(msg.encode(enc))
		else:
			sys.stderr.write(msg.encode(enc))
...
class EncodedFile(object):
	...
	def open(self, mode):
		fo = io.open(file=self.filename, mode=mode, encoding=self.encoding, newline=None)
		return fo
...
class Mailer(object):
	...
	def sendmail(self, send_from, send_to, subject, msg_content, content_filename=None, attach_filename=None):
		#print("Mailer.sendmail(); send_from = %s\nsend_to = %s\nsubject = %s\nmsg_content = %s" % (send_from, send_to, subject, msg_content))
		global smtplib
		global MIMEMultipart
		global MIMEText
		global MIMEBase
...
class ErrInfo(Exception):
	...
	def __init__(self, type, command_text=None, exception_msg=None, other_msg=None):
		...
		# 	"other"	: other_msg
		self.type = type
		self.command = None if not command_text else command_text.replace(u'\n', u'\n     ')
	...
	def script_info(self):
		if working_script and working_script.last_command:
			cmd = working_script.last_command
			em = u"\n     Script %s, line %d\n     " % (cmd.source, cmd.line_no)
			if cmd.command_type == "cmd":
				em = em + "Metacommand: %s" % cmd.sql
			else:
				em = em + "SQL statement: %s" % cmd.sql
			return em
		else:
			return None
	def eval_err(self):
		if self.type == 'db':
			self.error_message = u"***** Error in SQL statement:\n     %s\n     %s" % (self.command, self.exception.replace(u'\n', u'\n     '))
			if self.other:
				self.error_message += u"\n     %s" % self.other
		elif self.type == 'cmd':
			self.error_message = u"**** Error in metacommand:\n     %s" % self.command
			if self.exception:
				self.error_message += u"\n     %s" % self.exception.replace(u'\n', u'\n     ')
			if self.other:
				self.error_message += u"\n     %s" % self.other
		elif self.type == 'log':
			...
			self.error_message = u"**** Error: %s" % self.other
			if self.exception:
				self.error_message += u"\n     %s" % self.exception.replace(u'\n', u'\n     ')
		elif self.type == 'systemexit':
			self.error_message = u"**** Exit: %s" % self.other
...
def exit_immediately(exit_status, errinfo, logmsg=None):
	disable_gui()
	global exec_log
	if exit_status > 0:
		em = None
...
def fatal_error(error_msg=None):
	exit_immediately(1, ErrInfo("error", other_msg=error_msg))
		...
		if self.conn is None:
			...
			for drv in ssdrivers:
				...
				try:
					...
					break
			if not self.conn:
				raise ErrInfo(type="other", other_msg=u"Can't open SQL Server database %s on %s" % (self.db_name, self.server_name))
	...
	def read_and_parse_line(self, f):
		...
		def quote_in_quoted():
			if c == self.quotechar:
				...
				current_element[0] += c
				self._record_format_error(i+1, "Unexpected character following a closing quote")
				return _IN_UNQUOTED
		def in_unquoted():
			if c == self.delimiter:
				elements.append(current_element[0] if len(current_element[0]) > 0 else None)
...
class ScriptCommands(object):
	...
	def __init__(self, scriptfile_name, subvar_set, final_output_filename=None):
		self.scriptfile_name = scriptfile_name
		self.scripts = {}				# Dictionary of scripts defined with the BEGIN SCRIPT metacommand.
		# Python scoping rules prohibit passing the script level as an integer.
		class ScriptLevel(object):
			def __init__(self):
				self.level = 0
		self.scriptlevel = ScriptLevel()
		read_sqlscriptfile(scriptfile_name, self.scripts, None, self.scriptlevel)
		self.command_list = self.scripts.get(None, [])
		self.last_command = None
		# self.curr_pos is the index of the next command to be executed.
		self.curr_pos = 0
		self.outfile = final_output_filename
		self.subvars = subvar_set
		for k in os.environ.keys():
			self.subvars.add_substitution(u"&"+k, os.environ[k])
		self.counters = {}
		self.batch_level = 0
	...
	def __repr__(self):
		return u"ScriptCommands(%r, %r, %r)" % (self.scriptfile_name, self.subvars, self.outfile)
	def insert_commands(self, more_cmds):
		# Argument 'more_cmds' should be a list of SqlCmd objects
	...
	def read_sqlfile(self, sql_file_name):
		newscripts = {}
		read_sqlscriptfile(sql_file_name, newscripts, None, self.scriptlevel)
	...
	def script_lines(self, script_id):
		...
		if s in self.scripts.keys():
			return [cmd.sql for cmd in self.scripts[s]]
		else:
			return []
	def remove_substitution(self, template_str):
		self.subvars.remove_substitution(template_str)
	def add_substitution(self, template_str, sub_str):
		self.subvars.add_substitution(template_str, sub_str)
	def sub_exists(self, template_str):
		if self.subvars.sub_exists(template_str):
			return True
	...
	def current_script_line(self):
		...
		if self.curr_pos > 0:
			#cmdobj = self.command_list[self.curr_pos - 1]
	...
	def evaluate_metacommands(self, sqlcmd_obj, in_batch_only=False, when_false_only=False):
		...
		if something_tried:
			# but nothing applied.
			raise ErrInfo(type="cmd", command_text=sqlcmd_obj.sql, other_msg="Unrecognized metacommand")
		return None
	def run_db_sql(self, db, sql):
		# Execute a SQL command but do not commit it.
		db.execute(sql)
	def runone(self, sql, outfile):
		...
		try:
			if outfile is not None:
				sql = sql.strip()
				if sql[-1] == ';':
					sql = u'(' + sql[:-1] +u') as xxxxx'
				x_export(**{'queryname': sql, 'outputname': outfile, 'append': 'false', 'format': 'csv'})
			else:
				db = dbs.current()
		...
		if e:
			self.add_substitution("$LAST_ERROR", sql)
			status.sql_error = True
			if status.halt_on_err:
				exit_immediately(1, e)
			return
		self.add_substitution("$LAST_SQL", sql)
	...
	def run(self):
		...
		self.cmds_run = 0
		self.subvars.add_substitution("$LAST_ROWCOUNT", None)
		for cmd_index, cmditem in enumerate(self.command_list):
			self.subvars.add_substitution("$CURRENT_SCRIPT", cmditem.source)
			self.subvars.add_substitution("$SCRIPT_LINE", str(cmditem.line_no))
			self.subvars.add_substitution("$CANCEL_HALT_STATE", "ON" if status.cancel_halt else "OFF")
			self.subvars.add_substitution("$ERROR_HALT_STATE", "ON" if status.halt_on_err else "OFF")
			self.subvars.add_substitution("$METACOMMAND_ERROR_HALT_STATE", "ON" if status.halt_on_metacommand_err else "OFF")
			self.subvars.add_substitution("$CURRENT_TIME", datetime.datetime.now().strftime("%Y-%m-%d %H:%M"))
			self.subvars.add_substitution("$CURRENT_DIR", os.path.abspath(os.path.curdir))
			self.subvars.add_substitution("$CURRENT_ALIAS", dbs.current_alias())
			self.subvars.add_substitution("$AUTOCOMMIT_STATE", "ON" if dbs.current().autocommit else "OFF")
			self.subvars.add_substitution("$TIMER", unicode(datetime.timedelta(seconds=timer.elapsed())))
			self.subvars.add_substitution("$DB_USER", dbs.current().user if dbs.current().user else '')
			self.subvars.add_substitution("$DB_SERVER", dbs.current().server_name if dbs.current().server_name else '')
			self.subvars.add_substitution("$DB_NAME", dbs.current().db_name)
			self.subvars.add_substitution("$DB_NEED_PWD", "TRUE" if dbs.current().need_passwd else "FALSE")
			self.subvars.add_substitution("$RANDOM", str(random.random()))
			self.subvars.add_substitution("$UUID", str(uuid.uuid4()))
			self.last_command = cmditem
			while True:
				match_found = False
				cmditem.sql, match_found = self.subvars.substitute(cmditem.sql)
			...
			if self.all_true():
				if cmditem.command_type == 'sql':
					self.runone(cmditem.sql, (None, self.outfile)[self.outfile is not None and self.curr_pos == len(self.command_list)])
...
# GUI/message types
GUI_DISPLAY, GUI_CONNECT, GUI_CONSOLE, GUI_SELECTSUB, GUI_PAUSE, GUI_HALT, GUI_ENTRY, QUERY_CONSOLE = range(8)
...
class DisplayUI(object):
	def __init__(self, title, message, button_list, selected_button=0, no_cancel=False, column_headers=None, rowset=None, textentry=None, hidetext=False):
		...
		if textentry:
			...
			self.entryctrl = ttk.Entry(msgframe, width=60, textvariable=self.entryvar)
			if hidetext:
		...
		#else:
		#	sys.stderr.write("Can't reposition window; window geometry is: %s" % self.win.geometry())
		# Limit resizing
	...
	def activate(self):
		...
		self.entryvar = None
		return (self.button_clicked_value, self.return_value)
...
class EntrySpec(object):
	def __init__(self, name, prompt, required=False, initial_value=None, default_width=None, 
	...
	def __repr__(self):
		return "EntrySpec(%s, %s, %s, %s, %s, %s, %s, %s)" % (self.name, self.prompt, self.required, self.value,
				self.width, self.lookup_list, self.validation_rx.pattern if self.validation_rx else None, self.entry_type)
	...
	def __init__(self, title, message, entry_specs, column_headers=None, rowset=None):
		...
		def validate_entry(value_rx, key_rx, required):
			def checker(reason, new_val):
				...
				#if required and not new_val:
				#	return False
				if reason == 'key' and key_rx:
...
def x_halt(**kwargs):
	...
	exit_immediately(errlevel, None)
...
metacommands.append(MetaCommand(ins_table_rxs(r'^\s*HALT\s+MESSAGE\s+"(?P<errmsg>.+)"(?:\s+DISPLAY\s+',
...
def x_error_halt(**kwargs):
	...
	status.halt_on_err = flag == 'on'
	return None

metacommands.append(MetaCommand(r'\s*ERROR_HALT\s+(?P<onoff>ON|OFF)\s*$', x_error_halt))
...
def x_metacommand_error_halt(**kwargs):
	...
	status.halt_on_metacommand_err = flag == 'on'
	return None

metacommands.append(MetaCommand(r'\s*METACOMMAND_ERROR_HALT\s+(?P<onoff>ON|OFF)\s*$', x_metacommand_error_halt, set_error_flag=False))
...
def x_include(**kwargs):
	...
	exec_log.log_status_info("Including file %s; size: %d; date: %s" % (filename, sz, dt))
...
metacommands.append(MetaCommand(
	...
	x_import))
	#ins_table_rxs(r'^\s*IMPORT\s+TO\s+(?:(?P<new>NEW|REPLACEMENT)\s+)?', ins_fn_rxs(r'\s+FROM\s+', r'(?:\s+WITH(?:\s+QUOTE\s+(?P<quotechar>NONE|\'|")\s+DELIMITER\s+(?P<delimchar>TAB|UNITSEP|US|,|;|\|))?(?:\s+ENCODING\s+(?P<encoding>\w+))?)?\s*$')),
	#ins_table_rxs(r'^\s*IMPORT\s+TO\s+(?:(?P<new>NEW|REPLACEMENT)\s+)?', ins_fn_rxs(r'\s+FROM\s+', r'(?:\s+WITH\s+QUOTE\s+(?P<quotechar>NONE|\'|")\s+DELIMITER\s+(?P<delimchar>TAB|UNITSEP|US|,|;|\|))?\s*$')),
...
def x_prompt_ask(**kwargs):
	...
	if btn is None:
		...
		working_script.add_substitution(subvar, respstr)
...
def x_prompt_enter(**kwargs):
	...
	if btnval is None:
		...
		working_script.add_substitution(sub_var, txtval)
		script_name, lno = working_script.current_script_line()
		if as_pw:
...
metacommands.append(MetaCommand(
		ins_table_rxs(r'^\s*PROMPT\s+ENTER_SUB\s+(?P<match_str>\w+)\s+(?:(?P<password>PASSWORD)\s+)?MESSAGE\s+"(?P<message>.+)"(?:\s+DISPLAY\s+', r')?\s*$'),
		x_prompt_enter))
...
def x_prompt_entryform(**kwargs):
	...
	for r in spec_rows:
		...
		working_script.remove_substitution(subvar)
		entry_list.append(EntrySpec(subvar, prompt_msg, required=bool(v.get('required')), initial_value=initial_value,
							default_width=entry_width, lookup_list=lookups, validation_regex=v.get('validation_regex'),
	...
	if btn:
		for e in entries:
			if e.value:
				...
				working_script.add_substitution(e.name, value)
...
metacommands.append(MetaCommand(
	ins_table_rxs(r'^\s*PROMPT\s+ENTRY_FORM\s+', ins_table_rxs(r'\s+MESSAGE\s+"(?P<message>.+)"(?:\s+DISPLAY\s+', r')?\s*$', suffix="disp")),
	x_prompt_entryform))
...
metacommands.append(MetaCommand(
	ins_table_rxs(r'^\s*PROMPT\s+MESSAGE\s+"(?P<message>.+)"\s+DISPLAY\s+', r'\s*$'), x_prompt))


#####	PROMPT OPENFILE SUB
def x_prompt_openfile(**kwargs):
	try:
		prompt_openfile(kwargs['match'])
	except (ErrInfo, SystemExit):
		raise
	except:
	...
	if btn_val is None or (btn_val == 1 and selected_row is None):
		...
		if btn_val == 1:
			for i, item in enumerate(selected_row):
				...
				item = unicode(item)
				match_str = u"@" + hdrs[i]
				working_script.add_substitution(match_str, item)
				exec_log.log_status_info(u"Substitution string %s set to {%s} on line %d of %s" % (match_str, item, line_no, script))
	return None

metacommands.append(MetaCommand(
	ins_table_rxs(r'^\s*PROMPT\s+SELECT_SUB\s+', r'\s+MESSAGE\s+"(?P<msg>.+)"(?:\s+(?P<cont>CONTINUE))?\s*$'), x_prompt_selectsub))


#####	SUB
def x_sub(**kwargs):
	working_script.add_substitution(kwargs['match'], kwargs['repl'])
	return None
...
#####	RM_SUB
def x_rm_sub(**kwargs):
	working_script.remove_substitution(kwargs['match'])
	return None
...
def x_subdata(**kwargs):
	...
	working_script.add_substitution(subvar, dataval)
	return None
...
def x_selectsub(**kwargs):
	...
	for subvar in hdrs:
		subvar = u'@'+subvar
		if working_script.sub_exists(subvar):
			working_script.remove_substitution(subvar)
...
def x_sub_encrypt(**kwargs):
	working_script.add_substitution(kwargs['subvar'], Encrypt().encrypt(kwargs['plaintext']))
	return None
...
def x_sub_decrypt(**kwargs):
	working_script.add_substitution(kwargs['subvar'], Encrypt().decrypt(kwargs['crypttext']))
	return None
...
metacommands.append(MetaCommand(r'^\s*WRITE\s+"(?P<text>.*)"(?:(?:\s+(?P<tee>TEE))?\s+TO\s+(?P<outputname>[a-zA-Z0-9:._/\-\\]+))?\s*$', x_write))
...
metacommands.append(MetaCommand(r'^\s*AUTOCOMMIT\s+(?P<setting>ON|OFF)\s*$', x_autocommit))
...
def x_use(**kwargs):
	...
	working_script.add_substitution("$CURRENT_DBMS", dbs.aliased_as(db_alias).type.dbms_id)
	working_script.add_substitution("$CURRENT_DATABASE", dbs.aliased_as(db_alias).name())
	return None
...
def x_sub_tempfile(**kwargs):
	working_script.add_substitution(kwargs["match_string"], tempfiles.new_temp_fn())
...
def x_cancel_halt(**kwargs):
	...
	status.cancel_halt = flag == 'on'
	return None

metacommands.append(MetaCommand(r'^\s*CANCEL_HALT\s+(?P<onoff>ON|OFF)\s*$', x_cancel_halt))
...
metacommands.append(MetaCommand((
		r'^\s*PROMPT(?:\s+MESSAGE\s+"(?P<message>.+)")?\s+CONNECT\s+AS\s+(?P<alias>\w+)\s*$',
		r'^\s*CONNECT\s+PROMPT(?:\s+MESSAGE\s+"(?P<message>.+)")?\s+AS\s+(?P<alias>\w+)\s*$',
		r'^\s*PROMPT(?:\s+"(?P<message>.+)")?\s+CONNECT\s+AS\s+(?P<alias>\w+)\s*$',
		r'^\s*CONNECT\s+PROMPT(?:\s+"(?P<message>.+)")?\s+AS\s+(?P<alias>\w+)\s*$',
		), x_prompt_connect))
...
#####	SHOW SCRIPT
def x_showscript(**kwargs):
	script_id = kwargs["script_id"]
	lines = working_script.script_lines(script_id)
	for line in lines:
		output.write(u"%s\n" % line)

metacommands.append(MetaCommand(r'^\s*SHOW\s+SCRIPT\s+(?P<script_id>\w*)\s*$', x_showscript))
...
def x_debug_log_config(**kwargs):
	...
	exec_log.log_status_info("Config; GUI wait on exit = %s" % conf.gui_wait_on_exit)
...
def x_debug_write_config(**kwargs):
	...
	ofile.write(u"Config; GUI wait on exit = %s\n" % conf.gui_wait_on_exit)
...
def read_sqlscript(scriptfile_obj, script_dict, script_id, fn, nestlevel_obj):
	...
	#	* END SCRIPT
	#includeline = re.compile(r'^--\s*!x!\s*INCLUDE\s+("?)(?P<filename>\w+)\1\s*$', re.I)
	beginscript = re.compile(r'^--\s*!x!\s*BEGIN\s+SCRIPT\s+(?P<scriptname>\w+)\s*$', re.I)
				...
				if not is_comment_line:
					...
					if m:
						...
						if begs:
							...
							nestlevel_obj.level += 1
							read_sqlscript(scriptfile_obj, script_dict, scriptname, fn, nestlevel_obj)
						elif ends:
							# This is an END SCRIPT metacommand
							nestlevel_obj.level -= 1
							if nestlevel_obj.level < 0:
								raise ErrInfo(type="cmd", command_text=line, other_msg=u"Unmatched END SCRIPT metacommand on line %d of %s." % (scriptfile_obj.lno, scriptfile_obj.filename))
							ended = True
						else:
							# This is a non-IMMEDIATE metacommand
							sqllist.append(SqlCmd(fn, scriptfile_obj.lno, 'cmd', m.group('cmd').strip()))
					else:
...
def read_sqlscriptfile(fn, script_dict, script_id, nesting_level):
	...
	read_sqlscript(f, script_dict, script_id, fn, nesting_level)
...
def prompt_openfile(sub_name):
	# Use the standard file open dialog to prompt for an existing file and assign the
	# path and file name to the given substitution string.
	import Tkinter as tk
	import tkFileDialog
	working_script.remove_substitution(sub_name)
	script, lno = working_script.current_script_line()
	working_dir = os.path.dirname(os.path.abspath(script))
	win = tk.Tk()
	win.withdraw()
	fn = tkFileDialog.askopenfilename(initialdir=working_dir, parent=win, title="Filename for %s" % script)
	win.destroy()
	if not fn:
		if status.cancel_halt:
		...
		working_script.add_substitution(sub_name, fn)
		exec_log.log_status_info("Substitution string %s set to filename %s at line %d of %s" % (sub_name, fn, lno, script))

def prompt_savefile(sub_name):
	# Use the standard file open dialog to prompt for the name of a new or existing file and assign the
	# path and file name to the given substitution string.
	import Tkinter as tk
	import tkFileDialog
	working_script.remove_substitution(sub_name)
	...
	win = tk.Tk()
	win.withdraw()
	fn = tkFileDialog.asksaveasfilename(initialdir=working_dir, parent=win, title="Filename for %s" % script)
	win.destroy()
	if not fn:
		if status.cancel_halt:
...
def prompt_directory(sub_name):
	# Use the standard file open dialog to prompt for the name of a directory and assign the
	# path and file name to the given substitution string.
	import Tkinter as tk
	import tkFileDialog
	working_script.remove_substitution(sub_name)
	...
	win = tk.Tk()
	win._root().withdraw()
	fn = tkFileDialog.askdirectory(initialdir=working_dir, parent=win, mustexist=True, title="Directory for %s" % script)
	win.destroy()
	if not fn:
		if status.cancel_halt:
			msg = u"Halted from prompt for directory"
			exec_log.log_exit_halt(script, lno, msg)
			exit_immediately(2, None)
	else:
		working_script.add_substitution(sub_name, fn)
		exec_log.log_status_info("Substitution string %s set to directory %s at line %d of %s" % (sub_name, fn, lno, script))

def pause(message=None, max_time=None):
	if message:
		#sys.stdout.write(message+"\n")
		output.write(message+"\n")
	#sys.stdout.write("Press <Enter> to continue, <Esc> to quit. ")
...
def prettyprint_rowset(colhdrs, rows, output_dest, append=False, nd_val=u'', desc=None):
	...
	maxen = [max(0, len(colhdrs[j]), *(len(unicode(rows[i][j])) for i in rrows)) for j in rcols]
	names = u' '+u' | '.join([colhdrs[j].ljust(maxen[j]) for j in rcols])
	sep = u'|'.join([u'-'*(maxen[j]+2) for j in rcols])
	rows = [names, sep] + [u' '+u' | '.join(
			[unicode(rows[i][j] if rows[i][j] else nd_val).ljust(maxen[j])
...
def old_export_latex(outfile, hdrs, rows, append=False, querytext=None, desc=None):
	global conf
	script, lno = working_script.current_script_line()
	if outfile.lower() == 'stdout':
		f = output
	...
	if exit_status == 1:
		try:
			...
			if cmd:
				...
				raise ErrInfo(type="other", other_msg=u"Could not connect to %s" % db)
		else:
			dbs.add(alias, db)
	else:
...
def main():
	...
	if args and (len(args) == 2):
		if conf.db_type in ('a', 'l', 'd'):
			...
			if conf.server:
				...
				fatal_error(u'Command-line argument "%s" can\'t be interpreted.' % args[1])
	...
	global working_script
	working_script = ScriptCommands(script_name, subvars)
	...
	working_script.run()
	dbs.do_rollback = False
	if conf.gui_wait_on_exit:
...
if __name__ == "__main__":
	try:
		...
		exit_immediately(1, e)
		#if exec_log:
		#	exec_log.log_exit_error(e.write())
		#else:
		#	e.write()
		...
		msg2 = u"Uncaught exception %s (%s)" % (unicode(sys.exc_info()[0]), unicode(sys.exc_info()[1]))
		if exec_log:
			exec_log.log_exit_exception(msg2)
		output.write_err(u"%s\n" % msg1)
		exit_immediately(1, None)
