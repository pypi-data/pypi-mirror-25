...
class AppDescription(Jsonizable):
    ...
    def _validate_input_parameter(self, params):
        return copy.deepcopy(params) if isinstance(params, InputParameter) else InputParameter(params)
    ...
    def _validate_output_parameter(self, params):
        return copy.deepcopy(params) if isinstance(params, OutputParameter) else OutputParameter(params)
    ...
    def setproperty(self, key, value):
        ...
        if key == 'InputParameters' and isinstance(value, dict):
            for name, params in value.items():
                if not isinstance(params, InputParameter):
                    value[name] = InputParameter(value[name])
            new_value = value
        elif key == "OutputParameters" and isinstance(value, dict):
            for name, params in value.items():
                if not isinstance(params, OutputParameter):
                    value[name] = OutputParameter(value[name])
            new_value = value
        else:
            new_value = value
