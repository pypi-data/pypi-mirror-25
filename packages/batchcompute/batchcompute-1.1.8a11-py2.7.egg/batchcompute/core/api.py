...
from batchcompute.utils import (
    ...
    get_api_version, mock_pop_headers,
)
...
class Api(object):
    ...
    def __init__(self, endpoint, access_id, secret_key, auth_token, security, mock_pop=False, **headers):
        ...
        if mock_pop:
            self.port = constants.SERVICE_PORT_MOCKED
        elif not mock_pop and security:
            self.port = constants.SECURITY_SERVICE_PORT
        elif not mock_pop and not security:
            self.port = constants.SERVICE_PORT

        self.mock_pop = mock_pop
    ...
    def pop_mock_needed(self):
        return self.mock_pop
