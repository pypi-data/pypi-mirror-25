...
def read(files, verbose=False, ignore_read_error=False,
         ...
         follow_symlinks=False, um=None, chunk=True, _debug=False):
...
:Returns:
    ...
    out: `cf.FieldList`
        A list of the fields fpound in the input file(s).
...
If more than one field is found in the input files then an error is
raised. 

`cf.read` should be used unless it is known that only one field will
be created from the input file(s).
