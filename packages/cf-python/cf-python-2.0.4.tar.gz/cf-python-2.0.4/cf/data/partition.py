...
#            # file.
        elif (FileArray is not None and isinstance(subarray, FileArray)):
            # This partition contains a non-temporary file which is
            # not referenced by any other partition, so close the
            # file if the count of partitions pointing to it is 0.
            count = Partition.file_counter.get(subarray.file, None)
...
'''
    ...
    def _add_to_file_counter(self, i):
        '''Add i to the Partition.file_counter if self._subarray is an
...
:Returns:
    ...
    `None`
        ...
        '''
        if hasattr(self, '_subarray'):
            subarray = self._subarray
            if (not isinstance(subarray, _TempFileArray) and
                isinstance(subarray, FileArray)):
                filename = subarray.file
                if filename is not None:
                    count = Partition.file_counter.get(filename, 0)
                    Partition.file_counter[filename] = count + i
    #--- End: def
...
>>> p.close()
        ...
        config = self.config
        if _debug:
            print 'Partition.close: config =',config
        ...
        if config['serial']:
            ...
            # --------------------------------------------------------
            if _debug:
                print 'Partition.close: serial'
            if config['readonly']:
                if originally_on_disk:
                    if config.get('to_disk', False):
                        # 1.1.1.1 Original subarray was on disk and we
                        #         don't want to keep the current
                        #         subarray
                        if _debug:
                            print 'Partition.close: 1.1.1.1 revert'
                        self.revert()
                    elif FREE_MEMORY() <= FM_THRESHOLD():
                        # 1.1.1.2 Original subarray was on disk but
                        #         there is not enough memory to keep
                        #         the current subarray
                        if _debug:
                            print 'Partition.close: 1.1.1.2 revert ({} <= {})'.format(FREE_MEMORY(), FM_THRESHOLD())
                        self.revert()
                    else:
                        # 1.1.1.3 Original subarray was on disk and
                        #         there is enough memory to keep the
                        #         current subarray
                        ...
                        if _debug:
                            print 'Partition.close: 1.1.1.3 del masked ({} > {})'.format(FREE_MEMORY(), FM_THRESHOLD())
                        del self.masked       
                        pass

                else:
                    if _debug:
                        print 'Partition.close: subarray originally in memory'
                    if config.get('to_disk', False):
                        # 1.1.2.1 Original subarray was in memory and
                        #         we don't want to keep the current
                        #         subarray in memory
                        if _debug:
                            print 'Partition.close: 1.1.2.1 to_disk'
                        ...
                        if _debug:
                            print 'Partition.close: 1.1.2.2 to_disk'
                        self.to_disk(reopen=False)
                    else:
                        ...
                        if _debug:
                            print 'Partition.close: 1.1.2.3 pass'
                        pass
            else:
                ...
                if originally_on_disk:
                    if config.get('to_disk', False):
                        ...
                        if _debug: print 'Partition.close: 1.2.1.1 to_disk'
                        self.to_disk(reopen=False)
                    elif FREE_MEMORY() <= FM_THRESHOLD():
                        ...
                        if _debug: print 'Partition.close: 1.2.1.2 to_disk'
                        self.to_disk(reopen=False)
                    else:
                        # 1.2.1.3 Original subarray was on disk and
                        #         there is enough memory to keep it
                        if _debug: print 'Partition.close: 1.2.1.3 pass'
                        del self.masked       
                        pass
                else:
                    if config.get('to_disk', False):
                        ...
                        if _debug: print 'Partition.close: 1.2.2.1 to_disk'
                        self.to_disk(reopen=False)
                    elif FREE_MEMORY() <= FM_THRESHOLD():
                        # 1.2.2.2 Original subarray was an in memory
                        #         but there is not enough memory to
                        #         keep it
                        if _debug: print 'Partition.close: 1.2.2.2 to_disk'
                        self.to_disk(reopen=False)
                    else:
                        # 1.2.2.3 Original subarray was in memory and
                        #         there is enough memory to keep it
                        if _debug: print 'Partition.close: 1.2.2.3 pass'
                        del self.masked       
                        pass
        else:
...
#        if _debug:
#            print '    unique_array =', unique_array, self._subarray
            ...
            for mask in auxiliary_mask:
                if mask.any():
                    if not masked:
                        ...
                        masked = True
      #              print '-------------------'
     #               print type(p_data.mask), type(mask)
    #                print p_data.mask
   #                 print mask
  #                  print '-------------------'
#                    kk = (mask | p_data.mask).array
 #                   print kk
