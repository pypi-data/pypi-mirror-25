...
from ..functions  import (CHUNKSIZE, FM_THRESHOLD, RTOL, ATOL,
                          FREE_MEMORY, SET_FREE_MEMORY, parse_indices,
                          _numpy_allclose, _numpy_isclose, abspath,
                          pathjoin, hash_array, get_subspace,
                          set_subspace, broadcast_array)
...
place.
    ...
    def __init__(self, data=None, units=None, fill_value=None,
                 hardmask=True, chunk=True, loadd=None, dt=False):
        '''**Initialization**
        ...
        if value is cf_masked:
            ...
            if not isinstance(value, Data):
                ...
                if value.Units.equivalent(self.Units):
                    ...
                    raise ValueError(
"Some bad units {!r}, {!r}".format(self.Units, value.Units))
            #--- End: if
...
>>> d.chunk(100000)
>>> d.chunk(100000, [2])
>>> d.chunk(100000, [2], total=[2])
>>> d.chunk(100000, [2], omit_axes=[3, 4])
...
#        print 'OTHER.UNITS =', other.Units
#        calendar_arithmetic = data0.Units.isreftime and other.Units.iscalendartime
#        print '0 other.partitions[()] =',other.partitions[()]
        # ------------------------------------------------------------
        # Bring other into memory, if appropriate.
        # ------------------------------------------------------------
        other.to_memory()
#        print '1 other.partitions[()] =',other.partitions[()]
        # ------------------------------------------------------------
...
#            print 'other.Units =',other.Units
#            print 'other.partitions[()] =',other.partitions[()]
...
#                    f_partitions.expand_dims(Paxis, i=True)
            ...
            # --------------------------------------------------------
            new_auxiliary_mask = []
#            print 'data0._auxiliary_mask=',data0._auxiliary_mask # dch
#            print  'original_shape0=', original_shape0
...
#            print 'data1._auxiliary_mask=',data1._auxiliary_mask # dch
...
'''
        if index:
            ...
            raise ValueError(
                "Canx only convert a {} array of size 1 to a Python scalar".format(
    self.__class__.__name__))
        ...
        Return *data1* or, if possible, ``data1.datum(0)``.
            ...
            if shape1 == shape0:
                ...
                for n, m in zip(shape1[::-1], shape0[::-1]):
                    if n != m and n != 1:
                        raise ValueError(
"Can't assign by where: Can't broadcast data with shape {} to shape {}".format(
    shape1, shape0))
            else:
                raise ValueError(
"Can't assign by where: Can't broadcast data with shape {} to shape {}".format(
            ...
            if value is None or value is cf_masked:
                ...
                if not isinstance(value, d.__class__):
                    ...
                    if value.Units.equivalent(d.Units):
                        ...
                        raise ValueError(
"Some bad units {!r}, {!r}".format(d.Units, value.Units))
                #--- End: if
