        ...
        def _linear_weights(self, axis, comp, weights_axes, auto=False):
            ...
            if axis in weights_axes:
                if auto:
                    return
                raise ValueError(
"Multiple weights specifications for {0!r} axis".format(self.axis_name(axis)))
...
:Parameters:
    ...
    weights: *optional*
        ...
        components=True)``. See `cf.Field.weights` for details.  By
        default *weights* is `None` meaning that the collapse is
        unweighted
          ...
          *Example:*
            To automatically detect the best weights available from
            the metadata: ``weights='auto'``. See `cf.Field.weights`
...
#            return self._list_method('collapse', kwargs2)
        ...
        if _debug:
            print '    Initial cell methods =', f.cell_methods
...
"Can't collapse: Can't set 'axes' when 'method' is CF cell methods-like string")
        ...
        for axes in all_axes:
            ...
            for axis in axes:
                if axis == 'area':
                    ...
                    if len(a) != 2:
                        raise ValueError("sdf  222222")
                    axes2.extend(a)
                elif axis == 'volume':
                    a = self.axes(('X', 'Y', 'Z'), **kwargs)
                    if len(a) != 3:
                        raise ValueError("sdf 3  ")
                    axes2.extend(a)
                else:
                    a = self.axis(axis, key=True, **kwargs)
                    if a is None:
                        raise ValueError("i lhsb ouyvkjh m")
                    axes2.append(a)
            #--- End: for
        ...
        for method, axes, within, over in izip(all_methods, all_axes,
            ...
            if method2 is None:
                raise ValueError(
                    "Can't collapse: Unknown method: {0!r}".format(method))
...
#            size = reduce(operator_mul, f.axes_sizes(collapse_axes).values(), 1)
            collapse_axes_sizes = [int(x) for x in collapse_axes.values()]
#            collapse_axes_sizes = [int(x) for x in collapse_axes]
...
"Can't calculate {0} from fewer than {1} values".format(
    _collapse_cell_methods[method], min_size))
            ...
            if grouped_collapse:
                ...
                f = f._grouped_collapse(method, collapse_axes.keys()[0],
                                        within=within,
                                        over=over,
...
"Can't collapse: Bad parameter value: coordinate={0!r}".format(coordinate))
            ...
            # --------------------------------------------------------
            if _debug:
                print '    Initial cell methods  =', f.CellMethods
            ...
            if cell_method is not None:
                ...
                string = '{0}: {1}'.format(': '.join(collapse_axes), cell_method)
                cm = CellMethod(string)
                cm.axes = collapse_axes

                cms = f.CellMethods
                if not cms or not cms[-1].equivalent(cm):
                    cms.append(cm)
            #--- End: if

            if _debug:
                print '    Modified cell methods =', f.CellMethods
        #--- End: for
...
'''        
            ...
            try:
                ...
                raise ValueError(
                    "Can't collapse: Bad parameter value: {0}={1!r}".format(
                        parameter, selection))
            
            for c in iterator:
                if not isinstance(c, Query):
                    raise ValueError(
"Can't collapse: {0} sequence contains a non-{1} object: {2!r}".format(
...
"Must set the within_years parameter for a 'within years' climatalogical time collapse")
        ...
        # --------------------------------------------------------
        # Update the cell methods after grouped collapse
        # --------------------------------------------------------
        if _debug:
            print '        Initial cell methods =', f.cell_methods
...
#                print 'lastcm = ', repr(lastcm), repr(_collapse_cell_methods.get(lastcm.method)),  _co#llapse_cell_methods.get(lastcm.method) == _collapse_cell_methods.get(method)
#                print lastcm.axes ,(axis,) ,                    lastcm.axes   == (axis,)
#                print  lastcm.within
#                print  lastcm.over
                if (_collapse_cell_methods.get(lastcm.method) == _collapse_cell_methods.get(method)  and
...
#                print 'lastcm = ', repr(lastcm), repr(_collapse_cell_methods.get(lastcm.method)),  _c#ollapse_cell_methods.get(lastcm.method) == _collapse_cell_methods.get(method)
#                print lastcm.axes ,(axis,) ,                    lastcm.axes   == (axis,)
#                print  lastcm.within
#                print  lastcm.over
        ...
        #--- End: if
        if _debug:
            print '        End of grouped collapse cell methods =', f.cell_methods
...
:Parameters:
    ...
    kwargs: *optional*
        ...
        field. The string may be any string value allowed by *items*
...
(slice(0, 3, 1), slice(0, 5, 1), slice(0, 110, 1), slice(48, 106, 1))
        ...
        if _debug:
            print 'Field.indices:'
            print '    exact, envelope, full, compress =', exact, envelope, full, compress
...
"No {!r} axis indices found from: {}".format(identity, value))
                ...
                if contain:
                    ...
                    delete = [
                        ...
                        if not matplotlib_path_Path(zip(*vertices)).contains_point(points2)
                    ]
                    if delete:
    ...
    {+i}
        ...
        for k in sections:
            ...
            if masked or (mode == 'constant' and numpy_isnan(cval)):
                output_array = numpy_ma_MaskedArray(output_array,
                                                mask=numpy_isnan(output_array))
            #--- End: if
...
"Can't roll: {!r} axis has non-periodic dimension coordinates".format(
    ...
    def where(self, condition, x=None, y=None, i=False, item=None, **kwargs):
...
#            return self._list_method('where', kwargs2)
        ...
        if isinstance(condition, self_class):
            ...
            if item is None and not kwargs: 
                ...
                if key is None:
                    raise ValueError("No item could be found from {}".format(kwargs2))
...
**Defining a subspace in domain-space**

Defining a subspace in domain-space has the following features
...
* The field may be subspaced according to condiations on
  multidimensional items.
...
* Three types of subspace are available:
  ...
  * An "envelope" subspace is the smallest subspace which contains the
    selected elements and forms a contiguous bloack of te original
...
To assign to a subspace defined in domain-space, the equivalent
...
longitude(192)) m s-1>]
    ...
    def regrids(self, dst, method=None, src_cyclic=None,
                dst_cyclic=None, use_dst_mask=False, fracfield=False,
    ...
    method: `str`
        ...
        one of:
          ...
          *method*            Description
          ...
          ``'conservative'``  First-ordercConservative regridding will
                              be used (requires both of the fields to
                              have contiguous bounds).
...
#            return self._list_method('regrids', kwargs2)
        ...
        if method not in ('conservative', 'bilinear', 'nearest_stod', 'nearest_dtos'):
        ...
        for k in section_keys:
            ...
            if numpy_ma_is_masked(src_data):
                mask = src_data.mask
...
#            f.varray
    ...
    def regridc(self, dst, axes, method=None, use_dst_mask=False,
...
:Returns:
    ...
    out: `cf.{+Variable}`
        ...
        if not cyclic and not one_sided_at_boundary:
            d = numpy_ma_MaskedArray(d, mask=numpy_isnan(d))
        #--- End: if
