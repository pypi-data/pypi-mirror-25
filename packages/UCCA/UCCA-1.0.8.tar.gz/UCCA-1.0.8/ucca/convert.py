...
def from_json(lines, *args, **kwargs):
    ...
    passage = core.Passage(d["id"])
    l0 = layer0.Layer0(passage)
    for token in d["tokens"]:
        l0.add_terminal(text=token["text"], punct=token["requires_annotation"], paragraph=1)
    l1 = layer1.Layer1(passage)
    ...
    for unit in d["annotation_units"]:
        parent = l1.by_id(unit["parent_id"])
        node = l1.add_fnode(parent, unit["categories"][0]["name"], implicit=(unit["type"] == "IMPLICIT"))
        for category in unit["categories"][1:]:
            parent.add(category["name"], node)
    return passage


def to_json(passage, *args, **kwargs):
