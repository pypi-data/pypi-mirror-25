...
class ApiClientBase(object):
    ...
    def http_delete(self, url):
        if self.for_app_engine:
            ...
            response = requests.delete(self.base_url + url, auth=self.requests_auth)
            return self._process_response(response, response.content)
        else:
            with self._get_client() as _client:
                _client.request(method="DELETE", url=self.base_url + url, headers=self.headers)
                _response = _client.getresponse()
                return self._process_response(_response, _response.read())
...
class EntityClientBase(ApiClientBase):
    ...
    def create_bulk(self, entity):
        self._validate()
        return self.http_post_json(self.entity_endpoint_base_url + '/_bulk', entity)
    ...
    def delete_bulk(self, id):
        self._validate()
        return self.http_delete(self.entity_endpoint_base_url + '/_bulk')
