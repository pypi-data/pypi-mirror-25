...
def test_get_sso_redirect_with_invalid_email(sso_client: SSOClientUtils):
    expected = 'http://www.example.com/discourse/sso?' \
               'sso=bm9uY2U9Y2I2ODI1MWVlZmI1MjExZTU4YzAwZmYxMzk1ZjBjMGImcmVxdWlyZV9hY3RpdmF0aW9uPXRydWU%3D&' \
...
def test_decode_sso_data(sso_client: SSOClientUtils):
    user_data = sso_client.decode_client_data(b'bm9uY2U9Y2I2ODI1MWVlZmI1MjExZTU4YzAwZmYxMzk1ZjBjMGImbmFtZT1zYW0mdXNlcm5hbWU9\nc2Ftc2FtJmVtYWlsPXRlc3QlNDB0ZXN0LmNvbSZleHRlcm5hbF9pZD1oZWxsbzEyMyZyZXF1aXJl\nX2FjdGl2YXRpb249dHJ1ZQ==\n')
    print('MIK' + sso_client.get_param(user_data, 'nonce'))
    assert sso_client.get_param(user_data, b'nonce') == b'cb68251eefb5211e58c00ff1395f0c0b'
...
def test_generate_nonce(nonce_service: NonceService):
    n = nonce_service.generate_nonce()
    assert n is not None


def test_invalidate_nonce(nonce_service: NonceService):
    n = nonce_service.generate_nonce()
    assert nonce_service.is_nonce_valid(n) is True
    nonce_service.invalidate_nonce(n)
    assert nonce_service.is_nonce_valid(n) is False
