from setuptools import setup

setup(name="vpp_config",
      version="0.0.4a1",
      author="John DeNisco",
      author_email="jdenisco@cisco.com",
      description="VPP Configuration Utility",
      license = 'Apache-2.0',
      keywords="vppconfig",
      url = 'https://wiki.fd.io/view/VPP/VPP_CONFIG',
      py_modules=['vpp_config'],
      packages=['lib'],
      scripts=['scripts/vpp-config'],
      data_files=[('vpp_config/scripts', ['scripts/dpdk-devbind.py']),
                  ('vpp_config/configs', ['data/auto-config.yaml']),
                  ('vpp_config/dryrun/sysctl.d', ['data/80-vpp.conf.template']),
                  ('vpp_config/dryrun/default', ['data/grub.template']),
                  ('vpp_config/dryrun/vpp', ['data/startup.conf.template']),
      ],
      long_description="The VPP configuration utility can be used to easily configure VPP.",
      )
