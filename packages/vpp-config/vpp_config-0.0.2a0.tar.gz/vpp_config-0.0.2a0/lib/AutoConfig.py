...
from lib.VppPCIUtil import VppPCIUtil
from lib.VppHugePageUtil import VppHugePageUtil
from lib.CpuUtils import CpuUtils
from lib.VppGrubUtil import VppGrubUtil
from lib.QemuUtils import QemuUtils

__all__ = ["AutoConfig"]

DEFAULT_SYSTEM_CONFIG_FILENAME = "/usr/share/vpp/vpp-config/configs/system-config.yaml"
...
class AutoConfig(object):
    ...
    def _modify_cpu_questions(self, node, total_cpus, numa_nodes):
        ...
        print "\nYou can devote between 0 and 4 cpu cores to VPP" \
...
other than VPP? [0-{}][0]? '.format(str(total_cpus - 1))
        ...
        total_vpp_cpus = 0
        if max_vpp_cpus > 0:
            question = "How many core(s) shall we use for VPP workers[0-{}][0]? ". \
                format(max_vpp_cpus)
