...
VPP_DEFAULT_CONFIGURATION_FILE = '/usr/local/vpp/vpp-config/configs/auto-config.yaml'
VPP_SYSTEM_CONFIGURATION_FILE = '/usr/local/vpp/vpp-config/configs/system-config.yaml'
VPP_HUGE_PAGE_FILE = '/usr/local/vpp/vpp-config/dryrun/sysctl.d/80-vpp.conf'
VPP_STARTUP_FILE = '/usr/local/vpp/vpp-config/dryrun/vpp/startup.conf'
VPP_GRUB_FILE = '/usr/local/vpp/vpp-config/dryrun/default/grub'
...
def autoconfig_vpp_apply(node):
    ...
    diffs = autoconfig_diff(node, VPP_REAL_STARTUP_FILE, VPP_STARTUP_FILE)
    if diffs != '':
        print "These are the changes we will apply to"
...
def autoconfig_apply():
    ...
    if len(pkgs) == 0:
        print "VPP is not installed, Install VPP with option 4."
        return
...
def autoconfig_setup():
    ...
    if os.path.isfile(VPP_SYSTEM_CONFIGURATION_FILE) is True:
        acfg = AutoConfig(VPP_SYSTEM_CONFIGURATION_FILE)
    else:
    ...
    for i in nodes.items():
        ...
        if (os.path.isfile(VPP_STARTUP_FILE) is not True) and\
                (os.path.isfile(VPP_REAL_STARTUP_FILE) is True):
            autoconfig_cp(node, VPP_REAL_STARTUP_FILE, '{}'.format(VPP_STARTUP_FILE))
        if (os.path.isfile(VPP_HUGE_PAGE_FILE) is not True) and \
                (os.path.isfile(VPP_REAL_HUGE_PAGE_FILE) is True):
            autoconfig_cp(node, VPP_REAL_HUGE_PAGE_FILE, '{}'.format(VPP_HUGE_PAGE_FILE))
        if (os.path.isfile(VPP_GRUB_FILE) is not True) and\
                (os.path.isfile(VPP_REAL_GRUB_FILE) is True):
            autoconfig_cp(node, VPP_REAL_GRUB_FILE, '{}'.format(VPP_GRUB_FILE))
