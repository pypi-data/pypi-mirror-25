...
from lib.AutoConfig import AutoConfig
from lib.VPPUtil import VPPUtil

VPP_DEFAULT_CONFIGURATION_FILE = '/usr/share/vpp/vpp-config/configs/auto-config.yaml'
VPP_SYSTEM_CONFIGURATION_FILE = '/usr/share/vpp/vpp-config/configs/system-config.yaml'
VPP_HUGE_PAGE_FILE = '/usr/share/vpp/vpp-config/dryrun/sysctl.d/80-vpp.conf'
VPP_STARTUP_FILE = '/usr/share/vpp/vpp-config/dryrun/vpp/startup.conf'
VPP_GRUB_FILE = '/usr/share/vpp/vpp-config/dryrun/default/grub'
...
def autoconfig_hugepage_apply(node):
    ...
    diffs = autoconfig_diff(node, VPP_REAL_HUGE_PAGE_FILE, VPP_HUGE_PAGE_FILE)
    if diffs != '':
        ...
        # Copy and sysctl
        autoconfig_cp(node, VPP_HUGE_PAGE_FILE, VPP_REAL_HUGE_PAGE_FILE)
...
def autoconfig_vpp_apply(node):
    ...
    diffs = autoconfig_diff(node, VPP_STARTUP_FILE, VPP_REAL_STARTUP_FILE)
    if diffs != '':
        print "These are the changes we will apply to"
        ...
        autoconfig_cp(node, VPP_STARTUP_FILE, VPP_REAL_STARTUP_FILE)
    else:
...
def autoconfig_grub_apply(node):
    ...
    if answer == 'n':
        ...
        # Diff the file
        diffs = autoconfig_diff(node, VPP_REAL_GRUB_FILE, VPP_GRUB_FILE)
        if diffs != '':
            print "These are the changes we will apply to"
            ...
            autoconfig_cp(node, VPP_GRUB_FILE, VPP_REAL_GRUB_FILE)
            cmd = "update-grub"
            (ret, stdout, stderr) = VPPUtil.exec_command(cmd)
            if ret != 0:
                raise RuntimeError('{} failed on node {} {} {}'.
...
def autoconfig_setup():
    ...
    (ret, stdout, stderr) = VPPUtil.exec_command('ls {}'.format(VPP_SYSTEM_CONFIGURATION_FILE))
    if ret == 0:
        acfg = AutoConfig(VPP_SYSTEM_CONFIGURATION_FILE)
    else:
    ...
    print "\nBefore we change them, we'll create working copies in /usr/share/vpp/vpp-config/dryrun"
    ...
    for i in nodes.items():
        ...
        (ret, stdout, stderr) = VPPUtil.exec_command('ls {}'.format(VPP_STARTUP_FILE))
        if ret != 0:
            logging.debug(stderr)
            autoconfig_cp(node, filename, '{}'.format(VPP_STARTUP_FILE))
        filename = '/etc/sysctl.d/80-vpp.conf'
        (ret, stdout, stderr) = VPPUtil.exec_command('ls {}'.format(VPP_HUGE_PAGE_FILE))
        if ret != 0:
            logging.debug(stderr)
            autoconfig_cp(node, filename, '{}'.format(VPP_HUGE_PAGE_FILE))
        filename = '/etc/default/grub'
        (ret, stdout, stderr) = VPPUtil.exec_command('ls {}'.format(VPP_GRUB_FILE))
        if ret != 0:
            logging.debug(stderr)
            autoconfig_cp(node, filename, '{}'.format(VPP_GRUB_FILE))
