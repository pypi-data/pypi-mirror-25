...
class WhatStego(object):
    ...
    def get_flag_from_string(self, file_path):
        ...
        re_list = [
            (r'(?:key|KEY|flag|FLAG|ctf|CTF)+\{[^\{\}]{3,80}\}', re.S),
            (r'(?:key|flag|ctf)+\{[^\{\}]{3,80}\}', re.I),
            (r'(?:key|KEY|flag|FLAG|ctf|CTF)+[\x00-\x80]{3,80}', re.S),
            (r'(?:key|flag|ctf)+[\x00-\x80]{3,80}', re.I),
            # (r'[^\{\}]{0,20}\{[0-9a-zA-Z\-\_]{3,80}\}[^\{\}]{0,20}', re.S),
            (r'[^\{\}]{0,20}\{\w{3,80}\}[^\{\}]{0,20}', re.S),
        ]
    ...
    def png_check(self):
        if self.file_type == 'png':
            ...
            for t in out_list:
                t = t.strip()
                if t.startswith(b'chunk IDAT'):
                    try:
                        length = int(t.split(b' ')[-1])
