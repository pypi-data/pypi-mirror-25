...
class AccountCreate(generics.CreateAPIView):
    ...
    def post(self, request):
        if not settings.CONFIRM_EMAIL_ENABLED:
            request.data['is_confirmed'] = True
...
class PasswordReset(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        mail.send_mail(
            settings.RESET_PASSWORD_SUBJECT, settings.RESET_PASSWORD_BODY % token.key,
...
class ConfirmEmail(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        serializer.user.is_confirmed = True
        serializer.user.save()
