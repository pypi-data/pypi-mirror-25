...
class Dataset(object):
    ...
    def get_info(self,before=None,after=None):
        if before is not None:
            return {
                'title': self.title,
                'indices': self._indices[:before],
                'db_indices': self._db_indices[:before],
        ...
        return {
            'title': self.title,
            'indices': self._indices[after:],
            'db_indices': self._db_indices[after:],
            'middleware': self._middleware,
            'batch_size': self._batch_size,
            'size': self._size - after,
            'directory': self._dataset_dir
        }

    def split(self, train_part, test_part):
        divider = (self._size * train_part) // (test_part + train_part)
        return PartialDataset(self.get_info(before=divider)), PartialDataset(self.get_info(after=divider))
