...
class Daemon(object):
    ...
    """
    def __init__(self, pidfile=None, stdin=os.devnull,
                 stdout=os.devnull, stderr=os.devnull,
                 home_dir='.', umask=0o22, log_level=logging.WARNING,
    ...
    def log(self, data, level=logging.INFO):
    ...
    def daemonize(self):
        ...
        pid = str(os.getpid())
        with open(self.pidfile, 'w+') as pid_handle:
            pid_handle.write("%s\n" % pid)
    ...
    def get_pid(self):
        try:
            with open(self.pidfile, 'r') as pf:
                pid = int(pf.read().strip())
        except (IOError, SystemExit, ValueError):
            pid = None
        return pid
