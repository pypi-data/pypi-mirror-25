...
def get_user_c_data(u_id, item):
    ...
    userdata = mgdbfun.get_user_tbl('c').find_one(c_find_filter, item) or {}
    if not userdata:
        return {}
    if b_refine or park_refine:
        ...
        if 'b_img' in b_refine:
            ...
            b_refine.append('img')
        b_data = mgdbfun.get_user_tbl('b2').find_one(b_find_filter, b_refine) or {}
        if not pid:
            park_refine = []
            b_data = {}
    ...
    if park_refine:
        ...
        if park_name:
            ...
            park_name_dict = mgdbfun.get_park_info_tbl().find_one(park_find_filter, ['name'])
