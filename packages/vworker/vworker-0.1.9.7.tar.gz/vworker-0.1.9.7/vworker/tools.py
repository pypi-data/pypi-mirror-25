...
def get_user_c_data(u_id, item):
    ...
    userdata = mongofun.get_user_tbl('c').find_one(c_find_filter, item) or {}
    if not userdata:
        return {}
    if b_refine or park_refine:
        ...
        if 'b_img' in b_refine:
            ...
            b_refine.append('img')
        b_data = mongofun.get_user_tbl('b2').find_one(b_find_filter, b_refine) or {}
        if not pid:
            park_refine = []
            b_data = {}
    ...
    if park_refine:
        ...
        park_name = redisfun.redis_conn.hget(redies_key, park_id)
        if park_name:
            if isinstance(park_name,str):
                park_name = park_name.decode('utf-8')
        else:
            park_find_filter = {'_id': park_id}
            park_name_dict = mongofun.get_park_info_tbl().find_one(park_find_filter, ['name'])
            park_name = park_name_dict.get('name')
            redisfun.redis_conn.hset(redies_key, park_id, park_name)
