...
class TestWaitForUnits(unittest.TestCase):
    ...
    def test_errors_handling(self):
        ...
        # Because of the implementation, we can't guarantee the order of the
        # list in the first call to on_errors here:
        ...
        self.assertEqual(
            ...
            # [0][0][0] = first call, positional args, first positional arg
            sorted(on_errors.call_args_list[0][0][0], key=sort_key)
        )
        self.assertEqual(
            [{'Status': 'error', 'Name': 'django/0', 'Service': 'django'}],
            on_errors.call_args_list[1][0][0]
        )
...
class TestRaiseOnErrors(unittest.TestCase):
    ...
    def test_returned_callable(self):
        ...
        with self.assertRaises(ValueError) as cm:
            callback('bad wolf')
        self.assertEqual('bad wolf', cm.exception.args[0])
