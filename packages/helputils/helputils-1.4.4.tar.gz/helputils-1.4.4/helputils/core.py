...
def get_redirected(url, del_params=False):
...
def download(filename, download_link, timeout=6, proxies=None, tor=False):
    """Download file from given download link to given local filename path.

    timeout := Nearly all production code should use this parameter in nearly all requests. Failure to do so can cause your program to
               hang indefinitely:
    """
