...
def __process_record(ctx, odoo, record, context):
    ...
    if values:
        ...
        if not records:
            ids = [model.create(
                __process_values(ctx, odoo, model, 'create', values, fields, many2many, many2one, view_info, context))]
            click.secho('Creating a record with id=%s is successful' % ids[0], fg='green')
            records = model.browse(ids)
        else:
            ids = [x.id for x in records]
            records.write(
                __process_values(ctx, odoo, model, 'write', values, fields, many2many, many2one, view_info, context))
...
def yaml_load(ctx, datas):
    ...
    for data in datas:
        ...
        block = data.copy()
        if not set(['record', 'vars', 'message']) & set(block.keys()):
...
record:
    model: res.partner
    refs: Accepts domain [[...]], xml-id, name and ID  # else a new record will created if values
    ...
    functions:
        - func1
        - func2 arg1=value1 arg2=$value2$ => res1  # export the result of a function
    workflows:
        - signal1
        - signal2
    export:
