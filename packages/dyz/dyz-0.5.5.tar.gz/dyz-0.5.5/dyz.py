...
def __process_record(ctx, odoo, record, context):
    ...
    px = PrettyTable()
    px.field_names = ['ID', 'Display Name']
    for f in px.field_names:
        px.align[f] = "l"
    for px_record in records:
        px.add_row([px_record.id, px_record.display_name or '-'])
    click.secho('Found records of : %s, ref: %s, limit: %s' % (records._name, ref, limit), fg='blue')
    click.echo(px)
    # PROCESSING UNLINK
    if unlink:
        try:
