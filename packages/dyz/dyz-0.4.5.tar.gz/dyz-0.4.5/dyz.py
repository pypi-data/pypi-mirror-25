...
def process_domain(ctx, name, value):
    domain = []
    if value:
        for d in value:
...
def cli(ctx, database, host, port, user, password, superuserpassword, protocol, timeout, config, load, mode, export,
        prompt_login, prompt_connect, yes, no_context):
    """CLI for Odoo"""
    odoo = False
...
@click.pass_context
def truncate(ctx, model, domain, yes, batch_mode):
    """Truncate an object"""
    models = model
    odoo = ctx.obj['action_login']()
    for model in models:
...
@click.option('--yes', is_flag=True, default=False)
@click.pass_context
def update_data(ctx, model, values, domain, yes):
...
def __get_ref_records(ctx, odoo, model, ref, order, limit, context):
    ...
    if isinstance(ref, (list, tuple)) and isinstance(ref[0], basestring):
        new_ref = []
        for tmp_ref in ref:
            new_ref.append(yaml_eval(tmp_ref, context))
        ref = new_ref
    ...
    if isinstance(ref, basestring) and '.' in ref:
...
INT, CHAR, TEXT, FLOAT, BOOL, SELECTION = 'integer', 'char', 'text', 'float', 'boolean', 'selection'
...
def __process_values(ctx, odoo, model, mode, values, fields, many2many, view_info, context):
    ...
    for field, value in values:
        ...
        if field_type in [CHAR, TEXT]:
            ...
            if isinstance(value, (int, long)):
                ...
                ids = odoo.env[field_relation].search([('name', '=', value)])
                if len(ids) == 1:
                    vals[field] = ids[0]
                else:
                ...
                for line in value:
                    if len(line) != 3 and line not in ['&', '|', '!']:
                        ...
                        if len(line) == 3:
                            ...
                            domain.append(line)
                ids = odoo.env[field_relation].search(domain)
                if len(ids) == 1:
                    vals[field] = ids[0]
                else:
                    click.secho('Can not get values for %s on the field %s, model %s got %s ids' % (
            ...
            if isinstance(value, (int, long)):
                ...
                if value == '__ALL':
                    ids = odoo.env[field_relation].search([])
                    if ids:
            ...
            ctx.abort()
    all_onchange_fields = [__f for __f, __data in fields.items() if
                           __data.get('change_default', False) or __data.get('depends', False)]
    final_values = {}
    if mode == 'create':
        final_values = model.default_get(fields.keys())
    final_values.update(vals)
    all_fields_values = dict.fromkeys(fields.keys(), None)
    all_fields_values.update(final_values)
    changed_fields = final_values.keys()
    for field_name in list(set(changed_fields) & set(all_onchange_fields)):
        final_values.update(__onchange_values(model, all_fields_values, field_name, _onchange_spec(model, view_info)))
    return final_values
...
def __process_record(ctx, odoo, record, context):
    ...
    for tm2m_item in many2many:
        tm2m_data = tm2m_item.replace('=', '').split()
    ...
    # PROCESSING MESSAGE
    message_len = message and len(str(message).split('\n')[0]) or 0
    if message:
        click.secho('*' * message_len, fg='blue')
        click.secho(yaml_eval(message, context), fg='blue')
        click.secho('*' * message_len, fg='blue')
    ...
    if values:
        ...
        if not records:
            ids = [model.create(
                __process_values(ctx, odoo, model, 'create', values, fields, many2many, view_info, context))]
            click.secho('Creating a record with id=%s is successful' % ids[0], fg='green')
            records = model.browse(ids)
        else:
            ids = [x.id for x in records]
            records.write(__process_values(ctx, odoo, model, 'write', values, fields, many2many, view_info, context))
            click.secho('Updating %s records is successful' % (len(ids)), fg='green')
    ...
    for func_line in functions:
        func, func_export_name = func_line.split('=>', 1) if len(func_line.split('=>', 1) )>1 else (func_line, False)
        func_split = func.split(' ', 1)
        args = {}
        if len(func_split) == 2:
            func = func_split[0]
            func_splits = func_split[1].split(' ')
            for func_split in func_splits:
                if not '=' in func_split:
                    continue
                arg_key, arg_value = func_split.split('=')
                try:
                    arg_value = eval(arg_value)
                except:
                    pass
                args[arg_key] = yaml_eval(arg_value, context)
        click.secho('Execute the function %s with params %s' % (func, args), fg='blue')
        func_res = getattr(records, func)(**args)
        if func_export_name:
            context.update({func_export_name: func_res})
    ...
    for export_line in exports:
        if '=>' in export_line.lower():
            export_tuple = [x.strip() for x in export_line.split('=>')]
            export_expr, export_key = export_tuple
...
def load_yaml(ctx, datas):
    ...
    for data in datas:
        ...
        block = data.copy()
        if not set(['record','vars']) & set(block.keys()):
            click.secho('There are a problem in keys of the yaml file', fg='red')
            return
        blocks.append(block)
    context = dict(
        odoo.env.context,
