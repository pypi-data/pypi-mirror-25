...
from hunters.utils import decode_html
...
class BrowserConfig(object):
    ...
    def get_default_chrome_option(self):
        ...
        if os.name == 'posix':  #: Linux下只能用headless
            option.add_argument("--headless")
        elif self._headless:  #: 其他平台看配置
            option.add_argument("--headless")
...
class MiniBrowser(Browser):
    ...
    def __init__(self, browser_config=BrowserConfig()):
        ...
        self.session.headers['User-Agent'] = browser_config.user_agent
    ...
    def get(self, url, **kwargs):
        ...
        try:
            ...
            if "Set-Cookie" in r.headers:
                self._mrg_cookies(host, self.session.cookies.items())  #: 合并到会话级别, cookies中
            ...
            logger.exception("connect error:[%s] ,error[%s]" % (url, e))
            return r
        finally:
            b.mark("GET")
    ...
    def post(self, url, data=None, json=None, **kwargs):
        ...
        self._mrg_cookies(host, self.session.cookies.items())
        return self.__detect_encoding(r)

    def screenshot_as_png(self, filename):
        logger.warn("IGNORE screenshot_as_png for textBrowser")

    def window_size(self, width, height):
        logger.warn("IGNORE window_size for textBrowser")

    def scroll_by(self, x, y):
        logger.warn("IGNORE scroll_by for textBrowser")

    def wait(self, num):
        logger.warn("IGNORE scroll_by for textBrowser")

    def execute_js(self, js_str):
        logger.warn("IGNORE execute_js for textBrowser")

    def __detect_encoding(self, response):
        """ 尝试检测页面编码 """
        #: 'text/html; charset=GB2312'
        #: 'application/json'
        content_type = response.headers.get("Content-Type", "error/no-content-type")

        before_encoding = detect_encoding = response.encoding
