...
class Spider(object):
    ...
    def add_url(self, url, base_url=None, deep=0, headers=None, task_meta=None, extra={}):
        """
        ...
        :param deep: 当前深度
        :param task_id 用任务id区分url爬取任务. 可以实现对整体服务的重用, 比如每个task有独立的url(计数器, 深度), 跟总的计数器分开
        ...
        logger.info("Add To Queue [%s], origin:[%s], base_url:[%s] count[%s]", absolute_url, url, base_url, task_meta.count)
    ...
    def _mutli_thread_run(self, num, wait=True):
        ...
        try:
            ...
            if wait:
                ...
                for t in self._threadPool.values():
                    t.join()
...
class Tab(Browser, Response):
    ...
    def dom(self):
        ...
        if self._dom is None and Regex.RE_TYPE_HTML.search(self.headers.get("Content-Type")):
            text = self._response.text or ""
            if "" == text.strip():
                text = "<html></html>"
            try:
                self._dom = fromstring(text)  # cssselect
