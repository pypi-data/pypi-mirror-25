...
class MiniBrowser(Browser):
    ...
    def _check_cookies(self, host, response):
        if "Set-Cookie" in response.headers:
            self._mrg_cookies(host, self.session.cookies.items())  #: 合并到会话级别, cookies中
            #: 因为是单例, 如果单例以服务提供, 长期会导致cookie增大. 这里已经自己处理, 原来的就删除了.
    ...
    def get(self, url, **kwargs):
        ...
        try:
            ...
            logger.warning("connect error:[%s] ,error[%s]" % (url, e))
            return r
        finally:
            b.mark("GET")
