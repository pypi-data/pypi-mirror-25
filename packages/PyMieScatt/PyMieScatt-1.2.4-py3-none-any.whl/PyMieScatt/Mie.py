...
#  http://pymiescatt.readthedocs.io/en/latest/forward.html#Mie_SD
  ...
  if len(dp)<1000 and interpolate:
    dp = zoom(dp,10)
    ndp = zoom(ndp,10)
  ...
  Bext = trapz(Q_ext*aSDn,_logdp)
  Bsca = trapz(Q_sca*aSDn,_logdp)
  Babs = Bext-Bsca
  Bback = trapz(Q_back*aSDn,_logdp)
  Bratio = trapz(Q_ratio*aSDn,_logdp)
  bigG = trapz(g*Q_sca*aSDn,_logdp)/trapz(Q_sca*aSDn,_logdp)
  Bpr = Bext - bigG*Bsca
