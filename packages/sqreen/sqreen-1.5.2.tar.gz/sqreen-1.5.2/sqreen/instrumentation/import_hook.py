...
def get_original(module, module_class_name, module_object_name):
    ...
    if module_class is None:
        ...
        raise AttributeError(err_msg.format(module, module_class_name, dir(module)))

    # Get module object
    module_object = get_hook_parent(module_class, module_object_name)

    if module_object is None:
        err_msg = "Module '{!r}' class '{!r}'('{!r}'') has no attribute '{!r}'. Class attributes: {}"
        raise AttributeError(err_msg.format(module, module_class_name, module_class,
                                            module_object_name, dir(module_class)))
