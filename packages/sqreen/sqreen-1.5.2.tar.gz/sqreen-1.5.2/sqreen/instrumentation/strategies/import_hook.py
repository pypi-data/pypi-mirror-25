...
class ImportHookStrategy(BaseStrategy):
    ...
    def __init__(self, strategy_key, observation_queue, queue, import_hook, before_hook_point=None):
        super(ImportHookStrategy, self).__init__(observation_queue, queue,
    ...
    def hook(self):
        ...
        if self.hooked is False:
            self.import_hook.register_patcher(self.hook_module, self.hook_class,
                                              self.hook_name, self.import_hook_callback)
