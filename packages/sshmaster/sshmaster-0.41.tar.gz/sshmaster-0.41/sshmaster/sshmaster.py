...
def create_remote_file_multi_try(current_host, username, file_location, file_contents, file_mode, keyfile=None, key_string=None, file_owner='default', port=22, use_sudo=False, print_stdout=True, trys=10):
    """runs a commad via ssh on the host: current_host and returns a dictionary of the results
    :type current_host: string
    :param current_host: the host you want to run your command on
    :type username: string
    ...
    # create file
    ssh_cmd_multi_try(current_host=current_host, command=command_dict['create_file_command'], username=username, keyfile=keyfile, key_string=key_string, port=port, print_stdout=print_stdout, trys=trys)
    # execute file mode
    ssh_cmd_multi_try(current_host=current_host, command=command_dict['file_mode_command'], username=username, keyfile=keyfile, key_string=key_string, port=port, print_stdout=print_stdout, trys=trys)
    # change owners if requested
    if file_owner != 'default':
        ssh_cmd_multi_try(current_host=current_host, command=command_dict['file_owner_command'], username=username, keyfile=keyfile, key_string=key_string, port=port, print_stdout=print_stdout, trys=trys)


def scp_cmd(current_host, mode, username, local_file, remote_file, keyfile=None, key_string=None, recursive=False, preserve_times=False, port=22, file_owner='default', file_mode='default', use_sudo=False, print_stdout=True, trys=10):
    ...
    :param use_sudo: Wether to prefix all commands with 'sudo'
    :type print_stdout: boolean
    :param print_stdout: Wether to print the stdout to terminal for the commands to execute file mode, owner, etc commands
    :type trys: int
    ...
    if mode == 'put':
        if use_sudo:
            ...
            ssh_cmd_multi_try(current_host=current_host, command=command_dict['file_move_command'], username=username, keyfile=keyfile, key_string=key_string, port=port, print_stdout=print_stdout, trys=trys)
...
def scp_cmd_multi_try(current_host, mode, username, local_file, remote_file, keyfile=None, key_string=None, recursive=False, preserve_times=False, port=22, file_owner='default', file_mode='default', print_stdout=True, trys=10, use_sudo=False):
    ...
    while actual_trys != trys:
        ...
        try:
            scp_cmd(current_host=current_host, mode=mode, username=username, keyfile=keyfile, local_file=local_file, remote_file=remote_file, recursive=recursive, preserve_times=preserve_times, port=port, file_owner=file_owner, file_mode=file_mode, use_sudo=use_sudo, trys=trys )
...
def ssh_cmd(current_host, command, username, keyfile=None, key_string=None, port=22, print_stdout=True, use_sudo=False):
    ...
    :param username: the username to use when connecting
    :type keyfile: string
    :param keyfile: the absolute path to the private ssh key (Note: either keyfile or key_string must be provided, if both are provided, key_string will be tried first)
...
def ssh_cmd_multi_try(current_host, command, username, keyfile=None, key_string=None, port=22, print_stdout=True, use_sudo=False, trys=10):
