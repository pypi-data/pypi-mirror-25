...
class Structure(metaclass=TypeStructure):
    def __new__(cls, *args, **kwargs):
        ...
        self.__dict__.update([(k, getattr(cls, k)) for k in cls.__field_names__
...
class RangedInt(TypedField):
    ...
    def __check__(self, value):
        if self._min <= int(value) <= self._max:
            return int(value)
        raise ValueError("%s out of range (%s - %s)" % (value, self._min, self._max))
    ...
    def __repr__(self):
        return "<RangedInt:%s<=%s<=%s>" % (self._min, self.value, self._max)
