...
class Trainer (indexer.Indexer):
	...
	def rebuildIndex (self, breader):		
		...
		col = delune.collection (os.path.join (self.si.fs.getmaster (), "%s.ts" % self.si.alias), delune.APPEND, logger = self.si.logger)
	...
	def add_document (self, labeledDocument):
		...
		if self.indexer is None:
			...
			col = delune.collection (os.path.join (self.si.fs.getmaster (), "%s.ts" % self.si.alias), self.si.rebuild and delune.CREATE or delune.WRITE, self.si.analyzer, self.si.logger)
		...
		doc.add_field ("label", labeledDocument.label, delune.STRING)
		doc.add_field ("default", labeledDocument.text, delune.TEXT, lang = labeledDocument.lang )
