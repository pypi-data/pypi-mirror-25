...
class Segment:
	...
	def open (self, home, seg):
		...
		self.ti = _delune.TermInfo (self.fs_tii, self.fs_tis, bmode)
		self.ti.initialize ()
		
		self.tf = _delune.Posting (self.fs_tfq, self.fs_prx, bmode)
		self.tf.initialize ()
		
		self.fd = _delune.Document (self.fs_fdi, self.fs_fda, bmode)		
		self.fd.initialize ()
		
		self.sm = _delune.SortMap (self.fs_smp, bmode)
		self.sm.initialize ()
