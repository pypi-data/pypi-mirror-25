...
from delune.lib import logger as logs, flock, plock as _plock
import socket
...
class Segments:
	def __init__ (self, alias = ""):
		self.version = delune.__version__
		self.alias = alias
		self.N = 0
		self.segmentInfo = {}
...
class CollectionSetup:
	...
	def setReadingSegments (self):
		...
		self.lock.lock ("using-%s" % delune.PID, ",".join (segs))		
	
	def getReadingSegments (self):
		segs = self.lock.lockread ("using-%s" % delune.PID)
		if not segs: return []
	...
	def getReadingSegmentsAllProcesses (self):
		...
		for lname, ltime in locks:
			...
			isDeLuneServer = flock.isCurrentProcess (pid)
