...
def load_data (alias, numthreads, plock):
	...
	if 'classifier' in colopt:
		analyzer = delune.standard_analyzer (10000, numthreads, **colopt ["analyzer"])
		col = delune.model (colopt ["data_dir"], delune.READ, analyzer, plock = plock)
		actor = col.get_classifier (**colopt.get ('classifier', {}))
	else:
		analyzer = delune.standard_analyzer (8, numthreads, **colopt ["analyzer"])
		col = delune.collection	(colopt ["data_dir"], delune.READ, analyzer, plock = plock)	
...
@app.route ("/")
def index (was):
	return was.jstream ({'collections': list (delune.status ().keys ())})
...
def locks (was, alias):
	return was.jstream ({"locks": delune.get (alias).si.lock.locks ()})	
...
def query (was, alias, **args):
	...
	if type (q) is list:
		return was.jstream ([delune.query (alias, eq, o, f, s, w, r, l, analyze, data, limit = 1) for eq in q])
	return was.jstream (delune.query (alias, q, o, f, s, w, r, l, analyze, data, limit = 1))
...
def guess (was, alias, **args):
	...
	cond = args.get ("cond", '')
	if type (q) is list:
		return was.jstream ([delune.guess (alias, eq, l, c, top, cond) for eq in q])
	return was.jstream (delune.guess (alias, q, l, c, top, cond))
...
def cluster (was, alias, **args):
	...
	l = args.get ("l", 'en')	
	if type (q) is list:
		return was.jstream ([delune.cluster (alias, eq, l) for eq in q])	
	return was.jstream (delune.cluster (alias, q, l))
