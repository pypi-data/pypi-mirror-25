...
# 请求联动服务地址
service_url = 'http://pay.soopay.net/spay/pay/payservice.do'
...
class APIRequestor(object):
    ...
    def request_raw(self, method, params=None):
        post_data = urllib.urlencode(list(api_encode(params or {})))
    ...
    def interpret_response(self, rbody, rcode):
        ...
        try:
            ...
            if sign.verify(rbody_dict['plain'], rbody_dict['sign']):
                if rbody_dict.has_key('sign'):
                    del rbody_dict['sign']
                if rbody_dict.has_key('sign_type'):
                    del rbody_dict['sign_type']
                if rbody_dict.has_key('version'):
                    del rbody_dict['version']
                if rbody_dict.has_key('plain'):
    ...
    def _interpret_meta(self, rbody):
        ...
        try:
            ...
            content = soup.head.meta['content'].encode("utf-8")
