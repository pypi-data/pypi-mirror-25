...
def transport_handler(message):
    ...
    for annotation in span.get('annotations', []):
        annotation['endpoint'] = dict((get_javaname(k), v) for k, v in annotation.pop('host').iteritems() if v is not None)
    for binary_annotation in span.get('binary_annotations', []):
    ...
    def wapper(handler, *args, **kw):
        ...
        if existing_zipkin_attrs:
            with zipkin_span(
                ...
                add_binary_annotation(zipkin_context, request=handler.request, result=result)
                return result
        else:
            with zipkin_span(
                    service_name=service_name,
    ...
    def wrapper(*args, **kwargs):
        ...
        if existing_zipkin_attrs:
            with zipkin_span(
                ...
                zipkin_context.update_binary_annotations({'status_code': response.status_code})
                add_binary_annotation(zipkin_context, request=request, result=result)
                return result
        else:
            with zipkin_span(
                    service_name=service_name,
