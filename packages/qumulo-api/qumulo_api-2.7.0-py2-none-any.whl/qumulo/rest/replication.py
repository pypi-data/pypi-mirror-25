...
def create_source_relationship(
        ...
        target_path,
        address,
        should_disable_replication=False,
        target_port=None):

    body = {'source_root_id': source,
            'target_root_path': target_path,
            'target_address': address,
            'replication_enabled': not should_disable_replication}
    if target_port:
        body['target_port'] = target_port
...
@request.request
def enable_replication(conninfo, credentials, relationship_id):
    method = "POST"
    uri = "/v1/replication/source-relationships/{}/enable-replication"
    return request.rest_request(
        conninfo, credentials, method, uri.format(relationship_id))

@request.request
def disable_replication(conninfo, credentials, relationship_id):
    method = "POST"
    uri = "/v1/replication/source-relationships/{}/disable-replication"
    return request.rest_request(
        conninfo, credentials, method, uri.format(relationship_id))
