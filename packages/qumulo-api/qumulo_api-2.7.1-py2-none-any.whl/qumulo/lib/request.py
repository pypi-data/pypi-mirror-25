...
# QFS-14181: httplib doesn't set TCP_NODELAY on sockets. We set this for SSL
# sockets by monkeypatching ssl.wrap_socket. When Qumulo Core supports http
# (vs. https) we need an equivalent. Alternatively, we could bundle httplib
# modified, or switch to httplib2 or requests.
def _monkeypatch_nagle():
    old_wrap_socket = ssl.wrap_socket
    def new_wrap_socket(sock, *args, **kwargs):
        sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, True)
        return old_wrap_socket(sock, *args, **kwargs)
...
class Connection(object):
    ...
    def __init__(self, host, port, chunked=DEFAULT_CHUNKED,
        ...
        self.backdoor = host in LOCALHOSTS and os.geteuid() == 0
