...
def _to_unicode(s, charset='utf-8'):
    ...
    if 'MSIE' in ua:        
        if FILE_NORMALIZER and not IE_NORMALIZE:
            filename = IUserPreferredFileNameNormalizer(instance.REQUEST).normalize(
                    _to_unicode(filename, instance.getCharset()))
            base = filename
            ext = ""
            ...
            filename = urllib.quote(
                _to_unicode(filename, instance.getCharset()).encode('utf-8', 'replace'))
            header_value = contentDispositionHeader(
                            disposition='attachment',
                            filename=filename,
                            )
        else:
            filename = _to_unicode(filename, instance.getCharset()).encode('shift_jis', 'replace')
            m = emailMessage()
            disposition='attachment'
        ...
        filename = _to_unicode(filename, instance.getCharset()).encode(charset, 'replace')
        m = emailMessage()
        disposition='attachment'
        kw = {'filename' : filename}
        m.add_header('content-disposition', disposition, **kw)
...
def blob_wrapper_index_html(self, REQUEST=None, RESPONSE=None, charset='utf-8', disposition='inline'):
    """ make it directly viewable when entering the objects URL """
    ...
    if filename is not None:
        ...
        #     filename=filename)
        # RESPONSE.setHeader("Content-disposition", header_value)
        header_value = get_header_value(ua, filename, self)
        RESPONSE.setHeader("Content-disposition", header_value)
...
if IS_BLOB:
    ...
    field.BlobWrapper.index_html = blob_wrapper_index_html
