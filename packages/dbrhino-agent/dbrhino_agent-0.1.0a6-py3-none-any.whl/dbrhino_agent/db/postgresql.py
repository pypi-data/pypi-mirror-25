...
def create_user(cur, username, pw):
    ...
    """.format(username=username, password=pw))


def drop_role(cur, role):
...
class GrantOp(object):
    ...
    def render(self, **kwargs):
        return Template(self.jinja_templ).render(kwargs)
...
class DatabaseOp(GrantOp):
    def revoke_all(self):
        self.cur.execute("REVOKE ALL ON DATABASE {} FROM {}"
                         .format(self.catalog.database, self.username))

    valid_permissions = {"create", "connect", "temporary", "temp", "all"}
    jinja_templ = "GRANT {{perms}} ON DATABASE {{db}} TO {{user}}"

    def apply(self, stmt):
        self.validate(stmt)
        if statament.match is not None:
    ...
    def revoke_all(self):
        for schema in self.catalog.schemas:
            self.cur.execute("REVOKE ALL ON SCHEMA {} FROM {}"
                             .format(schema, self.username))

    valid_permissions = {"create", "usage", "all"}
    jinja_templ = "GRANT {{perms}} ON SCHEMA {{schema}} TO {{user}}"
...
class AllTablesInSchemaOp(GrantOp):
    ...
    jinja_templ = "GRANT {{perms}} ON ALL TABLES IN SCHEMA {{schema}} TO {{user}}"
...
class Postgresql(Database):
    def implement_grant(self, grant):
        logger.debug("implementing grant for {} in {}"
                     .format(grant.username, grant.database))
