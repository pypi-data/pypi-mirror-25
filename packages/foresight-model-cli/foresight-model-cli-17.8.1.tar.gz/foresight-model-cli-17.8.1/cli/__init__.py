...
class ForesightCli(object):
    ...
    _kube_ing_template_registry = {'predict': (['predict-ingress.yaml.template'], [])}
    ...
    def config(self):
        print("api_version: '%s'" % ForesightCli._pipeline_api_version)
    ...
    def clustered_yaml(self,
        ...
        model_clustered_template = os.path.join(template_path, ForesightCli._kube_clustered_template_registry['train'][0][0])
...
#        print("'%s' -> '%s'." % (filename, rendered_Dockerfile))
    ...
    def model_build(self,
        ...
        if build_type == 'docker':
            ...
            cmd = '%s build -t docker.qiotec.com:5555/predict-%s-%s-%s:%s --build-arg model_type=%s --build-arg model_name=%s --build-arg model_path=%s -f Dockerfile %s' % (docker_cmd, model_type, model_name, model_chip, model_tag, model_type, model_name, model_path, build_path)
    ...
    def model_yaml(self,
        ...
        model_predict_deploy_yaml_template_path = os.path.join(template_path, ForesightCli._kube_deploy_template_registry['predict'][0][0])
        ...
        model_predict_svc_yaml_template_path = os.path.join(template_path, ForesightCli._kube_svc_template_registry['predict'][0][0])
        print("'%s' -> '%s'." % (filename, rendered_filename))
        ...
        model_predict_ing_yaml_template_path = os.path.join(template_path,
                                                            ForesightCli._kube_ing_template_registry['predict'][0][0])
        print("'%s' -> '%s'." % (filename, rendered_filename))

        path, filename = os.path.split(model_predict_ing_yaml_template_path)
        rendered = jinja2.Environment(loader=jinja2.FileSystemLoader(path or './')).get_template(filename).render(
            context)
        rendered_filename = './%s-%s-%s-%s-ing.yaml' % (model_type, model_name, model_chip, model_tag)
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
        print("'%s' -> '%s'." % (filename, rendered_filename))

        model_predict_autoscale_yaml_template_path = os.path.join(template_path, ForesightCli._kube_autoscale_template_registry['predict'][0][0])
    ...
    def model_push(self,
        ...
        cmd = '%s push docker.qiotec.com:5555/predict-%s-%s-%s:%s' % (docker_cmd, model_type, model_name, model_chip, model_tag)
        process = subprocess.call(cmd, shell=True)
    ...
    def model_pull(self,
        ...
        cmd = '%s pull docker.qiotec.com:5555/predict-%s-%s-%s:%s' % (docker_cmd, model_type, model_name, model_chip, model_tag)
        process = subprocess.call(cmd, shell=True)
    ...
    def _model_deploy(self,
        ...
        full_model_url = "%s/api/%s/model/deploy/%s/%s" % (model_server_url.rstrip('/'), ForesightCli._pipeline_api_version, model_type, model_name)
    ...
    def _predict(self,
        ...
        full_model_url = "%s/api/%s/model/predict/%s/%s" % (model_server_url.rstrip('/'), ForesightCli._pipeline_api_version, model_type, model_name)
        print("")
    ...
    def _get_all_available_services(self):
        ...
        available_services = list(ForesightCli._kube_deploy_registry.keys())
    ...
    def _get_deploy_yamls(self, 
                          ...
                          git_version):
        try:
            (deploy_yamls, dependencies) = ForesightCli._kube_deploy_registry[service_name]
        except:
            dependencies = []
            deploy_yamls = []
    ...
    def _get_svc_yamls(self, 
                       ...
                       git_version):
        try:
            (svc_yamls, dependencies) = ForesightCli._kube_svc_registry[service_name]
        except:
            dependencies = []
            svc_yamls = []
