...
class Series(object):
    ...
    def next(self):
        ...
        with io.open(patch, 'r', encoding='utf-8') as fp:
            ...
            shutil.copy(os.path.join(self.directory, 'doc/index.rst'),
                        '/tmp/%02d-%s.txt' % (self.index, self.patches[self.index]))

            content = []
            for patch in p:
                hunks = []
                tpath = patch.target_file
                if tpath == '/dev/null':
                    continue
                for hunk in patch:
                    hunks.append(Hunk(
...
def apply(patchset, directory):
    ...
    def p(f):
        assert not f.startswith('/'), "Absolute paths are unsupported (%s)" % f
        if f.startswith(('a/', 'b/')):
            f = f[2:]
    ...
    for patch in patchset:
        if patch.target_file == '/dev/null':
            if patch.source_file != '/dev/null':
                os.remove(p(patch.source_file))
            continue

        source_lines = []
        if patch.source_file != '/dev/null':
            with io.open(p(patch.source_file), 'r', encoding='utf-8') as f:
        ...
        for hunk in patch:
            ...
            last = hunk.source_start + hunk.source_length
