...
from hpcli.hpclilib.common import MS_DEPLOY_URL_FRAGMENT, DRF_URL_ROOT
import os
...
class MicroServiceManager(object):
    ...
    def _get_docs_data(self):
        ...
        docs_files = {
            ...
            'Getting-Started.ipynb': 'get_started_url',
            'Use-Cases.ipynb': 'use_cases_url',
        }
    ...
    def _get_default_image_url(self):
        if 'icon-url' in self.config and self.config['icon-url']:
            return self.config['icon-url']
    ...
    def _get_friendly_name(self):
        ...
        # An oversite allowed 'friendly_name' which
        # is not consistent with the naming scheme.
        if 'friendly_name' in self.config:
            return self.config['friendly_name']
        if 'friendly-name' in self.config:
            return self.config['friendly-name']
        return ''

    def _get_drf_data(self):
        microservice_defn = self._get_post_data('')
        del microservice_defn['action']
        if 'x-api-key' in microservice_defn:
            del microservice_defn['x-api-key']
