...
def prepare_and_send(email_properties, smtp_server):
    ...
    msg['From'] = email_properties.get('from') or email_properties.get('From') or smtp_server.get('username')
    msg['To'] = email_properties.get('to') or email_properties.get('To')
    ...
    if email_properties.get('html_body'):
        msg.attach(MIMEText(email_properties.pop('body_html'), 'html'))

    if email_properties.get('html_text'):
        msg.attach(MIMEText(email_properties.pop('body_text'), 'plain'))
