...
from amino import IO, Left, Right, List, __, _, L, Empty, Just
...
class RecordCommand(Command):
    ...
    def spec(self):
        return (self._id_spec
                if self.args.head.exists(is_int) else
                self._data_spec)
    ...
    def _req(self, meth, path=Empty(), data=None):
        ...
        def task(i, body):
            return IO.delay(meth(self.client),
                              ...
                              body=body)
        def assemble(i, a):
            return cb(a).task('invalid arguments') // L(task)(i, _)
    ...
    def create(self, data: Callable=None, path=Empty()):
        ...
        sub = path / '/{}'.format | ''
        def assemble(path):
            return (cb(self.rest).task('invalid arguments') /
