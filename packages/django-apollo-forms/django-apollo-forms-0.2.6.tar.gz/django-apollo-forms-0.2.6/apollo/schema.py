...
class FormFieldType(DjangoObjectType):
    ...
    @graphene.resolve_only_args
    def resolve_input_type(self):
        return self.input_type

    @graphene.resolve_only_args
    def resolve_is_visible(self):
...
class CreateForm(graphene.Mutation):
    ...
    def mutate(root, args, context, info):
        ...
        form, form_created = Form.objects.update_or_create(name=name, defaults=dict(
            ...
            submit_button_text=data.get('submit_button_text'),
        ...
        for raw_field in data.get('fields'):
            ...
            field_layout_removed = raw_field_layout and raw_field_layout.get('id') and not layout.blocks.filter(id=raw_field_layout.get('id')).exists()
...
class CreateFormSubmission(graphene.Mutation):
    ...
    def mutate(root, args, context, info):
        try:
            Form.objects.get(id=args.get('form_id'))
        ...
        submission = FormSubmission(raw_data=args.get('submission_data'), form_id=args.get('form_id'))
        error_msg = False
...
class Query(graphene.AbstractType):
    ...
    @graphene.resolve_only_args
    def resolve_form_field_widget_choices(self):
        return [_[0].upper() for _ in FormFieldTemplate.FIELD_TYPES]

    @graphene.resolve_only_args
    def resolve_form_field_validation_rule_choices(self):
        return [_[0].upper() for _ in FormFieldTemplate.VALIDATION_RULES]

    @graphene.resolve_only_args
    def resolve_form_field_label_position_choices(self):
        return [_[0].upper() for _ in FormFieldTemplate.LABEL_POSITIONS]

    def resolve_all_form_field_templates(self, args, context, info):
        return FormFieldTemplate.objects.all()

    def resolve_form_field_template(self, args, context, info):
        id = args.get('id')

        return FormFieldTemplate.objects.get(id=id)

    @graphene.resolve_only_args
    def resolve_all_layouts(self):
        return Layout.objects.all()

    def resolve_form_field(self, args, context, info):
        id = args.get('id')

        return FormField.objects.get(id=id)

    @graphene.resolve_only_args
    def resolve_all_forms(self):
    ...
    def resolve_all_form_submissions(self, args, context, info):
        return FormSubmission.objects.filter(form_id=args.get('form_id')).select_related('form').all()

    def resolve_form_submission(self, args, context, info):
        id = args.get('id')
