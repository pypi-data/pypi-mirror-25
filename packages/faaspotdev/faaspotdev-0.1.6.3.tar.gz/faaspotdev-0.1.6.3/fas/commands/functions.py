...
class Functions(Command):
    ...
    def list(self):
        self.columns = ['id', 'name', 'runner_url']
        uri = 'functions/{0}/'.format(self.application)
        all = self.api.get(uri=uri)
        return all.get('items', [])

    @WaitCompletion(logger=logger)
    def create(self, name, source, runtime, public=False, context_file=None, **kwargs):
        self._validate_function_name(name)
        logger.debug('Going to create function `{0}` for `{1}`'.format(name, self.application))
        uri = 'functions/{0}/'.format(self.application)
        query_params = None
        encoded_file_content = self._encode_content(source)
    ...
    @WaitCompletion(logger=logger)
    def update(self, function_id, source=None, runtime=None, public=False, context_file=None, new_name=None, **kwargs):
        logger.debug('Going to update function: {0}'.format(function_id))
        if not (source or context_file or new_name):
            raise Exception('No action requested.')
        uri = 'functions/{0}/{1}'.format(self.application, function_id)
        query_params = None
        file_content = self._encode_content(source)
        context_file_content = self._encode_content(context_file)
        data = {}
        if file_content:
        ...
        if new_name:
            self._validate_function_name(new_name)
            data['name'] = new_name
        if runtime:
    ...
    def get(self, function_id, pretty_print=False):
        logger.debug('Going to retrieve function: {0}'.format(function_id))
        uri = 'functions/{0}/{1}'.format(self.application, function_id)
    ...
    @WaitCompletion(logger=logger)
    def delete(self, function_id, **kwargs):
        logger.debug('Going to delete deployment: {0}'.format(function_id))
        uri = 'functions/{0}/{1}'.format(self.application, function_id)
        response = self.api.delete(uri=uri)
        return response

    def run(self, function_id, parameters=None, **kwargs):
        """
        :param function_id:
        ...
        """
        logger.debug('Going to run `{0}`'.format(function_id))
        uri = 'functions/{0}/{1}/rpc/'.format(self.application, function_id)
        if isinstance(parameters, dict):
            ...
            data = None
        logger.debug('Going to run `{0}` with parameter: {1}'.format(function_id, data))
        wait = kwargs.get('wait', False)
        if wait:
            uri = 'sync/functions/{0}/{1}/rpc/'.format(self.application, function_id)
            return self.api.get(uri=uri, params=data)
        task_id = self.api.post(uri=uri, data=data)
        return task_id

    def run_bulk(self, function_id, parameters=None, **kwargs):
        """
        :param function_id:
        :param parameters: the input for the function.
            ...
            can be a list of [["key=value", ], ["key=value"]]
        """
        logger.debug('Goring to bulk run of `{0}`'.format(function_id))
        uri = 'functions/{0}/{1}/bulk_rpc/'.format(self.application, function_id)
