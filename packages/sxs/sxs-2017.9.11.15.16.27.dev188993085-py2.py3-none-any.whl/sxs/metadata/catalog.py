...
def symlink_runs(source_directory='Catalog', target_directory='CatalogLinks',
                 ...
                 use_relative_links=False, relative_directory_replacement=None, verbosity=1):
    ...
    use_relative_links: bool (default: False)
        ...
        names can change.
    relative_directory_replacement: (str, str) (default: None)
        If this path is not None, use relative links and do a regex replacement on the path matching
        element 0 of this pair with element 1.  This forces `use_relative_links` to be True.  Again,
        this can be helpful when the source and target directories are mounted as volumes in a
        docker container, but under different names.
    ...
    if relative_directory_replacement is not None:
    ...
    for name in catalog:
        ...
        if use_relative_links:
            ...
            if relative_directory_replacement is not None:
                # source_link = re.sub(relative_directory_replacement[0], relative_directory_replacement[1], source_link)
                print(source_link, relative_directory_replaced, relative_directory_replacement)
                source_link = source_link.replace(relative_directory_replaced, relative_directory_replacement)
        else:
