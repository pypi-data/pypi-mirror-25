...
class MP3(Audio):
  ...
  def write_tags(self, tags):
    ...
    for name, field in fields.items():
      setter = eval(field)
      value = tags[name]
      self.audio.add( setter( encoding=3, text=fstr("{value}") ) )
...
class Download(Base):
  ...
  @property
  def fullname(self):
    return fstr("{self.basename}.{self.extension}")
  ...
  def grab(self):
    ...
    logger.info( fstr("In: {self.url}") )
    logger.info( fstr("Out: {self.output}") )
    logger.info('')
  ...
  def release(self):
    ...
    for index, char in enumerate(title):
      try:
        ...
        return str( float( fstr("{consecutive}.{tenths}") ) )
...
class Enclosure(Base):
  ...
  def tag(self, audio):
    ...
    tags = {
      'album': ui.get('album', fstr("{podcast.title} ({self.episode.year})") ),
  ...
  def match(self):
    ...
    for root, dirs, files in os.walk(podcast.output):
      for file in files:
        ...
        logger.info( fstr("Matching episode '{episode.title}' against: '{file}'") )
  ...
  def download(self):
    ...
    logger.debug( fstr("Enclosure download url: {download.url}") )
    logger.debug( fstr("Enclosure download output: {download.output}") )
    if download.grab():
