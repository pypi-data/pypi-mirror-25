...
class Parser():
  ...
  def parse_episodes(self):
    ...
    if not self.entries:
      ...
      raise ParserException( fstr("Scope returned 0 episodes: {params}") )
    ...
    for index, entry in enumerate(self.entries):
      ...
      if enclosure_link:
        logger.debug( fstr("Parsing {enclosure_link}") )
      ...
      if not enclosure_link:
        for link in links:
          logger.debug( fstr("Parsing {link}") )
      ...
      for enclosure in episode.enclosures.order_by(Enclosure.priority):
        logger.debug( fstr("Parsing {enclosure.href}") )
        if audio:
          break
        if match:
