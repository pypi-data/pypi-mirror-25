...
class Polarbytebot:
    def __init__(self, signature, microservice_id, oauth_client_secret, oauth_username, oauth_client_id,
                 oauth_redirect_uri, refresh_token, praw_useragent, database_system, database_username, database_password,
                 database_host, database_dbname):
        self.microservice_id = microservice_id
        self.signature = signature

        self.reddit = praw.Reddit(client_id=oauth_client_id, client_secret=oauth_client_secret, refresh_token=refresh_token,
                                  redirect_uri=oauth_redirect_uri, user_agent=praw_useragent, username=oauth_username)
        self.reddit.auth.authorize()
        ...
        for tbsm in to_be_submitted:
            try:
                if tbsm.is_link:
                    ...
                    sub_obj = self.reddit.subreddit(tbsm.subreddit).submit(tbsm.title, text=tbsm.content)
