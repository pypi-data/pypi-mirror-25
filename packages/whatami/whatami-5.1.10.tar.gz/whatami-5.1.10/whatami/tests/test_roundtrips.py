...
TEST_CASES = ['simple', 'collections', 'nested', 'defaultdict']
...
def whatable2spectations(request):
    ...
    # defaultdicts in params
    elif request.param == 'defaultdict':
        an_empty_defaultdict = defaultdict(int)

        @whatable
        def anns(x=an_empty_defaultdict):
            return x

        expected_id = "anns(x=defaultdict(seq={}))"
        expected_what = What(name='anns',
                             conf={'x': an_empty_defaultdict})

        return anns, expected_id, expected_what
