...
from djcommon.dss.Serializer import serializer
from django.http import JsonResponse
from djcommon.common import Common
...
class TemplateView(TemplateView, Common):
    pass


class ListView(TemplateResponseMixin, MultipleObjectMixin, View, Common):

    def __get_result(self, request, *args, **kwargs):
        self.object_list = self.get_queryset()
        paginator = self.paginator if hasattr(self, 'paginator') else True
        if paginator:
            pre = self.pre if hasattr(self, 'pre') else 10
