...
class TensorportClient(object):
    ...
    def __init__(self, token="", username="", environment='', use_in_memory_schema=True, schema_json_path=None, api_schema_url=API_SCHEMA, api_url=API_URL):
        ...
        self.decoders = [codecs.CoreJSONCodec(), codecs.JSONCodec()]
    ...
    def client_action(self, actions_keys, params=None, validate=False):
        ...
        try:
            ...
            print("API Action error: %s" % exc.error)
    ...
    def create_job(self, project, display_name, description='Tensorport Job', parameters=None, git_hash=None, datasets=None):
        """
    ...
    def download_file(self, job_id, filename):
        ...
        """
        try:
            response = self.client_action(['file', 'read'], params={
                'job_id': job_id,
                'filename': filename,
            }, validate=True)
            return response
        except Exception as e:
            print("Couldn't find a job or file, error: %s" % str(e))
            raven_client.captureException()
            return False