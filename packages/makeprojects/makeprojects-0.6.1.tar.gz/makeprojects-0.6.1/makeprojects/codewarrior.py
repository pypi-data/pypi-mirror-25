...
# Copyright 2017 by Rebecca Ann Heineman becky@burgerbecky.com
...
class Defaults:
	...
	def defaults(self,solution,configuration='Debug'):
		self.environmentvariables = []
		self.burgersdkspaths = []
		self.systemsearchpaths = []
		self.defines = []
		...
		if solution.platform.iswindows():
			...
			self.defines = ['WIN32_LEAN_AND_MEAN','WIN32']

			# Add the default system directories
			if solution.projecttype!=core.ProjectTypes.library or solution.projectname!='burgerlib':
	...
	def loadjson(self,myjson):
		...
		for key in myjson.keys():
			if key=='systemsearchpaths':
				self.systemsearchpaths = burger.converttoarray(myjson[key])
			elif key=='burgersdkspaths':
				self.burgersdkspaths = burger.converttoarray(myjson[key])
			elif key=='environmentvariables':
				self.environmentvariables = burger.converttoarray(myjson[key])
			else:
...
class SETTING:
	...
	def write(self,fp,level=4):
		tabs = TAB * level
		fp.write(tabs + '<SETTING>')
...
class SearchPath:
	def __init__(self,platform,path,root=None,title='SearchPath'):
		...
		if platform.iswindows():
			self.settings.addsetting('Path',burger.converttowindowsslashes(path))
			format = 'Windows'
		else:
			self.settings.addsetting('Path',burger.converttolinuxslashes(path))
			format = 'Unix'
		self.settings.addsetting('PathFormat',format)
		if root!=None:
...
class MWProject_X86:
	def __init__(self,projecttype,filename):
		if projecttype == core.ProjectTypes.library:
			type = 'Library'
			extension = '.lib'
		else:
			type = 'Application'
		...
		self.settings = [
			SETTING('MWProject_X86_type',type),
...
class FILE:
	def __init__(self,platform,configuration,filename):
		if platform.iswindows():
			...
			self.filename = burger.converttolinuxslashes(filename)
			self.format = 'Unix'
		...
		if self.filename.endswith('.lib') or self.filename.endswith('.a'):
			...
			self.kind = 'Text'
			if configuration!='Release' and (self.filename.endswith('.c') or self.filename.endswith('.cpp')): 
				self.flags = 'Debug'
	...
	def write(self,fp,level=4):
		...
		tabs2 = tabs + TAB
		fp.write(tabs + '<FILE>\n')
		fp.write(tabs2 + '<PATHTYPE>Name</PATHTYPE>\n')
		fp.write(tabs2 + '<PATH>' + self.filename + '</PATH>\n')
		fp.write(tabs2 + '<PATHFORMAT>' + self.format + '</PATHFORMAT>\n')
		fp.write(tabs2 + '<FILEKIND>' + self.kind + '</FILEKIND>\n')
		fp.write(tabs2 + '<FILEFLAGS>' + self.flags + '</FILEFLAGS>\n')
		fp.write(tabs + '</FILE>\n')		
...
class FILEREF:
	...
	def write(self,fp,level=4):
		...
		if self.configuration!=None:
			fp.write(tabs2 + '<TARGETNAME>' + self.configuration + '</TARGETNAME>\n')		
		fp.write(tabs2 + '<PATHTYPE>Name</PATHTYPE>\n')
		fp.write(tabs2 + '<PATH>' + self.filename + '</PATH>\n')
		fp.write(tabs2 + '<PATHFORMAT>' + self.format + '</PATHFORMAT>\n')
		fp.write(tabs + '</FILEREF>\n')
...
class GROUP:
	...
	def write(self,fp,level=2):
		if level==1:
			groupstring = 'GROUPLIST'
		else:
			groupstring = 'GROUP'
		tabs = TAB * level
		fp.write(tabs + '<' + groupstring + '>')
		if self.name!=None:
			fp.write('<NAME>' + self.name + '</NAME>')
		fp.write('\n')
...
class SUBTARGET:
	...
	def write(self,fp,level=4):
		tabs = TAB * level
		tabs2 = tabs + TAB
		fp.write(tabs + '<SUBTARGET>\n')
		fp.write(tabs2 + '<TARGETNAME>' + self.target.name + '</TARGETNAME>\n')
		fp.write(tabs + '</SUBTARGET>\n')
...
class TARGET:
	...
	def write(self,fp,level=2):
		tabs = TAB * level
		tabs2 = tabs + TAB
		fp.write(tabs + '<TARGET>\n')
		fp.write(tabs2 + '<NAME>' + self.name + '</NAME>\n')

		fp.write(tabs2 + '<SETTINGLIST>\n')
		for item in self.settinglist:
			item.write(fp,level+2)
		fp.write(tabs2 + '</SETTINGLIST>\n')

		fp.write(tabs2 + '<FILELIST>\n')
		for item in self.filelist:
			item.write(fp,level+2)
		fp.write(tabs2 + '</FILELIST>\n')

		fp.write(tabs2 + '<LINKORDER>\n')
		for item in self.linkorder:
			item.write(fp,level+2)
		fp.write(tabs2 + '</LINKORDER>\n')

		fp.write(tabs2 + '<SUBTARGETLIST>\n')
...
class ORDEREDTARGET:
	...
	def write(self,fp,level=2):
		tabs = TAB * level
		fp.write(tabs + '<ORDEREDTARGET><NAME>' + self.target.name + '</NAME></ORDEREDTARGET>\n')
...
class Project:
	...
	def addtogroups(self,platform,configuration,parts):
		...
		while len(parts) and (parts[0]=='.' or parts[0]=='..'):
			parts.pop(0)
	...
	def write(self,fp):
		...
		fp.write('<?codewarrior exportversion="' + exportversion + '" ideversion="' + ideversion + '" ?>\n\n')
...
def generate(solution):
	...
	codefiles,includedirectories = solution.getfilelist([core.FileTypes.h,core.FileTypes.cpp,core.FileTypes.rc,core.FileTypes.hlsl,core.FileTypes.glsl])
	...
	listh = core.pickfromfilelist(codefiles,core.FileTypes.h)
	listcpp = core.pickfromfilelist(codefiles,core.FileTypes.cpp)
	listwindowsresource = []
	if solution.platform.iswindows():
		listwindowsresource = core.pickfromfilelist(codefiles,core.FileTypes.rc)
	...
	for configuration in solution.configurations:
		...
		if len(solution.codewarrior.environmentvariables):
		...
		if configuration=='Release':
			...
			definelist = ['_DEBUG']
		# Add in the codewarrior defaults
		definelist += solution.codewarrior.defines
		# Add in the user defaults
		definelist += solution.defines
		...
		if solution.platform.iswindows():
			...
			# x86 Target
			target.settinglist.append(MWProject_X86(solution.projecttype,solution.projectname + idecode + 'w32' + core.getconfigurationcode(configuration)))
		...
		if len(alllists) or len(liblist):
			filelist = []
	...
	projectpathname = os.path.join(solution.workingDir,codewarriorprojectfile.projectnamecode + '.mcp.xml')
	...
	codewarriorprojectfile.write(fp)
	fp.close()
	...
	if cwfile!=None and solution.platform.iswindows():
		...
		burger.perforceedit(mcppathname)
