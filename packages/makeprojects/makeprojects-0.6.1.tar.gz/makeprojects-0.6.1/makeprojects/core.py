...
class ProjectData:
	def __init__(self,name='project',projecttype=ProjectTypes.tool,suffixenable=False):
		...
		self.codewarrior = codewarrior.Defaults()

		## Create default Watcom object
		#self.watcom = watcom.Defaults()

		## Create default Codeblocks object
		#self.codeblocks = codeblocks.Defaults()
	...
	def process(self,myjson):
		...
		for item in myjson:
			if type(item) is dict:
				...
				error = visualstudio.generateold(self,IDETypes.vs2017)
			elif item=='vs2015':
				...
				error = codewarrior.generate(self)
