...
def main(working_dir):
	...
	parser = argparse.ArgumentParser(prog='makeprojects', description= \
		'Create project files. Copyright by Rebecca Ann Heineman. ' \
		'Given a .py input file, create project files')
	...
	parser.add_argument('-f', dest='jsonfiles', action='append', help='Input file to process')
	...
	if args.jsonfiles is None:
		...
		if len(sys.argv) == 1 and os.path.isfile(projectpathname) is True:
