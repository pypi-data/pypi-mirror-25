...
from stix_pattern.parser import generate_query
from stix_pattern import data_models
from stix_pattern import search_platforms
...
def main():
    ...
    parser.add_argument("--outputLanguage", help="language of translated query", choices=[s._value_ for s in SearchPlatforms], required=True, action=InputToEnumAction)
    parser.add_argument("--outputDataModel", help="Translate to this Data Model", choices=[d._value_ for d in DataModels], required=True, action=InputToEnumAction)
    parser.add_argument("pattern", help="The Query or Pattern to be translated.")
    args = parser.parse_args()

    result = translate(args.pattern, args.outputLanguage, args.outputDataModel)
    print(result)
    exit()
