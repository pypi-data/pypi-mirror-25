...
class BaseCallback(object):
    ...
    def set_hyperparams(self, total_epochs=None, batch_size=None, epoch_size=None, max_iterations=None,
                        optimizer_algorithm=None, learning_rate=None, total_batches=None, learning_rate_decay=None, **kwargs):
    ...
    def upload_images(self, model_hash, images, meta):
    ...
    def batch_command(self, event, data, flush=False):
        ...
        if event == EventTypes.BATCH_END:
            ...
            if self.batches_queue[i] is not None:
                batch, epoch = self.batches_queue[i][1].get('batch'), self.batches_queue[i][1].get('epoch')
                if batch and epoch:
                    self.batch_end_candidates.pop((batch, epoch))

            self.batches_queue[i] = command

            batch, epoch, candidate_index = data.get('batch'), data.get('epoch'), data.get('points_candidate')

            if batch is not None and epoch is not None and candidate_index is not None:
                self.batch_end_candidates[(batch, epoch)] = candidate_index
        ...
        elif event == EventTypes.BATCH_UPDATE:
            batch, epoch = data.get('batch'), data.get('epoch')
            if (batch, epoch) in self.batch_end_candidates:
                candidate = self.batch_end_candidates[(batch, epoch)]
                if candidate in self.points_candidate_indices:
                    batch_end = self.batches_queue[self.points_candidate_indices[candidate]][1]
                    batch_end.get('metricData', {}).update(data.get('metricData', {}))
                else:
                    index = len(self.batches_queue)
                    self.points_candidate_indices[candidate] = index
                    data['points_candidate'] = candidate
    ...
    def batch_update(self, batch, epoch, metric_data, **kwargs):
        batch = int(batch)
        epoch = int(epoch)
        ...
        data.update(kwargs)
        self.batch_command(EventTypes.BATCH_UPDATE, data)
    ...
    def _send_test_iteration_end(self, expected, predictions, partial_class_mapping, is_finished, **kwargs):
        data = {
            'test_token': self._test_token,
    ...
    def _test_iteration_end(self, expected, predictions, **kwargs):
        ...
        self._send_test_iteration_end(expected, predictions, partial_class_mapping, is_finished=is_finished, **kwargs)
