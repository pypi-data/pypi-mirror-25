...
class GradCAMInterface(BaseImageInterface):
    ...
    @classmethod
    def _get_heatmap(cls, image, heatmap, name=None, write_local=False, prefix=""):
        ...
        if write_local:
            ...
            cls.cv2.imwrite(prefix + "-" + str(name) + ".png", current_heatmap)
    ...
    def generate_grad_cam(self, uri, model, input_array=None, top_classes=5, top_images=1, class_mapping=None,
                          dim_order=ImageDimOrdering.NCHW, expected_class=None):
        pass

    def _generate_grad_cam(self, uri, model, input_array=None, top_classes=5, top_images=1, class_mapping=None,
        ...
        :param dim_order: order of dimentions for image
        ...
        for number in range(top_images):
            ...
            current_heatmap = self._get_heatmap(image, cur_cam_map_crops, name=number, prefix="_grad_")
...
class VisualBackPropInterface(BaseImageInterface):
    ...
    def _visual_back_prop(self, uri, model, input_val=None, dim_order=ImageDimOrdering.NCHW, write_local=False,
                          logger=None):
        self._set_logger(logger)
        self.validate_cv2()

        original_image = self._get_image(uri)

        if input_val is not None:
            feature_maps, output = self._get_feature_maps(model, input_val)
        else:
            shape = self._get_input_dim(model)
            feature_maps, output = self._get_feature_maps(model, original_image, shape=shape)
        ...
        if write_local:
            overlay = self.cv2.addWeighted(original_image, 1, final_heatmap, 0.8, 0)
        ...
        pure_image = self._convert_to_jpeg(original_image)
