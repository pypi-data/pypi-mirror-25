...
from .utils import hasharray, hashcombine
...
class KerasCallback(BaseCallback, ModelHashInterface, GradCAMInterface, VisualBackPropInterface):
    def __init__(self, owner_id, project_token, stopped_callback=None, host=None):
        super(KerasCallback, self).__init__(
            owner_id, project_token, stopped_callback=stopped_callback, host=host, framework='keras')
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        self.epoch_end(epoch, logs, weights_hash=model_hash)
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        self.batch_end(batch, self.current_epoch, metric_data)
    ...
    def generate_grad_cam(self, uri, model, input_array=None, top_classes=5, top_images=1, class_mapping=None,
                          dim_order=ImageDimOrdering.NHWC, expected_class=None, keep_origin=False):
        try:
            images_data = self._generate_grad_cam(
                uri, model, input_array=input_array, top_classes=top_classes, top_images=top_images,
            ...
            extra = {
                ...
                "top": [i["meta"] for i in images_data],
            }
            meta.update(extra)
    ...
    def visual_back_prop(
            self, uri, model, input_val=None, dim_order=ImageDimOrdering.NHWC, expected_output=None, keep_origin=False):
        try:
            result = self._visual_back_prop(uri, model, input_val=input_val, dim_order=dim_order, logger=self.logger)
        except MissingLinkException:
    ...
    def _create_test_function(self, original_test_function):
        def predict_classes(x, batch_size):
            proba = self.model.predict(x, batch_size=batch_size)
        ...
        def test_function(ins_batch):
            predictions = predict_classes(ins_batch[0], batch_size=len(ins_batch[0]))
            expected = np.argmax(ins_batch[1], axis=1)
            result = original_test_function(ins_batch)

            self._test_iteration_end(expected, predictions)
            return result
    ...
    def _ml_test_loop(self, f, ins, batch_size=32, verbose=0):
        test_samples_count = len(ins[0])
        steps = int(math.ceil(test_samples_count / float(batch_size)))
        self._on_test_begin(steps)
        test_function = self._create_test_function(f)

        return self._test_loop(test_function, ins, batch_size, verbose)
    ...
    def _patch_test_function(self):
        training_model = self._get_training_model()

        # noinspection PyProtectedMember
        training_model._make_test_function()

        training_model.test_function = self._create_test_function(training_model.test_function)
