...
from .settings import HyperParamTypes, CUSTOM_METRICS_FORMAT
from .interfaces import ModelHashInterface
from .utils import hasharray, hashcombine, hash_value
...
class Experiment(ModelHashInterface):
    ...
    def train(self, monitored_metrics=None, custom_metrics=None):
        ...
        yield
        if custom_metrics:
            custom_results = self._resolve_custom_metrics(custom_metrics)
    ...
    def _patch_tf_session_for_test(self, monitored_fetches):
        def patched_run(session, fetches, feed_dict=None, options=None, run_metadata=None):
            ...
            self._after_test_run(expected, predicted_classes)
    ...
    @staticmethod
    def _resolve_custom_metrics(custom_metrics):
        results = dict()
        for key, metrics_function in custom_metrics.items():
            results[CUSTOM_METRICS_FORMAT.format(key)] = metrics_function()
        return results
