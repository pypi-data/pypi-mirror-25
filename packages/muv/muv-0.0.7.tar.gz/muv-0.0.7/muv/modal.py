...
class ContentParser:
    ...
    def rst2markup(self, infile):
        html = self.rst2html(infile)
        return self.html2markup(html)

    def rst2html(self, infile):
    ...
    #@make_padding(6, 0)
    def _ol(self, element, padding=5):
        widgets = []
        i = 1
        for li in element:
            ...
            markup = self._li(li, padding=padding)
            if not isinstance(markup, urwid.Widget):
                markup = urwid.Text(('li',[str(i), '. ', markup]))
                i+=1
            widgets.append(markup)
        ol = urwid.Pile(widgets)
        return ol
        #return urwid.Padding(ol, align='left',
                                #width=('relative', 100), min_width=None, left=padding+1, right=1)
        #return ol

    #@make_padding(5, 0)
    def _ul(self, element, padding=6):
        widgets = []
        for li in element:
            if isinstance(li, NavigableString):
                continue
            widget = self._li(li, padding=padding)
            widgets.append(widget)
        ul = urwid.Pile(widgets)
        return ul

    def _li(self, element, markup_name=None, padding=6):
        """Convert html tag to plain text with tag as attributes recursively"""
        content = []
        for child in element.children:
            if child.name == 'ul':
                ul = self._ul(child, padding=padding+2)
                content.append(ul)
            elif child.name == 'ol':
                ol = self._ol(child, padding=padding+2)
                content.append(ol)
            elif isinstance(child, Tag):
                method = self.tag2method.get(child.name, getattr(self, '_'+child.name, self.tag2text))
                inner_content = method(child, markup_name)
                if not isinstance(inner_content, urwid.Widget):
                    inner_content = urwid.Text(('li', ['* ', inner_content]))
                pad = urwid.Padding(inner_content, align='left', width=('relative', 100), min_width=None, left=padding, right=1)
                content.append(pad)
            else:
                if not isinstance(child, NavigableString):
                    inner_content = urwid.Text(('li',['* ', child]))
                    pad = urwid.Padding(inner_content, align='left', width=('relative', 100), min_width=None, left=padding, right=1)
                    content.append(pad)
