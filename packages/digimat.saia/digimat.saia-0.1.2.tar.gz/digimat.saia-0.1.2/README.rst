...
Inputs, Outputs and Flags are boolean items. Registers, Timers and Counters are simple "32 bits uint values".
...
Timers are implemented *in the local node*. This means that any timer created will be automatically decremented until reaching 0
...
The default tickbaseTime is 100ms, which can be set on the timers object (decrement 1 each 100ms by default)
