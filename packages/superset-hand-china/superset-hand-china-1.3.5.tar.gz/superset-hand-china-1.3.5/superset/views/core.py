...
from superset.hand.models import PortalMenu
...
class DatabaseView(SupersetModelView, DeleteMixin):  # noqa
    ...
    order_columns = ['database_name', 'allow_run_sync', 'allow_run_async', 'allow_dml', 'modified']
    add_columns = [
...
class DatabaseTablesAsync(DatabaseView):
    ...
    label_columns = {
        'all_table_names': _("All Table Names"),
        'all_schema_names': _("All Schema Names"),
    }
...
class AccessRequestsModelView(SupersetModelView, DeleteMixin):
    ...
    list_title = _("List Access Request")
    show_title = _("Show Access Request")
    add_title = _("Add Access Request")
    edit_title = _("Edit Access Request")
    list_columns = [
...
class SliceModelView(SupersetModelView, DeleteMixin, SupersetFilter):  # noqa
    ...
    search_columns = (
        'slice_name', 'subtitle', 'description', 'viz_type',
    )
    list_columns = [
        'slice_link', 'viz_type', 'subtitle', 'datasource_link', 'creator', 'modified']
    order_columns = ['viz_type', 'subtitle', 'datasource_link', 'modified']
    edit_columns = [
        'slice_name', 'multi_language', 'description', 'subtitle', 'subtitleAble', 'viz_type', 'dashboards',
    ...
    label_columns = {
        ...
        'slice_name': _("Name"),
        'table': _("Table"),
        'multi_language': _("Multiple Language"),
        ...
        'perm': _("Perm"),
        'subtitle': _("Subtitle"),
        'subtitleAble': _("subtitleAble"),
    }

    def pre_update(self, obj):
        # check_ownership(obj)
        if not obj.slice_name.strip():
            raise Exception(__("Slice Name can't be empty!"))
        number_of_existing_slices = db.session.query(
            sa.func.count('*')).filter(
            models.Slice.slice_name == obj.slice_name,
            models.Slice.subtitle == obj.subtitle
        ).scalar()
        utils.validate_json(obj.multi_language)
        # table object is already added to the session
        if number_of_existing_slices > 1:
            raise Exception(__("Slice Name [{}] already exists, Please change slice name to save!").format(obj.slice_name))
    ...
    def add(self):
        ...
        datasources = []
        perms = self.get_view_menus('datasource_access')
        datasources = db.session.query(SqlaTable).filter(SqlaTable.perm.in_(perms)).all()
        if self.has_all_datasource_access():
            datasources = ConnectorRegistry.get_all_datasources(db.session)
        datasources = [
        ...
        return self.render_template(
            ...
            bootstrap_data=json.dumps({
                ...
                'common': BaseSupersetView.common_bootsrap_payload(self),
            }),
        )
    ...
    def edit(self, pk):
        ...
        resp = super(SliceModelView, self).edit(pk)
        ...
        if not item:
            ...
            number_of_existing_slices = db.session.query(
                sa.func.count('*')).filter(
                models.Slice.slice_name == item.slice_name,
                models.Slice.subtitle == item.subtitle
            ).scalar()
            if number_of_existing_slices > 1:
                return redirect('slicemodelview/edit/{}'.format(pk))
...
class DashboardModelView(SupersetModelView, DeleteMixin):  # noqa
    ...
    list_columns = ['dashboard_link', 'only_download', 'creator', 'modified']
    order_columns = ['only_download','modified']
    edit_columns = [
        'dashboard_title', 'multi_language', 'slug', 'slices', 'only_download', 'position_json', 'css', 'theme',
        'json_metadata', 'refreshAble']
    show_columns = edit_columns + ['table_names']
    search_columns = ('dashboard_title', 'slug', 'theme', 'refreshAble')
    ...
    description_columns = {
        ...
        'refreshAble': _("The window changes the size to refresh the slice"),
    }
    ...
    label_columns = {
        ...
        'theme': _("Theme"),
        'refreshAble': _("RefreshAble"),
        'only_download': _("Only Download"),
        'changed_on': _("Changed On"),
    }

    def pre_add(self, obj):
        if not obj.dashboard_title.strip():
            raise Exception(__("Dashboard Name can't be empty!"))
        number_of_existing_dashboards = db.session.query(
            sa.func.count('*')).filter(
            models.Dashboard.dashboard_title == obj.dashboard_title
        ).scalar()
        # table object is already added to the session
        # print(obj.__dict__)
        if len(obj.slices) == 0:
            if number_of_existing_dashboards > 0:
                raise Exception(__("Dashboard Name [{}] already exists, " +
                    "Please change dashboard name to add!").format(obj.dashboard_title))
        else:
            if number_of_existing_dashboards > 1:
                raise Exception(__("Dashboard Name [{}] already exists, " +
                    "Please change dashboard name to add!").format(obj.dashboard_title))
        ...
        security.merge_perm(sm, 'dashboard_access', obj.get_perm())
    ...
    def pre_update(self, obj):
        ...
        if number_of_existing_dashboards > 1:
            raise Exception(__("Dashboard Name [{}] already exists, Please change dashboard name to save!").format(obj.dashboard_title))
        obj.slug = obj.slug.strip() or None
        if obj.slug:
            obj.slug = obj.slug.replace(" ", "-")
            obj.slug = re.sub(r'\W+', '', obj.slug)
    ...
    def _delete(self, pk):
        ...
        try:
            ...
            number = db.session.query(PortalMenu).filter_by(dashboard_href=pk).count()
            if(number != 0):
                flash(__("portal using ")+item.dashboard_title,"danger")
    ...
    def edit(self, pk):
        """Simple hack to redirect to explore view after saving"""
        resp = super(DashboardModelView, self).edit(pk)
        if isinstance(resp, basestring):
            return resp
        item = self.datamodel.get(pk, self._base_filters)
        if not item:
            ...
            number_of_existing_dashboards = db.session.query(
                sa.func.count('*')).filter(
                models.Dashboard.dashboard_title == item.dashboard_title
            ).scalar()
            if number_of_existing_dashboards > 1:
                return redirect('dashboardmodelview/edit/{}'.format(pk))
...
class R(BaseSupersetView):
    ...
    def index(self, url_id):
        ...
        if url:
            ...
            flash(__("URL to nowhere..."), "danger")
            return redirect('/')
...
class Superset(BaseSupersetView):
    ...
    def datasources(self):
        ...
        for datasource in datasources:
            access = utils.can_access(appbuilder.sm, 'datasource_access', datasource.get_perm(), current_user)\
                or utils.can_access(appbuilder.sm, "all_datasource_access", "all_datasource_access", current_user)
            if access:
                ds.append(datasource)
        datasources = [o.short_data for o in ds]
    ...
    def explore_json(self, datasource_type, datasource_id):
        ...
        if request.args.get("excel") == "true":
            # print('===excel========')
            # print(viz_obj.get_excel())
            return Response(
                viz_obj.get_excel(),
                status=200,
        ...
        if payload.get('status') == QueryStatus.FAILED:
            status = 400
        #mutilanguage 
        vizT = ['ag_grid','echarts_bar','echarts_bar_waterfall','echarts_bar_h','echarts_area_stack',
                    'echarts_line','echarts_pie_m','echarts_pie_h','echarts_pie_g',
                    'echarts_multiple_ring_diagram','echarts_dash_board','echarts_big_number_compare',
                    'echarts_big_number','big_number_viz','echarts_radar_map','pivot_table',
                    ]
        if payload['form_data']['viz_type'] in vizT:
            if 'data' in payload:
                if  datasource_type == 'table':
                    ...
                    group_by =[]
                    metrics = []
                    metrics =db.session.query(SqlMetric).filter_by(table_id = datasource_id).all()          
                    columns = db.session.query(TableColumn).filter_by(table_id = datasource_id).all()
                    # columns
                    if payload['data']:
                        if 'columns' in payload['data']:
                            for pjc in payload['data']['columns']:
                                for metric in metrics:
                                    if pjc == metric.metric_name:
                                        cols.append(metric.nick_name)
                                for column in columns:
                                    if pjc == column.column_name:
                                        cols.append(column.nick_name)
                        print(cols)
                    # records
                    if payload['data']:
                        if  'records' in payload['data']:
                            for pjr  in payload['data']['records']:
                                keys = pjr.keys()
                                r = {}
                                for key in keys:
                                    for metric in metrics:
                                        if key  == metric.metric_name:
                                            r[metric.nick_name] = pjr[key]
                                    for column in columns:
                                        if key == column.column_name:
                                            r[column.nick_name] = pjr[key]
                                recs.append(r)

                    payload['data']['records'] = recs
                    payload['data']['columns'] = cols
    ...
    def explore(self, datasource_type, datasource_id):
        ...
        if slc:
            title = _("[slice] ") + slc.nick_name
        else:
            title = _("[explore] ") + table_name
        # columns = db.session.query(TableColumn).filter_by(table_id = datasource_id).all()
        # cols = []
        # for bfg in bootstrap_data['form_data']['groupby']:
        #     for col in columns:
        #         if bfg == col.column_name:
        #             cols.append(col.nick_name)
        # # bootstrap_data['form_data']['groupby'] = cols
        # metrics =db.session.query(SqlMetric).filter_by(table_id = datasource_id).all()
        # mets = []
        # for bfs in bootstrap_data['form_data']['metrics']:
        #     for met in metrics:
        #         if bfs == met.metric_name:
        #             mets.append(met.nick_name)
        # # bootstrap_data['form_data']['metrics'] = mets
    ...
    def save_or_overwrite_slice(
        ...
        subtitleAble = args.get('subtitleAble')
        ...
        slc.subtitle = subtitle
        slc.subtitleAble = subtitleAble == 'true' and True or False
        ...
        if request.args.get('add_to_dash') == 'existing':
            d = db.session.query(models.Dashboard).filter_by(id=int(request.args.get('save_to_dashboard_id')))
            dash = d.one()
        
            if ("\"" +str(slc.id) + "\"") not in dash.position_json :
                addPosition = '{"col": 0,"proportion": -1,"row": 0,"size_x": 0,"size_y": 0,"slice_id":'+"\"" +str(slc.id) + "\"" +'}]'
                position_json = dash.position_json.split("]")[0] + "," + addPosition
                d.update({
                    models.Dashboard.position_json: position_json
                })
            # flash(
            ...
            #     "info")
            flash(
                _("Slice [") + slc.slice_name + _("] was added to dashboard [")
                + dash.dashboard_title + "]",
                "info")
            ...
            dash = models.Dashboard(
                ...
                only_download=request.args.get('only_download') == 'true' and True or False,
                position_json='[{"col": 0,"proportion": -1,"row": 0,"size_x": 0,"size_y": 0,"slice_id":' + "\""+ str(slc.id) + "\"" +'}]',
                owners=[g.user] if g.user else [])
            security.merge_perm(sm, 'dashboard_access', dash.get_perm())
            # flash(
            ...
            #     "info")
            flash(
                _("Dashboard [") + dash.dashboard_title + _("] just got created and slice [")
                + slc.slice_name + _("] was added to it"),
                "info")
    ...
    def _set_dash_metadata(dashboard, data):
        ...
        dashboard.refreshAble = data['refreshAble']
    ...
    def dashboard(self, dashboard_id):
        ...
        dashboard_data.update({
            ...
            'dashboard_title': dash.nick_name,
        })
        ...
        standalone = request.args.get("standalone") == "true"
        if dash.only_download:
            return self.render_template(
                "hand/dashboard_download.html",
                entry='dashboard_download',
                dashboard_title=dash.nick_name,
                bootstrap_data=json.dumps(bootstrap_data),
                standalone_mode=standalone,
            ) 
    ...
    def sqllab_viz(self):
        ...
        database = db.session.query(models.Database).filter_by(id=data.get('dbId')).one()
    ...
    def sql_json(self):
        ...
        query = Query(
            ...
            limit=int(app.config.get('QUERY_SEARCH_LIMIT', None)),
            sql=sql,
            schema=schema,
            select_as_cta=request.form.get('select_as_cta') == 'true',
    ...
    def excel(self, client_id):
        ...
        logging.info("Exporting excel file [{}]".format(client_id))
        query = (
            db.session.query(Query)
            .filter_by(client_id=client_id)
            .one()
        ...
        if results_backend and query.results_key:
            logging.info(
                "Fetching excel from results backend "
                "[{}]".format(query.results_key))
            blob = results_backend.get(query.results_key)
        if blob:
            logging.info("Decompressing")
            ...
            columns = [c['name'] for c in obj['columns']]
            df = pd.DataFrame.from_records(obj['data'], columns=columns)
            logging.info("Using pandas to convert to excel")
            df.to_excel(writer,index=False, encoding='utf-8')
            writer.save()
            xlsx_data = output.getvalue()
        else:
            logging.info("Running a query to turn into excel")
            sql = query.select_sql or query.executed_sql
            df = query.database.get_df(sql, query.schema)
            # TODO(bkyryliuk): add compression=gzip for big files.
            df.to_excel(writer,index=False, encoding='utf-8')
            writer.save()
            xlsx_data = output.getvalue()
        response = Response(xlsx_data, mimetype='text/xlsx')
        response.headers['Content-Disposition'] = (
    ...
    def welcome(self):
        ...
        return self.render_template(
            'superset/welcome.html', utils=utils)
...
class CssTemplateModelView(SupersetModelView, DeleteMixin):
    ...
    add_columns = edit_columns
    search_columns = ['template_name', 'css', 'created_on']
    label_columns = {
