...
class ChartContainer extends React.PureComponent {
  ...
  getMockedSliceObject() {
    ...
    return {
      ...
      data: {
        excel_endpoint: getExploreUrl(props.formData, 'excel'),
  ...
  renderChartTitle() {
    let title;
    if (this.props.slice) {
      title = this.props.slice.nick_name;
    } else {
  ...
  query(mockSlice){
    let datasource =this.props.queryResponse.form_data.datasource.split('_')
    const table_id = datasource[0]
    let newdata = {}
    $.ajax({
      //要用post方式   
      type: "get",
      async:false,
      //方法所在页面和方法名
      url: "/hand/querytables/"+table_id,
      dataType: "json",
      success: function (data) {
        // 返回的数据用data.d获取内容
        let keys =Object.keys(mockSlice.formData)
        keys.forEach(function(key, index){
          if( key.indexOf('groupby') >= 0){
            let group = []
            for(let mf in mockSlice.formData[key]){
              for(let dg in data.groupby){
                if(mockSlice.formData[key][mf] == data.groupby[dg][0]){
                    group.push(data.groupby[dg][1])
                }
              }
            }
            if(group != []){
              mockSlice.formData[key] = group
            }
          }
          if(key.indexOf('metric') >= 0){
              if(typeof(mockSlice.formData[key]) == 'object'){
                let metric = []
                for(let mf in mockSlice.formData[key]){
                  for(let dm in data.metrics){
                    if(mockSlice.formData[key][mf] == data.metrics[dm][0]){
                ...
                if(metric != []){
                  mockSlice.formData[key] = metric
                }

            }
            if(typeof(mockSlice.formData[key]) == 'string'){
              let metric = ''
              for(let dm in data.metrics){
                if(mockSlice.formData[key] == data.metrics[dm][0]){
  ...
  renderViz() {
    ...
    if( vizT.indexOf(mockSlice.formData.viz_type) != -1){
      this.query(mockSlice)
    }
  ...
  render() {
    ...
    if (this.props.viz_type === 'markup' || this.props.viz_type === 'separator' || this.props.viz_type === 'filter_box_combination') {
      isImportExcel = false;
    }
    return (
