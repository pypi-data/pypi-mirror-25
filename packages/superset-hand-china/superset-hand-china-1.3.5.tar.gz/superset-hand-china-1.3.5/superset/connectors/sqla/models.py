...
class TableColumn(Model, BaseColumn):
    ...
    def nick_name(self):
        ...
        try:
            obj = json.loads(self.multi_language)
            return obj[language]
        except Exception as e:
            return self.column_name
 
    def column_link(self):
        title = escape(self.nick_name)
        return Markup(
            '<span>{title}</span>'.format(**locals()))
...
class SqlMetric(Model, BaseMetric):
    ...
    export_fields = (
        ...
        'description', 'is_restricted', 'd3format','multi_language')
...
class SqlaTable(Model, BaseDatasource):
    ...
    export_fields = (
        'table_name', 'multi_language', 'main_dttm_col', 'description', 'default_endpoint',
    ...
    def get_perm(self):
        return (
            "datasource|[{obj.database}].[{obj.table_name}]"
    ...
    def data(self):
        ...
        li =db.session.query(TableColumn).filter_by(table_id = self.id).all()
        for k in d['columns']:
            for i in li :
                if k['column_name'] == str(i.column_name):
                    k['nick_name'] = str(i.nick_name)
        newgb_cols = []
        for m in d['gb_cols']:
            for i in li:
                if m[0] ==  str(i.column_name):
                    t1=(m[0],str(i.nick_name))
                    newgb_cols.append(t1)
        d['gb_cols'] = newgb_cols

        metric =db.session.query(SqlMetric).filter_by(table_id = self.id).all()
        # multlanguage
        colmons = db.session.query(TableColumn).filter_by(table_id = self.id).all()
        order_by_choices = []
        cols = []
        for s in colmons:
            col = (str(s.column_name),str(s.nick_name),)
            cols.append(col)
            d['all_cols'] = cols
            order_by_choices.append((json.dumps([s.column_name, True]), s.nick_name + ' [asc]'))
            order_by_choices.append((json.dumps([s.column_name, False]), s.nick_name + ' [desc]'))
        combos = []
        filterable_cols = []
        for n in metric:
            for j in d['metrics']:
                if j['metric_name'] == str(n.metric_name):
                     j['nick_name'] = str(n.nick_name)
            combo = (str(n.metric_name),str(n.nick_name))
            combos.append(combo)
            filterable_col = (str(n.metric_name),str(n.nick_name))
            filterable_cols.append(filterable_col)
            order_by_choices.append((json.dumps([n.metric_name, True]), n.nick_name + ' [asc]'))
            order_by_choices.append((json.dumps([n.metric_name, False]), n.nick_name + ' [desc]'))
        d['metrics_combo'] = combos
        d['filterable_cols'] = filterable_cols
    ...
    def get_query_str(self, query_obj):
        ...
        # logging.info(self.is_package)
        if self.is_package:
            try:
                extra_filters = []
                try:
                    extra_filters = query_obj['form_data']['extra_filters']
                except Exception as e:
                    extra_filters = query_obj['form_data']['filters']
                logging.info(extra_filters)

                # get filter slices columns
                filterSliceColumns = []
                def f(filter):
                    return filter['filteringSliceId']
                filterSliceIds = map(f, extra_filters)

                # print(set(filterSliceIds))
                for filterSliceId in set(filterSliceIds):
                    filterSlice = db.session.query(Slice).filter(Slice.id == int(filterSliceId)).first()
                    filterModel = db.session.query(SqlaTable).filter(SqlaTable.id == filterSlice.datasource_id).first()
                    filterSliceColumns = filterSliceColumns + filterModel.columns
                cols = {col.column_name: col for col in filterSliceColumns}
                # logging.info(cols)

                for filter in extra_filters:
                    if filter['col'] != 'null' and filter['col'] != 'undefined' and filter['val'] != None \
                        and len(filter['val']) != 0 and filter['op'] in ('in'):
                        col = cols[filter['col']].sqla_col
                        regex = str(col) + '[\w\s]*=>[\w\d\s\'\"]*'
                        pattern = re.compile(regex, re.IGNORECASE)
                        list = pattern.findall(sql)
                        sql = sql.replace(list[0], str(col) + " => '" + str(filter['val'][0]) + "'")
            except Exception as e:
                logging.info('has no filter')
                pass
    ...
    def get_sqla_query(  # sqla
                ...
                if col_obj:
                    if op in ('in', 'not in'):
                        ...
                        if(isinstance(eq,str)):
                            streq = eq
                        if op == '==':
                            where_clause_and.append(col_obj.sqla_col == streq)
                        elif op == '!=':
                            where_clause_and.append(col_obj.sqla_col != streq)
                        elif op == '>':
                            where_clause_and.append(col_obj.sqla_col > streq)
                            ...
                            where_clause_and.append(col_obj.sqla_col.like(streq))
        if extras:
        ...
        if granularity:
            if template_kwargs['form_data']['viz_type'] == 'table':
                if is_timeseries:
        ...
        if orderby:
            ...
            if form_data['viz_type'] == 'echarts_word_cloud':
    ...
    def query(self, query_obj):
        ...
        # let df lower_col to origin_col
        dict = {}
        cols = {col.column_name: col for col in self.columns}
        if df is not None:
            for c in df.columns:
