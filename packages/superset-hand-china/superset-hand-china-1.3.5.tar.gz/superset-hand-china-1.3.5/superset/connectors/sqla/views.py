...
from superset.views.base import (
    ...
    get_datasource_exist_error_mgs, TableFilter
)
...
class TableColumnInlineView(CompactCRUDMixin, SupersetModelView, DeleteMixin):  # noqa
    ...
    edit_columns = [
        'column_name','multi_language', 'verbose_name', 'description',
        'type', 'groupby', 'filterable',
        'table', 'count_distinct', 'sum', 'min', 'max', 'expression',
        'is_dttm', 'python_date_format', 'database_expression']
    add_columns = edit_columns
    list_columns = [
        'column_link', 'verbose_name', 'type', 'groupby', 'filterable', 'count_distinct',
        'sum', 'min', 'max', 'is_dttm']
    page_size = 500
    description_columns = {
        'is_dttm': _(
...
class SqlMetricInlineView(CompactCRUDMixin, SupersetModelView):  # noqa
    ...
    list_columns = ['metric_link', 'verbose_name', 'metric_type']
    edit_columns = [
        'metric_name','multi_language','description', 'verbose_name', 'metric_type',
    ...
    def pre_add(self,metric):
        utils.validate_json(metric.multi_language)
...
class TableModelView(DatasourceModelView, DeleteMixin):  # noqa
    ...
    list_columns = [
        'link', 'database', 'is_package',
        'changed_by_', 'modified']
    order_columns = [
        'changed_on_']
    add_columns = ['database', 'schema', 'table_name', 'is_package']
    edit_columns = [
        'table_name', 'multi_language', 'sql', 'filter_select_enabled', 'is_package', 'slices',
    ...
    def pre_add(self, table):
        ...
        utils.validate_json(table.multi_language)
        ...
        try:
            ...
            logging.exception(e)
            # raise Exception(
            #     _("Table [") + table.name + _("] could not be found, please double" +
            #     " check your database connection, schema, and table name"))
            raise Exception(_(
    ...
    def post_add(self, table, flash_message=True):
        # update slice perm equals table perm
        slcs = db.session.query(superset_models.Slice).filter_by(datasource_id = table.id).all()
        for slc in slcs:
    ...
    def pre_update(self, table):
        utils.validate_json(table.multi_language)       
        if not table.table_name.strip():
            raise Exception(__("Table Name can't be empty!"))
        number_of_existing_tables = db.session.query(
            sa.func.count('*')).filter(
            models.SqlaTable.table_name == table.table_name,
        ).scalar()
        if number_of_existing_tables > 1:
            raise Exception(__("Table [{}] already exists, Please change table name to save!").format(table.table_name))
    ...
    def edit(self, pk):
        ...
        if not item:
            ...
            if number_of_existing_tables > 1:
                return redirect('tablemodelview/edit/{}'.format(pk))
