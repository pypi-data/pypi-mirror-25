    ...
    def get_query_str(self, query_obj):
        ...
        if self.is_package:
            try:
                ...
                filterSliceIds = map(f, query_obj['form_data']['extra_filters'])
                ...
                # logging.info(cols)
                # logging.info(query_obj['form_data']['extra_filters'])
                for filter in query_obj['form_data']['extra_filters']:
                    col = cols[filter['col']].sqla_col
                    if col != 'undefined' and filter['val'] != None and len(filter['val']) != 0 \
                            and filter['op'] in ('in'):
