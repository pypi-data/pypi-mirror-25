...
const defaultProps = {
  columns: [t('started'), t('duration'), t('rows')],
  queries: [],
  onUserClicked: () => {},
  onDbClicked: () => {},
};
...
class QueryTable extends React.PureComponent {
  ...
  render() {
    const data = this.props.queries.map((query) => {
      ...
      if (q.endDttm) {
        q[t('duration')] = fDuration(q.startDttm, q.endDttm);
      }
      ...
      q[t('started')] = moment(q.startDttm).format('HH:mm:ss');
      q[t('querylink')] = (
      ...
      if (q.resultsKey) {
        ...
        q[t('output')] = [schemaUsed, q.tempTable].filter(v => (v)).join('.');
      }
      q[t('progress')] = (
        <ProgressBar
          style={{ width: '75px' }}
