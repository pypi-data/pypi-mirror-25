    ...
    def latest_partition(cls, table_name, schema, database, show_first=False):
        ...
        if len(indexes[0]['column_names']) < 1:
            raise SupersetTemplateException(
                __("The table should have one partitioned field"))
        elif not show_first and len(indexes[0]['column_names']) > 1:
            raise SupersetTemplateException(
                __("The table should have a single partitioned field "
    ...
    def latest_sub_partition(cls, table_name, schema, database, **kwargs):
        ...
        for k in kwargs.keys():
            if k not in k in part_fields:
                ...
                msg = __("Field [{k}] is not part of the portioning key")
        ...
        if len(kwargs.keys()) != len(part_fields) - 1:
            ...
            msg = (
                __("A filter needs to be specified for ") + (len(part_fields)-1)
                + __(" out of the ") + (len(part_fields)) + __(" fields.")
                )
            raise SupersetTemplateException(msg)
...
class HiveEngineSpec(PrestoEngineSpec):
    ...
    def convert_dttm(cls, target_type, dttm):
        tt = target_type.upper()
        if tt == 'DATE':
            return __("CAST('{}' AS DATE)".format(dttm.isoformat()[:10]))
        elif tt == 'TIMESTAMP':
            return __("CAST('{}' AS TIMESTAMP)".format(
                dttm.strftime('%Y-%m-%d %H:%M:%S')))
        return "'{}'".format(dttm.strftime('%Y-%m-%d %H:%M:%S'))
