        ...
        for (const field in this.filters[filteringSliceId]) {
          if (!immuneToFields.includes(field)) {
            ...
            for (const i in this.filterBoxData) {
              ...
              if (this.filterBoxData[i].sliceId == filteringSliceId) {
    ...
    startPeriodicRender(interval) {
      ...
      const refreshAll = () => {
        dash.sliceObjects.forEach((slice) => {
          ...
          // slice.render(force);
          if ($.inArray(slice.formData.slice_id, filter_box_ids) !== -1) {
            setTimeout(() => { slice.render(force); }, 500);
          } else {
            slice.render(force);
          }
          // setTimeout(() => {
...
function startGridLayout(){
  ...
  $(document).ready(() => startGridLayout());