/* eslint camelcase: 0 */
import React from 'react';
import PropTypes from 'prop-types';
import { connect } from 'react-redux';

import { Modal, Alert, Button, Radio } from 'react-bootstrap';
import Select from 'react-select';
import { Checkbox } from 'react-bootstrap';
import { getExploreUrl } from '../exploreUtils';
import { t } from '../../locales';

const propTypes = {
  can_overwrite: PropTypes.bool,
  onHide: PropTypes.func.isRequired,
  actions: PropTypes.object.isRequired,
  form_data: PropTypes.object,
  user_id: PropTypes.string.isRequired,
  dashboards: PropTypes.array.isRequired,
  alert: PropTypes.string,
  slice: PropTypes.object,
  datasource: PropTypes.object,
};

class SaveModal extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      saveToDashboardId: null,
      newDashboardName: '',
      newSliceName: '',
      dashboards: [],
      alert: null,
      action: props.can_overwrite ? 'overwrite' : 'saveas',
      addToDash: 'noSave',
      newSubtitle: '',
      modalSliceShow: false,
      modalDashboardShow: false,
      subtitleAble: false,
      only_download: false,
    };
  }
  componentDidMount() {
    this.props.actions.fetchDashboards(this.props.user_id);
  }
  onChange(name, event) {
    switch (name) {
      case 'newSliceName':
        this.setState({ newSliceName: event.target.value });
        break;
      case 'saveToDashboardId':
        this.setState({ saveToDashboardId: event.value });
        this.changeDash('existing');
        break;
      case 'newDashboardName':
        this.setState({ newDashboardName: event.target.value });
        break;
      case 'newSubtitle':
        this.setState({ newSubtitle: event.target.value });
      case 'subtitleAble':
        this.setState({ subtitleAble: event.target.checked });
      case 'only_download':
        this.setState({ only_download: event.target.checked });
      default:
        break;
    }
  }
  changeAction(action) {
    this.setState({ action });
  }
  changeDash(dash) {
    this.setState({ addToDash: dash });
  }
  saveOrOverwrite(gotodash) {
    this.setState({ alert: null });
    this.props.actions.removeSaveModalAlert();
    const sliceParams = {};

    let sliceName = null;
    sliceParams.action = this.state.action;
    if (this.props.slice && this.props.slice.slice_id) {
      sliceParams.slice_id = this.props.slice.slice_id;
    }
    if (sliceParams.action === 'saveas') {
      sliceName = this.state.newSliceName;
      if (sliceName === '') {
        this.setState({ alert: t('Please enter a slice name') });
        return;
      }
      sliceParams.slice_name = sliceName;
      sliceParams.subtitle = this.state.newSubtitle;
      sliceParams.subtitleAble = this.state.subtitleAble;
      const _this = this;
      let sliceAlert = false;
      $.ajax({
        type: 'get',
        url: '/hand/querySliceData',
        dataType: 'json',
        async: false,
        success: function (data) {
          for (let i = 0; i < data.length; i++) {
            if (sliceParams.slice_name === data[i].slice_name && sliceParams.subtitle === data[i].subtitle) {
              sliceAlert = true;
              return sliceAlert;
            }
          }
          return sliceAlert;
        },
        error: function (error) {
          console.info(error);
        },
      });
      if(sliceAlert === true) {
        this.setState({ alert: t('The same name slice already exists, Please modify slice name or subtitle') });
        return;
      }
    } else {
      sliceParams.slice_name = this.props.slice.slice_name;
      sliceParams.subtitle = this.props.slice.subtitle;
      sliceParams.subtitleAble = this.props.slice.subtitleAble;
    }

    const addToDash = this.state.addToDash;
    sliceParams.add_to_dash = addToDash;
    let dashboard = null;
    switch (addToDash) {
      case ('existing'):
        dashboard = this.state.saveToDashboardId;
        if (!dashboard) {
          this.setState({ alert: t('Please select a dashboard') });
          return;
        }
        sliceParams.save_to_dashboard_id = dashboard;
        break;
      case ('new'):
        dashboard = this.state.newDashboardName;
        if (dashboard === '') {
          this.setState({ alert: t('Please enter a dashboard name') });
          return;
        }
        sliceParams.new_dashboard_name = dashboard;
        sliceParams.only_download = this.state.only_download;
        const _this = this;
        let dashboardAlert = false;
        $.ajax({
          type: 'get',
          url: '/hand/queryDashboardData',
          dataType: 'json',
          async: false,
          success: function (data) {
            for (let i = 0; i < data.length; i++) {
              if (sliceParams.new_dashboard_name === data[i].dashboard_title) {
                dashboardAlert = true;
                return dashboardAlert;
              }
            }
            return dashboardAlert;
          },
          error: function (error) {
            console.info(error);
          },
        });
        if (dashboardAlert === true) {
          this.setState({ alert: t('The same name dashboard already exists, Please modify dashboard name.') });
          return;
        }
        break;
      default:
        dashboard = null;
    }
    sliceParams.goto_dash = gotodash;

    const saveUrl = getExploreUrl(this.props.form_data, 'base', false, null, sliceParams);
    this.props.actions.saveSlice(saveUrl)
      .then((data) => {
        // Go to new slice url or dashboard url
        window.location = data;
      });
    this.props.onHide();
  }
  querydashboard(){
    let dashboards =[] 
    $.ajax({
      type: 'get',
      url: '/hand/queryDashboard',
      dataType: 'json',
      async: false,
      success: function (data) {
        for(let i = 0; i < data.length; i++){
            let dashboard = {}
            dashboard['label'] = data[i].nick_name
            dashboard['value'] = data[i].id
            dashboards.push(dashboard)
       }
      },
      error: function () {
      },
    });
        return dashboards;
  }
  removeAlert() {
    if (this.props.alert) {
      this.props.actions.removeSaveModalAlert();
    }
    this.setState({ alert: null });
  }
  render() {
    let dash = this.querydashboard()
    if (this.props.slice) {
      this.props.slice.subtitle = (this.props.slice.subtitle === undefined ||
        this.props.slice.subtitle === null) ? "" : this.props.slice.subtitle;
    }
    return (
      <Modal
        show
        onHide={this.props.onHide}
        bsStyle="large"
      >
        <Modal.Header closeButton>
          <Modal.Title>
            {t('Save A Slice')}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {(this.state.alert || this.props.alert) &&
            <Alert>
              {this.state.alert ? this.state.alert : this.props.alert}
              <i
                className="fa fa-close pull-right"
                onClick={this.removeAlert.bind(this)}
                style={{ cursor: 'pointer' }}
              />
            </Alert>
          }
          {this.props.slice &&
            <Radio
              id="overwrite-radio"
              disabled={!this.props.can_overwrite}
              checked={this.state.action === 'overwrite'}
              onChange={this.changeAction.bind(this, 'overwrite')}
            >
              {t('Overwrite slice: ') + `${this.props.slice.nick_name} ${this.props.slice.subtitle}`}
            </Radio>
          }

          <Radio
            id="saveas-radio"
            inline
            checked={this.state.action === 'saveas'}
            onChange={this.changeAction.bind(this, 'saveas')}
          > {t('Save as')} &nbsp;
          </Radio>
          <input
            name="new_slice_name"
            placeholder={t('[slice name]')}
            onChange={this.onChange.bind(this, 'newSliceName')}
            onFocus={this.changeAction.bind(this, 'saveas')}
          />
          &nbsp;&nbsp;&nbsp;
          <input
            name="subtitle"
            placeholder={t('[Subtitle]')}
            onChange={this.onChange.bind(this, 'newSubtitle')}
            onFocus={this.changeAction.bind(this, 'saveas')}
          />

          &nbsp;
          {t('Show subtitle in dashboard')}
          &nbsp;

          <input
            type="checkbox"
            onChange={this.onChange.bind(this, 'subtitleAble')}
            value
          />
          <br />
          <hr />

          <Radio
            checked={this.state.addToDash === 'noSave'}
            onChange={this.changeDash.bind(this, 'noSave')}
          >
            {t('Do not add to a dashboard')}
          </Radio>

          <Radio
            inline
            checked={this.state.addToDash === 'existing'}
            onChange={this.changeDash.bind(this, 'existing')}
          >
            {t('Add slice to existing dashboard')}
          </Radio>
          <Select
            options={dash}
            onChange={this.onChange.bind(this, 'saveToDashboardId')}
            autoSize={false}
            value={this.state.saveToDashboardId}
            placeholder={t('Select Dashboard')}
          />

          <Radio
            inline
            checked={this.state.addToDash === 'new'}
            onChange={this.changeDash.bind(this, 'new')}
          >
            {t('Add to new dashboard')} &nbsp;
          </Radio>
          <input
            onChange={this.onChange.bind(this, 'newDashboardName')}
            onFocus={this.changeDash.bind(this, 'new')}
            placeholder={t('[dashboard name]')}
          />
          <label style={{ marginLeft: 15 }}>
            <Checkbox
              onChange={this.onChange.bind(this, 'only_download')}
              value={ this.state.only_download }
            />
            {t('only download')}
          </label>

        </Modal.Body>

        <Modal.Footer>
          <Button
            type="button"
            id="btn_modal_save"
            className="btn pull-left"
            onClick={this.saveOrOverwrite.bind(this, false)}
          >
            {t('Save')}
          </Button>
          <Button
            type="button"
            id="btn_modal_save_goto_dash"
            className="btn btn-primary pull-left gotodash"
            disabled={this.state.addToDash === 'noSave'}
            onClick={this.saveOrOverwrite.bind(this, true)}
          >
            {t('Save & go to dashboard')}
          </Button>
        </Modal.Footer>
      </Modal>
    );
  }
}

SaveModal.propTypes = propTypes;

function mapStateToProps({ explore, saveModal }) {
  return {
    datasource: explore.datasource,
    slice: explore.slice,
    can_overwrite: explore.can_overwrite,
    user_id: explore.user_id,
    dashboards: saveModal.dashboards,
    alert: saveModal.saveModalAlert,
  };
}

export { SaveModal };
export default connect(mapStateToProps, () => ({}))(SaveModal);
