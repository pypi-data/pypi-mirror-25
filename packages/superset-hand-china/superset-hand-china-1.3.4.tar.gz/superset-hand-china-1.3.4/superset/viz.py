...
class BaseViz(object):
    ...
    def get_csv(self):
        ...
        try:
            columns = self.form_data['groupby'] + self.form_data['metrics']
        except Exception as e:
            columns = df.columns
        include_index = not isinstance(df.index, pd.RangeIndex)
        return df.to_csv(index=include_index, encoding="utf-8", columns=columns)
    ...
    def get_excel(self):
        ...
        try:
            ...
            columns = df.columns
        include_index = not isinstance(df.index, pd.RangeIndex)
        df.to_excel(writer,index=include_index, encoding="utf-8", columns=columns)
        writer.save()
...
class BoxPlotViz(NVD3Viz):
    ...
    def get_data(self, df):
        ...
        if whisker_type == "Tukey":
            ...
            raise ValueError(__("Unknown whisker type: ") + whisker_type)
...
class BubbleViz(NVD3Viz):
    ...
    def query_obj(self):
        ...
        d['row_limit'] = form_data.get('row_limit')
...
class NVD3DualLineViz(NVD3Viz):
    ...
    def query_obj(self):
        ...
        if m1 == m2:
            raise Exception(_("Please choose different metrics on left and right axis"))
        return d
...
class SankeyViz(BaseViz):
    ...
    def get_data(self, df):
        ...
        if cycle:
            # raise Exception(
            #     __("There's a loop in your Sankey, please provide a tree. "
            #     + "Here's a faulty link: ") + cycle)
            raise Exception(_(
