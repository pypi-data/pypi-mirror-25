...
from superset.hand.viz import viz_types
...
class Slice(Model, AuditMixinNullable, ImportMixin):
    ...
    subtitleAble = Column(Boolean, default=False)
    multi_language = Column(Text, default='{}')

    export_fields = ('slice_name', 'multi_language', 'datasource_type', 'datasource_name',
                     'viz_type', 'params', 'cache_timeout', 'subtitle', 'subtitleAble')
    ...
    def data(self):
        ...
        return {
            ...
            'subtitleAble': self.subtitleAble,
    ...
    def get_viz(self, url_params_multidict=None):
        ...
        slice_params['subtitle'] = self.subtitle
        slice_params['subtitleAble'] = self.subtitleAble
...
class Dashboard(Model, AuditMixinNullable, ImportMixin):
    ...
    refreshAble = Column(String(255), default='true')
    only_download = Column(Boolean, default=False)
    multi_language = Column(Text, default='{}')

    perm = 'dashboard|' + dashboard_title

    export_fields = ('dashboard_title', 'position_json', 'json_metadata',
                     'description', 'css', 'slug', 'theme', 'refreshAble', 'only_download', 'multi_language')
    ...
    def url(self):
        if self.json_metadata:
            ...
            if default_filters:
                ...
                return "/superset/dashboard/{}/".format(
    ...
    def import_obj(cls, dashboard_to_import, import_time=None):
        ...
        for slc in slices:
            slc_params = json.loads(slc.params)
            if slc.viz_type == 'filter_box_combination':
                filter_slcs_ids.extend(slc_params['filter_combination'])
        for slc in slices:
            ...
            if slc.id not in filter_slcs_ids:
        ...
        if existing_dashboard:
            ...
            security.merge_perm(sm, 'dashboard_access', copied_dash.get_perm())
            session.flush()
    ...
    def export_dashboards(cls, dashboard_ids):
        ...
        for dashboard_id in dashboard_ids:
            ...
            for slc in copied_dashboard.slices:
                ...
                slc_params = json.loads(slc.params)
                if slc.viz_type == 'filter_box_combination':
                    for filter_id in slc_params['filter_combination']:
                        filter_id = int(filter_id)
                        filter_slc = (
                            db.session.query(Slice)
                            .filter_by(id=filter_id).first()
                        )
                        datasource_ids.add(
                            (filter_slc.datasource_id, filter_slc.datasource_type))
                        # add extra params for the import
                        filter_slc.alter_params(
                            remote_id=filter_id,
                            datasource_name=filter_slc.datasource.name,
                            schema=filter_slc.datasource.name,
                            database_name=filter_slc.datasource.database.name,
                        )
                        filter_slcs.append(filter_slc)
        ...
        copied_dashboard.slices.extend(filter_slcs)
        return pickle.dumps({
    ...
    def get_perm(self):
        return ("dashboard|{obj.dashboard_title}").format(obj=self)
...
class Database(Model, AuditMixinNullable):
    ...
    def get_perm(self):
        return (
            "database|[{obj.database_name}].(id:{obj.id})").format(obj=self)
