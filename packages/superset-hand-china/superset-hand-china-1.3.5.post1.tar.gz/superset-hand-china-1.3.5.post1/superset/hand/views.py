...
class Hand(BaseSupersetView):
    ...
    # 查询talbes多语言字段
    @has_access
    @expose("/querytables/<talbe_id>", methods=['GET', 'POST'])
    def querytables(self,talbe_id):
        metrics =db.session.query(SqlMetric).filter_by(table_id = talbe_id).all()          
        columns = db.session.query(TableColumn).filter_by(table_id = talbe_id).all()
        mets=[]
        cols=[]
        for metric in metrics:
            met=[]
            met.append(metric.metric_name)
            met.append(metric.nick_name)
            mets.append(met)
        for column in columns:
            col=[]
            col.append(column.column_name)
            col.append(column.nick_name)
            cols.append(col)
        data = {}
        data['metrics'] = mets
        data['groupby'] = cols
