...
class Bitfinex(BaseClient):
  ...
  def ticker(self, ticker='btcusd'):
    ...
    '''
    return self._get('v1/pubticker/'+ticker, return_json=True)

  def ticker2(self, ticker='tBTCUSD'):
    '''
    Returns dictionary. 
    '''
    list = self._get('v2/ticker/'+ticker, return_json=True)
    return {
      'bid': list[0], 
      'bidSize': list[1], 
      'ask': list[2], 
