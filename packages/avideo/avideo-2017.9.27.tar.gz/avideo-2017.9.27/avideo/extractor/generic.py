...
class GenericIE(InfoExtractor):
    ...
    _TESTS = [
        ...
        # RSS feed
        {
            'url': 'http://phihag.de/2014/avideo/rss2.xml',
            'info_dict': {
                'id': 'http://phihag.de/2014/avideo/rss2.xml',
    ...
    def _real_extract(self, url):
        ...
        if not parsed_url.scheme:
                    ...
                    if default_search == 'auto_warning':
                        if re.match(r'^(?:url|URL)$', url):
                            raise ExtractorError(
                                'Invalid URL:  %r . Call avideo like this:  avideo -v "https://www.youtube.com/watch?v=BaW_jenozKc"  ' % url,
                                expected=True)
                        else:
            ...
            if default_search in ('error', 'fixup_error'):
                raise ExtractorError(
                    ...
                    'Set --default-search "ytsearch" (or run  avideo "ytsearch:%s" ) to search YouTube'
        ...
        if not full_response:
            ...
            # test whether it's HTML or not). According to avideo default Accept-Encoding
        ...
        if not found:
            ...
            # be supported by avideo thus this is checked the very last (see
            # https://dev.twitter.com/cards/types/player#On_twitter.com_via_desktop_browser)
