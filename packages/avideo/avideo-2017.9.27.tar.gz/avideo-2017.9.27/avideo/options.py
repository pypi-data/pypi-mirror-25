...
def parseOpts(overrideArguments=None):
    def _readOptions(filename_bytes, default=[]):
        ...
        try:
            # FIXME: https://notabug.org/GPast/avideo/commit/dfe5fa49aed02cf36ba9f743b11b0903554b5e56
    ...
    def _readUserConf():
        ...
        if xdg_config_home:
            userConfFile = os.path.join(xdg_config_home, 'avideo', 'config')
            if not os.path.isfile(userConfFile):
                userConfFile = os.path.join(xdg_config_home, 'avideo.conf')
        else:
            userConfFile = os.path.join(compat_expanduser('~'), '.config', 'avideo', 'config')
            if not os.path.isfile(userConfFile):
                userConfFile = os.path.join(compat_expanduser('~'), '.config', 'avideo.conf')
        ...
        if userConf is None:
            ...
            if appdata_dir:
                userConf = _readOptions(
                    os.path.join(appdata_dir, 'avideo', 'config'),
                    default=None)
                if userConf is None:
                    userConf = _readOptions(
                        os.path.join(appdata_dir, 'avideo', 'config.txt'),
                        default=None)

        if userConf is None:
            userConf = _readOptions(
                os.path.join(compat_expanduser('~'), 'avideo.conf'),
                default=None)
        if userConf is None:
            userConf = _readOptions(
                os.path.join(compat_expanduser('~'), 'avideo.conf.txt'),
                default=None)
    ...
    general.add_option(
        ...
        help='Use this prefix for unqualified URLs. For example "gvsearch2:" downloads two videos from google videos for avideo "large apple". Use the value "auto" to let avideo guess ("auto_warning" to emit a warning when guessing). "error" just throws an error. The default value "fixup_error" repairs broken URLs, but emits an error if this is not possible instead of searching.')
    general.add_option(
        '--ignore-config',
        action='store_true',
        help='Do not read configuration files. '
        'When given in the global configuration file /etc/avideo.conf: '
        'Do not read the user configuration in ~/.config/avideo/config '
        )
    general.add_option(
        '--config-location',
    ...
    authentication.add_option(
        ...
        help='Account password. If this option is left out, avideo will ask interactively.')
    authentication.add_option(
        '-2', '--twofactor',
    ...
    adobe_pass.add_option(
        ...
        help='Multiple-system operator account password. If this option is left out, avideo will ask interactively.')
    adobe_pass.add_option(
        '--ap-list-mso',
    ...
    verbosity.add_option(
        ...
        help='Contact the avideo server for debugging')
    verbosity.add_option(
        '--no-call-home',
        dest='call_home', action='store_false', default=False,
        help='Do NOT contact the avideo server for debugging')
    ...
    filesystem.add_option(
        ...
        help='Force resume of partially downloaded files. By default, avideo will resume downloads if possible.')
    filesystem.add_option(
        '--no-continue',
    ...
    filesystem.add_option(
        ...
        help='Location in the filesystem where avideo can store some downloaded information permanently. By default $XDG_CACHE_HOME/avideo or ~/.cache/avideo . At the moment, only YouTube player files (for videos with obfuscated signatures) are cached, but that may change.')
    filesystem.add_option(
    ...
    if overrideArguments is not None:
        ...
        if '--config-location' in command_line_conf:
            ...
            if os.path.isdir(location):
                location = os.path.join(location, 'avideo.conf')
            ...
            system_conf = _readOptions('/etc/avideo.conf')
