...
def single_factor_analysis(X, k=5, cycles=100, tol=.0001):
    ...
    tol : float, optional
        Tolerance value (default 0.0001)
    ...
    for i in range(cycles):
        ...
        MM = Psi_diag - PsiLambda.dot(np.linalg.inv(I +\
                Lambda.T.dot(PsiLambda))).dot(PsiLambda.T)
        dM = np.sqrt(np.linalg.det(MM))
        Beta = Lambda.T.dot(MM)
        # first moment of factors
        XXBeta_prime = XX.dot(Beta.T) 
        # compute second moment of factors
        EZZ = I - Beta.dot(Lambda) + Beta.dot(XXBeta_prime)

        # compute log likelihood 
        last_log_lkhd = log_lkhd
        log_lkhd = n_samples * const + n_samples * np.log(dM) - 0.5 * n_samples * np.sum(np.diag(MM.dot(XX)))
        lkhd_list.append(log_lkhd)

        # compute maximization 
        # update Lambda
        Lambda = XXBeta_prime.dot(np.linalg.inv(EZZ))
        # update Psi
        ...
        warnings.warn('Factor analysis did not converge for' +
                'tol : %.4f, you may want to increase the cycles' % tol)
