...
class GuessAndCheck:
	...
	def train_and_test (self, ratio, option):	
		fe = Features (self.model_path, self.analyzer)
		barrier = self.get_barrier ()
		print ("training %d data" % barrier)		
		train_data = fe.select (self.trainset [:barrier], *option [:4])
		...
		model.train (train_data, processor, featurerepr, normalize = 1, grid = 0)
		return fe, model.test (self.trainset [(self.ratio == 1 and -1000 or barrier):])			
