...
class DBInterface(object):
    ...
    @property
    def cursor(self):
        # Only construct cursor if not already active, this allows multiple operations
        # between a commit/rollback to use the same cursor
        if not self.cur:
    ...
    def commit(self):
        self.conn.commit()
        self.cur = None  # clears cursor for next operation
    ...
    def clear_cursor(self):
        self.cur = None  # in case it needs to be done by hand
    ...
    def create_table(self, cursor, tablename, fields=None, commit=True, verbose=True):
        if fields is None:
            fields = self.db_structure['tables'][tablename]
    ...
    def _create_indexes(self, index_map, primary, commit=True, verbose=True):
        for tablename, indexspec in index_map.items():
            ...
            for columns in indexes:
                self.create_index(tablename, columns, primary=primary, commit=False, verbose=verbose)
        if commit:
            self.commit()
        return True
    ...
    def create_keys(self, commit=True, verbose=True):
        ...
        result = self._create_indexes(index_map, primary=True, commit=commit, verbose=verbose)
        if verbose:
    ...
    def create_indexes(self, commit=True, verbose=True):
        ...
        result = self._create_indexes(index_map, primary=False, commit=commit, verbose=verbose)
        if verbose:
