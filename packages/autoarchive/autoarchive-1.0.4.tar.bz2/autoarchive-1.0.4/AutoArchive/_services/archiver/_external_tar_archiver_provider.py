...
from ._tar_archiver_provider_base import *
...
class _ExternalTarArchiverProvider(_TarArchiverProviderBase):
    ...
    # {{{ _TarArchiverProviderBase overrides or IArchiver implementation
    ...
    def supportedBackupTypes():
        "See :attr:`.IArchiver.supportedBackupTypes`"
    ...
    def backupFiles(self, backupDefinition, compressionStrength = None):
        ...
        super().backupFiles(backupDefinition, compressionStrength)
        ...
        workingBackupFilePath = self.getWorkingPath_(backupFilePath)
    ...
    def backupFilesIncrementally(self, backupDefinition, compressionStrength = None, level = None):
        ...
        super().backupFilesIncrementally(backupDefinition, compressionStrength, level)
        ...
        if level < 0 or level > maxBackupLevel:
            raise ValueError(str.format(
                "'level' must be from interval 0 <= level <= maxBackupLevel(). The passed value was {}.", level))
    ...
    def removeBackupIncrements(self, backupDefinition, level = None):
        ...
        while backupExists:
            ...
            if backupExists:
                ...
                externalTarIncrementalUtility.tryRemoveSnapshotFile(removeLevel)
            removeLevel += 1

        if self.getMaxBackupLevel(backupDefinition.backupId) > level:
    ...
    def __processTarOutput(self, archiverProcess):
        ...
        try:
            ...
            # capture program's standard output and standard error and use IComponentUi interface to print captured
    ...
    def __propagateArchiverMessage(self, message, sentToStderr = False):
        ...
        Parses the passed ``message``, evaluates it and fires :meth:`.IArchiver.backupOperationError` event if it is
        an (non-fatal) error message or :meth:`.IArchiver.fileAdd` otherwise."""
    ...
    class __ExternalTarIncrementalUtility:
        ...
        def getSnapshotFileName(self, level):
            "Returns full path to snapshot file for a certain backup level."

            return os.path.join(self.__snapshotsDir,
                                self.__backupId + self.__getLevelSuffix(level) + self.__SNAPSHOT_SUFFIX)
        ...
        def tryRemoveSnapshotFile(self, level):
            ...
            snapshotFileName = self.getSnapshotFileName(level)
        ...
        def getLevelFromFileName(fileName):
            ...
            try:
                levelToken = os.path.splitext(os.path.splitext(fileName)[0])[1]
