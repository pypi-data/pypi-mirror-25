...
class ArchiverTestUtils(metaclass = ABCMeta):
    ...
    def createArchiverMock():
        """Creates a mock of :class:`_IArchiver`.

        :return: Mock of the :class:`_IArchiver`.
        :rtype: :class:`mock.Mock<_IArchiver>`"""

        def backupFilesSideEffect(backupDefinition, compressionStrength = None, level = None):
        ...
        archiverMock = Mock(spec_set = IArchiver)
    ...
    @classproperty
    def _irrelevantValidFilePath(cls):
    ...
    def _extractBackup(cls, backupFilePaths):
        ...
        :param backupFilePath: Path to the backup file or files.  In case of incremental backup, list should be provided
        otherwise a single path only.
        :type backupFilePath: ``str`` or ``Iterable<str>``
        :param levels: How many levels from an incremental backup shall be extracted.  If 0 then archive will be
           treated as non-incremental.
        :type levels: ``int``
