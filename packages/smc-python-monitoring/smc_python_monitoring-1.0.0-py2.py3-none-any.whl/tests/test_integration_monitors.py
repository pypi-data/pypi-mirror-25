...
from smc_monitoring.models.formatters import RawDictFormat, InvalidFieldFormat
...
class Test(unittest.TestCase):
    ...
    def test_blacklist_with_invalid_foramt(self):
        """
        Combined filters are not supported when using formatters. You must
        use a single filter type as a combined filter returns data in a customized
        format.
        """
        query = BlacklistQuery('foo')
        with self.assertRaises(InvalidFieldFormat):
