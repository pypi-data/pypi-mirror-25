...
class ClientProcess(multi.Process):
    ...
    def terminate(self):
        ...
        kill_proc_tree(self.pid, children_only=True)
        try:
...
class BaseController(ClientProcess):
    ...
    def start_process(self, target, name, ps_args=(), ps_kwargs={}):
        ...
        logger.info("CHILD PROCESSES %s" % self.child_processes)
    ...
    def stop_process(self, pid=None):
        ...
        # # old method
        # logger.info("STOPPING ps named %s" % name)
        # try:
        #     logger.info("CHILD PROCESSES %s" % self.child_processes)
        #     for child_ps in self.child_processes:
        #         logger.info("Stopping ps: %s" % child_ps)
        #         logger.info("Comparing names: %s and %s"
        #                     % (child_ps.name, name))
        #         if child_ps.name == name:
        #             child_ps.terminate()
        # except psutil.NoSuchProcess:
        #     logger.info("No " + name + " to stop")
        # except Exception:
        #     logger.warning("Unexpected exception stopping " + name,
        #                    exc_info=True)
...
class MultiClientController(BaseController):
    ...
    Attributes:
        ...
        keepers - a dictionary with a k,v paid for each keeper that
            ...
            case. Normal kickoff() usage won't need this.
    """
...
def kill_proc_tree(pid=None, including_parent=True, exc_pid=None,
                   children_only=False):
    ...
    # Otherwise, kill the process and all child processes
    try:
        if not check_pid(pid):
            return
        logger.warning("Killing proc tree for pid %s" % pid)
