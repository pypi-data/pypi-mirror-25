...
class MotorClient(BaseClient):
    ...
    def check_motor(self, update_command=True):
        ...
        state = {"status": status, "state": "Ready", "model": "ADP-055-18",
            "manufacturer": "Copley", "instrument_type": "CopleyADP-055-18",
            "flags": flags, 'nmt_states': self.card.nmt_states,
            "connection": self.card.channel}
        self.update_gateway_state({'instruments': [state], "status": "Ready"})
        if update_command:
        ...
        while True:
            ...
            try:
                ...
                if self.setup["category"] == "CanBus":
                    self.run_canbus()
                if self.setup["category"] == "Misc" and self.setup["arg"] == "check_scope":
    ...
    def run_canbus(self):
        ...
        if not incoming_frame:
            ...
            logger.error("No frames received after " + str(MAX_RETRIES) +
                         " attempts")
            self.update_commands(status="No Frames Received")
            return
        else:
        ...
        self.update_commands(status="CAN Bus In Progress")
        output_frames = []
        for frame_to_write in frames_to_write:
            ...
            if "expression" in frame_to_write.keys():
                if frame_to_write["expression"] == "DOWNLOAD":
                    ...
                    raise ValueError("unsure what to do about expression: %s" % frame_to_write["expression"])
            else:
    ...
    def run_motor_trace(self):
        ...
        while True:
            ...
            logger.info("Motor state is: "+str(state) + " setup is: " +
                        str(self.setup["id"])+" pos is: "+str(self.card.property_getter("actual_position")))
