...
class Searcher:
	...
	def do_status (self, *arg, **karg):
		...
		return {
			"version": self.si.segments.version,			
			"lastupdated": max (self.mod_time, max ([x [-1] for x in segmentinfos])),
	...
	def do_query (self, qs, offset = 0, fetch = 10, sort = "", summary = 30, returns = "", lang = "un", analyze = 1, data = 1, limit = 1, *arg, **karg):
		try:
			offset = int (offset)
			fetch = int (fetch)
			summary = int (summary)
		...
		if cached and (len (cached [-1]) >= offset + fetch or len (cached [-1]) == cached [0]):
			...
			result ["time"] = 0.0
			if data:
				result ["result"] = self.do_fetch (bucket  [offset:offset + fetch], summary, highlights, returns)								
			else:
		...
		try:
			...
			if data:
				result ["result"] = self.do_fetch (q.bucket  [offset:offset + fetch], summary, list(q.highlights.keys ()), returns)
	...
	def do_fetch (self, bucket, summary, highlights, returns, *arg, **karg):
		f = fetcher.Fetcher (self.segments, bucket, summary, highlights, returns)		
		try:
			result = f.fetchall ()
		finally:			
			f.searcher = None			
