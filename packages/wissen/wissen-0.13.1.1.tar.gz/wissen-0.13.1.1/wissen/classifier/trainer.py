...
class Trainer (indexer.Indexer):
	...
	def rebuildIndex (self, breader):		
		...
		col = wissen.collection (os.path.join (self.si.fs.getmaster (), "%s.ts" % self.si.alias), wissen.APPEND, logger = self.si.logger)
	...
	def add_document (self, labeledDocument):
		...
		if self.indexer is None:
			...
			col = wissen.collection (os.path.join (self.si.fs.getmaster (), "%s.ts" % self.si.alias), self.si.rebuild and wissen.CREATE or wissen.WRITE, self.si.analyzer, self.si.logger)
