...
class Classifier (searcher.Searcher):
	...
	def do_refresh (self, *arg, **karg):
		...
		self.clfs [wissen.MULTIPATH].setopt (clfs = subclfs, cluster = self.clfs [wissen.TERMCLUSTER])
	...
	def do_cluster (self, qs, lang = 'un', *arg, **karg):
		...
		clusters = self.get_classifier (wissen.TERMCLUSTER).cluster (mem, qs, lang)
		result = {
			"code": 200,
