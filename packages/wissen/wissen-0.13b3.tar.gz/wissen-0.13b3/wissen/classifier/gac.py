...
class GuessAndCheck:
	...
	def guess (self, document, lang = "en"):
		...
		rs = []
		for cl in (wissen.META, wissen.NAIVEBAYES, wissen.FEATUREVOTE, wissen.ROCCHINO, wissen.MULTIPATH, wissen.TFIDF, wissen.SIMILARITY):
	...
	def train (self, selector = wissen.IG, select = 2000):
		learner = self.get_learner ()	
		r = learner.train (selector = selector, select = select, orderby = wissen.MAX, dfmin = 0, dfmax = 0)
		learner.close ()			
		return r
	...
	def test (self, cl = wissen.NAIVEBAYES):
