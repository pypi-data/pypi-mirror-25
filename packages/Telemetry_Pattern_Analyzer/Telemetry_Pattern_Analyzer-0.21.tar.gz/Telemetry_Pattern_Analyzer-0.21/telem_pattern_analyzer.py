...
class Telemetry_Pattern_Analyzer(object):
    ...
    def GenerateMarkerUniqueCode(errorMarkers_unique):
        uncode_value=[]
        index=0
    ...
    def GetMarkerDescription(code):
        code_string = code
        code_string_tuple_len = len(code_string)
        marker=str()
        for i in range(0,code_string_tuple_len,4):
            code_string_tuple=code_string[i:i+4]
            marker += self.df_uncode_value[df_uncode_value['key']==code_string_tuple].iloc[0][1]+'|'

        return marker
    
    def GetMarkerCode(marker):
    ...
    def GetDNA_Set(self,dna_strain):
        nibbles=[]
