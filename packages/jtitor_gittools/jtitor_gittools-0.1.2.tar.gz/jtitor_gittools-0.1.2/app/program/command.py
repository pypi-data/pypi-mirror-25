...
class CommandRunner(object):
	...
	def _enterDirAndExecute(self, commandLabel, runDirectory, command):
	...
	def addRemote(self, repoPath, remoteName, remoteUrl):
		if self._dryRun:
			...
			self._enterDirAndExecute("addRemote", repoPath, ["git", "remote", "add", remoteName, remoteUrl])
	...
	def pull(self, repoPath):
		...
		if self._dryRun:
			...
			self._enterDirAndExecute("pull", repoPath, ["git", "pull", "origin"])
	...
	def getCurrentBranch(self, repoPath):
		...
		if self._dryRun:
			...
			branchOutput = self._enterDirAndExecute("get-branch", repoPath, kCommand).decode()
