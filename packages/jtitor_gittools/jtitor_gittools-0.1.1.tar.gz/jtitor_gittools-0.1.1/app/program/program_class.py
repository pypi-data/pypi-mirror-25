...
from .directory_traversal import DirectoryRunner, GitPullDelegate, BuildDictionaryDelegate
...
class Program(object):
	...
	def __init__(self, args):
		...
		self.log = Logger(self._minLogLevel)
		self._parsedRepoDict = None
		self._command = CommandRunner(self.log, self.dryRun)
	...
	def _parseRepoFile(self, repoFilePath):
		...
		jsonFile = None
		try:
			self.log.debug("Opening file '{0}'".format(repoFilePath))
	...
	def _reportStatus(self, statusFormatString):
		...
		'''
		errors = self.log.entriesOfLevel(LogLevel.Error)
		warnings = self.log.entriesOfLevel(LogLevel.Warn)
		...
		self.log.logMessage(resultStatus, statusFormatString.format(statusString))
		...
		if numErrors + numWarnings > 0:
			...
			if numErrors > 0:
				...
				self.log.printLogEntries(errors)
			if numWarnings > 0:
				self.log.info("Warnings:")
				self.log.printLogEntries(warnings)
		...
		try:
			self.log.dumpEntriesToFile(defaultLogPath)
		except IOError:
	...
	def _jsonRun(self, statusFormatString, validateOnly, runFailedString, runRaisedExceptionString):
		...
		try:
			...
			cloneTreeRunner = JsonRunner(outPath, self.log, self._command, self._parsedRepoDict)
			treeErrors = cloneTreeRunner.run(validateOnly)
			if treeErrors:
				self.log.error(runFailedString)
		except RuntimeError as e:
			self.log.error(runRaisedExceptionString + ": {0}".format(str(e)), e)
	...
	def _pullRepos(self):
		...
		try:
			directoryRunner = DirectoryRunner(self.log)
			directoryRunner.run(treeRootPath, GitPullDelegate(self._command))
		except RuntimeError as e:
			self.log.error("Fatal error while pulling repositories: {0}".format(str(e)), e)
		...
		if canRunExport:
			try:
				...
				if not self.dryRun:
					...
					self.log.debug("Dry run: export to file '{0}'".format(destinationFilePath))
					self.log.debug(("Dry run: data to export: "
					"'{0}'").format(directoryRunner.directoryStructure()))
			except RuntimeError as e:
				self.log.error("Fatal error while exporting directory tree: {0}".format(str(e)), e)
