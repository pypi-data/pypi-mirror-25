...
class TerminalCopyMachine:
    ...
    def tab(self, conf):
        ...
        for c in conf.ordered_choices:
            ...
            if conf.choices[c].get('checkbox', False):
                ...
                checkbox.state = self._checkboxes.get(self._current_tab, {}).get(c, False)
    ...
    def item_chosen(self, button, choice):
        ...
        """
        if 'text' in self.choices[choice].keys() and self.choices[choice]['text'] is not None:
    ...
    def checkbox_toggled(self, checkbox, state, key):
        ...
        """
        try:
            self._checkboxes[self._current_tab][key] = state
        except KeyError:
            self._checkboxes[self._current_tab] = {}
            self._checkboxes[self._current_tab][key] = state
