...
class FormValidationError(CustomException):
    errcode = 2001
    errmsg = '表单验证错误'

    def __init__(self, form, errmsg=None, show_first_error=False):
        if not errmsg and show_first_error:
            print('this is me', errmsg)
            errmsg = next(iter(form.errors.values()))[0]
        super(FormValidationError, self).__init__(errmsg=errmsg,
                                                  errors=form.errors)
