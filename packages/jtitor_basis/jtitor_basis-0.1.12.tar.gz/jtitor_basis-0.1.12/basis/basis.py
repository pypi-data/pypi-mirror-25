	...
	def _doRun(self, stepMethod, forceCheck):
		...
		try:
			...
			try:
				...
				self._log.error("Failed to initialize logging: {0}! This run might not be saved to logs!")
			...
			dumpPath = ""
			try:
				dumpPath = ShellHelpers.defaultLogFilePath(context)
				self._log.dumpEntriesToFile(dumpPath)
			except Exception as e:
				self._log.error("Failed to dump logs to '{0}': {1}! This run won't be saved to logs!".format(dumpPath, str(e)))
