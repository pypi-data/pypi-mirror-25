...
class Message(Item):
    ...
    def forward(self, to_recipients, cc_recipients=None, bcc_recipients=None, subject_prefix='Fwd:'):
        # Helper method to create a new message and send (forward) it to the specified recipients
        new_msg = self.__class__(**{f.name: getattr(self, f.name) for f in self.supported_fields()})
        new_msg.account = self.account
        new_msg.to_recipients = to_recipients
        new_msg.cc_recipients = cc_recipients
        new_msg.bcc_recipients = bcc_recipients
        new_msg.subject = ' '.join([subject_prefix, new_msg.subject])
        new_msg.send()
        return new_msg
