...
class Tokenizer:
    ...
    def _validate_delimiter_tokens(self):
        ...
        for token in Tokens.get_list():
            ...
            # # Usually "-" is used as a delimiter when everything inside a
            # # bracket is separated by it, if there is another delimiter inside
            # # the brackets (or outside) then "-" is not a delimiter
            # if delimiter == '-':
            #     last_bracket = Tokens.find_previous(token, TokenFlags.BRACKET)
            #     next_bracket = Tokens.find_next(token, TokenFlags.BRACKET)
