...
class Chessboard:
    ...
    def set_pos(self, x, y, user, check=False):
        '''Set a chess'''
        if isinstance(x, str):
    ...
    def handle_input(self, input_str, user, check=False):
        '''Transfer user input to valid chess position'''
        input_str = input_str.replace(' ', '')
        pos_str = input_str.split(',')
        if (len(pos_str) != 2):
            raise PositionError('Error number of coordinates!')
        try:
            x, y = int(pos_str[0]), int(pos_str[1])
        except:
            raise PositionError('Coordinates must be integers!')
        else:
            if x > self.board_size or y > self.board_size:
                raise PositionError('Coordinates must be smaller than size of chessboard!')
            else:
                try:
        ...
        for ang in self.angle:
            ...
            while True:
                ...
                for ind, a in enumerate(angs):
                    target_x = int(x - 1 + radius*(sign(math.cos(a)))) if direction[ind] else -1
                    target_y = int(y - 1 - radius*(sign(math.sin(a)))) if direction[ind] else -1
...
if __name__ == '__main__':
    cb = Chessboard(board_size=3, win=3)
    while True:
        pos = input('Input:')
        player = 2 - cb.game_round % 2
        a = cb.handle_input(pos, player, True)
        if a:
