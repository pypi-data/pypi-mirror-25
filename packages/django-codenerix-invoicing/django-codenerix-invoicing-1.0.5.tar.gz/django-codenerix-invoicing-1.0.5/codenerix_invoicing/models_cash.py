...
class CashDiary(CodenerixModel):
    ...
    def amount_cash(self):
        total = self.cash_movements.filter(kind=PAYMENT_DETAILS_CASH).aggregate(total=Sum('amount'))
        if total:
            return total['total']
        else:
            return 0.0
    ...
    def amount_cards(self):
        total = self.cash_movements.filter(kind=PAYMENT_DETAILS_CARD).aggregate(total=Sum('amount'))
        if total:
            return total['total']
        else:
            return 0.0
