...
def kitchen_delete(backend, kitchen, yes):
    ...
    try:
        ...
        raise click.ClickException('Cannot get kitchen children for kitchen %s' % kitchen)

    if not yes:
        confirm = raw_input('\nAre you sure you want to delete the remote copy of the Kitchen %s and its children ? [yes/No]' % kitchen)
...
def recipe_delete(backend,kitchen,name, yes):
    ...
    if not yes:
        confirm = raw_input('Are you sure you want to delete the remote copy Recipe %s? [yes/No]' % name)
        if confirm.lower() != 'yes':
            return
...
def recipe_get(backend, recipe, force):
    ...
    check_and_print(DKCloudCommandRunner.get_recipe(backend.dki, kitchen_name, recipe_name, force, start_dir))
...
@click.option('--message', '-m', type=str, required=True, help='change message')
@click.pass_obj
def file_update_all(backend, message):
    """
    ...
    check_and_print(DKCloudCommandRunner.update_all_files(backend.dki, kitchen, recipe, recipe_dir, message))
...
def file_delete(backend, kitchen, recipe, message, filepath):
    """
    Delete one or more Recipe files. Provide the file path(s) relative to the recipe root.
...
@click.option('--kitchen', '-k', type=str, required=False, help='kitchen name')
@click.pass_obj
def kitchen_settings_get(backend, kitchen):
    """
    ...
    click.secho("%s - Getting a local copy of kitchen-settings.json for Kitchen '%s'" % (get_datetime(), use_kitchen), fg='green')
    check_and_print(DKCloudCommandRunner.kitchen_settings_get(backend.dki, use_kitchen))
...
@click.option('--kitchen', '-k', type=str, required=False, help='kitchen name')
@click.pass_obj
def kitchen_settings_update(backend, filepath, kitchen):
    """
    ...
    click.secho("%s - Updating the settings of Kitchen '%s'" % (get_datetime(), use_kitchen), fg='green')
    check_and_print(DKCloudCommandRunner.kitchen_settings_update(backend.dki, use_kitchen, filepath))
