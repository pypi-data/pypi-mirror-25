...
from utils.DKFileUtils import DKFileUtils
...
class DKCloudCommandRunner(object):
    ...
    def print_kitchen_children(dk_api, kitchen):
        ...
        if rc.ok():
            ...
            if kl is not None and len(kl) > 0:
                tree_kitchen_dict = DKCloudCommandRunner.prepare_tree_kitchen_dict(kl)
                # Do Print Children
                print 'Child kitchens for kitchen %s:' % kitchen
    ...
    @staticmethod
    @check_api_param_decorator
    def get_recipe(dk_api, kitchen, recipe_name_param, force, start_dir=None):
        rc = DKReturnCode()
        try:
    ...
    @staticmethod
    @check_api_param_decorator
    def update_all_files(dk_api, kitchen, recipe_name, recipe_dir, message):
        """
        reutrns a string.
    ...
    def update_file(dk_api, kitchen, recipe_name, recipe_dir, message, files_to_update_param):
        ...
        if len(rl['only_local']) > 0:
            for file_to_update in files_to_update:
                ...
                index = file_to_update.rfind("/")
                if index > -1:
                    file_path = "/" + file_to_update[0:index]
                    file_name = file_to_update[index+1:]
                else:
                    file_path = ''
                    file_name = file_to_update
                complete_path = recipe_name + file_path
                if complete_path in rl['only_local']:
                    file_list_in_path = rl['only_local'][complete_path]
    ...
    def recipe_variation_list(dk_api,kitchen, recipe_name, get_remote):
        ...
        try:
            if get_remote:
                ...
                msg = DKCloudCommandRunner.get_local_recipe_variation_list(recipe_name)
            rc.set(DKReturnCode.DK_SUCCESS, msg)
        except Exception as e:
            rc.set(rc.DK_FAIL, e.message)
        return rc
    ...
    @staticmethod
    def get_local_recipe_variation_list(recipe_name):
        current_path = os.getcwd()
        recipe_path = current_path[0:current_path.find(recipe_name)+len(recipe_name)]
    ...
    def file_diff(dk_api, kitchen, recipe, recipe_dir, file_path):
        ...
        try:
            ...
            file_content = dk_api.get_file(kitchen, recipe, file_path)
            ...
            aux_full_path = '%s/%s/%s/%s' % (base_working_dir, kitchen, recipe, file_path)
            DKFileUtils.write_file(aux_full_path, file_content)

            # Call diff tool
            template_dict = dict()
            template_dict['remote'] = aux_full_path
            template_dict['local'] = '%s/%s' % (recipe_dir, file_path)
    ...
    def kitchen_settings_get(dk_api, kitchen):
        ...
        if not rc.ok():
            rc.set_message('kitchen_settings_get error. %s\nmessage: %s' % (kitchen, rc.get_message()))
        else:
            message = 'DKCloudCommand.kitchen_settings_get %s succeeded\n' \
    ...
    def kitchen_settings_update(dk_api, kitchen, filepath):
        ...
        if not rc.ok():
            rc.set_message('kitchen_settings_update error. %s\nmessage: %s' % (kitchen, rc.get_message()))
        else:
            rc.set_message('DKCloudCommand.kitchen_settings_update %s succeeded\n' % kitchen)
        return rc
