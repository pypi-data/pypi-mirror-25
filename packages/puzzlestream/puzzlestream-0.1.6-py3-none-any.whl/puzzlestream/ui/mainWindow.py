...
from puzzlestream.backend.manager import PSManager
...
class PSMainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.__editor = PyCodeEdit(server_script=server.__file__,
                                   args=["-s" + os.getcwd()])
    ...
    def __createToolBarActions(self):
        ...
        self.__openProjectAction = QAction(
            QIcon(os.path.join(currentDir, "../Icons//openproject.png")),
            'openProject', self
        )

        self.__saveFileAction = QAction(
            QIcon(os.path.join(currentDir, "../Icons//savefile.png")),
            'Save File', self
        )

        self.__undoAction = QAction(
            QIcon(os.path.join(currentDir, "../Icons//back.png")),
            'Back', self
        )

        self.__redoAction = QAction(
            QIcon(os.path.join(currentDir, "../Icons//forward.png")),
            'Forward', self
        )
        ...
        self.__runAction = QAction(
            QIcon(os.path.join(currentDir, "../Icons//run.png")),
            'Run Puzzle', self
        )

        self.__stopAction = QAction(
            QIcon(os.path.join(currentDir, "../Icons//stop.png")),
            'Stop Puzzle', self
        )
    ...
    def __newProject(self):
        ...
        if foldername != "":
            if len(os.listdir(foldername)) == 0:
                self.__manager.newProject(foldername)
                self.setWindowTitle(
    ...
    def __showPlots(self, puzzleItem):
        plotview = PSPlotView(puzzleItem.streamSection.data, self)
