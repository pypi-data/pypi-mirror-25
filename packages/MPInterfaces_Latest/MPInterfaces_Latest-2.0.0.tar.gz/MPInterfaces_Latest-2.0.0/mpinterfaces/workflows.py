...
def StepVASP0(my_project, struct_list):
    """
    Template for a general starting workflow for initial calibrations of
    input structures which may be relaxations or calibrations over input
    parameters. Reads in the Structure List and the project naming parameters
    """
    ...
    # Decide the Kpoints density per atom setting here, Pseudopotential choice and Structures 
    turn_knobs= OrderedDict({'POSCAR': struct_list,'KPOINTS':Workflow_Params['KPOINTS'],\
                             'POTCAR_functional':Workflow_Params['PSEUDOPOTENTIAL']})

    if 'Other_Knobs' in list(Workflow_Params.keys()):
...
def StepVASP1(my_project):
   ...
   incar_dict.update(Workflow_Params['Incar_Update'])
   ...
   run_cal(turn_knobs, qadapter, job_cmd, job_dir,
            ...
            reuse=True, is_matrix=False, Grid_type=Workflow_Params['Kpt_Grid'])
