...
from mpinterfaces.utils import get_ase_slab, get_magmom_string, get_magmom_afm, \
    get_magmom_mae, print_exception, get_mueller_kpoints, get_magmom_hunds, get_magmom_custom,\
    get_hse_kpoints_file, get_hse_prep_incar, get_hse_incar
...
class Calibrate(MSONable):
    ...
    def __init__(self, incar, poscar, potcar, kpoints, system=None,
                 ...
                 turn_knobs=OrderedDict([('ENCUT', []),
                                         ('KPOINTS', [])]),
                 finer_kpoint=None, checkpoint_file=None, cal_logger=None):
        """
    ...
    def setup_matrix_job(self):
        ...
        #print ("list of keys ", keys, n_items, self.turn_knobs)
    ...
    def recursive_jobs(self, n, keys, i):
        ...
        job_dir = '__'.join(
            [self.job_dir, self.key_to_name(keys[i])])
        #### Testing ####
        if i == n - 1 and i != 0:
    ...
    def set_poscar(self, scale=None, poscar=None):
        ...
        """
        #print ('POSCAR setting from set_poscar')
        try:
         #print ('Comes here')
         if scale is not None:
            ...
            self.logger.info('Poscar is being set')
            #print (poscar)
           #print (poscar,type(poscar))
            self.poscar = poscar
        except:
         print_exception()
        ...
        if not self.database:
            ...
            if self.Grid_type == 'M':
                ...
                self.kpoints = Kpoints.automatic_linemode(divisions=kpoint,
                                                          ibz=HighSymmKpath(
                                                              poscar.structure))
            elif self.Grid_type == 'Mueller_server_algorithm':
                self.kpoints = get_mueller_kpoints(int(kpoint))
            ...
            elif self.Grid_type == 'HSE_bands_prep':
                if kpoint:
                   self.kpoints = Kpoints.automatic_gamma_density(poscar.structure,kpoint)
                else:
                   self.kpoints = Kpoints.automatic_gamma_density(poscar.structure,200)

            elif self.Grid_type == 'HSE_bands':
                if kpoint:
                   self.kpoints = get_hse_kpoints_file(poscar.structure, ibzkpth, density=kpoint)
                else:
                   self.kpoints = get_hse_kpoints_file(poscar.structure, ibzkpth)

        # applicable for database runs
        # future constructs or settinsg can be activated via a yaml file
        # database yaml file or better still the input deck from its specification
        ...
        if scale_list:
            ...
            for pos in poscar_list:
                ...
                if self.reuse:
                    ...
                    try:
                        ...
                        if self.Grid_type == 'hse_bands_2D_prep':
                            ...
                            incar_dict = get_hse_prep_incar(incar_dict)
                            self.set_kpoints(poscar=poscar)
                            self.logger.info('updating INCAR KPOINTS for HSE Prep step')

                        elif self.Grid_type == 'HSE_bands':
                            # most general HSE bands wherein a general incar update would 
                            # be provided by the user via a yaml, for example for some
                            # material one may want to do HSE+SOC otherwise the user can 
                            # use a HSE input set that has been tried for 2D materials with 
                            # good success
                            if self.reuse_incar == 'HSE_optimized':
                               incar_dict = get_hse_incar(incar_dict) 
                               
                            self.set_kpoints(poscar=poscar, ibzkpth=pos + os.sep + 'IBZKPT')
                            self.logger.info('updated input set for HSE calculation\
                                             using IBZKPT from {0}'.format(pos + os.sep +\
                                             'IBZKPT'))
                ...
                # case for non - reuse
                    ...
                    self.logger.info('Setting POSCAR from non-reuse {}'.format(type(pos)))
                    poscar = pos
                    #print (poscar)
                    #print (self.database)
                ...
                # Magnetism  
                if self.magnetism=='Hunds':
                    incar_dict['MAGMOM'] = get_magmom_hunds(poscar)
                elif self.magnetism=='Custom':
                    incar_dict['MAGMOM'] = get_magmom_custom(poscar)

                self.incar = Incar.from_dict(incar_dict)
                #print ('Going to call set_poscar and set_potcar')
