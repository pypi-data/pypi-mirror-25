...
def set_config():
    ...
    """
    global MPINT_CONFIG, MP_API, VASP_PSP, QUEUE_TEMPLATE
    global USERNAME, VASP_STD_BIN, VASP_TWOD_BIN, VDW_KERNEL, QUEUE_SYSTEM

    try:
       MPINT_CONFIG = loadfn(os.path.join(PACKAGE_PATH, 'mpint_config.yaml'))
    except:
       MPINT_CONFIG = {}
       warnings.warn('mpint_config.yaml file not configured. Be careful of running VASP jobs')

    if MPINT_CONFIG:
# set environ variables for MAPI_KEY and VASP_PSP_DIR
       if MPINT_CONFIG.get('potentials',None):
          os.environ['PMG_VASP_PSP_DIR'] = MPINT_CONFIG['potentials']# '')
          VASP_PSP = MPINT_CONFIG['potentials']
       else:
          warnings.warn('Potentials path not set. Please set because it is a necessary environment variable')
       MP_API = MPINT_CONFIG.get('mp_api',None)
       if MP_API:
          os.environ['MAPI_KEY'] = MP_API
       else:
          warnings.warn('MAPI KEY not set for MaterialsProject queries')
       ...
       QUEUE_TEMPLATE = MPINT_CONFIG.get('queue_template', None) + '/qtemplate.yaml'
...
def get_struct_from_mp(formula, MAPI_KEY="", all_structs=False):
    ...
    """
    MPINT_CONFIG, MP_API, VASP_PSP, QUEUE_TEMPLATE,\
         USERNAME, VASP_STD_BIN, VASP_TWOD_BIN, VDW_KERNEL, QUEUE_SYSTEM = set_config()
    if not MP_API:
        MP_API = os.environ.get("MAPI_KEY", "")
        if not MP_API:
