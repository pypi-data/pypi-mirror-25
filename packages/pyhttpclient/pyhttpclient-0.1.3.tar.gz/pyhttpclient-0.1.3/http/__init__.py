...
def put(url, data=None, headers=None, timeout=socket._GLOBAL_DEFAULT_TIMEOUT):
...
def delete(url, data=None, headers=None, timeout=socket._GLOBAL_DEFAULT_TIMEOUT):
...
def post(url, query=None, data=None, headers=None, timeout=socket._GLOBAL_DEFAULT_TIMEOUT, dataGzip=False, dataJson=False):
    opener = urllib2.build_opener(urllib2.HTTPHandler)
...
def get(url, data=None, headers=None, timeout=socket._GLOBAL_DEFAULT_TIMEOUT, dataGzip=False):
    opener = urllib2.build_opener(urllib2.HTTPHandler)
    data = data and urllib.urlencode(data) or ''
    url = '%s?%s' % (url, data)
    if dataGzip:
