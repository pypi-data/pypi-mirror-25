...
from nlp.pos.lstm import model as lstm_model, bilstm_model
from nlp.pos.lstm.dataset import rawdata
...
class ModelLoader(object):
    def __init__(self, args, ckpt_path):
        ...
        print("Initializing pos_tagger model...")
        self.model = self.init_ner_model(self.session, self.ckpt_path)

    def init_ner_model(self, session, ckpt_path):

        with tf.variable_scope("pos_var_scope"):
            if self.method == "lstm":
                model = lstm_model.POSTagger(is_training=False, config=self.config)
            else:
                model = bilstm_model.POSTagger(is_training=False, config=self.config)
