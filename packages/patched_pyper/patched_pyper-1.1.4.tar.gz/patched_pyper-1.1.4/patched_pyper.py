...
class R(object):  # "del r.XXX" fails on FePy-r7 (IronPython 1.1 on .NET 2.0.50727.42) if using old-style class
    ...
    DataFrameStr <- function(x) {
        ...
        for (i in seq(x)) {
            ...
            if (is.character(xi)) {
                ...
                xi <- paste(xi)
                ctp[i] <- paste0('("', cnms[i], '", "<q")' ) }
            else if (is.double(xi)) {
                xi <- paste(xi)
                ctp[i] <- paste0('("', cnms[i], '", "<g")' ) }
        ...
        x <- as.matrix(x)
        x <- apply(x, 1, function(r) paste0('(', paste(r, collapse=','), if(length(r)<2) ',)' else ')', sep=''))
        if (has_numpy) tailstr <- paste0(', dtype=[', paste(ctp, collapse=','), ']', tailstr, sep='')
        x <- paste(headstr, '[', paste0(x, collapse=','), ']', tailstr, sep='')
        return(x) }
    ...
    def __init__(self, RCMD='R', max_len=1000, use_numpy=True, use_pandas=True, use_dict=None,
        ...
        if use_numpy and not has_numpy:
            raise ValueError("required numpy not installed.")

        if use_pandas and not has_pandas:
            raise ValueError("required pandas not installed.")
