...
def remove_autoinject():
    ...
    # If we are running inside of a virtualenv, a `site` from Python2.6 will be
    # used. In Python3.x, this can cause an issue where our agent terminates
    # early due to a KeyError being raised when we remove our `sitecustomize`.
    # This occurs while executing `_find_and_load_unlocked` in the `_bootstrap`
    # module of `importlib`. To work around this, remove our sitecustomize
    # after attempting to find the real one first. If there isn't one, the
    # ImportError is silenced from `site` during `execsitecustomize`.
