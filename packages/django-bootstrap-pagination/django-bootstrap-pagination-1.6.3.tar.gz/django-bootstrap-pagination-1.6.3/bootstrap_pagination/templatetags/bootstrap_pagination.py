...
def get_page_url(page_num, current_app, url_view_name, url_extra_args,
...
class BootstrapPagerNode(Node):
    ...
    def render(self, context):
        ...
        previous_label = mark_safe(
            kwargs.get("previous_label", "Previous Page"))
        next_label = mark_safe(kwargs.get("next_label", "Next Page"))
        previous_title = mark_safe(
            kwargs.get("previous_title", "Previous Page"))
        ...
        if page.has_previous():
            previous_page_url = get_page_url(page.previous_page_number(),
                                             get_current_app(context),
                                             url_view_name, url_extra_args,
                                             url_extra_kwargs, url_param_name,
                                             url_get_params, url_anchor)

        next_page_url = None
        if page.has_next():
            next_page_url = get_page_url(page.next_page_number(),
                                         get_current_app(context),
                                         url_view_name, url_extra_args,
                                         url_extra_kwargs, url_param_name,
                                         url_get_params, url_anchor)
...
class BootstrapPaginationNode(Node):
    ...
    def render(self, context):
        ...
        for curpage in page_range:
            ...
            url = get_page_url(curpage, get_current_app(context),
                               url_view_name, url_extra_args, url_extra_kwargs,
                               url_param_name, url_get_params, url_anchor)
            page_urls.append((curpage, index_range, url))

        first_page_url = None
        if current_page >= 1:
            first_page_url = get_page_url(1, get_current_app(context),
                                          url_view_name, url_extra_args,
                                          url_extra_kwargs, url_param_name,
                                          url_get_params, url_anchor)
        ...
        if current_page <= page_count:
            last_page_url = get_page_url(page_count, get_current_app(context),
                                         url_view_name, url_extra_args,
                                         url_extra_kwargs, url_param_name,
                                         url_get_params, url_anchor)
