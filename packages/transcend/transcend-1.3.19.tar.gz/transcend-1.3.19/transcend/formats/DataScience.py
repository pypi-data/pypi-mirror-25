...
def raw_to_data_science(data, serviceURI):
    params = copy.deepcopy(datascience_format_options[serviceURI])
...
class RawToDataScience(object):
    def __init__(self, data, name, now = None, dt_str_strip = '%Y-%m-%dT%H:%M:%S.%fZ',
                 ...
                 additional_details = ['updatedAt', 'createdAt', 'latestPull'],
                 try_dates = None):
        '''
    ...
    def pre_process(self):
        ...
        for attr in self.additional_details:
            details[attr] = self.allData[attr] if attr in self.allData else ''
    ...
    def to_json(self):
        ...
        for attr in self.attributes:
            response['attributes'][attr] = json.loads(getattr(self, attr).to_json(orient='records'))
