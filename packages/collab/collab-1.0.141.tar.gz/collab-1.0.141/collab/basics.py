...
class Tag:
    ...
    """
    def __init__(self, own_identifier, symbol_identifier, arguments, comment=None, weight=None):
        argument_identifiers = [a if isinstance(a, Identifier) else a.identifier for a in arguments]
...
class ObjectManager:
    ...
    def get_all_objects(self, object_type):
        ...
        """
        res = []
        for k,v in self.identifier_to_object_dict.items():
...
def parse_object_manager(dictionary):
    ...
    res.tag_argument_backreferences = { parse_identifier(kv[0]) : parse_identifier(kv[1]) for kv in dictionary['tag_argument_backreferences'] }
