...
def get_inputs():
    ...
    res = []
    for a in inputs:
        obj = basics.parse_file_object(a)
...
def request_program_execution(program_identifier, *argument_identifiers):
    ...
    """
    request = basics.ProgramExecutionRequest().program(program_identifier).arguments(argument_identifiers)
...
def create_option(trigger_rule, display, actions, description=None):
    ...
    """
    option = basics.Option(basics.create_preliminary_identifier('option'), description, trigger_rule, display, actions)
