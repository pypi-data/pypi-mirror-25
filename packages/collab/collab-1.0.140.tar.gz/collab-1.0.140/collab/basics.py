...
class Tag:
    ...
    """
    def __init__(self, own_identifier, symbol_identifier, argument_identifiers, comment=None, weight=None):
    ...
    def to_json(self):
        ...
        res = {
            'identifier' : self.identifier.to_json(),
            'symbol_identifier' : self.symbol_identifier,
...
def parse_tag(dictionary):
    ...
    """
    identifier = parse_identifier(dictionary['identifier'])
    symbol_identifier = dictionary['symbol_identifier']
    argument_identifiers = [parse_identifier(a) for a in dictionary['argument_identifiers']]
    comment = dictionary['comment']
    weight = dictionary['weight']
    return Tag(identifier, symbol_identifier, argument_identifiers, comment=comment, weight=weight)
...
class ObjectManager:
    ...
    def __init__(self):
        ...
        self.tag_argument_backreferences = collections.defaultdict(list)
    ...
    def get_all_objects(self, object_type):
        ...
        """
        res = []
        for k,v in self.identifier_to_object_dict:
    ...
    def to_json(self):
        ...
        res = {
            ...
            'identifier_to_identifier_model_id' : { k.to_json() : v for k,v in self.identifier_to_identifier_model_id },
            'tag_argument_backreferences' : { k.to_json() : v.to_json() for k,v in self.tag_argument_backreferences.items() },
...
def parse_object_manager(dictionary):
    ...
    """
    res = ObjectManager()
    res.identifier_to_object_dict = { parse_identifier(k) : parse_object_according_to_type(k['type'], v) for k,v in dictionary['identifier_to_object_dict'] }
    res.identifier_to_identifier_model_id = { parse_identifier(k) : v for k,v in dictionary['identifier_to_identifier_model_id'] }
    res.tag_argument_backreferences = { parse_identifier(k) : parse_identifier(v) for k,v in dictionary['tag_argument_backreferences'] }
    res.preliminary_identifier_mapping = { parse_identifier(k) : parse_identifier(v) for k,v in dictionary['preliminary_identifier_mapping'] }
    return res
...
class DisplayMessageRequest(EventRequest):
    ...
    def text(self, message_text):
        ...
        return self
    def feedback_buttons(self, add=True):
