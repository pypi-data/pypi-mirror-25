...
from .basics import Identifier, parse_identifier, create_preliminary_identifier, FileObject, parse_file_object, Tag, parse_tag, Event, ProgramExecutionRequest, DisplayMessageRequest, parse_object_manager
...
def get_object_manager():
    ...
    if _object_manager is None:
        _object_manager = parse_object_manager(_get_input()['object_manager'])
...
def get_current_step():
    ...
    """
    return get_object_manager().current_step
...
def get_event_of_this_execution():
    ...
    """
    identifier = parse_identifier(_get_input()['event_identifier'])
...
def create_tag(symbol, *arguments):
    ...
    For any arguments, you may use either an Identifier or any class that has an identifier as a field.
    """
    global _output_objects
    ...
    for arg in arguments:
        if isinstance(arg, Identifier):
            ...
            cleaned_arguments.append(arg.identifier)
    tag = Tag(identifier, cleaned_arguments, symbol_name=symbol)
...
def request_program_execution(program_identifier, *argument_identifiers):
    ...
    The program may be identified as an Identifier object,
