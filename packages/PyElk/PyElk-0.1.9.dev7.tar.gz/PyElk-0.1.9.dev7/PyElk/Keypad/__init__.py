...
class Keypad(Node):
    ...
    def unpack_event_user_code_entered(self, event):
        ...
        if self._area > 0:
            self._pyelk.AREAS[self._area].last_user_num = user
            self._pyelk.AREAS[self._area].last_user_at = event.time
            self._pyelk.AREAS[self._area].last_user_name = self._last_user_name
            self._pyelk.AREAS[self._area].last_keypad_num = self._number
            self._pyelk.AREAS[self._area].last_keypad_name = self._description
