...
if __name__ == '__main__':
    ...
    parser.add_argument("-G", "--cfgfile", type=string,
                    help="Customize the config file") 
    parser.add_argument("-S", "--storage", type=string,
                    help="Customize the storage. Option: FILE | MOMERY ")  
    parser.add_argument("-X", "--prefix", type=string,
    ...
    app = Flask(__name__)
    restApi = FlatApi(app, config=_cfgfile, prefix = _prefix , storage = _storage)
