...
class PDFTransformer(object):
    ...
    @staticmethod
    def render(survey, response):
        buffer = BytesIO()
        doc = SimpleDocTemplate(buffer, pagesize=A4)
        doc.build(PDFTransformer.get_elements(survey, response))
    ...
    @staticmethod
    def render_to_file(survey, response):
    ...
    def get_elements(survey, response):
        ...
        heading_data = [[Paragraph(survey['title'], style_h)]]
        ...
        for question_group in filter(lambda x: 'title' in x, survey['question_groups']):
            ...
            for question in filter(lambda x: 'text' in x, question_group['questions']):
                if question['question_id'] in response['data']:
                    ...
                    elements.append(Paragraph(str(answer), style_answer))
...
def main(args):
    ...
    output = PDFTransformer.render(survey, data)
