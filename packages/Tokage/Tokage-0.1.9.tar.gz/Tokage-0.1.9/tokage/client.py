...
class Client:
    ...
    async def get_id(self, query: str):
        """Parse a google query and return the ID."""
        query = "site:myanimelist.net" + query
        params = {
            'q': query,
            'safe': 'on'
        }
        ...
        async with self.session.get('https://www.google.com/search', params=params, headers=headers) as resp:
            if resp.status != 200:
