...
import auto_gui.simulate as simulate
...
class TabSimulate(simulate.SimulateUI):

    def __init__(self, parent, db, experiment, etab, is_new):
        simulate.SimulateUI.__init__(self, parent)
        ...
        # I need a persistent reference to this sizer for adding and removing
        # controls
    ...
    def on_run(self, event):
        ...
        self.etab.top.statusbar.SetStatusText('',3)
        ...
        if msg:
            ...
            self.etab.top.statusbar.SetStatusText('Running experiment, please wait.',0)
            ...
            if exception:
                self.last_run = { }
                self.etab.top.statusbar.SetStatusText('Run returned errors!',3)
            else:
                # Turn the results (a list of dicts) into Simulation
                # objects.
                ...
                metabolite_map = [(metabolite.name, metabolite) for
                                                metabolite in new_metabolites]
                ...
                self.etab.build_basis_functions()
                self.etab.update_plot_controls()
                self.set_control_enabled()

            self.etab.top.statusbar.SetStatusText(' ',0)
    ...
    def initialize_controls(self):
        """Called once during init. This could could just as well be inlined
        in __init__().
        """
    ...
    def pulse_sequence_setup(self):
        """Sets up the panel appropriately based on which pulse seq.
        is selected"""
        i = self.ComboPulseSequence.GetCurrentSelection()
        if i == wx.NOT_FOUND:
            # No selection
    ...
    def set_control_enabled(self):
        ...
        if not self.is_new:
            ...
            if self.etab.experiment.is_public == True:
