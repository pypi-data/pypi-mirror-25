...
class AnalysisMenuBar(common_menu.VespaMenuBar):
    ...
    def __init__(self, main):
        ...
        file_, processing, view_start, view_raw, view_prep_fidsum, view_prep_timeseries,\
        ...
        # Last but not least, we need to check to see if the user specified
        # any items in "analysis_import_menu_additions.ini". If so, we add them to 
        # the self._user_menu_items dict here. That dict is keyed by the id
        # of the menu item (which we generate dynamically) and the values
        # are 2-tuples of (reader class, ini file name). main.py uses this
        # dictionary if any of the user-specified menu items are invoked.
        ...
        for module_name in menu_config.keys():
            # I add a separator before the first item
            if not self._user_menu_items:
            ...
            path = section["path"]
            # There are lots of ways that this could go wrong; we're loading a 
            # module of user code so we could get SyntaxError, ImportError,
            # etc. Since the only people using this feature are presumably
            # Python programmers, we just let all errors percolate up to the
            # user.
            # However, we check for the common error of a missing file or 
            # bad path just to be polite. 
            if os.path.exists(path):
                ...
                id_ = wx.NewId()
                menu_item = wx.MenuItem(self.file_import_menu, id_, 
                                        text=menu_item_text)
                self.file_import_menu.AppendItem(menu_item)

                # Save the reader class & INI file name associated with this
                # menu item.
                klass = getattr(module, section["class_name"])
                self._user_menu_items[id_] = (klass, section["ini_file_name"])

                main.Bind(wx.EVT_MENU, main.on_import_user_item, menu_item)
            else:
                msg = """I couldn't find the file "{0}" referenced in analysis_import_menu_additions.ini."""
                msg = msg.format(path)
...
def _get_menu_data(main):
    ...
    file_ = (
            ...
            ("&Import", (
                ("Bruker ",                   main.on_import_bruker),       
                ("GE PROBE (*.7) ",           main.on_import_ge_probe),       
                ("Philips (*.spar/sdat)",     main.on_import_philips_spar),       
                ("Philips Sum FIDs",          main.on_import_philips_fidsum),
                ("Siemens DICOM",             main.on_import_siemens_dicom), 
                ("Siemens DICOM Sum FIDs",    main.on_import_siemens_dicom_fidsum), 
#                ("Siemens DICOM Timeseries",  main.on_import_siemens_dicom_timeseries), 
                ("Siemens Export (*.rda)",    main.on_import_siemens_rda),       
                ("Varian",                    main.on_import_varian), 
                ("VASF (*.rsd/rsp)",          main.on_import_vasf),       
                ("VASF Sum FIDs (*.rsd/rsp)", main.on_import_vasf_fidsum),       
...
#            ("Add Fitting Tab - Giso",  main.on_add_giso_tab, wx.ITEM_NORMAL),
    ...
    view_watref = (
            ("Output", (
                ("View Results -> Text", main.on_menu_view_output, wx.ITEM_NORMAL, ViewIdsWatref.OUTPUT_RESULTS_TEXT),
                       )
            ),
           )
