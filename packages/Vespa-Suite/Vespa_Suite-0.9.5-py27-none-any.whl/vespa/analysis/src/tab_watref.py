...
class TabWatref(tab_base.Tab, watref.PanelWatrefUI):
    ...
    def on_menu_view_output(self, event):
        ...
        if event_id == util_menu.ViewIdsRaw.OUTPUT_RESULTS_TEXT:
            pass
    ...
    def on_reference_browse(self, event):
        ...
        if ref_dataset:
            ...
            self.TextReferenceDataset.SetValue(block.data_source)
            self.process_and_plot()
        ...
        if filename:
            ...
            if os.path.isfile(filename):
                with open(filename, 'r+') as f:
                    data = f.readlines()
                    if len(data) == len(val):
                        for item1,item2 in zip(data,val):
                            item1 = item1.replace("\n","")
                            item2 = item2.replace("\n","")
            ...
            if new_flag:
                for item1, item2 in zip(hdr,val):
                    res.append(item1+","+item2+"\n")
    ...
    def process(self, entry='all'):
        ...
        if self._plotting_enabled:
            ...
            label1  = 'Relaxation Corrected Water Concentration = '
