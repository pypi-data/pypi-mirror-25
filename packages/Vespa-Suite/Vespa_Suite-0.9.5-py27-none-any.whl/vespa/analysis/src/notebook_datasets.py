...
class NotebookDatasets(vespa_notebooks.VespaAuiNotebook):
    ...
    def global_poll_associated_tabs(self, dataset):
        """
        At this point, we are only polling for tabs associated by being part of
        and "Edited" dataset with On, Off, Sum, and Dif datasets.  We may 
        expand this in the future.
        """
        tabs = []

        raw = dataset.blocks['raw']
        if not isinstance(raw, block_raw_edit_fidsum.BlockRawEditFidsum):
            return tabs
