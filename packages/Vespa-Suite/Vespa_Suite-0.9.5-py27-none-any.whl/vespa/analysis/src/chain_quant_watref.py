...
class ChainQuantWatref(chain_quant_identity.ChainQuantIdentity):
    ...
    def run(self, voxels, entry='all'):
        ...
        wat_mole_wt = 18.015  # g/mol - by definition
        const1      = 1.0 / wat_mole_wt
        
        if block.set.apply_metabolite_correction:
            met_relax_corr = 1.0 / np.exp(-1.0*set.sequence_te/set.metabolite_t2)
        else:
            met_relax_corr = 1.0
        ...
        if block.set.apply_water_correction:
            wat_relax_corr  = (const1*(set.tissue_content_gm /100.0) * set.water_content_gm  * np.exp(-set.sequence_te/set.water_t2_gm))
            wat_relax_corr += (const1*(set.tissue_content_wm /100.0) * set.water_content_wm  * np.exp(-set.sequence_te/set.water_t2_wm))
            wat_relax_corr += (const1*(set.tissue_content_csf/100.0) * set.water_content_csf * np.exp(-set.sequence_te/set.water_t2_csf))
        else:
            wat_relax_corr = 1.0 * const1
            
        plot_results = {'met_relax_corr' : unicode(met_relax_corr), 
                        'wat_relax_corr' : unicode(wat_relax_corr), 
        ...
        for voxel in voxels:
            ...
            fit_results = fit.fit_results[:,voxel[0],voxel[1],voxel[2]]

            # create calibration scale value
            scale = wat_relax_corr * met_relax_corr * avgs_corr / water_area
            
            # local copy of fitted results
            results = fit_results.copy()
