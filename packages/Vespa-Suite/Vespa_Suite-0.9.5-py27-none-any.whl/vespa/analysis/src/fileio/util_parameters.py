...
def map_parameters(file_, identification=None, measurement=None, data=None):
    ...
    # mid PPM
    if "frequency_offset" in measurement:
        # Determine resonance PPM
        if d["nucleus"] == "1H":
            resppm = constants.DEFAULT_PROTON_CENTER_PPM
        else:
            resppm = constants.DEFAULT_XNUCLEI_CENTER_PPM

        d["midppm"] = resppm + float(measurement["frequency_offset"])
