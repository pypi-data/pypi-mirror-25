...
# Where is the evolve python source? Use pkg_resources to avoid importing which will fail with
# python3
_EVOLVE_LOCATION = osp.join(pkg_resources.get_distribution('hg-evolve').location,
                            'hgext', 'evolve.py')
...
class HGRepo(EntityAdapter):
    ...
    configs = [
        ...
        # we want obsolescence markers and rely on the allprecursors revset. Specify location
        # explicitly in case it's installed in a virtual env while hg isn't
        'extensions.hgext.evolve={0}'.format(_EVOLVE_LOCATION),
    ]
    path = None
    ...
    def update_hidden(self, cnx, hgrepo):
        ...
        for new_hide in hide:
            ...
            data = {'r': self.entity.eid,
                    'p': str(ctx)}
            successors_changesets = [cs.node[:12].decode('ascii') for cs in hgrepo.log(revrange='successors(%s)' % ctx.node().decode('ascii'))]
    ...
    def import_revision(self, hgrepo, i):
        ...
        parent_changesets = [n.node[:12].decode('ascii') for n in parent_changesets] if parent_changesets else []
        precursors_changesets = [cs.node[:12].decode('ascii') for cs in hgrepo.log(revrange='allprecursors(%s)' % i)]
