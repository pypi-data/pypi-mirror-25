...
class ScanManager(QtCore.QObject):
    ...
    def addValue(self, name, value):
        ...
        if self.names:
            try:
                index = self._names.index(name)  # there might be values from previous scans, we skip them
            except ValueError:
                return
    ...
    def run(self, params):
        ...
        self.images = [f'{str(uuid.uuid4())}.cbf' for _ in range(len(self.points))]
        self.names = self.images[:]
        self._names = self.images[:]
    ...
    def motorArrived(self):
        if self.running and self.imageName:
            name = self.imageName
            self.imageName = None
