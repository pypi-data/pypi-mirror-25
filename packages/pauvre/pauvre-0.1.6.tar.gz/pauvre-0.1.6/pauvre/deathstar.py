...
def deathstar(args):
    ...
    if 'False' not in args.query:
        ...
        if len(samFile) == 0:
            ...
            again with a less stringent test.  Deathstar plotter
            exiting""")
    ...
    start_radius_dict = {1: 10,
                         ...
                         30: 20,
                         35: 90}
    print("The number of rows is: {}".format(len(samFile.index)))
    for key in sorted(start_radius_dict):
        if len(samFile.index) >= key:
    ...
    # now plot ticks around the interior
    ...
    for extension in args.fileform:
        plt.savefig('deathstar.{}'.format(extension), dpi=args.dpi,
                    transparent=False, frameon=True, fc='white')
