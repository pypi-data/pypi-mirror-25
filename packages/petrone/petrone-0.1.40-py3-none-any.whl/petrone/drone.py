...
# BaseFunctions Start
    ...
    def checkDetail(self):
        ...
        while len(self._bufferHandler) > 0:
            ...
            # 오류 출력
            if (self._receiver.message != None):
                self._printError(self._receiver.message)
    ...
    def _eventLinkEvent(self, data):
        ...
        elif data.eventLink == EventLink.Connected:
        ...
        # 로그 출력
        self._printLog(data.eventLink)



    def _eventLinkEventAddress(self, data):
        if   data.eventLink == EventLink.Connected:
            self._flagConnected = True
    ...
    def _printLog(self, message):
        ...
        if self._flagShowLogMessage:
            print(Fore.GREEN + "[{0:12.03f}] {1}]".format((time.time() - self.timeStartProgram), message) + Style.RESET_ALL)
    ...
    def _printError(self, message):
        ...
        if self._flagShowErrorMessage:
            print(Fore.RED + "[{0:12.03f}] {1}]".format((time.time() - self.timeStartProgram), message) + Style.RESET_ALL)
