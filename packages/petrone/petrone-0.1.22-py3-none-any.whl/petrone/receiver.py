        ...
        if self.section == Section.Start:
            ...
            if self.index == 0:
                ...
                if self.crc16received == self.crc16calculated:
                    ...
                    self.message = "Success / Receiver / Section.End / Receive complete / [receive: 0x{0:04X}]".format(self.crc16received)

                else:
                    self.state = StateLoading.Failure
                    self.message = "Error / Receiver / Section.End / CRC Error / [receive: 0x{0:04X}, calculate: 0x{1:04X}]".format(self.crc16received, self.crc16calculated)
                    return self.state
