...
# BaseFunctions Start
    ...
    def _eventLinkEvent(data):
        if data.eventLink == EventLink.Scanning:
            self._flagDiscover = True
            self._devices = []

        if data.eventLink == EventLink.ScanStop:
            self._flagDiscover = False



    def _eventLinkDiscoveredDevice(self):
        self.devices.append(data)



    def connect(self, deviceName = "None", portName = "None"):
        
        # 이벤트 핸들링 함수 등록
        self.setEventHandler(DataType.LinkEvent, self._eventLinkEvent)
        self.setEventHandler(DataType.LinkDiscoveredDevice, self._eventLinkDiscoveredDevice)
        ...
        # 이벤트 핸들링 함수 해제
        self.setEventHandler(DataType.LinkEvent, None)
        self.setEventHandler(DataType.LinkDiscoveredDevice, None)
