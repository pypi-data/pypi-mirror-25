...
def convertByteArrayToString(dataArray):
    ...
    if (not isinstance(dataArray, bytearray)) and (not isinstance(dataArray, list)):
        return ""
...
# BaseFunctions Start
    ...
    def check(self):
        while self._bufferQueue.empty() == False:
            ...
            if (dataArray != None) and (len(dataArray) > 0):
                ...
                if self._flagShowReceiveData:
                    print(Back.MAGENTA + Fore.BLACK + convertByteArrayToString(dataArray) + Style.RESET_ALL, end=' ')
        ...
        while len(self._bufferHandler) > 0:
            ...
            # 오류 출력
            if (self._flagShowErrorMessage) and (stateLoading == StateLoading.Failure) and (self._receiver.message != None):
                # 수신 데이터 출력(줄넘김)
                if self._flagShowReceiveData:
                    print("")

                print(Fore.RED + self._receiver.message + Style.RESET_ALL)
                

            # 로그 출력
            if (self._flagShowLogMessage) and (stateLoading == StateLoading.Loaded) and (self._receiver.message != None):
                # 수신 데이터 출력(줄넘김)
                if self._flagShowReceiveData:
                    print("")

                print(Fore.GREEN + self._receiver.message + Style.RESET_ALL)
    ...
    def checkDetail(self):
        ...
        while len(self._bufferHandler) > 0:
            ...
            # 오류 출력
            if (self._flagShowErrorMessage) and (self._receiver.message != None):
                print(Fore.RED + self._receiver.message + Style.RESET_ALL)
    ...
    def _eventLinkEvent(self, data):
        if   data.eventLink == EventLink.Scanning:
            self._flagDiscover = True
            self._devices.clear()

        elif data.eventLink == EventLink.ScanStop:
            self._flagDiscover = False

        elif data.eventLink == EventLink.ReadyToControl:
            self._flagConnected = True
        
        elif data.eventLink == EventLink.Disconnected:
            self._flagConnected = False

        # 로그 출력
        if self._flagShowLogMessage:
            print(Fore.GREEN + "{0}".format(data.eventLink) + Style.RESET_ALL)
    ...
    def _eventLinkDiscoveredDevice(self, data):
        ...
        # 로그 출력
        if self._flagShowLogMessage:
            print(Fore.GREEN + "LinkDiscoveredDevice / {0} / {1} / {2} / {3}".format(data.index, convertByteArrayToString(data.address), data.name, data.rssi) + Style.RESET_ALL)



    def connect(self, portName = "None", deviceName = "None"):
        ...
        if not self.isOpen():
            # 오류 출력
            if self._flagShowErrorMessage:
                print(Fore.RED + "Could not connect to PETRONE LINK." + Style.RESET_ALL)
        ...
        if eq(deviceName, "None"):
            ...
            if length > 0:
                ...
                # 오류 출력
                if self._flagShowErrorMessage:
                    print(Fore.RED + "Could not find PETRONE." + Style.RESET_ALL)
            ...
            if len(self._devices) > 0:
                for i in range(len(self._devices)):
                    ...
                    # 로그 출력
                    if self._flagShowLogMessage:
                        print(Fore.GREEN + "Compare {0} / {1} / {2}".format(deviceName, self._devices[i].name, (deviceName == self._devices[i].name)) + Style.RESET_ALL)
                        print(Fore.GREEN + "Compare {0} / {1} / {2}".format(convertByteArrayToString(deviceName.encode('ascii', 'ignore')), convertByteArrayToString(self._devices[i].name.encode('ascii', 'ignore')), (deviceName == self._devices[i].name)) + Style.RESET_ALL)
                        print(Fore.GREEN + "Compare {0} / {1} / {2}".format(convertByteArrayToString(deviceName.encode()), convertByteArrayToString(self._devices[i].name.encode()), (deviceName == self._devices[i].name)) + Style.RESET_ALL)

                    if (deviceName == self._devices[i].name):
                ...
                if targetDevice != None:
                    ...
                    # 오류 출력
                    if self._flagShowErrorMessage:
                        print(Fore.RED + "Could not find " + deviceName + "." + Style.RESET_ALL)
