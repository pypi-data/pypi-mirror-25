...
def _log_hosts(awsaddresses):
    ...
    for instance in awsaddresses:
        logger.info("%s  %s  %s  %s\n", instance.name, instance.publicdns, instance.privateip, instance.shard)
...
@task
def deploycode(branch,nltk=False,doCollectStatic=True):
    tar_from_git(branch)
    remote_inflate_code()
    pip_install()
 
    if nltk in TRUTH_VALUES:
        download_nltk_data()
...
@task
@parallel
def deployParallel(nltk=False,doCollectStatic=True):
    remote_inflate_code()
    pip_install()
    if nltk in TRUTH_VALUES:
        download_nltk_data()
