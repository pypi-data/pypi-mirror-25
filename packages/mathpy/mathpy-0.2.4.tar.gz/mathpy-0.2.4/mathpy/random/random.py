...
def clcg_32bit(n=10):
    ...
    Where :math:`m_1` is the modulus of the LCG, :math:`Y_{i,j}` is the :math:`i`th input from the :math:`j`th
    LCG and :math:`X_i` is the :math:`i`th random generated value.
    ...
    range :math:`[1, m_1 - 1] \space \text{and} \space [1, m_2 - 1]`, respectively.
...
def clcg_16bit(n=10):
    ...
    y1, y2, y3 = np.random.randint(1, m1 - 1, 1), \
                 np.random.randint(1, m2 - 1, 1), \
                 np.random.randint(1, m3 - 1, 1)
