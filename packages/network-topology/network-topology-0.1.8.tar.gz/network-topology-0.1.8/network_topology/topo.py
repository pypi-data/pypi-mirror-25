...
def _findSegmentsAndIntersections(G, turnThreshold=20.0):
    ...
    ids = []
    to_remove = []
    for node, data in BG.nodes(data=True):
        if data['nnodes'] == 1:
            p = G.node[data['graph'].nodes().key()[0]]
            data['geom'] = Point(p['x'], p['y'])

    for node, data in BG.nodes(data=True):
        if data['nnodes'] > 1:
            current = data['graph'].nodes().key()[0]
            ...
            for terminal in BG[node]:
                d = BG.node[terminal]
                if d['graph'].nodes().key()[0] in G[sequence[0]]:
                    startNode = terminal
                if d['graph'].nodes().key()[0] in G[sequence[-1]]:
                    endNode = terminal
