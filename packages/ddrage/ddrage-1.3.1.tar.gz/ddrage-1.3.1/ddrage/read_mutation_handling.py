...
def shallow_plain_copies(template_read, coverage, meta_info="type:'common'"):
    ...
    """
    template_meta = template_read.meta_info
...
def mutation_copies(rad_read, coverage, heterozygous, mutation_model):
    ...
    if heterozygous:
        ...
        # reads_allele_1 = [copy.deepcopy(template_read_1) for _ in range(coverage_allele_1)]
        ...
        # reads_allele_2 = [copy.deepcopy(template_read_2) for _ in range(coverage_allele_2)]
        ...
        if mutated_template_read != "dropout NA":
            ...
            coverage = 0
        # mutated_reads = [copy.deepcopy(mutated_template_read) for _ in range(coverage)]
...
def id_copies(template_read, coverage, rec_sites):
    ...
    id_reads = [copy.deepcopy(template_read) for _ in range(coverage)]
    for id_read in id_reads:
        id_read.add_meta_info("type:'ID'")
