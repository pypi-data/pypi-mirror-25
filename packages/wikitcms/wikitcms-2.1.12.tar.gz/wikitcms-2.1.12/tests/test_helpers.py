...
"""
    ...
    def test_read_creds(self):
        credtext = "user1 pass1\nuser2".encode('utf-8')
        (tempfh, tempfn) = tempfile.mkstemp()
        os.write(tempfh, credtext)
        os.close(tempfh)
        creds = hl.read_creds(filenames=[tempfn])
        assert creds == {'user1': 'pass1', 'user2': None}
    ...
    def test_cid_to_event(self, fakecurr):
        ...
        assert bran == ('24', 'Branched', '20160314.n.2')
        rawh = hl.cid_to_event('Fedora-Rawhide-20160314.n.1')
        assert rawh == ('24', 'Rawhide', '20160314.n.1')
        prod = hl.cid_to_event('Fedora-24-20160314.1')
        assert prod == ('24', 'Alpha', '1.2')
