...
class TcmsGeneratorList(mwl.GeneratorList):
    ...
    def _get_tcms(self, name, info=()):
        ...
        if (name.startswith('Test Results:') or
            ...
            nightly_patt = re.compile(r'Test Results:Fedora (\d{1,3}) '
            ...
            accept_patt = re.compile(r'Test Results:Fedora (\d{1,3}) '
                                     ...
                                     r'(\d{1,2})$')
            ms_patt = re.compile(r'Test Results:Fedora (\d{1,3}) '
                                 r'([^ ]+?) ([^ ]+?) (.+)$')
            cat_patt = re.compile(r'Category:Fedora (\d{1,3}) '
                                  r'(.*?) *?Test Results$')
            ...
            if match:
                ...
                page = pg.NightlyPage(
                    self.site, release=match.group(1), testtype=match.group(5),
                    milestone=match.group(2), compose=match.group(3),
                    info=info)
            ...
            if match:
                if match.group(4) == 'Summary':
                    raise NoPageWarning(name)
                page = pg.ComposePage(
                    self.site, release=match.group(1), testtype=match.group(4),
                    milestone=match.group(2), compose=match.group(3),
                    info=info)
                return self._check_page(name, page)
            ...
            if match:
                if not match.group(2):
                    page = ValidationCategory(
                        self.site, match.group(1), info=info)
                    return self._check_page(name, page)
                elif match.group(2) == 'Nightly':
                    page = ValidationCategory(self.site, match.group(1),
