...
def read_creds(filenames=None):
    """Read wiki credentials (username and possibly password) from
    the locations listed in the 'files' iterable. If files is not
    specified, will read from the default locations:
    ~/.fedora/credentials and /etc/fedora/credentials. The user file
    is read first, meaning if both exist, the first entry in the dict
    will be from the user file (this is usually used as the default).
    Returns a dict of username: password pairs. If no credentials
    files are found, will return an empty dict. If a username but no
    password is given on any line in the credentials file, the dict
    will contain an item with the user name as the key and None as the
    value.
    """
    creds = OrderedDict()
    if not filenames:
        filenames = (os.path.expanduser('~/.fedora/credentials'),
                     '/etc/fedora/credentials')

    for filename in filenames:
        try:
            credfh = open(filename, 'r')
            for line in credfh:
                linetup = line.split()
                if len(linetup) == 2:
                    creds[linetup[0].strip()] = linetup[1].strip()
                elif len(linetup) == 1:
                    creds[linetup[0].strip()] = None
        except IOError:
            pass
...
def cid_to_event(cid):
    ...
    if dist != "Fedora":
        # we only have validation events for 'Fedora' composes ATM -
