...
class Wiki(mwclient.Site):
    ...
    def login(self, username=None, password=None, *args, **kwargs):
        """Login method, overridden to read credentials from file if
        not provided."""
        if not self.credentials and (not username or not password):
            # self.credentials being populated means a *previous* call
            # to this method set username and password, we should not
            # override them.
            creds = hl.read_creds()
            if creds:
                if not username:
        ...
        super(Wiki, self).login(username, password, *args, **kwargs)
    ...
    def get_validation_event(self, release='', milestone='', compose='',
        ...
        """
        if cid:
            (release, milestone, compose) = hl.cid_to_event(cid)
    ...
    def report_validation_results(self, reslist, allowdupe=False):
        ...
        iterable containing up to 14 values in the following order:
        (testtype, release, milestone, compose, testcase, section,
        testname, env, status, user, bugs, comment, bot, cid). They
        can also be any mapping type (e.g. a dict) with enough of the
        14 keys set (see below for requirements).
        ...
        * 'testtype', 'release', 'milestone', 'compose' and 'cid' must
        identify a single validation page using get_validation_page()
        (at least 'testtype' must always be set).
        ...
        *'status' must indicate the result status.
        ...
        add_results() method.
        """
        pagedict = dict() # KEY: (testtype, release, milestone, compose)
        insufficients = list()
        dupes = list()
