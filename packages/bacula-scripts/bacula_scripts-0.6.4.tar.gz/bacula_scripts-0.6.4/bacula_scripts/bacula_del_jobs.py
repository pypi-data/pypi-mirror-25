...
def main():
    ...
    try:
        ...
        if jobnames:
            ...
            query += operator + query2
        if storagenames:
            data += storagenames
            query2 = "(s.name IN (%s) OR j.name IN (%s));" % (storagenames_placeholders, jobnames_placeholders)
            query += operator + query2
        if newer_as:
            data += newer_than_starttime
            query += operator + "j.starttime >= %s::timestamp;"
        elif older_as:
            data += older_than_starttime
            query += operator + "j.starttime <= %s::timestamp;"
