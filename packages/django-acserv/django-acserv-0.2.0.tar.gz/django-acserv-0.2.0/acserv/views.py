...
User = get_user_model()
conf = getattr(settings, 'ANYCONNECT', {})
AUTH_HEADER = conf.get('AUTH_HEADER', {})
HANDLER = conf.get('HANDLER', None)
...
def message_or_default(key_path):
    messages = conf.get('FORM_MESSAGES', {})
...
class ACView(View):
    ...
    def render_auth_succeed(cls, request, auth_ctx, user):
        ...
        banner = render_to_string('acserv/banner.html', context=dict(
            service_due=user.service_due,
