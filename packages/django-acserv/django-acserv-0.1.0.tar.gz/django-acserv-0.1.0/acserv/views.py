...
class ACView(View):
    ...
    if getattr(settings, 'ACSERV_HANDLER', None):
        import importlib
        tokens = settings.ACSERV_HANDLER.split('.')
        pkg = importlib.import_module('.'.join(tokens[:-1]))
        handler = getattr(pkg, tokens[-1])
    else:
        from acserv.handlers import BaseHandler
        handler = BaseHandler

    def __init__(self, **kwargs):
        super(ACView, self).__init__(**kwargs)

    def dispatch(self, request, *args, **kwargs):
        # 如果提供了验证头的配置，就一定要匹配，否则403
        if settings.ACSERV_AUTH_HEADER:
            for k, v in settings.ACSERV_AUTH_HEADER.items():
    ...
    def get_user_or_response(cls, request, auth_ctx):
        if not auth_ctx or not auth_ctx.user_pk:
            return None, cls.render_account_form(request, '请输入账号')

        user = User.objects.filter(pk=auth_ctx.user_pk).first()
        if user:
            return user, None
        else:
            return None, cls.render_account_form(request, '未找到该用户，请重新输入邮箱账号')
    ...
    @classmethod
    def render_account_form(cls, request, message='请输入账号'):
        return cls.render_xml(request, 'acserv/account.xml',
                              message=message)

    @classmethod
    def render_password_form(cls, request, message='请输入密码'):
        return cls.render_xml(request, 'acserv/password.xml',
                              message=message)

    @classmethod
    def render_clients_form(cls, request, data, message='已经达到同时在线个数限制，请选择您所要替换的客户端'):
        for client in data:
    ...
    def render_auth_succeed(cls, request, auth_ctx, user):
        if not user.is_in_service():
            return cls.render_account_form(request, "尚未购买服务，或服务已过期")

        exceed_clients_data = cls.handler.get_exceed_clients_data_on_auth_success(user)
        if exceed_clients_data:
            return cls.render_clients_form(request, exceed_clients_data)

        token = cls.handler.on_auth_succeed_get_token(request, auth_ctx, user)
        ...
        current_clients, max_clients = cls.handler.get_client_count(user)
        banner = """
                欢迎使用！

                服务期：{service_due}
                终端数：{current_clients}/{max_clients}
                """.format(service_due=user.service_due,
...
class UsernameACView(ACView):
    def post(self, request, auth_ctx):
        ...
        if not username:
            return self.render_account_form(request, '请输入账号')

        # 用户不存在
        user = User.objects.filter(username=username).first()
        if not user:
            return self.render_account_form(request, '找不到该用户，请确认账号是否输入正确')

        auth_ctx.user_pk = user.pk

        # 有上次登录记录的，直接登录
        if auth_ctx.device_id and self.handler.validate_device_id(user, auth_ctx.device_id):
            return self.render_auth_succeed(request, auth_ctx, user)

        # 没有登录记录的，提示输入密码
        return self.render_password_form(request, '请输入密码')
...
class PasswordACView(ACView):
    def post(self, request, auth_ctx):
        ...
        if not user.check_password(password):
            return self.render_password_form(request, "密码不正确, 请重新输入")
...
class KickSessionACView(ACView):
    def post(self, request, auth_ctx):
        ...
        self.handler.on_kick_session(user, token_to_expire)
