...
BASIC_PROFILE_RUN_WORKER_JOB_REGISTRY = BaseWorkerJobRegistry()

BASIC_PROFILE_RUN_WORKER_JOB_REGISTRY.add_job(job = BaseWorkerJob(command = 'update', action='self', allowed_job_types=['synchronous', 'local', 'asynchronous'], allowed_priorities=[], timeout_ttl=3600))
BASIC_PROFILE_RUN_WORKER_JOB_REGISTRY.add_job(job = BaseWorkerJob(command = 'update', action='test', allowed_job_types=['asynchronous'], allowed_priorities=[], timeout_ttl=3600))
BASIC_PROFILE_RUN_WORKER_JOB_REGISTRY.add_job(job = BaseWorkerJob(command = 'update', action='delete', allowed_job_types=['local'], allowed_priorities=[], timeout_ttl=3600))

class BasicProfileRunWorker(BaseWorker):
    ALLOWED_JOBS = BASIC_PROFILE_RUN_WORKER_JOB_REGISTRY
    ...
    def process_response(self, worker_response):
        ...
        if worker_response.command == 'update':
            ...
            raise ValueError('The command/action for BasicProfileRun was not recognized!')
...
register_worker(BasicProfileRunWorker)