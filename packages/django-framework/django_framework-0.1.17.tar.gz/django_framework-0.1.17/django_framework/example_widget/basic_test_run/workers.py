...
BASIC_TEST_RUN_WORKER_JOB_REGISTRY.add_job(job = BaseWorkerJob(command = 'update', action='self', allowed_job_types=['synchronous', 'local', 'asynchronous'], allowed_priorities=[], timeout_ttl=3600))
BASIC_TEST_RUN_WORKER_JOB_REGISTRY.add_job(job = BaseWorkerJob(command = 'update', action='test', allowed_job_types=['asynchronous'], allowed_priorities=[], timeout_ttl=3600))
BASIC_TEST_RUN_WORKER_JOB_REGISTRY.add_job(job = BaseWorkerJob(command = 'update', action='delete', allowed_job_types=['local'], allowed_priorities=[], timeout_ttl=3600))
