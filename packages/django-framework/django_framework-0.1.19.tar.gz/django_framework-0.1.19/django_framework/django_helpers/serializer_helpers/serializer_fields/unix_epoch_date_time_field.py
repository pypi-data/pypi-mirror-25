...
class UnixEpochDateTimeFieldHuman(serializers.SerializerMethodField):
    ...
    def to_representation(self, value):
        value = self.parent.Meta.model._meta.get_field(self.source_name) # get the value of interest!
        
        if value == None:
            response = None
        else:
            response = arrow.get(value).format('YYYY-MM-DD HH:mm:ss') # 24 hour formatted human readable time.  It is in UTC
        return response
