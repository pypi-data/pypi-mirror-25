...
class Scheduler:
    ...
    async def _handle_service_queue(self, service):
        try:
            while True:
                ...
                if len(worker['messages']) < self.DEFAULT_MAX_MESSAGES_PER_WORKER: # schedule another task?
    ...
    async def _handle_worker_message(self, worker_id, multipart_message):
        ...
        if message_type == SchedulerCode.READY:
            ...
            if len(worker['messages']) == (self.DEFAULT_MAX_MESSAGES_PER_WORKER - 1):
            ...
            if worker_id in self.workers:
                ...
                for _ in range(service['workers']['next_worker'].qsize()): # Remove worker from next_worker queue
                    value = service['workers']['next_worker'].get_nowait()
                    if value[1] != worker_id:
                        service['workers']['next_worker'].put_nowait()
