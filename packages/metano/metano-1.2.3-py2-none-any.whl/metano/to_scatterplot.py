...
class Scatterplot(object):
    ...
    def do_analysis(self):
        ...
        for filename in self.filenames:
            ...
            pparser = ParamParser()
            # Parse file, get maxmin, name of objective function, and solver name
            maxmin, obj_str, solver, num_iter, lb_vec, ub_vec = pparser.parse(filename)
            ...
            if self.method in ["fba", "moma"]:
                ...
                _, solution, _ = fba.runOnModel(model, fba_params, True, False, True)
                ...
                minmax, solution, _, _, _ = fva.runOnModel(model, fba_params, tolerance)

                if not minmax:
                    print "FVA Warning: Model is infeasible or unbounded. Nothing to do."
                    exit()
                for rea in model.reactionDict:
                ...
                solution, _ = mfm.runOnModel(model, fba_params, tolerance, minimize=True)
                ...
                mfm = MfmAnalyzer(solver)
                solution_min, _ = mfm.runOnModel(model, fba_params, tolerance, minimize=True)
                solution_max, _ = mfm.runOnModel(model, fba_params, tolerance, minimize=False)

                split_ratios = solution.computeAllSplitRatios(model, 0., False, False)
    ...
    def set_plot_style(self, stylefile=None):
        ...
        try:
            style = {
                ...
                "ids_in_names" : bool(config.get("DEFAULT", "ids_in_names")),
                "unique_names" : bool(config.get("DEFAULT", "unique_names")),
                ...
                "plot_grid" : bool(config.get("DEFAULT", "plot_grid")),
    ...
    def do_scatterplot(self, outputfile=None, obj_fun=None, stylefile=None):
        ...
        style = self.set_plot_style(stylefile)

        # calculate obj jun coefficient if obj fun is given
        if obj_fun:
            try:
                obj_coeff = self.result[obj_fun][0][0] / self.result[obj_fun][1][0]
            except KeyError:
        ...
        if self.method in ["fva", "mva"]:
            name, x, xmin, xmax, y, ymin, ymax = zip(*[
                ([rea] + self.result[rea][0] + self.result[rea][1]) for rea in self.result])
        else:
            name, x, y = zip(*[([rea] + self.result[rea][0] + self.result[rea][1])
        ...
        if self.method in ["fva", "mva"]:
            ...
            if style["show_errorbar"] == "significant":
                plt.errorbar(x=[x[i] for i in range(len(x)) if significance[i] == 1],
                             y=[y[i] for i in range(len(y)) if significance[i] == 1],
            ...
            significance = [1 if abs(x[i]-y[i]*obj_coeff) >= style["significance_threshold"] else 0
