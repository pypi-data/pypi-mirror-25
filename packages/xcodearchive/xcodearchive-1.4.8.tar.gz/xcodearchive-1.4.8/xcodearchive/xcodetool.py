...
def get_project_basic_info():
    ...
    target, index = pick.pick (tartgets, 'Please select the corresponding target', '=>')
    scheme, index = pick.pick (schemes, 'Please select the corresponding scheme', '=>')
    configuration, index = pick.pick (configurations, 'Please select the corresponding configuration', '=>')
...
def archive_project():
    ...
    if check_upload_ipa ( ):
        ...
        # 选择导出方式
        method, index = pick.pick (__export_method, __export_title, '=>')
        # 输出命令
        print (archive_cmd)
        # 执行命令
...
def main():
    # pass
    method, index = pick.pick (__enter_method, __enter_title, indicator='=>')
    if index == 0:
        # 编译工程文件
        build_project ( )
        ...
        if archive_path is not  None:
            AssetCarAnalysis.AssetImageAnalysis (archive_path, base_dir)
        else:
        ...
        ipa_list = auto_recogonize_ipa_file (base_dir)
        if len (ipa_list) > 0:
            ipa, _ = pick.pick (ipa_list, "current ipa list", '=>')
