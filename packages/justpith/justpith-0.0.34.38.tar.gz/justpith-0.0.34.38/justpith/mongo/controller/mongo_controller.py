...
class MongoController(Mongo):
    ...
    def deactive_job(self, job_id):
        selected_collection = self.connection[self.collection_job]
        result = selected_collection.update_one({"_id": job_id}, {"$set": {"active": 0}})
    ...
    def is_voted_news(self, news_id, user_id):
        ...
        result = selected_collection.find({'_id':str(news_id), 'register.'+str(user_id):{'$exists': True}}).count()
    ...
    def get_user_raccomandations(self, user_id, category):
        ...
        result = selected_collection.find({'_id': str(user_id)}, {'raccomandations':1})['raccomandations']
        return result
