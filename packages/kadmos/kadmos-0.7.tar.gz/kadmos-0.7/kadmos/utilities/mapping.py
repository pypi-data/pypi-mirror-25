...
# def OLD_get_nodes_by_xpaths_full_auto(xpaths, use_case_dir_path, refUIDs = {}):
#     """
#     This function searches the provided xpaths in other Basefiles in the knowledge base and checks whether the found
#     elements have the referenced UIDs in their ancestry. If they do, they are added to a template tree and returned.
#
...
#
#     # get each basefile in subdirectories
#     basefile_paths = [os.path.join(dir, file) for dir in subdirs for file in os.listdir(dir) if file.endswith("-base.xml")]
#
#     # loop through basefiles, parse them
#     baseTrees = {}
#     for path in basefile_paths:
#         baseTrees[path] = etree.parse(path)
#
#     # loop through xpaths and check whether they are found in basetrees
#     addElements = []
#     for xpath in xpaths:
#
#         try: # if all all nodes for this xpath found, return to outermost loop and continue with next xpath
#
#             # copy reference uids; list is modified during iteration
#             c_refUIDs = copy.deepcopy(refUIDs)
#
#             # get searchpath; loop through basetrees and check whether nodes are found using searchpath
#             searchPath = xpath_to_searchpath(xpath)
#             for baseTreePath, baseTree in baseTrees.iteritems():
#
#                 # check whether there is a found node whose path carries all referenced uids
#                 for node in baseTree.findall(searchPath):
#
#                     # loop through ancestors, check whether ancestor paths in refUIDs
#                     checkedPaths = []
#                     noMatch = False
#                     for anc in node.iterancestors():
#                         ancPath = xpath_to_clean_xpath(baseTree.getpath(anc))
#
#                         # check whether anc path is in refUIDs
#                         if ancPath in c_refUIDs:
#                             uids = c_refUIDs[ancPath]
#                             if anc.get('uID') in uids: # if uid matches, add to checkedPaths
#                                 checkedPaths.append((ancPath, anc.get('uID')))
#                             else: # if uid does not match, break loop and skip element
#                                 noMatch = True
#                                 break
#
#                     # if any ancestor does not match the referenced UIDs, skip node
#                     if noMatch: break
#
#                     # add node to list of added elements
#                     addElements.append((node, baseTreePath))
#
#                     # check whether checked paths have uids
#                     if checkedPaths:
#                         # if any checked path has more than one reference uid, remove the one referenced in added node
#                         # this ensures that the other referenced node can still be added
#                         if any(len(c_refUIDs[path])>1 for path, uid in checkedPaths):
#                             for path, uid in checkedPaths:
#                                 if len(c_refUIDs[path])>1:
#                                     c_refUIDs[path].remove(uid)
#                             continue
#                         else:
#                             raise Continue
#                     else:
#                         raise Continue # if no checked paths, continue outermost loop
#
#         except Continue: # continue exception to continue outermost loop
#             pass
#
#     # iterate through addElements, create isolated trees, merge trees
#     templ_tree = etree.ElementTree(etree.Element('cpacs'))
#     for node, tree in addElements:
#         iso_tree = isolate_element_path_in_tree(node)
#         templ_tree = merge_XML_trees(templ_tree, iso_tree)
#
#     # print added elements
#     if addElements:
#         printAdded = []
#         for node, treePath in addElements:
#             btree = baseTrees[treePath]
#             nodePath = xpath_to_uid_xpath(btree.getpath(node), node)
#             nodeVal = node.text
#             try: # try to display node value in scientific notation
#                 nodeVal = "{0:.4e}".format(float(nodeVal))
#             except ValueError:
#                 pass
#             printAdded.append((nodePath, nodeVal, os.path.basename(treePath)))
#
#         print "\nThe following nodes were added to the Basefile:"
#         PRI.print_in_table(printAdded, headers=["Xpath", "Value", "Basefile"], print_indeces=True)
#     else:
#         print "\nNo elements added to Basefile."
#
#     return templ_tree

#
# def append_template_nodes_to_tree(main_tree, templ_tree):
#     """
#     This function iterates through the nodes in the templ_tree and tries to append them to existing nodes in the
#     main_tree. For this purpose, each ancestor of the leaf nodes in the templ_tree is iterated to find a matching node
#     in the main_tree. A match occurs if two nodes have the same xpath.
#     At the moment, the highest MATCHING node in the main_tree serves as the parent node, and the descendant
#     node(s) of the templ_tree are appended to that one.
#
#     NOTE: if node in templ_tree matches only one node in main_tree,
#
#      Example:
#      main_tree: <a><b><c/></b></a>
#      templ_tree: <a><b><d/><e/></b></a>
#      result: <a><b><c/><d/><e/></b></a>
#
#     :param main_tree:
#     :param templ_tree:
#     :return: main_tree: contains appended nodes
#     """
#
#     # loop through templ_tree leaf nodes
#     for elem in templ_tree.iter():
#         elemPath = templ_tree.getpath(elem)
#         if not elem.getchildren():
#
#             # iterate ancestors and search main_tree for existing node to merge into
#             descList = [elem] # get list of descendants for later check
#             for anc in elem.iterancestors():
#                 ancPath = xpath_to_clean_xpath(templ_tree.getpath(anc))
#
#                 # search for existing nodes to append descendants to
#                 if anc == templ_tree.getroot(): # if nodes found to append to, append to root
#                     appendToElems = [main_tree.getroot()]
#                 else:
#                     searchPath = xpath_to_searchpath(ancPath)
#                     appendToElems = main_tree.findall(searchPath)
#
#                 # if existing elements found, ask user which one to add templ_tree nodes to
#                 if appendToElems:
#
#                     if len(appendToElems)>1:
#
#                         # get UID xpaths of elements to append to (for easier identification in prompt)
#                         appendElemDict = {}
#                         for el in appendToElems:
#                             uid_xpath = xpath_to_uid_xpath(main_tree.getpath(el), el)
#                             appendElemDict[uid_xpath] = el
#
#                         # prompt user on where to add element
#                         msg = "The following elements were found viable parent nodes to merge '{}': ".format(elemPath)
#                         PRI.print_indexed_list(*appendElemDict.keys(), message=msg)
#                         sel_msg = "Please select parent node to merge into (skip with blank)."
#                         usr_sel = PRO.user_prompt_select_options(*appendElemDict.keys(), message=sel_msg, allow_empty=True, allow_multi=True)
#                         if usr_sel:
#                             appendToElems = [appendElemDict[uid_xpath] for uid_xpath in usr_sel]
#                         else:
#                             # redefine descendant for next iteration and continue
#                             descList.insert(0,anc)  # this needs to be defined because the descendants are appended to parent node
#                             continue
#
#                     # copy node and remove all irrelevant descentants
#                     # if more than one, only the relevant descendants are added to copyEl
#                     if len(descList)>1:
#                         copyEl = copy.deepcopy(descList[0])
#                         parent = copyEl
#                         for desc in descList[1:]:
#                             for child in parent.iterchildren():
#                                 parent.remove(child)
#                             cDesc = copy.deepcopy(desc)
#                             parent.append(cDesc)
#                             parent = cDesc
#                     else:
#                         copyEl = next(iter(descList))
#
#                     # append templ_tree node to selected (or only present) parent
#                     for appElem in appendToElems:
#                         appElem.append(copyEl)
#                     break
#
#                 # redefine descendant for next iteration and continue
#                 descList.insert(0, anc) # this needs to be defined because the descendants are appended to parent node
