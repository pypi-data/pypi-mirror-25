...
class PSO_new(object):
	...
	def CreateDump(self, stringa):
		self.save_path = stringa + self.FITNESS.__name__
		print self.save_path
		with open(self.save_path, 'w' ) as f:
			pass
	def CreateDump2(self, stringa):
		self.save_path2 = stringa + self.FITNESS.__name__
		print self.save_path2
		with open(self.save_path2, 'w' ) as f:
			pass
	def CreateDump3(self, stringa):
		self.save_path3 = stringa + self.FITNESS.__name__
		print self.save_path3
		with open(self.save_path3, 'w' ) as f:
			pass

	def CreateDumpNew(self, stringa):
		with open(stringa, 'w' ) as f:
			pass
	...
	def StopDump(self):
		with open(self.save_path , 'a' ) as f:
			f.write("\n")
	def StopDump2(self):
		with open(self.save_path2 , 'a' ) as f:
			f.write("\n")
	...
	def Iterate(self, verbose=False):
		...
		if not self.ParallelFitness:
			...
			self.UpdateCalculatedFitnessParallel()
		"""
	...
	def Generate(self, lista, use_log=True, verbose=False): 
		...
		if use_log:
			for i in range(len(lista)):
				#minimo = self.Boundaries[i][0]
				#massimo = self.Boundaries[i][1]
				...
				# ret.append( minimo*((massimo/minimo)**random.random()) )
		...
		for i in xrange(len(self.Solutions)):			
			...
			if self.Solutions[i].CalculatedFitness < self.Solutions[i].CalculatedBestFitness:
				...
				if self.Solutions[i].CalculatedFitness < self.G.CalculatedFitness:
					...
					# print "new global best", i, "has fitness", self.Solutions[i].CalculatedFitness, self.G.X
					self.SinceLastGlobalUpdate = 0			
					self.GIndex = i
			else:
				if verbose:
		...
		for numpart, p in enumerate(self.Solutions):
			...
			if self.UseRestart:
				if self.getBestIndex != numpart:
					distance_from_global_best = numpy.linalg.norm( array(self.G.X) - array(p.X) ) 
