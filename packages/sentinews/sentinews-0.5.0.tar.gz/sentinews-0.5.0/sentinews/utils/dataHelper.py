...
def load_data_and_labels(file_path,
    ...
    raw_data = list(open(file_path, 'r', encoding='utf-8').readlines())
    # parse label
    labels = [
    ...
    texts = [list(filter(lambda a: a != '', s.split(" "))) for s in texts]
    # support multi-label
    labels = [list(filter(lambda a: a != '', s.split(" "))) for s in labels]
    if is_shuffle:
