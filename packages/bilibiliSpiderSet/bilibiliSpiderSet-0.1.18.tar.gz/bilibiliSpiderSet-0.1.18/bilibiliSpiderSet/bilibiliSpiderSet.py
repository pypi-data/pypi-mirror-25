...
# @Last Modified time: 2017-10-10 21:24:25
...
class AVInfoSpider():
	...
	def analysis(self, response):
		...
		if link == None:
			Logger.warning('这个视频不存在,或者是会员的世界')
			return None
		else:
...
class CoverSpider():
	...
	def get(self, raw_url):
		...
		article_url_choices = ['https://www.bilibili.com/read/cv', 'http://www.bilibili.com/read/cv']
		av_url_choices = ['https://www.bilibili.com/video/av', 'http://www.bilibili.com/video/av']
		live_url_choices = ['https://live.bilibili.com/', 'http://live.bilibili.com/']

		if raw_url.startswith(tuple(av_url_choices)):
			return self.get_av_cover(raw_url)
		elif raw_url.startswith(tuple(article_url_choices)):
			return self.get_article_cover(raw_url)
		elif raw_url.startswith(tuple(live_url_choices)):
