...
class Secrets(object):
    ...
    def decrypt(self, json_secrets_path):
        ...
        unecrypted_secrets_file = json_secrets_path.split('.enc')[0]
        if os.path.isfile(unecrypted_secrets_file) and not os.path.isfile(json_secrets_path):
            try:
                secrets = json.loads(open(unecrypted_secrets_file).read())
            ...
            try:
                ...
                # TODO perhaps only write to file in the dev enviroment
                with open(unecrypted_secrets_file, 'w') as f:
                    json.dump(secrets, f, sort_keys=True, indent=4)
            except ValueError as e:
