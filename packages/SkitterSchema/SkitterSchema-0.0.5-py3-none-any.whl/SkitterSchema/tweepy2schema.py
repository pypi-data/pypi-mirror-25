...
def process_status(status):
    ...
    # TODO: Add support to the new 280 char tweet. This has to wait for twitter to change its API
    if status.truncated:
        return outcome
...
def process_favourites(favorites, user_id):
    outcome = ProcessedOutcome()

    for status in favorites:
        outcome += process_status(status)
        outcome += UUEdge.get_favorite_uuedge(user_id, outcome.last_user().id)
        outcome += USEdge.get_favorite_usedge(user_id, outcome.last_status().id)
