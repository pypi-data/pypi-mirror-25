...
class PipelineCli(object):
    ...
    def version(self):
        ...
        print('')
        print('capabilities_enabled: %s' % ['server', 'prediction', 'version'])
        print('capabilities_disabled: %s' % ['cluster', 'optimizer', 'traffic'])
        print('')
    ...
    def cluster_connect(self,
        ...
        self._cluster_wildcard_connect(service_name=service_name,
                                       kube_namespace=kube_namespace,
                                       local_port=local_port,
                                       service_port=service_port)


    def _cluster_wildcard_connect(self,
                                  service_name,
                                  kube_namespace="default",
                                  local_port=None,
                                  service_port=None):
    ...
    def _get_cluster_node_resources(self):
    ...
    def server_build(self,
        ...
        if build_type == 'docker':
            generated_Dockerfile = self._build_init(model_type=model_type, 
                                                                model_name=model_name,
                                                                model_tag=model_tag,
                                                                templates_path=templates_path,
                                                                build_path=build_path)
    ...
    def cluster_rollout(self,
        ...
        with warnings.catch_warnings():
            ...
            if found:
                ...
                subprocess.call(cmd, shell=True)
                print("")
                print("Check status with 'pipeline cluster-status'.")
                print("")
            else:
                print("")
                print("App '%s' is not running." % service_name)
    ...
    def optimizer_generate(self,
        ...
        print('optimize_server_url: %s' % optimize_server_url)
        print('timeout: %s' % timeout)
        ...
        full_optimize_server_url = "%s/api/%s/model/optimize/%s/%s/%s" % (optimize_server_url.rstrip('/'), PipelineCli._pipeline_api_version, model_type, model_name, model_tag)

        with open(tar_path, 'rb') as fh:
            files = [(upload_key, (upload_value, fh))]
            print("")
            print("Optimizing model tar.gz '%s' to '%s'." % (tar_path, full_optimize_server_url))
            headers = {'Accept': 'application/json'}
            try:
                response = requests.post(url=full_optimize_server_url,
                                         headers=headers,
                                         files=files,
                                         timeout=timeout)
    ...
    def prediction_loadtest(self,
        ...
        self._prediction_test(model_type=model_type,
                              model_name=model_name,
                              model_tag=model_tag,
    ...
    def _prediction_test(self,
        ...
        with ThreadPoolExecutor(max_workers=test_request_concurrency) as executor:
            for _ in range(test_request_concurrency):
                executor.submit(self._predict(predict_server_url=predict_server_url,
    ...
    def cluster_status(self):
        ...
        self._cluster_volumes()

        print("")
        print("Cluster Resources")
        print("*****************")        
        self._get_cluster_node_resources()
        ...
        #    for svc in services:
        #        self._get_cluster_resources(service_name=svc.metadata.name)
        ...
        #    services = response.items
        #    for svc in services:
        #        self._cluster_describe(service_name=svc.metadata.name)
    ...
    def cluster_shell(self,
        ...
        self._cluster_wildcard_shell(service_name=service_name,
                                     kube_namespace=kube_namespace)


    def _cluster_wildcard_shell(self,
                                service_name,
                                kube_namespace='default'):
    ...
    def cluster_logs(self,
        ...
        self._cluster_wildcard_logs(service_name=service_name,
                                    kube_namespace=kube_namespace)


    def _cluster_wildcard_logs(self,
                               service_name,
                               kube_namespace='default'):
    ...
    def cluster_describe(self,
        ...
        self._cluster_wildcard_describe(service_name=service_name)


    def _cluster_wildcard_describe(self,
                                   service_name):
    ...
    def cluster_scale(self,
        ...
        self._cluster_wildcard_scale(service_name=service_name,
                                     replicas=replicas,
                                     kube_namespace=kube_namespace)

    def _cluster_wildcard_scale(self,
                                service_name,
                                replicas,
                                kube_namespace='default'):
    ...
    def cluster_quarantine(self,
                           instance_name):
        # TODO:  Split out pod from cluster for forensic purposes
        pass


    def traffic_register(self,
                         model_type,
                         model_name,
                         model_tag):
        # TODO:  Add routing
    ...
    def traffic_deregister(self,
                           model_type,
                           model_name,
                           model_tag):
        # TODO:  Remove model cluster from accepting traffic
        pass
    ...
    def traffic_split(self,
                      ...
                      model_tag,
                      percentage_traffic):
        # TODO:  Split percentage_traffic
        pass


    def traffic_shadow(self,
                       model_type,
                       model_name,
                       model_tag,
                       percentage_traffic):
        # TODO:  Shadow percentage_traffic
        pass


    def traffic_status(self):
        # TODO:  Show all traffic statuses for all model clusters
        pass

  
    def traffic_describe(self,
                         model_type,
                         model_name,
                         model_tag):
        # TODO:  Describe traffic for a model server cluster
        pass    
...
#        print("")
    ...
    def cluster_stop(self,
        ...
        service_name = '%s-%s-%s-%s' % (build_prefix, model_type, model_name, model_tag)
        self._cluster_wildcard_stop(service_name=service_name, 
                                    kube_namespace=kube_namespace)


    def _cluster_wildcard_stop(self,
                               service_name,
                               kube_namespace='default'):
