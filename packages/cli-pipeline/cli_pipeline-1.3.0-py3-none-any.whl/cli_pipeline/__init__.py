...
class PipelineCli(object):
    ...
    def version(self):
        ...
        print('capabilities_enabled: %s' % ['predict', 'server', 'version'])
        print('capabilities_disabled: %s' % ['train', 'cluster', 'optimize'])
    ...
    def cluster_upgrade(self,
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            response = kubeclient_v1_beta1.list_deployments_for_all_namespaces(watch=False,
                                                                               pretty=True)
            found = False
            ...
            if found:
                ...
                subprocess.call(cmd, shell=True)
                print("")
                print("Check status with 'pipeline cluster-describe'.")
                print("")
            else:
                print("")
                print("App '%s' is not running." % service_name)
    ...
    def cluster_quarantine(self,
                           service_name,
                           instance_name):
        # TODO:  Split out node from cluster.  Save for forensic study.
        pass
    ...
    def cluster_route(self,
                      service_name_list,
                      pct_list):
        # TODO:  Route traffic to service_name_list according to pct list.
        pass
    ...
    def cluster_start(self,
        ...
        print("")
        print("Check service status with 'pipeline cluster-describe'.")
        print("")
    ...
    def cluster_stop(self,
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            response = kubeclient_v1_beta1.list_deployments_for_all_namespaces(watch=False, pretty=True)
            found = False
            deployments = response.items
            for deploy in deployments:
                if service_name in deploy.metadata.name:
