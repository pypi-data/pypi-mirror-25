...
def ATR(security_list, check_date, timeperiod=14):
    ...
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close','high','low'] , skip_paused = True, count = timeperiod*2)
        ...
        if nan_count == len(security_data['close']):
            ...
            temp = -timeperiod
            while temp < 0:
                t = max( high_ATR[temp]-low_ATR[temp], abs(close_ATR[temp-1]-high_ATR[temp]), abs(close_ATR[temp-1]-low_ATR[temp]) )
                list_mtr.append(t)
                temp += 1
...
def BIAS(security_list,check_date, N1=6, N2=12, N3=24):
    ...
    maxN = max(N1, N2, N3)
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = maxN*2)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            bias1[stock] = np.nan
...
def CCI(security_list, check_date, N=14):
    ...
    cci = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close','high','low'] , skip_paused = True, count = N*2)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            cci[stock] = np.nan
            ...
            cci[stock] = talib.CCI(np.array(high_CCI), np.array(low_CCI), np.array(close_CCI), N)[-1]
    return cci
...
def KDJ(security_list, check_date, N =9, M1=3, M2=3):
    ...
    k = {}; d = {}; j = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['high', 'low', 'close'] , skip_paused = True, count = n*10)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            k[stock] = np.nan
            ...
            kValue, dValue = talib.STOCHF(high, low, close, N , M2, fastd_matype=0)
            # 求K值(等于RSV的M1日移动平均)
...
def MFI(security_list, check_date, timeperiod=14):
    ...
    mfi = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields =['high','low','close','volume'], skip_paused = True, count = timeperiod+1)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            mfi[stock] = np.nan
            ...
            mfi[stock] = talib.MFI(high_MFI,low_MFI,close_MFI,volume_MFI)[-1]
    return mfi
...
def MTM(security_list, check_date, timeperiod=12):
    ...
    mtm = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date,frequency = '1d', fields = ['close'] , skip_paused = True, count = timeperiod + 1)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            mtm[stock] = np.nan    
...
def ROC(security_list, check_date, timeperiod = 12):
    ...
    roc = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d',fields = ['close'],skip_paused = True, count = timeperiod * 20)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
            roc[stock] = np.nan
...
def RSI(security_list, check_date, N1=6):
    ...
    rsi = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = N1*20)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            rsi[stock] = np.nan
...
def ACCER(security_list, check_date, N = 8):
    ...
    specificCount = N * 4
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = specificCount)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
            accer[stock] = np.nan
...
def ADTM(security_list, check_date, N = 23, M = 8):
    ...
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['open', 'high', 'low'] , skip_paused = True, count = specificCount)
        ...
        if nan_count == len(security_data['open']):
            ...
            # 计算DTM
            dtm = [0.0 if diff <= 0 else max(hg_minus_open, open_minus_ref) for diff, hg_minus_open, open_minus_ref in zip(diff_open, high_ADTM-open_ADTM, open_ADTM-ref_open)]
            # 计算DBM
            dbm = [0.0 if diff >= 0 else max(open_minus_low, open_minus_ref) for diff, open_minus_low, open_minus_ref in zip(diff_open, open_ADTM-low_ADTM, open_ADTM-ref_open)]
            # 计算STM
            ...
            # 计算ADTM
            adtm = [sms / _stm if sms > 0 else (0.0 if sms == 0 else sms / _sbm) for sms, _stm, _sbm in zip(stm_minus_sbm, stm, sbm)]
            # 计算MAADTM
...
def BIAS_36(security_list, check_date,  M = 6):
    ...
    for stock in security_list:
        ...
        if nan_count == len(security_data['close']):
            ...
            # 计算BIAS612
            preBias612 = talib.MA(close_BIAS36, N2)-talib.MA(close_BIAS36, N3)
            # 计算MABIAS
...
def DKX(security_list, check_date, M = 10):
    ...
    specificCount = maxN * 4
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'low', 'high', 'open'] , skip_paused = True, count = specificCount)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)        
...
def KD(security_list, check_date, N = 9, M1 = 3, M2 = 3):
    ...
    maxN = max(N, M1, M2)
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'low', 'high'] , skip_paused = True, count = maxN * 7)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
            kd_K[stock] = np.nan
            ...
            # 计算 RSV
            list_RSV, temp = talib.STOCHF(high_KD, low_KD, close_KD, N, N, fastd_matype=0)
            # 计算 K
...
def LWR(security_list, check_date, N = 9, M1 = 3, M2 = 3):
    ...
    maxN = max(N, M1, M2)
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'low', 'high'] , skip_paused = True, count = maxN * 6)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
            lwr1[stock] = np.nan
            ...
            lwr1[stock] = list_lwr1[-1]
            lwr2[stock] = list_lwr2[-1]
    return lwr1, lwr2
...
def OSC(security_list, check_date, N = 20, M = 6):
    ...
    maxN = max(N, M)
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = maxN * 10)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
            osc[stock] = np.nan
            ...
            except Exception, e: # 在security_data['close']不为全空，list_osc为全空时，talib.EMA会出错(只有新股才出现这种情况)
                osc[stock] = np.nan
                maosc[stock] = np.nan
            else:
...
def SKDJ(security_list, check_date, N = 9, M = 3):
    ...
    maxN = max(N, M)
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'low', 'high'] , skip_paused = True, count = maxN * 5)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
            skdj_K[stock] = np.nan
            ...
            # 计算 RSV
            list_preRSV, temp = talib.STOCHF(high_SKDJ, low_SKDJ, close_SKDJ, N, N, fastd_matype=0)
...
def UDL(security_list, check_date, N1 = 3, N2 = 5, N3 = 10, N4 = 20, M = 6):
    ...
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = maxN * 4)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
            udl[stock] = np.nan
            ...
            # 计算UDL
            list_udl = (talib.MA(close_UDL, N1) + talib.MA(close_UDL, N2) + talib.MA(close_UDL, N3) + talib.MA(close_UDL, N4)) / 4
            # 计算MAUDL
...
def WR(security_list, check_date, N = 10, N1 = 6):
    ...
    maxN = max(N, N1)
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'high', 'low'] , skip_paused = True, count = maxN * 4)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
            wr1[stock] = np.nan
...
def CYF(security_list, check_date, N = 21):
    ...
    for stock in security_list:
        ...
        if start >= dates[-1] or (dates[0] <= start and start <= dates[-1]) or end < dates[-1]:
            ...
            turnover_ratio = [get_fundamentals(q, date=day)['turnover_ratio'][0] for day in dates]
            # 计算EMA(HSL,N)
...
def FSL(security_list, check_date):
    ...
    dates = jqdata.get_trade_days(end_date=check_date, count=specificCount)
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'volume'] , skip_paused = True, count = specificCount)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
            ...
            try:
                ...
                circulating_cap = np.array([get_fundamentals(q, day)['circulating_cap'][0] for day in dates])
            except IndexError:
                log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
                continue
            except Exception, e:
            ...
            pre_max = [100 * sum_t / (N1 * cap) for sum_t,cap in zip(sum_N2,circulating_cap)]
...
def TAPI(index_stock, security_list, check_date, M=6):
    ...
    for stock in new_security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'money'] , skip_paused = True, count = specificCount)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            if point == 0:
                # 大盘数据全为空(这种情况不会发生)
...
def CHO(security_list, check_date, N1 = 10, N2 = 20, M = 6): 
    ...
    cho_macho = {}
    for stock in security_list:
        security_data = get_price(stock, end_date=check_date, frequency='daily', fields=['low', 'high', 'close', 'volume'], skip_paused=True, count=maxN * 4)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            ...
            temp = -maxN * 2
            while temp < 0:
                m  = volume_CHO[temp] * (2 * close_CHO[temp]-high_CHO[temp]-low_CHO[temp]) / (high_CHO[temp] + low_CHO[temp])
                if temp != -maxN * 2:
                    m += mid[temp-1]
                mid[temp] = m
                temp += 1
...
def CYE(security_list, check_date):
    ...
    M = 20
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = M * 2)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
...
def DBQR(index_stock, security_list, check_date, N = 5, M1 = 10, M2 = 20, M3 = 60):
    ...
    # 计算ZS
    zs = []
    index_data = get_price(index_stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = maxN * 3)
    nan_count = list(np.isnan(index_data['close'])).count(True)
    if nan_count == len(index_data['close']):
        log.info("大盘： %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % index_stock)
        dbqr_zs[index_stock] = np.nan
    else:
        close_Index = index_data['close']
        temp = -2*maxN
        while temp < 0:
            t = (close_Index[temp]-close_Index[temp-N]) / close_Index[temp-N]
            zs.append(t)
            temp += 1
    ...
    # 计算 DBQR
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = maxN * 3)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
            ...
            temp = -2 * maxN
            while temp < 0:
                t = (close_DBQR[temp]-close_DBQR[temp-N]) / close_DBQR[temp-N]
                gg.append(t)
                temp += 1
...
def DMA(security_list, check_date, N1 = 10, N2 = 50, M = 10):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date=check_date, frequency='daily', fields=['close'], skip_paused=True, count=maxN * 2)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
...
def DMI(security_list, check_date, N=14,  MM = 6):
    ...
    dmi_adxr = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['high', 'low' ,'close'] , skip_paused = True, count = N * 20)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            ...
            temp = -N *5
            while temp < 0:
                m = max(high_DMI[temp]-low_DMI[temp], abs(high_DMI[temp]-close_DMI[temp-1]), abs(close_DMI[temp-1]-low_DMI[temp]))
                maxList.append(m)
                temp += 1
...
def DPO(security_list, check_date, N=20,  M = 6):
    ...
    dpo_madpo = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = N * 20)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
...
def EMV(security_list, check_date, N = 14, M = 9):
    ...
    maemv = {}
    for stock in security_list:
        security_data = get_price(stock, end_date=check_date, frequency='daily', fields=['low', 'high', 'close', 'volume'], skip_paused=True, count= N * 10)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            emv[stock] = np.nan
            ...
            temp = -5*N
            while temp < 0:
                t = mid[temp] * volume[temp] * hgMinusl[temp] / ma_hgMinusl[temp]
                pre_emv.append(t)
                temp += 1
            # 获取 EMV
...
def GDX(security_list, check_date, N = 30, M = 9):
    ...
    gdx_zcx = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'high', 'low'] , skip_paused = True, count = N * 10)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
...
def JS(security_list, check_date, N = 5, M1 = 5, M2 = 10, M3 = 20):
    ...
    for stock in security_list:
        ...
        if nan_count == len(security_data['close']):
            ...
            temp = -2 * maxN
            while temp < 0:
                t = 100 * (close_JS[temp]-close_JS[temp-N]) / (N * close_JS[temp-N])
                js.append(t)
                temp += 1
...
def MACD(security_list, check_date, SHORT = 12, LONG = 26, MID = 9):
    ...
    macd_macd = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = SHORT * 20)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            ...
            dic = talib.MACD(np.array(close_macd), fastperiod= SHORT, slowperiod=LONG, signalperiod=MID)
...
def QACD(security_list, check_date, N1 = 12, N2 = 26, M = 9):
    ...
    maxN = max(N1, N2)
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = maxN * 20)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
...
def QR(index_stock, security_list, check_date, N = 21):
    ...
    # 计算大盘值
    dp = []
    index_data = get_price(index_stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = N * 3)
    nan_count = list(np.isnan(index_data['close'])).count(True)
    if nan_count == len(index_data['close']):
        log.info("大盘： %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % index_stock)
        ...
        temp = -2*N
        while temp < 0:
            t = (close_Index[temp]-close_Index[temp-N]) / close_Index[temp-N] * 100
            dp.append(t)
            temp += 1
    ...
    if isinstance(security_list, (str, unicode)):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = N * 5)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
            qr_gg[stock] = np.nan
            ...
            temp = -2 * N
            while temp < 0:
                t = (close_QR[temp]-close_QR[temp-N]) / close_QR[temp-N] * 100
                gg.append(t)
                temp += 1
...
def UOS(security_list, check_date, N1 = 7, N2 = 14, N3 = 28, M = 6):
    ...
    maxN = max(N1, N2, N3)
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['high', 'low', 'close'] , skip_paused = True, count = maxN * 10)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
            ...
            ultiIndicator = (acc1 * N2 * N3 + acc2 * N1 * N3 + acc3 * N1 * N2) * 100 / (N1*N2 + N1*N3 + N2*N3)
            #计算 mauos
...
def VMACD(security_list, check_date, SHORT = 12, LONG = 26, MID = 9):
    ...
    vmacd_macd = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'volume'] , skip_paused = True, count = SHORT * 20)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
...
def VPT(security_list, check_date, N = 51, M = 6):
    ...
    vpt_mavpt = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['volume', 'close'] , skip_paused = True, count = N * 2 + 1)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
...
def WVAD(security_list, check_date, N = 24, M = 6):
    ...
    wvad_mawvad = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['low', 'high', 'open', 'close', 'volume'] , skip_paused = True, count = N * 10)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
            ...
            pre_WVAD = []
            while temp < 0:
                t = (close_WVAD[temp]-open_WVAD[temp]) / (high_WVAD[temp]-low_WVAD[temp]) * volume_WVAD[temp]
                pre_WVAD.append(t)
                temp += 1
            # 获取wvad 和 mawvad
...
def PSY(security_list, check_date, timeperiod=12):
    ...
    psy = {}
    for stock in security_list:
        security_data = get_price(stock, end_date=check_date, frequency='daily', fields=['close'], skip_paused=True, count=timeperiod+1)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            psy[stock] = np.nan
...
def VR(security_list, check_date, N=26, M=6):
    ...
    for stock in security_list:
        ...
        if nan_count == len(security_data['close']):
            ...
            # 获取计算TH, TL, TQ的源数据
            pre_th = [vol if c_m_ref > 0 else 0.0 for vol, c_m_ref in zip(volume_VR, close_minus_ref)]
            pre_tl = [vol if c_m_ref < 0 else 0.0 for vol, c_m_ref in zip(volume_VR, close_minus_ref)]
            pre_tq = [vol if c_m_ref == 0 else 0.0 for vol, c_m_ref in zip(volume_VR, close_minus_ref)]
...
def BRAR(security_list, check_date, N=26):
    ...
    specificCount = N + 2
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'high', 'low', 'open'], skip_paused = True, count = specificCount)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)
...
def CR(security_list, check_date, N=26, M1=10, M2=20, M3=40, M4=62):
    ...
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['high', 'low'] , skip_paused = True, count = specificCount)
...
def CYR(security_list, check_date, N = 13, M = 5):
    ...
    specificCount = maxN * 5
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['volume', 'money'] , skip_paused = True, count = specificCount)
...
def MASS(security_list, check_date, N1=9, N2=25, M=6):
    ...
    specificCount = maxN * 4
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['high', 'low'] , fq='pre', skip_paused = True, count = specificCount)
        nan_count = list(np.isnan(security_data['high'])).count(True)
...
def OBV(security_list, check_date, timeperiod=30):
    ...
    OBV = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close','volume'] ,skip_paused = True, count = timeperiod )
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            OBV[stock] = np.nan
...
def AMO(security_list, check_date, M1 = 5, M2 = 10):
    ...
    specificCount = maxN * 4
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['money'] , skip_paused = True, count = specificCount)
...
def CCL(futures_list, check_date, M=5):
    ...
    for stock in futures_list:
        futures_data = get_extras('futures_positions', stock, end_date=check_date, count=specificCount)
...
def DBLB(index_stock, security_list, check_date, N=5, M=5):
    ...
    specificCount = N * 2
    for stock in new_security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['volume'] , skip_paused = True, count = specificCount)
        nan_count = list(np.isnan(security_data['volume'])).count(True)
        if nan_count == len(security_data['volume']):
...
def HSL(security_list, check_date, N = 5):
    ...
    specificCount = N * 1
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['volume', 'high'] , skip_paused = True, count = specificCount)
        nan_count = list(np.isnan(security_data['volume'])).count(True)
        if nan_count == len(security_data['volume']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)        
            ...
            circulating_cap = np.array([get_fundamentals(q, day)['circulating_cap'][0] for day in dates])
            # 计算HSL，通达信获取的流通股本是聚宽的1/10000，而计算公式中是volume_HSL / (circulating_cap / 100)，所以此处化简为下面的式子
...
def VRSI(security_list, check_date, N1=6, N2=12, N3=24):
    ...
    import talib
    import numpy as np
    # 计算SMA(X, N, M)， 即X的N日移动平均，M为权重, 若Y=SMA(X,N,M) 则 Y = (M*X+(N-M)*Y')/N, 其中Y'表示上一周期Y值,N必须大于M。返回一个值
    def SMA(X, N, M=1):
...
def BBI(security_list, check_date, timeperiod1=3, timeperiod2=6, timeperiod3=12, timeperiod4=24):
    ...
    bbi={}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = timeperiod4*2)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            bbi[stock] = np.nan
...
def MA(security_list, check_date, timeperiod=5):
    ...
    ma = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = timeperiod*2)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            ma[stock] = np.nan
...
def EXPMA(security_list, check_date, timeperiod = 12):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', skip_paused = True, fields = ['close'], count = timeperiod)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            expma[stock] = np.nan
...
def HMA(security_list, check_date, timeperiod = 12):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', skip_paused = True, fields = ['high'], count = timeperiod)
        nan_count = list(np.isnan(security_data['high'])).count(True)
        if nan_count == len(security_data['high']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            hma[stock] = np.nan
...
def LMA(security_list, check_date, timeperiod = 12):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', skip_paused = True, fields = ['low'], count = timeperiod)
...
def VMA(security_list, check_date, timeperiod = 12):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', skip_paused = True, fields = ['low', 'high', 'open', 'close'], count = timeperiod)
        nan_count = list(np.isnan(security_data['low'])).count(True)
        if nan_count == len(security_data['low']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            vma[stock] = np.nan
        else:
            vv = np.array((security_data['low'] + security_data['close'] + security_data['high'] + security_data['open']) / 4)
...
def ALLIGAT(security_list, check_date, timeperiod = 21):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', skip_paused = True, fields = ['high', 'low'], count = timeperiod)
        nan_count = list(np.isnan(security_data['high'])).count(True)
        if nan_count == len(security_data['high']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            up[stock] = np.nan
...
def AMV(security_list, check_date, timeperiod = 13):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', skip_paused = True, fields = ['volume', 'open', 'close'], count = timeperiod)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            amv[stock] = np.nan
        else:
            amov = np.array(security_data['volume'] * (security_data['close'] + security_data['open']) / 2)
            amv[stock] = (talib.SUM(amov, timeperiod) / talib.SUM(np.array(security_data['volume']), timeperiod))[-1]
    return amv
...
def BBIBOLL(security_list, check_date, N = 11, M = 6):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', skip_paused = True, fields = ['close'], count = 23 + N)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            upr[stock] = np.nan
            ...
            one_bbi = (talib.MA(CV, 3) + talib.MA(CV, 6) + talib.MA(CV, 12) + talib.MA(CV, 24)) / 4
...
def Bollinger_Bands(security_list, check_date, timeperiod=20, nbdevup=2, nbdevdn=2):
    ...
    for stock in security_list:
        ...
        if nan_count == len(security_data['close']):
            ...
            up, mid, low = talib.BBANDS(np.array(closeBOLL), timeperiod, nbdevup, nbdevdn)
            upperband[stock], middleband[stock], lowerband[stock] = up[-1], mid[-1], low[-1]
...
def ENE(security_list, check_date, N = 25, M1 = 6, M2 = 6):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', skip_paused = True, fields = ['close'], count = N)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            upper[stock] = np.nan
...
def MIKE(security_list, check_date, timeperiod = 10):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', skip_paused = True, fields = ['low', 'high', 'close'], count = timeperiod * 3)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            stor[stock] = np.nan
...
def PBX(security_list, check_date, timeperiod = 9):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', skip_paused = True, fields = ['close'], count = timeperiod * 4)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            pbx[stock] = np.nan
            ...
            close = np.array(security_data['close'])
            tem_pbx = (talib.EMA(close, timeperiod)[-1] + np.mean(close[-timeperiod * 2:]) + np.mean(close[-timeperiod * 4:])) / 3.0
            pbx[stock] = tem_pbx
    return pbx
...
def XS(security_list, check_date, timeperiod = 13):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['volume', 'close'], skip_paused = True, count =  timeperiod * 4)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
                log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
                sup[stock] = np.nan
            ...
            var2 = close * volume
            var3 = talib.EMA((talib.EMA(var2, 3) / talib.EMA(volume, 3) + talib.EMA(var2, 6) / talib.EMA(volume, 6) + talib.EMA(var2, 12) / talib.EMA(volume, 12) + talib.EMA(var2, 24) / talib.EMA(volume, 24)) / 4, timeperiod)
...
def XS2(security_list, check_date, N = 102, M = 7):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', skip_paused = True, fields = ['low', 'high', 'close'], count = max(N, M))
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            pass1[stock] = np.nan
...
def SMA(security_list, check_date, N = 7, M = 1):
    ...
    sma = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = N*2)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            sma[stock] = np.nan
...
def BDZX(security_list, check_date, timeperiod = 40):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'low', 'high'], skip_paused = True, count = timeperiod)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            aa[stock] = np.nan 
...
def CDP_STD(security_list, check_date, timeperiod = 2):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'low', 'high'], skip_paused = True,count = timeperiod+1)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            cdp[stock] = np.nan
...
def CYHT(security_list, check_date, timeperiod = 60):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['open', 'close', 'low', 'high'], skip_paused = True, count = timeperiod)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)   
            sk[stock] = np.nan
            ...
            var2 = llv(low, 34)
            var3 = hhv(high, 34)
            t_sk = talib.EMA((var1[33-timeperiod:]-var2) / (var3-var2)*100, 13)
...
def JFZX(security_list, check_date, timeperiod = 30):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'open', 'volume'], skip_paused = True, count = timeperiod)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            most[stock] = np.nan
...
def JYJL(security_list, check_date, N = 120, M = 5):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['volume'], skip_paused = True, count = N)
        nan_count = list(np.isnan(security_data['volume'])).count(True)
        if nan_count == len(security_data['volume']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
            com[stock] = np.nan
...
def LYJH(security_list, check_date, M = 80, M1 = 50):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'low', 'high'], skip_paused = True, count = M)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)   
            empty[stock] = np.nan
            ...
            high = np.array(security_data['high'])
            low = np.array(security_data['low'])
            var1 = (hhv(high, 36)-close[35-M:]) / (hhv(high, 36)-llv(low, 36))*100
            empty[stock] = SMA(var1, 2, 1)[-1]
            var2 = (close[8-M:]-llv(low, 9)) / (hhv(high, 9)-llv(low, 9))*100
...
def TBP_STD(security_list, check_date, timeperiod=30):
    ...
    std_ktts = {}
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'high', 'low'] , skip_paused = True, count = timeperiod)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)        
            ...
            # 计算TR0
            tr0 = [max(h_m_l, abs(h_m_r), abs(l_m_r)) for (h_m_l, h_m_r, l_m_r) in zip(high_minus_low[1:], high_minus_refclose, low_minus_refclose)]
            # 计算MF0:=C-REF(C,2);
            ...
            # 计算DIRECT1
            pre_direct1 = [True if f0 > f1 and f0 > f2 else False for (f0, f1, f2) in zip(mf0[2:], mf1[1:], mf2)]    
            last_direct1 = max(np.nonzero(np.array(pre_direct1) == True)[0])
            direct1 = len(pre_direct1)-last_direct1-1
            # 计算DIRECT2
            pre_direct2 = [True if f0 < f1 and f0 < f2 else False for (f0, f1, f2) in zip(mf0[2:], mf1[1:], mf2)]
            ...
            tbp = close_STD[-3] + (min_mf0_mf1 if direct0 > 50 else max_mf0_mf1)
...
def ZBCD(security_list, check_date, timeperiod = 10):
    ...
    if isinstance(security_list, six.string_types):
        security_list = [security_list]
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'open', 'low' ,'high', 'volume', 'money'], skip_paused = True, count = timeperiod*5)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
                log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" %stock)
                cd[stock] = np.nan
...
def ZLMM(security_list, check_date):
    ...
    for stock in security_list:
        ...
        if nan_count == len(security_data['close']):
            ...
            # 计算MAX(CLOSE-LC,0)
            max_close_lc = [max(close-l, 0.0) for close, l in zip(close_ZLMM, lc)]
            # 计算ABS(CLOSE-LC)
...
def RAD(index_stock, security_list, check_date, D=3, S=30, M=30):
    ...
    ii = []
    for stock in new_security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['open', 'close', 'high', 'low'], skip_paused = True, count = specificCount)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            if i == 0:
                log.info("大盘股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)        
...
def SHT(security_list, check_date, N=5):
    ...
    for stock in security_list:
        ...
        if nan_count == len(security_data['close']):
            ...
            vol_minus_pre = [(vol-pre_vol) / pre_vol for vol, pre_vol in zip(volume_SHT, pre_volume)]
            # 计算VAR1
...
def CYW(security_list, check_date):
    ...
    specificCount = N * 4
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['close', 'high', 'low', 'volume'] , skip_paused = True, count = specificCount)
        nan_count = list(np.isnan(security_data['close'])).count(True)
        if nan_count == len(security_data['close']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)        
            ...
            var4 =  [pre_v if high > low else 0.0 for pre_v, high, low in zip(pre_var4, high_CYW, low_CYW)]
            # 计算CYW
...
def CYS(security_list, check_date):
    ...
    specificCount = N * 2
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['money', 'volume', 'close'] , skip_paused = True, count = specificCount)
        nan_count = list(np.isnan(security_data['money'])).count(True)
        if nan_count == len(security_data['money']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)        
...
def ZSDB(index_stock, check_date):
    ...
    zsdb_zszf = {}
    specificCount = 2
    security_data = get_price(index_stock, end_date = check_date, frequency = '1d', fields = ['close'] , skip_paused = True, count = specificCount)
    nan_count = list(np.isnan(security_data['close'])).count(True)
    if nan_count == len(security_data['close']):
        log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)        
        zsdb_a[stock] = np.nan
...
def AROON(security_list, check_date, N = 25):
    ...
    specificCount = N
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['high'] , skip_paused = True, count = specificCount)
        nan_count = list(np.isnan(security_data['high'])).count(True)
        if nan_count == len(security_data['high']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)        
...
def CFJT(security_list, check_date, MM = 200):
    ...
    for stock in security_list:
        ...
        if nan_count == len(security_data['close']):
            ...
            # 计算A1X
            a1x = [(num_ema-num_tp) * 100 / num_tp for num_ema, num_tp in zip(ema_M_close, ema_N_close)]
...
def ZX(security_list, check_date):
    ...
    specificCount = 1
    for stock in security_list:
        security_data = get_price(stock, end_date = check_date, frequency = '1d', fields = ['volume', 'money', 'close', 'high', 'low'] , skip_paused = True, count = specificCount)
        nan_count = list(np.isnan(security_data['volume'])).count(True)
        if nan_count == len(security_data['volume']):
            log.info("股票 %s 输入数据全是 NaN，该股票可能已退市、未上市或刚上市，返回 NaN 值数据。" % stock)        
            zx_av[stock] = np.nan
