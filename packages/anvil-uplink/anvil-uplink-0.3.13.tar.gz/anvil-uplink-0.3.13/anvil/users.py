...
if is_server_side():
    ...
    #!defFunction(anvil.users,!,[show_signup_option=])!2: "Display a login form and allow user to log in. Returns user object if logged in, or None if cancelled.\n\nIf signups are enabled, the form will also show the option to sign up for a new account. This can be disabled by passing show_signup_option=False." ["login_with_form"]
    def login_with_form(show_signup_option=True):
        lp = LinearPanel()
        email_box = None
        passwd_box = None
