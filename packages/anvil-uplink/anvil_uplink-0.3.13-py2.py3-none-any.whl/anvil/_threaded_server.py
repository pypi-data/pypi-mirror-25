...
import threading, random, string, json, re, sys, time
            ...
            try:
                if 'liveObjectCall' in self.json:
                    ...
                    for reg in registrations:
                        ...
                        raise Exception('No server function matching "%s" has been registered' % command)
                ...
                try:
                    ...
                    raise _server.AnvilSerializationError("Tried to store illegal value in a anvil.server.session. " + e.args[0])

                resp = {"id": self.json["id"], "response": response, "sessionData": sjson}
                if call_info.enable_profiling:
