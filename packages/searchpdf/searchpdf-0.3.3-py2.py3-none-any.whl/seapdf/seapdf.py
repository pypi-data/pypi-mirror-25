...
class pdf(object):
    ...
    def transform(self):
        ...
        try:
                                ...
                                if file_or_dir == '.pdf':
                                    for file in f:
                                        if len(inpath.split('/')) == 1:
                                            ...
                                            if file == inpath.split('/')[-1] and '/'.join(inpath[:-1]) in r:
                                                if os.path.isfile(os.path.join(r, file)):
                                                    l += [os.path.join(r, file)]
                                elif file_or_dir == '':
            ...
            if l == []:
                if file_or_dir=='': 
                        raise ValueError("This is either a non-existent directory or no pdf file exists in this directory")
                elif file_or_dir=='.pdf':
                        raise ValueError("This is an non-existent pdf file")
                else:
