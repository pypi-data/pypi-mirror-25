...
class Geometry(object):
    ...
    def addLineLoopsFromDictionary(self, lineLoopsDictionary):
        ...
        # self.lineLoops.keys
        for lineLoop in lineLoopsDictionary.keys():
    ...
    def addPhysicalLineIDsFromDictionary(self, physicalIDsDictionary):
        ...
        for physicalID in physicalIDsDictionary:
            ...
            if self.physicalLineIDs.has_key(physicalID):
    ...
    def addPhysicalSurfaceIDsFromDictionary(self, physicalIDsDictionary):
        ...
        for physicalID in physicalIDsDictionary:
            ...
            if self.physicalSurfaceIDs.has_key(physicalID):
    ...
    def writeGmshGeoFile(self, geoFilename):
        ...
        import datetime
        import sys
        from .. import get_git_sha_key, get_version
        ...
        gmshFile.write('// Gmsh .geo file produced by qmesh version '+qmesh_version+' (git sha key: '+qmesh_git_sha_key+').\n')
        ...
        # write again the points used to define the sphere.
        ...
        if self.lineSegmentsCount() > 0:
            gmshFile.write('\n// Definitions of ' + str(self.lineSegmentsCount()) + ' line-segments:\n')
        ...
        if self.bsplinesCount() > 0:
            gmshFile.write('\n// Definitions of ' + str(self.bsplinesCount()) + ' B-splines:\n')
            for index in self.bsplines.keys():
        ...
        if self.lineLoopsCount() > 0:
            gmshFile.write('\n// Definitions of ' + str(self.lineLoopsCount()) + ' line-loops:\n')
            for index in self.lineLoops.keys():
        ...
        if self.surfacesCount() > 0:
            gmshFile.write('\n// Definitions of ' + str(self.surfacesCount()) + ' surfaces:\n')
            for index in self.surfaces.keys():
        ...
        if self.physicalLineIDsCount() > 0:
            gmshFile.write('\n// Definitions of ' + str(self.physicalLineIDsCount()) + ' physical line ID`s:\n')
            for index in self.physicalLineIDs.keys():
        ...
        if self.physicalSurfaceIDsCount() > 0:
            gmshFile.write('\n// Definitions of ' + str(self.physicalSurfaceIDsCount()) + ' physical surface ID`s:\n')
            for index in self.physicalSurfaceIDs.keys():
        ...
        gmshFile.close()
        LOG.info('Done writing '+geoFilename)
...
class Domain(object):
    ...
    def shp2geo(self, surfaceRadius=6.37101e+6):
        ...
        for feature in self.geometryPolygonShapes.featureList:
            featureGeometry = feature.geometry()
            shapeType = featureGeometry.wkbType()
            if not (shapeType == qgis.core.QGis.WKBPolygon or \
        ...
        for ringIndex in ringIDPointID_dictionary.keys():
            pointsOnRing = ringIDPointID_dictionary[ringIndex]
            for pointIndex in pointsOnRing:
                if pointIndex in pointIDRingID_dictionary.keys():
        ...
        for ringKey in ringIDPointID_dictionary.keys():
            lineList = []
        ...
        for ringKey in ringIDLineID_dictionary:
            ...
            while len(lineList) > 0:
                ...
                if np.sign(contiguousLinesList[-1]) >= 0:
        ...
        ringIDrenumbering = {}
        for ringID in ringIDLineID_dictionary.keys():
        ...
        for ringID in ringIDPointID_dictionary.keys():
        ...
        for pointID, ringIDs in zip(pointIDRingID_dictionary.keys(), pointIDRingID_dictionary.values()):
            renumbered_ringIDs=[]
        ...
        for polygon in polygons:
            ...
            for ring in polygon:
                ringID = ringID_dictionary.keys()[ringID_dictionary.values().index(ring)]
        ...
        if len(linePhysIDList) > 0:
            for line, lineIndex in zip(linePhysIDList, range(len(linePhysIDList))):
                if str(line) != 'NULL' and str(line) != 'None':
                    if physLineIDDictionary.has_key(line):
        ...
        if len(polygonPhysIDList) > 0:
            for polygon, polygonIndex in zip(polygonPhysIDList, range(len(polygonPhysIDList))):
                if physSurfaceIDDictionary.has_key(polygon):
...
class Mesh():
      ...
      def readGmsh(self, gmshFileName, crs):
          ...
          for counter in range(nodecount): 
              ...
              self.vertexCoordinates.append(map(float,line[1:4]))
          ...
          for counter in range(elementcount):
              ...
              if (element[1]=="1"):#Lines on boundaries of the domain
                  ...
                  elementLabels = map(int,element[3:3+numberOfLabels])
      ...
      def writeShapefile(self, filename):
          ...
          meshEdgeShapes.setShapeType(qgis.core.QGis.WKBLineString)
      ...
      def reProjectVertices(self, toCrs):
          ...
          if targetCoordReferenceSystem == 'PCC' and type(self.crs) == qgis.core.QgsCoordinateReferenceSystem:
              ...
              for pointCounter in range(len(self.vertexCoordinates)):
                  sourceCRS_point = qgis.core.QgsPoint(self.vertexCoordinates[pointCounter][0],
          ...
          # to authalic sphere using GFD_basisChangeTools, and then convert to target CRS
              ...
              for pointCounter in range(len(self.vertexCoordinates)):
                  ...
                  sourceCRS_point = qgis.core.QgsPoint(pointCoordinates[0], pointCoordinates[1])
                  targetCRS_point = CoordTransformer.transform(sourceCRS_point)
              ...
              if targetCoordReferenceSystem.toWkt != self.crs.toWkt:
                  ...
                  CoordTransformer = qgis.core.QgsCoordinateTransform(sourceCRS, targetCoordReferenceSystem)
                  CoordTransformer.initialise()
                  # Check if coordinate transformation is correctly initialised
                  if not(CoordTransformer.isInitialised()):
