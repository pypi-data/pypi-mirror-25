...
class Shapes(object):
    ...
    def setShapeType(self, shapeType):
        ...
        expectedShapeTypes = [qgis.core.QGis.WKBLineString,
                              qgis.core.QGis.WKBLineString25D,
                              qgis.core.QGis.WKBMultiLineString,
                              qgis.core.QGis.WKBMultiLineString25D,
                              qgis.core.QGis.WKBMultiPoint,
                              qgis.core.QGis.WKBMultiPoint25D,
                              qgis.core.QGis.WKBMultiPolygon,
                              qgis.core.QGis.WKBMultiPolygon25D,
                              qgis.core.QGis.WKBPoint,
                              qgis.core.QGis.WKBPoint25D,
                              qgis.core.QGis.WKBPolygon,
                              qgis.core.QGis.WKBPolygon25D]
    ...
    def writeInfo(self):
        ...
        if self.shapeType == qgis.core.QGis.WKBLineString:
            LOG.debug(', of type line-string.')
        elif self.shapeType == qgis.core.QGis.WKBLineString25D:
            LOG.debug(', of type 2.5 dim. line-string.')
        elif self.shapeType == qgis.core.QGis.WKBMultiLineString:
            LOG.debug(', of type multi-line-string.')
        elif self.shapeType == qgis.core.QGis.WKBMultiLineString25D:
            LOG.debug(', of type 2.5 dim. multi-line-string.')
        elif self.shapeType == qgis.core.QGis.WKBMultiPoint:
            LOG.debug(', of type multi-point.')
        elif self.shapeType == qgis.core.QGis.WKBMultiPoint25D:
            LOG.debug(', of type 2.5 dim. multi-point.')
        elif self.shapeType == qgis.core.QGis.WKBMultiPolygon:
            LOG.debug(', of type multi-polygon.')
        elif self.shapeType == qgis.core.QGis.WKBMultiPolygon25D:
            LOG.debug(', of type 2.5 dim. multi-polygon.')
        elif self.shapeType == qgis.core.QGis.WKBPoint:
            LOG.debug(', of type point.')
        elif self.shapeType == qgis.core.QGis.WKBPoint25D:
            LOG.debug(', of type 2.5 dim. point.')
        elif self.shapeType == qgis.core.QGis.WKBPolygon:
            LOG.debug(', of type polygon.')
        elif self.shapeType == qgis.core.QGis.WKBPolygon25D:
        ...
        for feature in self.featureList:
            ...
            if featGeometry.wkbType() == qgis.core.QGis.WKBPoint or \
               featGeometry.wkbType() == qgis.core.QGis.WKBMultiPoint or \
               featGeometry.wkbType() == qgis.core.QGis.WKBPoint25D or \
               featGeometry.wkbType() == qgis.core.QGis.WKBMultiPoint25D:
            ...
            elif featGeometry.wkbType() == qgis.core.QGis.WKBLineString or\
                 featGeometry.wkbType() == qgis.core.QGis.WKBMultiLineString or \
                 featGeometry.wkbType() == qgis.core.QGis.WKBLineString25D or \
                 featGeometry.wkbType() == qgis.core.QGis.WKBMultiLineString25D:
                ...
                for point in line:
                   LOG.debug('                Point Coords: '+point.toString())
            elif featGeometry.wkbType() == qgis.core.QGis.WKBPolygon or \
                 featGeometry.wkbType() == qgis.core.QGis.WKBMultiPolygon or \
                 featGeometry.wkbType() == qgis.core.QGis.WKBPolygon25D or \
                 featGeometry.wkbType() == qgis.core.QGis.WKBMultiPolygon25D:
                ...
                for field in feature.fields():
                   LOG.debug('            '+field.name()+' : ' + str(feature.attribute(field.name())))
                for (ringIndex, ring) in zip(range(len(polygon)), polygon):
    ...
    def projectUnitDiskToLonLat(self):
        ...
        for feature in self.featureList:
            ...
            if shapeType == qgis.core.QGis.WKBPolygon or \
               shapeType == qgis.core.QGis.WKBPolygon25D:
                ...
                for pointList in polygon:
                    for point in pointList:
                        ...
                        point.setY(latitude)
                feature.setGeometry(qgis.core.QgsGeometry.fromPolygon(polygon))
            elif shapeType == qgis.core.QGis.WKBPoint or \
                 shapeType == qgis.core.QGis.WKBPoint25D:
                ...
                for point in pointList:
                    ...
                    point.setY(latitude)
                feature.setGeometry(qgis.core.QgsGeometry.fromPolyline(pointList))
            elif shapeType == qgis.core.QGis.WKBLineString or \
                 shapeType == qgis.core.QGis.WKBLineString25D:
    ...
    def changeCoordRefSystem(self, targetCRS_string):
        ...
        """
        #Make sure input is correct and create new CRS object.
        ...
        if targetCRS_string != 'PCC':
            ...
            targetCRS.createFromWkt('PROJCS["unnamed",GEOGCS["Normal Sphere (r=0.5)",DATUM["unknown",SPHEROID["sphere",0.5,0]],PRIMEM["Greenwich",0],UNIT["degree",0.0174532925199433]],PROJECTION["Polar_Stereographic"],PARAMETER["latitude_of_origin",90],PARAMETER["central_meridian",90],UNIT["Meter",1]]')
        #Figure out shape-type
        if self.shapeType == qgis.core.QGis.WKBMultiPoint or\
           self.shapeType == qgis.core.QGis.WKBMultiPoint25D or\
           self.shapeType == qgis.core.QGis.WKBPoint25D or\
           self.shapeType == qgis.core.QGis.WKBPoint:
            shapeTypeString = 'point'
        elif self.shapeType == qgis.core.QGis.WKBLineString or\
           self.shapeType == qgis.core.QGis.WKBLineString25D or\
           self.shapeType == qgis.core.QGis.WKBMultiLineString or\
           self.shapeType == qgis.core.QGis.WKBMultiLineString25D:
            shapeTypeString = 'line'
        elif self.shapeType == qgis.core.QGis.WKBPolygon or\
           self.shapeType == qgis.core.QGis.WKBMultiPolygon or\
           self.shapeType == qgis.core.QGis.WKBMultiPolygon25D or\
           self.shapeType == qgis.core.QGis.WKBPolygon25D:
        ...
        if not(CoordTransformer.isInitialised()):
            raise Exception('ERROR: Change of Coordinate Reference System failed. Initialisation of coordinate transformation object failed')
        # Ensure the shape-type of the features is single-type, not mutli-type
        self.decomposeMultiFeatures()
            ...
            if shapeType == qgis.core.QGis.WKBPolygon or \
               shapeType == qgis.core.QGis.WKBPolygon25D:
                ...
                msg = 'Error in Coordinate Reference System Change: '+\
                      ' shape-type of object was not admitted.'
                raise Exception(msg)
    ...
    def decomposeMultiFeatures(self):
        ...
        for feature in self.featureList:
            ...
            shapeType = featureGeometry.wkbType()
            #Locate multi-points.
            if shapeType == qgis.core.QGis.WKBMultiPoint or \
               shapeType == qgis.core.QGis.WKBMultiPoint25D:
            ...
            #Locate multi-lines.
            if shapeType == qgis.core.QGis.WKBMultiLineString or \
               shapeType == qgis.core.QGis.WKBMultiLineString25D:
                multiLines = featureGeometry.asMultiPolyline()
                for line in multiLines:
                    newFeature = qgis.core.QgsFeature()
                    newFeature.setGeometry(qgis.core.QgsGeometry.fromPolyline(line))
                    #Set fields and attributes of new lines, by copying the fields
                    # and attributes of the multi-line.
                    newFeature.setFields(fields)
                    for field in feature.fields().toList():
            ...
            if shapeType == qgis.core.QGis.WKBMultiPolygon or \
               shapeType == qgis.core.QGis.WKBMultiPolygon25D:
                multiPolygons = featureGeometry.asMultiPolygon()
                for polygon in multiPolygons:
                    newFeature = qgis.core.QgsFeature()
                    newFeature.setGeometry(qgis.core.QgsGeometry.fromPolygon(polygon))
                    #Set fields and attributes of new lines, by copying the fields
                    # and attributes of the multi-line.
                    newFeature.setFields(fields)
                    for field in feature.fields().toList():
    ...
    def makeLinesContiguous(self):
        ...
        for currentLine in self.featureList:
            ...
            if not(shapeType != qgis.core.QGis.WKBLineString or \
                   shapeType != qgis.core.QGis.WKBLineString25D):
        ...
        for currentLine in non_contiguous_lineFeatures:
            ...
            if not foundStartPoint:
                ...
                currentLine.setGeometry(qgis.core.QgsGeometry.fromPolyline(pointsList))
...
def lines2polygon(outerRingFeatures, internalRings):
    ...
    for feature in outerRingFeatures:
        ...
        if (not (geometry.wkbType() == qgis.core.QGis.WKBLineString or
                 geometry.wkbType() == qgis.core.QGis.WKBMultiLineString)):
            raise BadArguments('Input geometry does not appear to be of line-type.')
        if geometry.wkbType() == qgis.core.QGis.WKBMultiLineString:
            temp_lines = geometry.asMultiPolyline()
            polyLine = []
            for t in temp_lines:
                polyLine.extend(t)
            outerRingLines.append(polyLine)
        else:
            outerRingLines.append(geometry.asPolyline())
    ...
    for ringFeatures in internalRings:
        ...
        for feature in ringFeatures:
            ...
            geometry = feature.geometry()
            #Ensure feature describes a line.
            if geometry.wkbType() != qgis.core.QGis.WKBLineString:
                raise BadArguments('Input geometry does not appear to be of line-type.')
        ...
        polygonList.append(ring)

    #Create a polygon-feature from the polygon-list and return.
    polygonFeature = qgis.core.QgsFeature()
    polygonFeature.setGeometry(qgis.core.QgsGeometry.fromPolygon(polygonList))
...
def insertRegions(receivingLines, receivingPolygons,
    ...
    for feature in receivingLines.getFeatures():
        ...
        if 'PhysID' not in fieldNames:
            error_msg = 'Error: Feature in file '+receivingLines_shpFile+\
                        ' does not seem to have an PhysID attribute.\n'+\
    ...
    for feature in receivingPolygons.getFeatures():
        ...
        #Check 'PhysID' is in the above-created list.
        if 'PhysID' not in fieldNames:
            error_msg = 'Error: Feature in file '+receivingPolygons_shpFile+\
                        ' does not seem to have an PhysID attribute.\n'+\
                        '        Feature '+str(feature.id())+ '\n'
            for fieldName in fieldNames:
                error_msg += '            '+fieldName+' : ' +\
    ...
    for feature in insertedLines.getFeatures():
        ...
        #Check 'PhysID' is in the above-created list.
        if 'PhysID' not in fieldNames:
            error_msg = 'Error: Feature in file '+insertedLines_shpFile+\
                        ' does not seem to have an PhysID attribute.\n'+\
                        '        Feature '+str(feature.id())+ '\n'
            for fieldName in fieldNames:
                error_msg += '            '+fieldName+' : ' +\
    ...
    for feature in insertedPolygons.getFeatures():
        ...
        #Check 'PhysID' is in the above-created list.
        if 'PhysID' not in fieldNames:
            error_msg = 'Error: Feature in file '+insertedPolygons_shpFile+\
                        ' does not seem to have an PhysID attribute.\n'+\
                        '        Feature '+str(feature.id())+ '\n'
            for fieldName in fieldNames:
                error_msg += '            '+fieldName+' : ' +\
    ...
    #receiving_polygonsLayer and inserted_polygonsLayer may contain
    # multiple polygons. We must locate where each inserted polygon
    # lies into, and insert it there.
    ...
    outputLines.setShapeType(qgis.core.QGis.WKBLineString)
    ...
    outputPolygons.setShapeType(qgis.core.QGis.WKBPolygon)
...
def identifyLoops(inputShapes,
    ...
    if not(inputShapes.shapeType != qgis.core.QGis.WKBLineString or \
           inputShapes.shapeType != qgis.core.QGis.WKBLineString25D or \
           inputShapes.shapeType != qgis.core.QGis.WKBMultiLineString or \
           inputShapes.shapeType != qgis.core.QGis.WKBMultiLineString25D) :
    ...
    if inputShapes.fields.indexFromName('PhysID') == -1:
        ...
        haveLinePhysID = True
    #
    loopIDfield = qgis.core.QgsField("LoopID", PyQt4.QtCore.QVariant.Int)
    physIDfield = qgis.core.QgsField("PhysID", PyQt4.QtCore.QVariant.Int)
    fields = qgis.core.QgsFields()
    ...
    for feature in inputShapes.featureList:
        ...
        # into lines.
        if not(geom.wkbType() != qgis.core.QGis.WKBLineString or \
               geom.wkbType() != qgis.core.QGis.WKBMultiLineString):
            msg = 'Method identifyLoops can only be invoked on line-type'+\
                  ' objects.\n'
            raise Exception(msg)
        if geom.wkbType() == qgis.core.QGis.WKBMultiLineString:
            temp_lines = geom.asMultiPolyline()
            for t in temp_lines:
                t = geom.fromPolyline(t)
                ...
                if extraPointsPerVertex != 0:
                    ...
                    for pointCounter in range(numbOriginalPoints-1):
                        ...
                        for newPointCounter in range(newNumbPointsPerVertex):
                            newXi = endPoint1.x() + \
                                    (newPointCounter/(newNumbPointsPerVertex-1))*deltaXi
                            newEta = endPoint1.y() + \
                                    (newPointCounter/(newNumbPointsPerVertex-1))*deltaEta
    ...
    lineShapes.setShapeType(qgis.core.QGis.WKBLineString)
            ...
            while not isLoop:
                ...
                for nextLine in lineShapes.featureList:
                    ...
                    if pointsList[-1] == currentEndPoint:
                        ...
                        pointsList.reverse()
                        nextLine.setGeometry(qgis.core.QgsGeometry.fromPolyline(pointsList))
    ...
    outputShapes.setShapeType(qgis.core.QGis.WKBLineString)
...
def identifyPolygons(loopShapes,
    ...
    if loopShapes.shapeType != qgis.core.QGis.WKBLineString:
    ...
    for loopID in loops:
        ...
        if qgis.core.QgsGeometry.fromPolygon([loopPoints]) == None:
        ...
        polygon.setGeometry(qgis.core.QgsGeometry.fromPolygon([loopPoints]))
    ...
    for hierarchyLevel, hierarchyLevelCounter in zip(polygonHierarchy, np.arange(len(polygonHierarchy))):
            ...
            if len(polygonTrees) == 0:
                isMeshed = True
                for loopID, polygon in hierarchyLevel.items():
            ...
            #Otherwise locate the correct tree and add the polygon as a node.
                ...
                for parentloopID, parentPolygon in polygonHierarchy[hierarchyLevelCounter-1].items():
                    for loopID, polygon in hierarchyLevel.items():
    ...
    areaCalculator.setSourceCrs(loopShapes.getCoordRefSystem().srsid())
    areaCalculator.setSourceAuthId(loopShapes.getCoordRefSystem().authid())
        ...
        for (node, childNodes) in tree.nodesWithChildren():
            ...
            for childNode in childNodes:
                ...
                if smallestNotMeshedArea != None and not isChildMeshed:
                    area = areaCalculator.measure(childPolygon.geometry())
                    if area < smallestNotMeshedArea:
                        ...
                        parentPolygon.geometry().addRing(childPolygon.geometry().asPolygon()[0])
                else:
                    parentPolygon.geometry().addRing(childPolygon.geometry().asPolygon()[0])
    ...
    for tree in polygonTrees:
        for node in tree.nodes():
            ...
            if smallestMeshedArea != None and isMeshed:
                area = areaCalculator.measure(polygon.geometry())
    ...
    if LOG.getEffectiveLevel() == 10:
        ...
        for tree in polygonTrees:
            for node in tree.nodes():
                ...
                if isMeshed:
                    meshedCounter += 1
                    if largestMeshedArea < area or largestMeshedArea == None:
                        largestMeshedArea = area
                    if smallestMeshedArea > area or smallestMeshedArea == None:
                        smallestMeshedArea = area
                else:
                    nonMeshedCounter += 1
                    if largestNotMeshedArea < area or largestNotMeshedArea == None:
                        largestNotMeshedArea = area
                    if smallestNotMeshedArea > area or smallestNotMeshedArea == None:
    ...
    outputShapes.setShapeType(qgis.core.QGis.WKBPolygon)
