...
class raster(object):
    ...
    def changeCoordRefSystem(self, targetCRS_string):
        ...
        for xiCounter in range(self.numb_xiPoints):
            for etaCounter in range(self.numb_etaPoints):
                ...
                sourceCRS_point = qgis.core.QgsPoint(xi, eta)
    ...
    def getRasterPeripheryPoints(self):
        ...
        for etaCounter in range(self.numb_etaPoints):
            ...
            point = qgis.core.QgsPoint(xi, eta)
            peripheryPoints.append(point)
        for xiCounter in reversed(range(self.numb_xiPoints)):
            xi = self.xiData[self.numb_etaPoints-1, xiCounter]
            eta = self.etaData[self.numb_etaPoints-1, xiCounter]
            point = qgis.core.QgsPoint(xi, eta)
            peripheryPoints.append(point)
        for etaCounter in reversed(range(self.numb_etaPoints)):
    ...
    def fromFile(self, inputFileName):
        ...
        fileInfo = PyQt4.QtCore.QFileInfo(inputFileName)
        ...
        if not rasterLayer.isValid():
            raise Exception('Error: failed to load raster file '+filename)
        ...
        for xiCounter in range(self.numb_xiPoints):
            for etaCounter in range(self.numb_etaPoints):
                ...
                gridPoint = qgis.core.QgsPoint(xi,eta)
    ...
    def fromNetCDF(self, inputFileName, crs_string=None, loadInVariableName=None):
        ...
        dimensionNames = inputNetCDF.dimensions.keys()
        ...
        variableNames = inputNetCDF.variables.keys()
    ...
    def fromIGF(self, inputFileName, crs_string=None, NODATA_value=np.nan):
        ...
        for line in inputFile:
            if line.split()[0] in gridSpecification.keys():
    ...
    def getRasterPartitionPeripheryPoints(self, low_xiIndex, high_xiIndex, \
        ...
        for partition_xiCounter in reversed(range(low_xiIndex, high_xiIndex+1)):
            partitionPeripheryPoints.append( qgis.core.QgsPoint(
                             self.xiData[high_etaIndex, partition_xiCounter],
                             self.etaData[high_etaIndex, partition_xiCounter]))
        for partition_etaCounter in reversed(range(low_etaIndex, high_etaIndex+1)):
            partitionPeripheryPoints.append( qgis.core.QgsPoint(
    ...
    def setGlobal(self, isGlobal=True):
        """ Set the boolean value of isGlobal. isGlobal helps to enable the correct differentiation of global bathymetries (+/- 180 longitude line).
    ...
    def gradientMagnitude(self, targetCRS_string=None,
        ...
        :arg str targetCRS_string: An optional argument to specify the coordinate reference system on which the magnitude is calculated.
...
def maximumRaster(rasterList):
    ...
    """
    return minmax(rasterList, operator='max')

def minimumRaster(rasterList):
    ...
    :raises Exception: if the input list holds less than 2 objects.
    """
    return minmax(rasterList, operator='min')

def minmax(rasterList, operator=None):
...
class gradationToShapes(raster):
    ...
    def writeDistanceNetCDF(self, outputRasterFilename):
        ...
        delta_xi = abs(self.xiMax - self.xiMin)/(self.numb_xiPoints-1)
        delta_eta = abs(self.etaMax - self.etaMin)/(self.numb_etaPoints-1)
