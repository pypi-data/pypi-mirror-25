...
def integrate_section(x, y, x_begin, x_end):
    """return the integral over a section.
    Adds extra points for start and end as they likely don't align with the input  spectra
    Used to convert solar spectrum to 10 nm intervals"""
    mask = (x_begin < x) & (x < x_end)
    y_begin = np.interp(x_begin, x, y)
    y_end = np.interp(x_end, x, y)
    x_section = np.concatenate(([x_begin], x[mask], [x_end]), axis=0)
    y_section = np.concatenate(([y_begin], y[mask], [y_end]), axis=0)
    section_trapz = np.trapz(y_section, x_section)
    print(section_trapz)
    return section_trapz


def wavelength_interval(wavelength, spectra, wl_min, wl_max, interval):
    """spectra with constant interval
    Returns spectra in W/m2 (NOT W/m2/nm)
    given a spectrum (W/m2/nm) as a function of wavelength (nm)
    wavelength minimum (nm) and wavlength maximum (nm) """
    new_wavelength = np.arange(wl_min, wl_max + interval, interval)
    new_spectra = new_wavelength * 1.1
    # should do this with nditer, maybe?
    for idx, wl in enumerate(new_wavelength):
        section_min = wl - interval / 2
        section_max = wl + interval / 2
        print(idx, wl, section_min, section_max)
        new_spectra[idx] = integrate_section(wavelength, spectra, section_min, section_max)
    print(new_spectra)
    return new_wavelength, new_spectra
