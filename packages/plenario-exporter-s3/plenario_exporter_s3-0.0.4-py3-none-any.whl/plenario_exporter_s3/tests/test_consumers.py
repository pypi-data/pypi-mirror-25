...
class TestConsumers(ChannelTestCase):
    ...
    def test_export(self):
        # run the payload through the view to get everything wired up properly
        payload = json.dumps({
        ...
        # get the message off the channel
        message = self.get_next_message('async-export')

        # actual testing begins here
        consumers.export(message)

        job = ExportJob.objects.get(pk=job_id)
        self.assertIsNotNone(job.date_started)
        self.assertIsNotNone(job.date_completed)
        self.assertIsNotNone(job.bucket)

        response = requests.get(job.bucket)
