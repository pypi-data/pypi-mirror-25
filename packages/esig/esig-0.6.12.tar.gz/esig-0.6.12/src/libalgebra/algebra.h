...
use as the first arg of sparse_vector::add_scal_prod().  
...
public:
	...
	inline void triangularbufferedmultiplyandcombine(const algebra& rhs, algebra& result, Transform fn) const
	{
		...
		// segment buffer according to the degree of the elements
		std::vector<typename std::vector<std::pair<KEY, SCALAR> >::const_iterator>
			iterators(DEPTH1 + 1,buffer.end());
		unsigned deg = 0;
		for (typename std::vector<std::pair<KEY, SCALAR> >:: const_iterator j0 = buffer.begin(); j0 != buffer.end(); j0++)
		{
		...
		{
			const KEY& k = i->first;
			unsigned rhdegree = DEPTH1 - VECT::basis.degree(k);
