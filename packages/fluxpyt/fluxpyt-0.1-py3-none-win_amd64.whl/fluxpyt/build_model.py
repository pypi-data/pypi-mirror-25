...
reading model from excel and building it
Note 2Jul2016: this program reads excel model into stoichiometric matrix with list of rxns and metabolites
Stoichimetric coeffiecients are added to existing values (line 185,196). This allows creation of matrices with following syntax also.
'a -> b + b'; 
in addition to following syntax: 'a -> 2 b'
...
def read_mfa_model(filename):
    ...
    this will return the model in lists that separate columns'''
    #import numpy as np
    file = open(filename)
    ...
    #read the reactions
    rxnID = [];formulae = [];atm_trans = [];rates = [];rxnType = []; basis = [];deviation = []
    excludedMetabolites = []
    inputSubstrates = []
    measuredIsotopes = []
    mids = [[],[],[]]
    ...
    for i in range(len(rxnID)):
        if rxnType[i] == 'F' or rxnType[i] == 'FR' or rxnType[i] == 'R' or rxnType[i] == 'B' or rxnType[i] == 'BR' or rxnType[i] == 'BRR': #metabolites balnce needed
    ...
    for i in range(len(rxnID)):
        if rxnType[i] == 'F' or rxnType[i] == 'FR' or rxnType[i] == 'R' or rxnType[i] == 'S' or rxnType[i] == 'SF':
    ...
    return model_metabolite,model_isotope,excludedMetabolites,inputSubstrates,mids
...
def metabolite_list(reaction_formulae,excludedMetabolites=[]):
    ...
    for formula in reaction_formulae:
        ...
        for el in fr:
            ...
            if not(el == '->' or el == '+' or el.find('.') >= 0 or el.isnumeric()):
...
def modify_rxn_for_S(rxn,excludedMetabolites=[]):
    ...
    ''''remove + sign excluded metabolites and their stoichiometric coeff.'''
    ...
    while i < len(rxn_split):
        el = rxn_split[i]
        if (i != len(rxn_split)-1) and (el.find('.') >= 0 or el.isnumeric()): #if coeff
...
def make_stoich_matrix(reactions,reaction_formulae,excludedMetabolites=[]):
    ...
    for rxn in reaction_formulae:
        ...
        while c < len(fr):
            ...
            if c+1 > len(fr):
                '''this condition was added because when there are no products in equation, error was shown'''
                
                break #loops out of while loop as there is not element after this
                
            
            element = fr[c]
            
           
            #check if the character is stoichiometric coefficient of metabolite
            if element.isnumeric() or element.find('.') >= 0:
                ...
                if c+1 == len(fr):
                    ...
                    m = fr[c+1] #the metabolite will be next to the coefficient
...
def build_model(fileName):
    [model_metabolite,model_isotope,excludedMetabolites,inputSubstrates,mids] = read_mfa_model(fileName)

    mets = metabolite_list(model_metabolite[1],excludedMetabolites)
    stoich_matrix = make_stoich_matrix(model_metabolite[0],model_metabolite[1],excludedMetabolites)
    return stoich_matrix,model_metabolite,model_isotope,mets,mids
    

def test_model_build():
        
    [model_metabolite,model_isotop,excludedMetabolites,inputSubstrates,mids] = read_mfa_model('glut_WT.csv')
    
    mets = metabolite_list(model_metabolite[1],excludedMetabolites)

    S = make_stoich_matrix(model_metabolite[0],model_metabolite[1],excludedMetabolites)
