...
Written in order to find the atom transition equation
Inputs:
    rxnList = reaction list
    atm_transitions = atm transitions for the reactions
    molecule = molecule for which all possible atomic transitions are to be found
...
def insert_hash(list):
    ''' inset hyphen between each element of a list'''
    new_list = []
    for el in list:
        new_list.append('#')
        new_list.append(el)
...
def get_coeff(rxn):
    ...
    '''give coefficients of each molecule in a reaction'''
    output = [[],[]]
    rxn_split = split_rxn(rxn)
