 ...
 *
 *		$Revision: 25798 $
 *		$Date: 2017-08-21 11:48:48 +0200 (lun, 21 ago 2017) $
 *		$Author: lillo $
 *
...
{
	PyObject *result;
	CL_NetInterface IFs[16];
	uint32 i, numIFs;
	
	Py_BEGIN_ALLOW_THREADS
	numIFs = CL_NetInterface::Enumerate(IFs, 16);
	Py_END_ALLOW_THREADS
...
{
	if (PyThreadState_Get()->thread_id == sMainThreadID) {
		{
			CL_AutoLocker locker(&MGA::gThreadsLock);
