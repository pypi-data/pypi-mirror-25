...
def fk_to_params(field):
    ...
    params['rel_model'] = "migrator.orm['%s']" \
...
class Column(VanilaColumn):
    ...
    def get_field_parameters(self):
        ...
        params.update(
            {k: repr(v) if v not in repr_ignore
             and k not in repr_ignore else v
...
"""
    ...
    meta = INDENT + NEWLINE.join([
        ...
        if Model._meta.schema else '',
        (INDENT + 'primary_key = pw.CompositeKey{0}'
         .format(Model._meta.primary_key.field_names))
        if isinstance(Model._meta.primary_key, pw.CompositeKey) else '',
        (INDENT + 'order_by = {0}'.format(get_order_by(Model)))
        if Model._meta.order_by else '',
    ])
...
def change_fields(Model, *fields, **kwargs):
    return "migrator.change_fields('%s', %s)" % (
        Model._meta.db_table, (',' + NEWLINE)
        .join([field_to_code(f, False) for f in fields])
    )


def change_not_null(Model, name, null):
    operation = 'drop_not_null' if null else 'add_not_null'
    return "migrator.%s('%s', %s)" % (
        operation, Model._meta.db_table, repr(name))
