...
class PunchTool(Tool):
    ...
    def process_nb(self, nb: NotebookNode, nb_path: Path) -> ProcessingResultType:
        ...
        if args.punched:
            if not args.tags:
                freq, nb_punched = PunchedNotebookProcessor(nb, args).punch_nb()
        ...
        if args.chads:
            if not args.tags:
                _, nb_chads = ChadsNotebookProcessor(nb, args).punch_nb()
    ...
    def config_tool_args_parsing(self, group: _ArgumentGroup) -> None:
        ...
        group.add_argument('-s', '--source',
                           ...
                           ' (default: "{}")'.format(args.punch_source))
    ...
    def print_tool_args(self) -> None:
        ...
        if args.fill:
            if args.source and not args.tags:
                print('  Fill holes from source notebook: {}'.format(args.source))
            else:
