...
def load_config_file(tool: str=package_name,
    ...
    if file_path.exists():
        try:
            config_all: Dict[str, Dict[str, Any]] = json.load(file_path.open(encoding='utf-8'))
            ...
            sys.exit(1)  # TODO do this more graceously (but it may have been called from tool constructor)
        if verbose:
...
def load_config(tool: str=package_name, verbose: bool=False) -> Namespace:
    ...
    * ~/.CONFIG_FILE_NAME (a dot file in the home directory)
    ...
    :param ns: namespace to load configuration into (default: empty namespace)
    :param verbose: whether to print some diagnostic info
    :return: updated ns
    """
