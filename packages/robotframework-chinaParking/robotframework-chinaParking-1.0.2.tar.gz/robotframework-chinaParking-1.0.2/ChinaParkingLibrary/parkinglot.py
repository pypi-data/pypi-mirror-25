...
class ParkingLot(object):
    ...
    def app_PaidFee(self,plateNo,tccid,jrsdm,yptzh,RCLSH,yjfy):#车牌，停车场ID，接入商代码,云平台账号,驶入流水号，应缴费用
        '''模拟获取驶离前已缴费用XML(用于支付宝代扣的车辆,在驶离前向后台请求应缴纳的费用,后台判断如是支付宝代扣的,则直接返回应缴费用为0,反之返回应缴费用)
        参数说明：
        plateNo:车牌号
        tccid:停车场ID
        jrsdm:接入商代码
        ...
        示例：
        | ${xml}= | app_PaidFee | 浙CSK306 | hzlpdh | keytop | RCLSHXXXXXX | 0 |

        '''
        ##算出秘钥
        unsign = yptzh + tccid + RCLSH + plateNo + yjfy + jrsdm
        java2same = unsign.encode(encoding='GBK')  # 这里必须转成GBK，与java里编码一致，不然MD5值会不一样
        sign = get_md5_value(java2same)
        ...
        outxml = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:fac="http://facade.ws.service.innotek.com/">\n' \
                 ...
                 '</fac:getBeforeLeavePaidFee>\n' \
                 '</soapenv:Body>\n' \
                 '</soapenv:Envelope>'.format(yptzh=yptzh, tccid=tccid, RCLSH=RCLSH, plateNo=plateNo,
                                              yjfy=yjfy, jrsdm=jrsdm,sign=sign)
