...
def get_resources(json):
    ...
    from FHIR. Returns a list with all discovered resource objects. Returns None if no valid resource objects
    could be found.
    :param json: The response json from FHIR
    :type json: json
    ...
    try:
        ...
        # Check for a bundle.
            ...
            # Get the list.
            list = [entry['resource'] for entry in json['entry']]
...
def flatten_questionnaire_response(incoming_json):
    ...
    # Get each question.
    for question in [item for item in q_resource.get('item', []) if item.get('linkId') is not None]:
        ...
        for index, answer in enumerate(answers, 1):
            ...
            sub_answer = next((item['answer'][0]['valueString'] for item in a_resource.get('item', [])
                               if item['linkId'] == '{}-{}'.format(question['linkId'], index)), None)
            ...
            # Add it.
            response[question['text']].append(answer_value)
