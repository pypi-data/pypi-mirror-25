...
class FHIRServices:
    ...
    FHIR_URL = None
    FHIR_CONSENT_QUIZ_ID = None
    FHIR_QUESTIONNAIRE_ID = None
    FHIR_QUESTIONNAIRE_ID_NEER = None
    ...
    def query_patient_record(self, email=None, flatten_return=False):
        ...
        if flatten_return:
            response_json = flatten_participant(response.json())
        else:
            response_json = response.json()
    ...
    def query_questionnaire_responses(self, email=None, questionnaire_id=None, flatten_return=False):
        ...
        :param email: Optional. A patient's email address.
        :param questionnaire_id: Optional. A Questionnaire ID.
    
        One or both parameters should be provided to limit responses.
        """
    ...
    def remove_consent_response(self, participant_email):
        consent_responses = self.query_questionnaire_responses(participant_email, self.FHIR_CONSENT_QUIZ_ID)
        ...
        try:
            ...
            logger.debug("[P2M2][DEBUG][remove_consent_response] - Could not find consent response to remove.")
            return

        consent_responses = self.query_questionnaire_responses(participant_email, self.FHIR_GUARDIAN_CONSENT_QUIZ_ID)
    ...
    def remove_questionnaire(self, participant_email, project):

        questionnaire_responses = self.query_questionnaire_responses(participant_email, self.choose_questionnaire_id(project))
        try:
            ...
            logger.debug("[P2M2][DEBUG][remove_questionnaire] - Could not find questionnaire to remove.")
            return
        return

    def choose_questionnaire_id(self, users_project):
        question_id = None

        if users_project == "neer":
            question_id = self.FHIR_QUESTIONNAIRE_ID_NEER
        elif users_project == "autism":
            question_id = self.FHIR_QUESTIONNAIRE_ID
