    ...
    with aiohttp.ClientSession(loop=loop) as session:
        ...
        with async_timeout.timeout(10):
            if args.method == SEARCH_FOR_STATION:
                ...
                stations = yield from train_api.search_train_stations(args.station)
                for station in stations:
                    print(station.name + " " + station.signature)
            elif args.method == GET_TRAIN_STOP:
                from_station = yield from train_api.get_train_station(args.from_station)
                to_station = yield from train_api.get_train_station(args.to_station)
                ...
                train_stop = yield from train_api.get_train_stop(from_station, to_station, time)
                ...
                if args.date_time is not None:
                    ...
                    time = datetime.now()
                train_stop = yield from train_api.get_next_train_stop(from_station, to_station, time)
