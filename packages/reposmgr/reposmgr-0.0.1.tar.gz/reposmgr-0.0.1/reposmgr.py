...
REPOS_ROOT         = os.environ.get('REPOS_ROOT', '/srv')
DEFAULT_REPOS_FILE = os.environ.get('REPOS_FILE', '/srv/.repos')
...
def display_repo_property(repo_name, prop_names):
    repos = load_repos()
    print get_properties(repos, repo_name, prop_names)


def display_all():
    repos = load_repos()
    for repo in repos.values():
        print get_properties(repos, repo.name, ['name', 'url', 'path'], separator='\t')


def display_properties(prop_names):
    repos = load_repos()
    for repo in repos.values():
        print get_properties(repos, repo.name, prop_names, separator='\t')
