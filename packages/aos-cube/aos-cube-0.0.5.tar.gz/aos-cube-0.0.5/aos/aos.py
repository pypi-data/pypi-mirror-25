...
# base url for all aos related repos (used as sort of index)
aos_base_url = 'https://code.aliyun.com/aos'
# default aos url
aos_os_url = 'https://code.aliyun.com/aos/aos.git'
...
def warning(msg):
    for line in msg.splitlines():
        sys.stderr.write("[aos] WARNING: %s\n" % line)
    sys.stderr.write("---\n")

def error(msg, code=-1):
    for line in msg.splitlines():
        sys.stderr.write("[aos] ERROR: %s\n" % line)
    sys.stderr.write("---\n")
    sys.exit(code)
...
class Hg(object):
    ...
    def isurl(url):
        ...
        if m_url and not re.match(regex_build_url, m_url.group(1)):
            return re.match(regex_hg_url, m_url.group(1)) or re.match(regex_aos_url, m_url.group(1))
        else:
            return False
        '''
        return None
    ...
    def check_repo(self, show_warning=None):
        ...
        if not os.path.isdir(self.path):
            err = (
                ...
                "You can also use \"aos sync\" to synchronize and remove all invalid library references." % (os.path.basename(self.lib), self.path))
            ...
            err = (
                ...
                "You can also remove library reference \"%s\" and use \"aos sync\" again." % (os.path.basename(self.lib), self.path, self.lib))
...
def _run_make(arg_list):
    ...
    win_make = os.path.join(aosder_dir,'cmd/win32/make.exe -e').replace('\\','/')
...
@subcommand('new',
    ...
    dict(name='--program', action='store_true', help='Force creation of an aos program. Default: auto.'),
    dict(name='--component', action='store_true', help='Force creation of an aos component. Default: auto.'),
    dict(name='--aoslib', action='store_true', help='Add the aos component instead of aos into the program.'),
    dict(name='--create-only', action='store_true', help='Only create a program, do not import aos or aos component.'),
    dict(name='--depth', nargs='?', help='Number of revisions to fetch the aos OS repository when creating new program. Default: all revisions.'),
    dict(name='--protocol', nargs='?', help='Transport protocol when fetching the aos OS repository when creating new program. Supported: https, http, ssh, git. Default: inferred from URL.'),
    help='Create new aos program or component',
    description=(
        "Creates a new aos program if executed within a non-program location.\n"
        "Alternatively creates an aos component if executed within an existing program.\n"
        "When creating new program, the latest aos release will be downloaded/added\n unless --create-only is specified.\n"
        "Supported source control management: git, hg"))
def new(name, scm='git', program=False, component=False, aoslib=False, create_only=False, depth=None, protocol=None):
    global cwd_root
    ...
    if d_type == 'program':
        ...
        if not create_only and not p.get_os_dir() and not p.get_aoslib_dir():
            url = aos_lib_url if aoslib else aos_os_url+'#latest'
            d = 'aos' if aoslib else 'aos'
            try:
                with cd(d_path):
    ...
    if d_type == 'program':
        ...
        target_file_list = ('template/main.c', 'template/aos_config.h', 'README.md', 'template/template.mk')
        temp_dir = os.path.join(d_path, 'aos/template/program')
        os.mkdir(os.path.join(d_path, target_name))
    else:
        target_dir = d_path
        target_name = name
        target_file_list = ('template.c', 'template.h', 'README.md', 'template.mk', 'test/README.md', 'test/main.c', 'test/aos_config.h', 'test/test.mk')
        temp_dir = os.path.join(p_path, 'aos/template/component')
    ...
    if top:
        ...
        if p and not p.is_cwd:
            error("Cannot import program in the specified location \"%s\" because it's already part of a program \"%s\".\n"
                  "Please change your working directory to a different location or use \"aos add\" to import the URL as a component." % (os.path.abspath(repo.path), p.name), 1)
...
@subcommand('add',
    ...
    description=(
        "Adds aos component and its dependencies from a source control based URL\n"
        "(GitHub, Bitbucket, aos.org) into an existing program.\n"
        "Use 'aos import <URL>' to import as a program"))
...
@subcommand('remove',
    ...
    description=(
        ...
        "You can re-add the component from it's URL via 'aos add <component URL>'."))
def remove(path):
    repo = Repo.fromrepo()
...
def publish(all_refs=None, msg=None, top=True):
    ...
    if repo.is_local:
        error(
            ...
            "Read more about publishing local repositories here:\nhttps://code.aliyun.com/aos/aos-cube/#publishing-local-program-or-component" % ("Program" if top else "Library", repo.name, repo.path, repo.scm.name), 1)
...
def update(rev=None, clean=False, clean_files=False, clean_deps=False, ignore=False, top=True, depth=None, protocol=None):
    ...
    if top and not rev and repo.isdetached():
        error(
            ...
            "You can checkout a branch using \"%s checkout <branch_name>\" command before running \"aos update\"." % (cwd_type, repo.scm.name), 1)
...
'''
# Compile command which invokes the aos OS native build system
@subcommand('compile',
    ...
    dict(name=['--profile'], action='append', help='Path of a build profile configuration file. Example: aos/tools/profiles/debug.json'),
    ...
    dict(name='--app-config', dest="app_config", help="Path of an app configuration file (Default is to look for 'aos_app.json')"),
    help='Compile code using the aos build tools',
    description=("Compile this program using the aos build tools."))
...
def test_(toolchain=None, target=None, compile_list=False, run_list=False, compile_only=False, run_only=False, tests_by_name=None, source=False, profile=False, build=False, clean=False, test_spec=None, app_config=None):
    ...
    with cd(program.path):
        ...
        if run_list:
            popen(['aosgt', '--test-spec', test_spec, '--list']
                  + (['-n', tests_by_name] if tests_by_name else [])
                  + (['-V'] if verbose else [])
                  + args,
                  env=env)
        ...
        if run_only or build_and_run_tests:
            popen(['aosgt', '--test-spec', test_spec]
                  + (['-n', tests_by_name] if tests_by_name else [])
                  + (['-V'] if verbose else [])
                  + args,
                  env=env)
...
def export(ide=None, target=None, source=False, clean=False, supported=False):
    ...
    # Remember the original path. this is needed for compiling only the libraries and tests for the current folder.
    orig_path = os.getcwd()
    # Change directories to the program root to use aos OS tools
    with cd(program.path):
        tools_dir = program.get_tools()
...
# Test command
@subcommand('detect',
    help='Detect connected aos targets/boards\n\n',
    description=(
        "Detects aos targets/boards connected to this system and shows supported\n"
        "toolchain matrix."))
def detect():
    # Gather remaining arguments
    args = remainder
    ...
    if tools_dir:
        ...
        warning("The aos tools were not found in \"%s\". \nLimited information will be shown about connected aos targets/boards" % program.path)
...
@subcommand('makelib',
    ...
    dict(name=['-r', '--arch'], help='The arch for static library compiling. E.g. "Cortex-M3", "Cortex-M4", "Cortex-M4F", "ARM968E-S", "linux"'),
...
def makelib(path, arch=None, new=False):
    ...
    if arch:
        _run_make(['LIB_DIR='+path, 'HOST_ARCH='+arch, '-f build/aos_library_makefile.mk'])
        return
...
@subcommand('config',
    dict(name='var', nargs='?', help='Variable name. E.g. "aosder", "protocol"'),
...
@subcommand('target',
    ...
    description=(
        ...
        "This is an alias to 'aos config [--global] target [name]'\n"))
...
@subcommand('toolchain',
    ...
    description=(
        "Set or get default toolchain\n"
        "This is an alias to 'aos config [--global] toolchain [name]'\n"))
