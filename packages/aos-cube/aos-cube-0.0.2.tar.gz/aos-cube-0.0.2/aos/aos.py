...
def makelib(path, new=False):
    if new:
        ...
        with open(os.path.join(root_dir,'aos/template/makefiles/aos_library_makefile/template.mk'), 'r') as f:
            content = f.read()
    ...
    for arch in host_arch:
        _run_make(['LIB_DIR='+path, 'HOST_ARCH='+arch, '-f aos/makefiles/aos_library_makefile.mk'])
