...
from wifi_tools.settings import IWCONFIG_COMMAND_PATH, IWLIST_COMMAND_PATH, NETWORK_SETUP_COMMAND_PATH
...
def iwconfig():
    ...
    try:
        proc = Popen([IWCONFIG_COMMAND_PATH], stdout=PIPE, stderr=DN)
        iw_config_detected = True
    except OSError as ex:
        proc = Popen([NETWORK_SETUP_COMMAND_PATH, '-listallhardwareports'], stdout=PIPE, stderr=DN)
...
def get_iface(interfaces):
    ...
    for iface in interfaces:
        count = 0
        proc = Popen([IWLIST_COMMAND_PATH, iface, 'scan'], stdout=PIPE, stderr=DN)
        for line in proc.communicate()[0].decode('utf8').split('\n'):
