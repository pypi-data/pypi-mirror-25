...
class QgridWidget(widgets.DOMWidget):
    ...
    df : DataFrame
        ...
        QgridWidget. The DataFrame passed in here WILL BE CHANGED by qgrid
        when you make changes in the qgrid UI. If you need to get your original
        DataFrame back after making changes via the qgrid UI, use the
        ``get_unchanged_df`` method.
    grid_options : dict
    ...
    def _update_df(self):
        ...
        # self._unfiltered_df is used to keep track of changes in
        # sorting/editing, but not filtering. it's the starting point for
        # applying filters, and also serves a snapshot of the state we need
        # to return to when all filters are removed.
        self._unfiltered_df = self.df.copy()
        # self._unchanged_df is a backup copy of the dataframe which won't
        # reflect any sorting/filtering/editing changes. it allows the user to
        # get their unchanged data back, in case they made unwanted edits, for
        # example.
    ...
    def get_unchanged_df(self):
        """Get a backup copy of the DataFrame that was originally used to
        generate the QgridWidget instance. This DataFrame will NOT reflect
        any sorting/filtering/editing changes that were made via the UI.

        Returns:
            DataFrame: A copy of the DataFrame that was originally used to
            generate the QgridWidget instance.
        """
