...
class DBService:
    ...
    def connect(self):
        db_path = os.path.dirname(os.path.abspath(__file__)) + '/database/ssh.db'
    ...
    def _db_exist(self, db_name):
        cursor = self.conn.execute('select count(*)  from sqlite_master where type=\'table\' and name = \'%s\';'%db_name)
        res = cursor.fetchone()
        return res[0]
    ...
    def get_all_ssh_list(self,order_by='id'):
        cursor = self.conn.execute("SELECT id,name,passwd,host,timestamp from SSHList order by %s desc limit 100"%order_by)
        ssh_list = []
        for row in cursor:
            ssh_list.append({'id': row[0], 'name': row[1], 'passwd': row[2],'host':row[3], 'timestamp':row[4]});

        # ssh_list = list(reversed(ssh_list))
        return ssh_list

    def put_ssh_key(self,name,passwd,host,timestamp):
        sql = "INSERT INTO SSHList (id,name,passwd,host,timestamp) \
              VALUES (null,'%s','%s','%s',%d)" % (name,passwd,host,timestamp)
        self.conn.execute(sql)
        self.conn.commit()
    ...
    def get_ssh_key(self, host):
        ...
        row = cursor.fetchone()
        if row is not None:
            return {'id': row[0], 'name': row[1], 'passwd': row[2],'host':row[3], 'timestamp':row[4]}
        else:
            return None
