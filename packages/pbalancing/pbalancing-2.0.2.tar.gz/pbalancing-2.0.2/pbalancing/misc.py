...
def rerevoke_sbtab(sbtab_file):
    ...
    '''
    rerevoked_sbtab = '!!SBtab SBtabVersion='"'1.0'"' TableType='"'QuantityType'"'\n'
    ...
    for row in config.split('\n'):
        ...
        if row.startswith('!!'): continue
            ...
            if splitrow[o_id] == 'pH': pass
                ...
                log.append('Warning: There is an irregular option in your config file: %s'%(splitrow[o_id]))
                continue
...
def valid_prior(prior,delimiter):
    ...
    if m_column == None:
        validity.append('Error: The SBtab prior table does not have the required "'"!PriorMedian"'" column. Please add it to continue.')
        return prior,validity
    if s_column == None:
