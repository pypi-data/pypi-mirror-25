...
header_names = ['QuantityType','SBMLReactionID','SBMLSpeciesID','Median','Mean','Std','Unit','Provenance','Type','Reference','logMean','logStd']
...
class ParameterBalancing:
    ...
    def getParameterInformations(self):
        ...
        header = ['QuantityType', 'Symbol', 'Unit', 'Constant', 'Related Element', 'Scaling', 'Dependence', 'Prior/pseudo median', 'Prior/pseudo std.dev.', 'SBML element ', 'Abbreviation', 'MatrixInfo']
        ...
        for index,column_name in enumerate(header):
            ...
            elif column_name == 'Prior/pseudo median': self.median_column = index
    ...
    def makeEmptySBtab(self,pmin,pmax,parameter_dict):
        ...
        '''
        sbtab_string = 'QuantityType\tSBMLReactionID\tSBMLSpeciesID\tMedian\tMean\tStd\tUnit\tProvenance\tType\tReference\n\t\t\t\t\t\t\t\t\t\n'
        header = header_names
    ...
    def getColumnIndices(self,header):
        ...
        for i,name in enumerate(header):
            ...
            elif name == 'Median': self.med_column = i                
            ...
            elif name == 'logMean': self.logm_column = i
            elif name == 'logStd': self.logs_column = i
    ...
    def newRow(self,name,quantity):
        ...
        row[6] = self.quantity_type2unit[quantity]
        row[7] = 'No value found'
        if quantity in self.prior_list: row[8] = 'Basic'
        else: row[8] = 'Derived'
        ...
        for row in self.rows:
            for i in range(0,amount):
                ...
                if further:
                    ...
                    if value_dict:
                        new_row[3] = str(value_dict['Median'])
                        new_row[4] = str(value_dict['Mean'])
                        new_row[5] = str(value_dict['Std'])
                        new_row[7] = 'Mean of several entries'
                    else:
                        new_row[4] = row[self.m_column]
                        new_row[5] = row[self.std_column]
                        new_row[3] = str(round(numpy.exp(self.normal2log([float(new_row[4])],[float(new_row[5])],[new_row[0]])[0])[0],4))
                        new_row[7] = 'Entry from collected data'
                    if quantity in self.quantity_type2mean_std.keys(): new_row[3] = new_row[4]
                    new_row[6] = self.quantity_type2unit[row[self.qt_column]]
                    if quantity in self.prior_list: new_row[8] = 'Basic'
                    else: new_row[8] = 'Derived'
                    new_row[9] = row[self.src_column]
                    
                    #columns ten and eleven are the logmean and logstd
                    #optional columns (columns twelve and more)
                    j = 12
                    if self.temp_column:
        ...
        if priors:
            ...
            for row in sbtab_file:
                if len(row) == len(first_row):
                    if row[4] == '':
                        try:
                            row[4] = str(priors[row[0]][0])
                            row[5] = str(priors[row[0]][1])
                        except: pass
        ...
        if pseudos:
            ...
            for row in sbtab_file:
                if len(row) == len(first_row):
                    if row[4] == '':
                        try:
                            row[4] = str(pseudos[row[0]][0])
                            row[5] = str(pseudos[row[0]][1])
                        except: pass
    ...
    def addConfig2log(self):
        ...
        '''
        self.log += '\n### You have used a config file for the parameter balancing. ###\nThe options are as follows:\n'
        ...
        if self.bounds_inc.count((None,None)) != len(self.bounds_inc):# and False:                #and False ---> mustn't be loaded in web interface
            ...
            for i,value in enumerate(new_medians):
                if value+0.001 < proper_boundaries[i][0] or value-0.001 > proper_boundaries[i][1]:
                    self.types_inc[i]
                    print('NEW_MEDIAN value out of bound: ',value,' [bounds: ',proper_boundaries[i],']')
    ...
    def getNewSBtab(self,new_sbtab=False,posterior_sample=False,header=False):
        ...
        for i,row in enumerate(self.new_rows):
            if len(row) == len(self.new_header):
                ...
                row[4] = means[row_number]
                row[5] = self.stds_post[row_number]
                if row[0] in self.quantity_type2mean_std.keys(): row[3] = means[row_number]
                else:
                    (log_mean,log_std) = self.normal2log([row[4]],[row[5]],row[0])
                    row[3] = numpy.exp(log_mean[0])
                    #row[3] = means[row_number]
                row[7] = 'Balanced parameter value'
                row[9] = ''
                row[self.logm_column] = str(self.log_mean_post[row_number])
                row[self.logs_column] = str(self.log_stds_post[row_number])
    ...
    def checkHighAndLow(self,row,first):
        ...
        elif float(row[4]) < self.pmin[row[0]]:
            if first:
        ...
        elif float(row[4]) > self.pmax[row[0]]:
            if first:
