...
header_names = ['QuantityType','SBMLReactionID','SBMLSpeciesID','Median','Unit','Mean','Std','logMean','logStd']
...
class ParameterBalancing:
    ...
    def makeEmptySBtab(self,pmin,pmax,parameter_dict):
        ...
        '''
        sbtab_string = 'QuantityType\tSBMLReactionID\tSBMLSpeciesID\tMedian\tUnit\tMean\tStd\n\t\t\t\t\t\t\n'
        header = header_names
    ...
    def getNewSBtab(self,new_sbtab=False,posterior_sample=False,header=False):
        ...
        for i,row in enumerate(self.new_rows):
            if len(row) == len(self.new_header):
                ...
                row[5] = means[row_number]
                row[6] = self.stds_post[row_number]
