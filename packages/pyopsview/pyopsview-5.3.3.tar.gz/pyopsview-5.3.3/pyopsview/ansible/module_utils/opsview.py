...
def _compare_recursive(old, new):
    ...
    if isinstance(new, dict):
        ...
        # TODO(JG): Make sorted more deterministic
        new, old = sorted(new), sorted(old)
...
class OpsviewAnsibleModuleAdvanced(object):
    ...
    def __init__(self, object_type, find_params=None, secrets=[]):
    ...
    def _build_argspec(self):
        ...
        for (field_name, field) in six.iteritems(fields):
            ...
            argspec_field['no_log'] = (name in self._secrets)
    ...
    def _requires_update(self, old_object, new_object):
        ...
        with open('/tmp/old.json', 'w') as fh:
            json.dump(old_encoded, fh)

        with open('/tmp/new.json', 'w') as fh:
            json.dump(new_encoded, fh)
