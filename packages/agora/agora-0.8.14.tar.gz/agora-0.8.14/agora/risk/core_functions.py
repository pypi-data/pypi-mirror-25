...
def WithRiskValueTypes(cls):
    ...
    # -------------------------------------------------------------------------
    def HistoricalReturns(self, graph, ndays):
        start = graph("Risk", "StartDate")
        end = graph("Risk", "EndDate")
        ...
        for sec, weight in graph(self, "Weights").items():
            ...
            prcs = prcs / fx
            # --- we reverse the array so that the eventually unused elements
            #     (when the length of the array is not a whole multiple of
            #     ndays) are the oldest
            rts = np.diff(np.log(prcs.values[::-1][::ndays][::-1]))
            dts = prcs.dates[::-1][::ndays][::-1]
            rts_crv = Curve(dts[1:], rts)

            if agg_returns is None:
                agg_returns = rts_crv*weight
            else:
                rts_crv, agg_returns = CurveIntersect([rts_crv, agg_returns])
                agg_returns += rts_crv*weight
    ...
    def ForwardVol(self, graph):
        ...
        rets = graph(self, "HistoricalReturns", 1).crop(start, end).values
        if len(rets):
            rets -= rets.mean()
    ...
    def VaR(self, graph, ndays=1, pctl=95.0):
        ...
        rets = graph(self, "HistoricalReturns", ndays).crop(start, end).values
        if len(rets):
            rets -= rets.mean()
    ...
    def AdjVaR(self, graph, ndays=1, pctl=95.0, nbos_adj=60):
        ...
        if len(rets):
            ...
            return adj*graph(self, "VaR", ndays=ndays, pctl=pctl)
        else:
            return 0.0

    # -------------------------------------------------------------------------
    def cVaR(self, graph, ndays=1, pctl=95.0):
        start = graph("Risk", "VaRStartDate")
        end = graph("Risk", "VaREndDate")
        rets = graph(self, "HistoricalReturns", ndays).crop(start, end).values
        if len(rets):
    ...
    def MaxLoss(self, graph, ndays=1):
        ...
        #     ndays interval
        rets_crv = graph(self, "HistoricalReturns", ndays)
