...
class TestORM(unittest.TestCase):
    ...
    ## def test_slorg_example_102(self):
        ## class RomTestSlorg(Model):
            ## col = Text(required=True, index=True, keygen=FULL_TEXT, prefix=True)

        ## d = RomTestSlorg(col='an apple')
        ## d.save()
        ## self.assertEqual(RomTestSlorg.query.startswith(col='an').startswith(col='a').count(), 1)


    ## def test_luis_106(self):
        ## class RomTestLuis1(Model):
            ## col = String(required=True, unique=True, index=True, keygen=rom.SIMPLE)

        ## self.assertEqual(RomTestLuis1(col='Luis').save(), 2)
        ## self.assertNotEqual(RomTestLuis1.get_by(col='Luis'), None)
        ## self.assertNotEqual(RomTestLuis1.query.filter(col='Luis').all(), [])
