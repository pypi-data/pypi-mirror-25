...
class Setup:
    ...
    def basic(self, curl, payload):
        ...
        curl.setopt(pycurl.CONNECTTIMEOUT, payload.get('connecttimeout', 30))
        curl.setopt(pycurl.TIMEOUT, payload.get('timeout', 30))
        ...
        if cookies:
            ...
            for cookie in cookies:
                ...
                cookie_str = '\t'.join([res.tld, "FALSE", "/", "FALSE", "0", cookie[0], cookie[1]])
                # print(cookie_str,'?????')
                curl.setopt(pycurl.COOKIELIST, cookie_str)
