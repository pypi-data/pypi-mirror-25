...
class CurlBoy:
    ...
    async def _runs(self, payload=None, info=None, curl_loop=None):
        ...
        #         payload = pre_ret
        curl, result = self._setup_curl(payload)
        resp = await self.run_async(curl, payload.get('aiohttp_timeout', 30), curl_loop)
        resp = self._update_response(payload, resp, result)
    ...
    async def run_async(self, curl, aiohttp_timeout=30, curl_loop=None):
...
if __name__ == '__main__':
    ...
    r = boy.run(data)
    # print(r)
    print(json.dumps(boy.view(r, ["title", "url", "state", "error_desc", "error_code", "total_time", "mode", "charset"]), indent=2,
                     ensure_ascii=False))
    print(len(r))
