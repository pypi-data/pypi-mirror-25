...
def build_work_plan(logger, options, sts_client):
    ...
    for account in options.accounts:
        ...
        for region in options.regions:
            ...
            if options.profile:
                s=boto3.session.Session(profile_name=options.profile)
                env["AWS_ACCESS_KEY_ID"] = s.get_credentials().access_key
            if options.role:
            ...
            if options.profile:
                env["AWS_SECRET_ACCESS_KEY"] = s.get_credentials().secret_key
            if options.role:
