...
import os
import json
from azureml.api.schema.schemaUtil import _generate_service_schema, load_service_schema
...
def generate_main(user_file, schema_file, main_file_name):
    ...
    # Try to load the specified schema file to ensure compatibility with publishing SDK
    if os.path.exists(schema_file):
        load_service_schema(schema_file)
    ...
    user_pkg = os.path.split(user_file)[1].replace('.py', '')
    main_src = main_src.replace('<user_script>', user_pkg)\
