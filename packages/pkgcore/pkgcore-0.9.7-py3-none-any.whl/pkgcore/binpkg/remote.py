...
def find_best_savings(stream, line_prefix):
    ...
    return max(stream, key=lambda k_v: (len(k_v[0]) + line_overhead) * k_v[1])[0]
...
class PackagesCacheV0(cache.bulk, metaclass=cache_meta):
    ...
    def _read_data(self):
        ...
        defaults = dict(iter(self._deserialized_defaults.items()))
