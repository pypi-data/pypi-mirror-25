...
class ProfileNode(object, metaclass=caching.WeakInstMeta):
    ...
    def pkg_use(self, data):
        ...
        c.update_from_stream(
            chain.from_iterable(iter(self._parse_package_use(data).values())))
        c.freeze()
        return c
    ...
    def masked_use(self):
        ...
        if self.pkg_use_mask:
            ...
            c.update_from_stream(
                chain.from_iterable(iter(self.pkg_use_mask.values())))
            c.freeze()
        return c
    ...
    def stable_masked_use(self):
        ...
        if self.pkg_use_stable_mask:
            c.update_from_stream(
                chain.from_iterable(iter(self.pkg_use_stable_mask.values())))
        c.freeze()
        return c
    ...
    def forced_use(self):
        ...
        if self.pkg_use_force:
            c = c.clone(unfreeze=True)
            c.update_from_stream(
                chain.from_iterable(iter(self.pkg_use_force.values())))
            c.freeze()
        return c
    ...
    def stable_forced_use(self):
        ...
        if self.pkg_use_stable_force:
            c.update_from_stream(
                chain.from_iterable(iter(self.pkg_use_stable_force.values())))
        c.freeze()
        return c
    ...
    def default_env(self, data):
        ...
        for parent in self.parents:
            rendered.update(iter(parent.default_env.items()))

        if data is not None:
            data = read_bash_dict(data, vars_dict=rendered)
            rendered.update(iter(data.items()))
...
class ProfileStack(object):
    ...
    def default_env(self):
        d = dict(iter(self.node.default_env.items()))
        for incremental in const.incrementals:
            ...
            if v:
                if incremental in const.incrementals_unfinalized:
                    ...
                    if v:
                        d[incremental] = tuple(v)
        return ImmutableDict(iter(d.items()))
...
class OnDiskProfile(ProfileStack):
    ...
    def split_abspath(path):
        ...
        chunks = [_f for _f in path.split("/") if _f]
        try:
            # poor mans rindex.
