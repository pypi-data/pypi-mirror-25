...
class OverlayedLicenses(Licenses):
    ...
    @klass.jit_attr_none
    def licenses(self):
        return frozenset(chain(*list(map(iter, self._license_instances))))
...
class BundledProfiles(object):
        ...
        try:
            for line in iter_read_bash(fp):
                ...
                d[key].append(_KnownProfile(
                    '/'.join([_f for _f in profile.split('/') if _f]), status))
        except EnvironmentError as e:
            if e.errno != errno.ENOENT:
                raise
    ...
    def paths(self, status=None):
        ...
        if status == 'deprecated':
            ...
            for profile_path, profile_status in chain.from_iterable(iter(self.arch_profiles.values())):
