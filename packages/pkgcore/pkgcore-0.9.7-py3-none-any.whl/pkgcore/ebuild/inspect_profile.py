...
class status(_base, metaclass=_register_command):
    ...
    def __call__(self, namespace, out, err):
        ...
        statuses = [(path, status) for path, status in chain.from_iterable(iter(arch_profiles.values()))
...
class provided(_base, metaclass=_register_command):
    ...
    def __call__(self, namespace, out, err):
        ...
        for pkg_name, pkgs in sorted(iter(targets.items()), key=operator.itemgetter(0)):
            out.write(
