...
def parse_time(s):
    ...
    date = re.match(r'^(\d+)([%s])$' % ''.join(list(units.keys())), s)
    if date:
        value = int(date.group(1))
        unit = date.group(2)
    else:
        raise argparse.ArgumentTypeError(
            "invalid date: %r (valid units: %s)" % (s, ', '.join(list(units.keys()))))
...
def parse_size(s):
    ...
    size = re.match(r'^(\d+)([%s])$' % ''.join(list(units.keys())), s)
    if size:
        value = int(size.group(1))
        unit = size.group(2)
    else:
        raise argparse.ArgumentTypeError(
            "invalid size: %r (valid units: %s)" % (s, ', '.join(list(units.keys()))))
...
def _dist_validate_args(parser, namespace):
    ...
    namespace.remove = (
        ...
        filter(namespace.filters.run, targets))
...
def _pkg_validate_args(parser, namespace):
    ...
    namespace.remove = (
        ...
        filter(namespace.filters.run, (pkg.path for pkg in pkgs)))
