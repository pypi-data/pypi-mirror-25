...
class MergeEngine(object):
    ...
    replace_hooks = {x: [] for x in
        set(list(install_hooks.keys()) + list(uninstall_hooks.keys()))}
    ...
    def install(cls, tempdir, pkg, offset=None, observer=None,
        ...
        hooks = {k: [y() for y in v]
                 for (k, v) in cls.install_hooks.items()}
    ...
    def uninstall(cls, tempdir, pkg, offset=None, observer=None,
        ...
        hooks = {k: [y() for y in v]
                 for (k, v) in cls.uninstall_hooks.items()}
    ...
    def replace(cls, tempdir, old, new, offset=None, observer=None,
        ...
        hooks = {k: [y() for y in v]
                 for (k, v) in cls.replace_hooks.items()}
