...
class domain(config_domain):
    ...
    def __init__(self, profile, repositories, vdb, name=None,
        ...
        for k in profile.profile_only_variables.intersection(list(settings.keys())):
            del settings[k]
        ...
        self.repos_raw = {
            collapsed.name: repo for (collapsed, repo) in zip(
        ...
        for k, v in profile.default_env.items():
        ...
        rev_names = {repo: name for name, repo in self.repos_raw.items()}
    ...
    def ebuild_repos_raw(self):
        ...
        return util.RepositoryGroup(
            x for x in self.repos_configured.values()
