...
class TestContentsSet(TestCase):
    ...
    def test_remove(self):
        ...
        list(map(cs.remove, self.all))
        cs = contents.contentsSet(self.all, mutable=True)
        list(map(cs.remove, (x.location for x in self.all)))
    ...
    def iterobj(self, name, obj_class=None, forced_name=None):
        ...
        if obj_class is not None:
            list(map(post_curry(self.assertTrue, obj_class), s2))
        ...
        if obj_class is not None:
            list(map(post_curry(self.assertFalse, obj_class), s3))
