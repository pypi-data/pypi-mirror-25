...
@click.option('--compression',
    help="Compression algorithm used ('gzip', 'bz2', or 'lzma')")
@click.option('--o', default=None, 
    help=("Specify an alternate output filename."
          "(Default: Original filename with .csv extension)"))
@click.option('--excel', is_flag=True, help="Format CSV for Microsoft Excel.")
@click.argument('filename', nargs=1)
def json2csv(filename, o, compression=None, excel=False):
    if excel:
        json_to_csv(filename=filename,
            output=o, compression=compression, recsep='\n')
    else:
        json_to_csv(filename=filename,
            output=o, compression=compression)