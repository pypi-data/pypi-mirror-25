...
@click_cli.command('debug-access')
@click.argument("reason", nargs=-1, required=True)
def request_debug_access(reason):
    '''Request debug access to the cluster'''
    user = access_request_username(None)
    request_access("debug", None, user, ' '.join(reason))
    click.echo("Requested debug access for {}.".format(user))


@click_cli.command('approve-debug-access')
@click.argument("username", required=True)
def approve_debug_access(username):
    '''Approve a debug access to the cluster'''
    if not username:
