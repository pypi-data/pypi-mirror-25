...
class BaseAbstractModel(DynamicModelFieldsMixin, models.Model):
    ...
    def db_info(self):
        """
        Уникальный идентификатор объекта, нужен для нескольких частей проектов.
        В основном для определения generic связей ( виджеты, блоки, галерея)
        """
    ...
    def _build_serializer_fields(cls, fields):
        ...
        Добавляет поле с идентификатором модели
        """
        fields = super(BaseAbstractModel, cls)._build_serializer_fields(fields)
        fields.append('db_info')
        return fields
