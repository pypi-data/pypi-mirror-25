...
__rtl_emoticon = [
    r"(?<![\w])",
    r"(?:[(){}\[\]<>|/\\]+|(?<![\d\.\,])[0](?![\d\.])|(?![\d\*,.@#&])[*,.@#&]|[$]|(?<![a-zA-Z])[DOosSxX])",  # mouth
    r"(?:[-‑^])?",  # optional nose
    r"(?:['\",])?",  # optional tears
    r"(?:[xX]|[:=|]|[%#]|[$8](?![\d\.])|[;]|\*)",  # eyes
    r"(?:[O]|[0]|[|{><=])?",  # optional hat
...
__fd1 = "(?:{})".format("".join([__full_date_parts[0] + "?", __full_date_parts[1], __full_date_parts[2]]))
__fd2 = "(?:{})".format("".join([__full_date_parts[0], __full_date_parts[1], __full_date_parts[2] + "?"]))
