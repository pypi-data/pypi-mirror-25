...
class AMaaSUser(object):
    ...
    def check_book_permissions(self, asset_manager_id, book_id, permission) -> None:
        ...
        is_admin = next((rel for rel in self.relationships
                         if rel.asset_manager_id == asset_manager_id
                         and rel.relationship_type == 'Administrator'), None)

        if not is_admin:
            for book_permission in self.book_permissions:
                if (book_permission.book_id == book_id or book_permission.book_id == '*') and \
                    book_permission.asset_manager_id == asset_manager_id:
