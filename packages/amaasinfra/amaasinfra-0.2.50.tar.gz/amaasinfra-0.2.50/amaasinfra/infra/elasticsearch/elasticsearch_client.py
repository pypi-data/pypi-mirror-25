...
class ElasticsearchProxy(object):
    ...
    def get_indexed_fields(self):
        indexed_fields = set()
        text_fields = set()
        mappings = self.index.get_mapping().get('assets', {}).get('mappings', {}).values()
    ...
    def search(self, asset_manager_ids, queries=None, filters=None, sort_fields=None,
        ...
        field_filters = [{field if field not in self.text_fields else '{}.keyword'.format(field): filter_value}
                         for field, filter_value in filters.items()] if filters else None

        for query_type in ['multi_match', 'query_string']:
            search = Search(using=self.client, index=self.index._name) \
            ...
            if sort_fields:
                sort_fields = ['{}.keyword'.format(field) for field in sort_fields if field in self.text_fields]
