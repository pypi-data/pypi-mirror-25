    ...
    for path in conf_path:
        ...
        if os.path.exists(path) and os.path.isfile(path):
            try:
                ...
                if not conf:
                    sys.exit()
                return type('ReadOnlyObject', (object,), conf)()
...
def load_argParser():
    ...
    conf = load_config()
    langset = conf.langset if conf.langset else LANG.get("default")
    lang = LANG[langset]
