...
class Base(ToolsBox):
    ...
    def _db(self):
        db_file = os.path.expanduser(conf.DB_FILE)
        try:
...
class SShHandler(Base):
    ...
    def auth_key(self):
        """获取本地private_key"""
        key_path = os.path.expanduser(conf.KEY_PATH)
...
class HostHandle(SShHandler):
    ...
    def _getInfo(self, ip):
        ...
        self.info = {
            ...
            "ip" : ip,
            "user" : option.user if option.user else conf.USER,
            "passwd" : option.passwd if option.passwd else conf.PASS,
            "port" : option.port if option.port else conf.PORT,
        }
