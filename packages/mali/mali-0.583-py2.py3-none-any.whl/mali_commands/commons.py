...
def url_encode(d, doseq=False):
    try:
        # noinspection PyUnresolvedReferences
        from urllib.parse import urlencode
        return urlencode(d, doseq=doseq)
    except ImportError:
        import urllib
        # noinspection PyUnresolvedReferences
        return urllib.urlencode(d, doseq=doseq)
...
def handle_api(ctx, http_method, method_url, data=None):
    if ctx.id_token is None:
        click.echo('Please run: "mali auth init" to setup authorization', err=True)
        exit(1)

    url = urljoin(ctx.api_host, base_url, method_url)

    result = None
    for retries in range(3):
        headers = {'Authorization': 'Bearer {}'.format(ctx.id_token)}
        r = http_method(url, headers=headers, json=data)

        if r.status_code == 401:
            update_token(ctx.config_prefix, ctx.client_id, ctx.refresh_token, ctx.auth0)
            continue
...
def update_token(config_prefix, client_id, refresh_token, auth0):
    r = requests.post('https://{}/delegation'.format(build_auth0_url(auth0)), json={
        'client_id': client_id,
    ...
    config.set(get_prefix_section(config_prefix, 'token'), 'id_token', data['id_token'])
    config.save()
...
def pixy_flow(ctx):
    ...
    redirect_uri = app.redirect_uri if app is not None else ctx.host + '/console/auth/init'
    query = {
        'response_type': 'code',
        'scope': 'openid offline_access user_external_id org orgs email picture name given_name user_metadata',
    ...
    authorize_url = 'https://{}/authorize?{}'.format(build_auth0_url(ctx.auth0), url_encode(query))
...
def output_result(ctx, result, fields, formatters=None):
