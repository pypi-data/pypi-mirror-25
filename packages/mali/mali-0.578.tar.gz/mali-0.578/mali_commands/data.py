...
from mali_commands.commons import add_to_data_if_not_none
...
def has_change_set(repo, ref='HEAD'):
    ref = ref.encode('ascii')

    try:
        ref_sha = repo.refs[ref]

        head_tree_sha = repo[ref_sha].tree
    except KeyError:  # in case of empty tree
        head_tree_sha = dulwich.objects.Tree().id
...
@click.option('--dataPath', 'data_path', required=False)
@click.pass_context
def _cmd_add_dataPath(ctx, volumeid, data_path):
    result = ctx.obj.handle_api(ctx.obj, requests.get, "data_volumes/" + str(volumeid))
...
@click.option('--dataPath', required=False)
...
def _commit_data_volume(repo, message):
    tree_id = has_change_set(repo)
    ...
    try:
        commit_hash = repo.do_commit(message=make_bytes(message), tree=tree_id).decode('ascii')

        repo.metadata.commit(commit_hash, tree_id)

        repo.end_transactions()
    except Exception:
        repo.rollback_transactions()
        raise
...
@click.pass_context
def commit_data_volume(ctx, path, message):
    with with_repo(ctx, path) as repo:
        _commit_data_volume(repo, message)
...
def add_to_metadata(ctx, volumeid, files, data, update):
    ...
    def update_enumerate_function(current_batch):
        return repo.metadata.get_head_data(current_batch)
    ...
    with with_repo(ctx, volumeid) as repo:
        ...
        insert_batch_size = min(total_files // 20, 2000)  # FIXME: hardcoded
        ...
        with click.progressbar(length=total_files) as bar:
            ...
            for file_path_chunks in chunks(enumerate_path(files), insert_batch_size):
                ...
                repo.metadata.add_data(data=data_batch)
