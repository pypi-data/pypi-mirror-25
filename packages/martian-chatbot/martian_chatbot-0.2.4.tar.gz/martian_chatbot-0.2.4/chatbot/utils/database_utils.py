...
class DatabaseUtils(object):
    ...
    def generate_new_fb_user(self, sender):
        ...
        current_sender = None
        try:
            print('Fetching user({0}) data from Facebook({1}).....'.format(sender, self.fbaccesstoken))
            resp = requests.get(
            ...
            print('Facebook response: {0}'.format(str(resp.text)))
            ...
            print('Created new user: {0}'.format(current_sender['user_id']))
        except:
    ...
    def generate_new_user(self):
        ...
        print('generated user = {0}'.format(user_id))
        current_sender = None
        current_user = user.find({
            'user_id': str(user_id)})
        for document in current_user:
        ...
        if current_sender is None:
            try:
                ...
                current_sender["is_input_expected"] = False
                current_sender["expected_input"] = ""
                print('Created new user: {0}'.format(user.insert_one(current_sender).inserted_id))
            except:
                print('Error creating new user')
        return current_sender
