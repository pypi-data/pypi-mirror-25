...
class Connection(object):
    ...
    def insert(self, table, dataframe):
        with timer('INSERT ' + table):
            dataframe.to_sql(table, self._engine, if_exists='append', index=False)

    def replace(self, table, dataframe):
        with timer('REPLACE ' + table):
            with self as transaction:
                transaction.execute("TRUNCATE " + table)
                dataframe.to_sql(table, transaction._connection, if_exists='append', index=False)
    ...
    def _dataframe(self, sql, bindings):
        ...
        with timer("dataframe:"):
            ...
            dataframe = pandas.read_sql(sql=sql, con=connection, params=bindings)
            connection.close()
            return dataframe
    ...
    def __execute(self, sql, bindings):
        ...
        with timer("sql:"):
            if self._connection:
                ...
                self._engine.execute(sql, bindings)
