...
def create_virtual_env():
    ...
    if os.path.exists(env.bin_lore):
        return
...
def launch_virtual_env():
    if env.launched():
        if sys.version_info[0:3] != env.python_version_info[0:3]:
            sys.exit(
                ...
                'and rerun lore install: ' + os.linesep + os.linesep + env.bin_python +
                os.linesep
            )
        return

    if not os.path.exists(env.bin_lore):
        sys.exit(ansi.error() + ' %s virtualenv is missing. Please run:\n '
            '$ lore install\n' % (env.project)
        )

    sys.argv[0] = env.bin_lore
...
def freeze_requirements():
    ...
    print(ansi.success('EXECUTE') + ' ' + env.bin_pip + ' freeze -r ' + source)
    vcs = split_vcs_lines()
    pip = subprocess.Popen(
    ...
    needed = [m.group(1) for l in missing for m in [regex.search(l)] if m]

    if needed:
        args = [env.bin_pip, 'install'] + needed
        print(ansi.success('EXECUTE ') + ' '.join(args))
        subprocess.check_call(args)
...
def check_requirements():
    if not os.path.exists(env.requirements):
        sys.exit(
            ansi.error() + ' %s is missing. Please run:\n '
                           '$ lore install\n' % ansi.underline(env.requirements)
        )
    
    with open(env.requirements, 'r', encoding='utf-8') as f:
        dependencies = f.readlines()

    vcs = [d for d in dependencies if
           re.match(r'^(-e )?(git|svn|hg|bzr).*', d)]
