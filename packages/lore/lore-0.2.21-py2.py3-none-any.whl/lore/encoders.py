...
class Norm(Continuous):
    ...
    def transform(self, data):
        with timer(('transform %s:' % self.name), logging.DEBUG):
            ...
            bad_nan = numpy.isnan(data)
            bad_inf = numpy.logical_not(numpy.isfinite(data))
            bad = numpy.logical_or(bad_nan, bad_inf)
            good = numpy.logical_not(bad)
            result = numpy.empty(len(data))
            result[good] = (
                numpy.minimum(self.__range, numpy.maximum(0, data[good] - self.__min)
                           ) / self.__range)
            if (numpy.any(bad)):
                result[bad] = self.missing_value
...
class Unique(Base):
    ...
    def reverse_transform(self, series):
        with timer(('reverse transform %s:' % self.name), logging.DEBUG):
...
class Token(Unique):
    ...
    def tokenize(self, data):
        ...
        return lowered.str.split(expand=True)
