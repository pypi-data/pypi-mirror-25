...
config_database_path = os.path.join(root, 'config', 'database.cfg')
config_aws_path = os.path.join(root, 'config', 'aws.cfg')
color = {
...
def set_python_version(version):
    ...
    if python_version:
        python_version_info = tuple([int(i) for i in version.split('.')])
        if pyenv:
            prefix = os.path.join(
                pyenv,
                'versions',
