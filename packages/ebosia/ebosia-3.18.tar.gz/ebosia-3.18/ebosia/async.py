...
def publish(topic, payload, connection=None):
    ...
    LOGGER.debug("Published event %s %s", topic, payload)
...
def start_pump(name, interval=180, limit=None):
    """
    Start a coroutine that is designed to keep the connection to the eventbus for publishing
    open. Without this docker swarm will shut down connections after 15 minutes without data
    traveling over them
    """
    count = 0
    LOGGER.info('Starting async pump coroutine for %s', name)
    while (not limit) or (limit > count):
        try:
            yield from publish('keepalive', {'count': count, 'name': name})
            count += 1
            yield from asyncio.sleep(interval)
        except Exception as e: # pylint: disable=broad-except
            LOGGER.exception("Death in the async pump thread %s at count %s: %s", name, count, e)
