import os, shutil, tempfile, logging, urllib, requests
...
def get_attrs_from_rule(rule):
    ...
    logging.debug('Parsing rule: %s', rule.selectorText)
    ...
    if len(category) == 0:
        for _, dirnames, filenames in os.walk(path):
            for subdir in dirnames:
                logging.debug('Deleting image category %s', subdir)
                shutil.rmtree(path + '/' + subdir)
            break
    else:
        for category_dir in category:
            logging.debug('Deleting image category %s', category_dir)
            print('path=%r' % path)
...
def archive_images(path, dest, filename_prepend=''):
    ...
    logging.debug('Running archive creation process')
    ...
    try:
        for _, dirnames, filenames in os.walk(path):
            if len(dirnames) == 0:
                logging.warn('No files to archive, moving on')
                return False
            ...
            logging.debug('Archiving images: %s', tmp_archive_file.split('/')[-1])
...
def delete_archive(path):
    ...
    logging.debug('Running archive deletion process')
...
def write_image_to_file(logo_attrs, file_types, dest):
    ...
    logging.debug('Writing images to file in %s', dir_path)
    filepaths = []

    for extension in file_types:
        if extension not in SUPPORTED_EXTS:
            logging.debug('Skipping %s', extension)
            continue
