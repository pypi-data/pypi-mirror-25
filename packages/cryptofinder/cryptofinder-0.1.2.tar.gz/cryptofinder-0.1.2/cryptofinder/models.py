...
class Ticker(Base):
  ...
  @property
  def stale(self):
    if datetime.datetime.now() > ( self.parsed_at + datetime.timedelta(minutes = 10) ):
      return True
  ...
  def refresh(self):
    try:
      data = list(requests.get('https://api.coinmarketcap.com/v1/ticker/').json())
    except ConnectionError:
      return

    for entry in tqdm(data, desc="Caching CoinMarketCap ticker", unit=" coins"):
    ...
    ticker = Ticker.create(results=len(data))
    return ticker
