...
def create(project, remote, id, name):
    """
    ...
    if remote:
        if id:
            project_dict = dict(id=id)
        elif name:
            project_dict = ProjectClient().get_project_id_by_name(access_token.username, name)
        else:
            # 初始化参数错误
            russell_logger.error("Init argument is wrong")
            return
    else:
        project_dict = ProjectClient().create(access_token.username, project)
    if project_dict:
        ...
        russell_logger.error("Project \"{}\" initialization failed in current directory".format(project))
...
@click.argument('id', nargs=1)
def log(id, tail, sleep_duration=1):
    """
    ...
    lines = experiment_client.get_log_stream(id)
    if not lines:
        print("No logs....")
    for line in lines:
        print(line)
