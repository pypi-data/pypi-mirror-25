...
def get_col_doc(engine, metadata):
    ...
    def _tbl_doc(tbl):
        name, comment = tbl['Name'], tbl['Comment'].strip()
        if not comment:
            comment = u'table {0}'.format(name)
    ...
    db_comments_map = {tbl['Name']: _tbl_doc(tbl) for tbl in tbl_rst}
    ...
    def _col_doc(col):
        ...
        if not comment:
            comment = u'自增主键' if stricmp(col['Extra'], 'auto_increment') and stricmp(col['Key'], 'PRI') else \
                ...
                u'更新时间' if stricmp(field, 'uptime') else u'field {0}'.format(field)
...
class ModelClass(Model):
    ...
    def render(self, db_comments_map=None):
        ...
        text += '    u"""{0}"""\n'.format(db_comments_map[self.table.name]['Comment'])
        ...
        for attr, column in self.attributes.items():
            if isinstance(column, Column):
                ...
                doc = db_comments_map[self.table.name]['Columns'][column.name]['Comment']
