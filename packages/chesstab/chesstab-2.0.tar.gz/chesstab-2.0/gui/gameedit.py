...
from rmappsup.workarounds.workarounds import text_count
...
from pgn.core.parser import PGNEdit, PGNMove, get_fen_string
...
class GameEdit(Game):
    ...
    def get_insertion_point_at_end_of_rav(self, insert_point_limit):
        ...
        """
        end_rav = self.score.tag_nextrange(RAV_END_TAG, insert_point_limit)
        next_move = self.score.tag_nextrange(
            NAVIGATE_MOVE, insert_point_limit)
        if not end_rav:
            point = self.score.tag_nextrange(EDIT_RESULT, insert_point_limit)
            if not point:
                return self.score.index(tkinter.END)
            return point[0]
        if not next_move:
            return end_rav[0]
        if self.score.compare(next_move[0], '>', end_rav[0]):
            return end_rav[0]
        # Raise exception or issue warning dialogue are better options than
        # 'end of next move plus one character'.
        # Bug 2013-06-19 note.
        ...
        return self.score.index(next_move[1] + '+1char')
    ...
    def insert_empty_move_after_currentmove(self, event_char):
        ...
        if self.current is None:
            ...
            widget.mark_set(
                tkinter.INSERT,
                self.get_insertion_point_at_end_of_rav(end_current))
