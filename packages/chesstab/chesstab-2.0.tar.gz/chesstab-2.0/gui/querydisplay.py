...
from rmappsup.workarounds import dialogues
from basesup.api.where import WhereError
...
class QueryDisplay(ChessException, Display):
    ...
    def insert_selection_rule_database(self, event=None):
        ...
        if not updater.value.process_query_statement(
            ...
            if not updater.value.where_error:
                ...
                message=''.join((
                    ...
                    updater.value.get_name_text(),
                    '\n\nto database.\n\n',
                    updater.value.where_error.get_error_report(datasource)))):
                dialogues.showinfo(
                    title='Insert Game Selection Rule',
                    message=''.join(('Add game selection rule to ',
                                     'database abandonned.')))
    ...
    def on_game_change(self, instance):
        ...
        if self.sourceobject is not None:
            if instance == self.sourceobject:
                self.blockchange = True
            if self is self.ui.selection_items.active_item:
                key = None
                p = self.ui.selection_games
                for e, k in enumerate(p.keys):
                    if instance.key.recno != k[0]:
                        key = k
                        break
                p.close_client_cursor()
                p.datasource.get_selection_rule_games(
                    self.get_selection_key_selection_rule(),
                    self.sourceobject)
...
class DatabaseQueryEdit(DatabaseQueryInsert):
    ...
    def update_selection_rule_database(self, event=None):
        ...
        original.value.set_database(datasource.dbhome)
        original.value.dbset = self.ui.base_games.datasource.dbset
