...
from basesup.api.record import KeyData, Value, ValueText, Record
from basesup.api.segmentsize import SegmentSize
...
class ChessDBrecordGameImport(Record):
    ...
    def import_pgn(self, database, source, sourcename, reporter=None):
        ...
        count = 0
        for d in self.value.read_games(
            source,
...
class ChessDBvalueRepertoireUpdate(PGNRepertoireUpdate, ChessDBvaluePGN):
    ...
    def pack(self):
        ...
        if self.is_pgn_valid():
            ...
            index[OPENING_ERROR_FIELD_DEF] = [self.gamesource]
        return v
