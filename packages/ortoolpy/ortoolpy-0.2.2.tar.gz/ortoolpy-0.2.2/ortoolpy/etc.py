...
def graph_from_table(dfnd, dfed, directed=False, node_label='id',
        from_label='node1', to_label='node2', from_to=None, **kwargs):
    """表からグラフを作成"""
...
def chinese_postman(g, weight='weight'):
    ...
    for i,j in nx.max_weight_matching(h,True).items():
        if i < j:
            g.add_edge(i, j, **{weight:dd[i][j]})
    return (sum(d[weight] for dct in g.edge.values()
            for dc in dct.values() for d in dc.values())/2,
