...
def chinese_postman(g_, weight='weight'):
    """
    中国人郵便配達問題
    入力
        g: グラフ
    ...
    g = g_.copy()
    subnd = [nd for nd,dg in g.degree().items() if dg%2==1] # 奇数次数ノード群
    dd = nx.floyd_warshall(g) # 完全距離表
    mx = max(d for dc in dd.values() for d in dc.values()) # 最大距離
    h = nx.Graph()
    for i,j in combinations(subnd,2):
        h.add_edge(i, j, weight=mx-dd[i][j])
    for i,j in nx.max_weight_matching(h).items():
        if i < j:
            g.add_edge(i, j, weight=dd[i][j])
    return (sum(d[weight] for dc in g.edge.values()
        for d in dc.values())/2, list(nx.eulerian_circuit(g)))
...
def sudoku(s, checkOnlyOne=False):
    ...
    for i in range(9):
        for j in range(9):
            ...
            if data[i*9+j].isdigit():
                m += v[i,int(data[i*9+j])-1,j] == 1 # fix (6)
    m.solve()
    if m.solve() != 1:
        return None, None
    res = (np.arange(9) @ np.vectorize(value)(v).astype(int)+1).tolist()
    if checkOnlyOne:
        fr = [v[i,res[i][j]-1,j] for i in range(9)
              for j in range(9) if not data[i*9+j].isdigit()]
        m += lpSum(fr) <= len(fr)-1
