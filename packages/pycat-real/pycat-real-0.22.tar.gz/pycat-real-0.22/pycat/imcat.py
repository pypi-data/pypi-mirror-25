...
class PrintableImage:
    def __init__(self, path_or_pil_or_uint_rgb_arr, full_width=False, use_h_res_augmentation_method=True):
        ...
        if isinstance(path_or_pil_or_uint_rgb_arr, basestring):
            ...
            self.im = Image.fromarray(np.clip(auto_norm(path_or_pil_or_uint_rgb_arr), 0, 255).astype(np.uint8))
        else:
...
def auto_norm(imgs):
    ...
    min_v = np.min(imgs)
    max_v = np.max(imgs)
    if 0<np.max(imgs)-np.min(imgs) < 11.:  # something wrong, perform normalisation
        return ((imgs-(min_v+max_v)/2.)/(max_v-min_v))*255+255/2.
    return imgs
