...
class BrokerClientRedis(BrokerClient):
    def __init__(self, buject):
        ...
        self.pool_size = buject.bubot.get('redis_pool_size', 3)
    ...
    async def initialize(self):
        res = Action('BubotClientRedis.initialize')
        try:
    ...
    async def get_buject_config(self, buject_name):
    ...
    async def save_buject_config(self, buject_name, buject_config):
    ...
    async def handler_msg(self):
        ...
        while True:
            ...
            if msg_redis:
                ...
                try:
                    msg = self.class_msg().init_from_json(msg_redis.value)
                    try:
                    ...
                    await self.buject.error('{0}.bad format redis msg: {1}'.format(self.id, e))
        ...
        while True:
            ...
            if msg_redis:
                ...
                try:
                    msg = self.class_msg().init_from_json(msg_redis.value)
                    try:
                        config = await self.get_buject_config(msg.receiver)
                        buject_class = BubotConfig.get_class(
                            'buject.{buject}.{buject}.{buject}'.format(buject=config['param']['buject']))
                        buject = buject_class(id=msg.receiver, config=config, bubot=self.buject.bubot, db=self.db,
                                              loop=self.buject.loop)
                        buject.init()
                        res.add_stat(await getattr(buject, 'handler_{0}'.format(msg.type))(msg))
                    except BujectError as e:
                        res.error = e.to_json()
                    except Exception as e:
                        res.error = BujectError(e, action=res).to_json()
