...
class Task:
    ...
    async def init_task(self, data):
        ...
        changes = BubotConfig.compare_dict(self.data, new_task_data)
    ...
    def initialize(self, data):
        ...
        try:
            if self.data.get('handler', None):
                self.handler = BubotConfig.get_class(self.data['handler'])
    ...
    def create_new_stage(self, stage_name, event):
        ...
        if 'create_handler' in stage:
            for handler in stage['create_handler']:
                ...
                if temp_result:
                    self.data = BubotConfig.update_dict(event.data, temp_result)
