...
def config_docker_ubuntu_xenial():
    """
    All the application will use docker, check if it's installed, if not, install it
    :return:
    """
...
def teste():
    ...
    if sys.argv[0]=='config_server':
        config_server(sys.argv[1])
    else:
        if sys.argv[0] == 'config_server':
            config_server(sys.argv[1])
