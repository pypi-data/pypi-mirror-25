...
class GetAppChangelogRequest(TLObject):
    ...
    def to_bytes(self):
        return b''.join((b'o\xef\x10\x90',TLObject.serialize_string(self.prev_app_version),))
...
class SaveAppLogRequest(TLObject):
    ...
    def to_bytes(self):
        return b''.join((b'H\xf7\x02o',b'\x15\xc4\xb5\x1c',struct.pack('<i', len(self.events)),b''.join(x.to_bytes() for x in self.events),))
...
class SetBotUpdatesStatusRequest(TLObject):
    ...
    def to_bytes(self):
        return b''.join((b'\xcd\xcf"\xec',struct.pack('<i', self.pending_updates_count),TLObject.serialize_string(self.message),))
