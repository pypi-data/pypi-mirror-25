...
class CSFtpUploadBakYMD:
    ...
    """
    def __init__(self, dictHPUP, iYmdBeginIndex=-1):
        ...
        self.iYmdBeginIndex = iYmdBeginIndex
        PrintTimeMsg('CSFtpUploadBakYMD.iYmdBeginIndex=(%s)' % self.iYmdBeginIndex)
        self.sHintFileName = ''
    ...
    def sftpUploadBakupFile(self, sFileName, sDirRemote, sDirLocal, sDirBakup):
        ...
        sSubR = ''
        sSubB = ''
        if self.iYmdBeginIndex >= 0:
            sYmd = sFN[self.iYmdBeginIndex, self.iYmdBeginIndex+8]
            if len(sYmd) == 8:
                sY = sYmd[0:4]
                sM = sYmd[4:6]
                sD = sYmd[6:8]
                sSubR = '/'.join([sY, sM, sD])
                sSubB = os.path.join([sY, sM, sD])
        ...
        for sDir in lsDir:
            ...
            if os.path.isfile(sL):  # 一般文件
                ...
                if GetTimeInteger() - int(s.st_mtime) >= 60*5:  # 仅上传5分钟前的
                    if self.iYmdBeginIndex >= 0:
                        sYmd = sDir[self.iYmdBeginIndex, self.iYmdBeginIndex+8]
                        if len(sYmd) == 8:
                            sY = sYmd[0:4]
                            sM = sYmd[4:6]
                            sD = sYmd[6:8]
