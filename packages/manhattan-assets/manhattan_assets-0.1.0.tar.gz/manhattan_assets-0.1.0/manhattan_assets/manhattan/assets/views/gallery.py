...
gallery_chains.set_link(factories.get_document())
gallery_chains.set_link(factories.decorate('gallery'))
gallery_chains.set_link(factories.render_template('gallery.html'))
...
def store_assets(state):
    ...
    if state.assets:
        for asset in state.assets:
            ...
            if not hasattr(frame_cls, '_variations'):
                continue

            if not state.gallery_field in frame_cls._variations:
                continue

            # Generate the variations for the asset
            asset_mgr.generate_variations(
                asset,
                frame_cls._variations[state.gallery_field]
            )
