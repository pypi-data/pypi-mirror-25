...
def main():
    import csv,googlemaps,math, sys,os                #imports:
    ...
    try:                              #trys to access googlemaps api
        ...
        print("Cant access googlemaps Check config at",os.path.join(os.path.dirname(__file__), 'config.cfg'))   
        quit()
    ...
    for item in array:
        ...
        try:                        
            ...
            tobewritten = [check,lat,lon,output]
            write.writerow(tobewritten)  #writes to csv

        except:                          #if error write input to output
            if item != ",,":
                tobewritten = item.split(",")
                write.writerow(tobewritten)
                if debug == True:
                    print(item.split(","))
            else:
                write.writerow(["","",""])
