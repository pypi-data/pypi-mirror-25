...
class MatrixWithCoords:
    ...
    def roi(self, x1, x2, y1, y2, mask=None):
        ...
        ret.xc = self.xc[xidx]
        ret.yc = self.yc[yidx]
        ret.d = self.d[t1*t2].reshape((len(ret.yc),len(ret.xc)))
        
        if mask:
            idx = mask[t1*t2].reshape((len(ret.yc),len(ret.xc)))
            ret.d[idx] = np.nan
    ...
    # collapse the matris into an array along x coordinates if axis=0, along y coordinates if axis=1
    ...
    # merge with the list of data given, not necessarily
...
class Data2d:
    ...
    def conv_Iqphi(self, Nq, Nphi, mask=None, cor_factor=1):
