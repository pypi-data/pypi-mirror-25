...
def update_config_files():
    root.verify_root_access("Root access needed to write files in '/opt/openpyn/files'")
    try:
        subprocess.check_call(
            "sudo wget -N https://nordvpn.com/api/files/zip -P /opt/openpyn/".split())
        subprocess.check_call(
            "sudo unzip -u -o /opt/openpyn/zip -d /opt/openpyn/files/".split())
        subprocess.check_call("sudo rm /opt/openpyn/zip".split())
...
def print_latest_servers(server_set):
    ...
    serverFiles = subprocess.check_output(
        "ls /opt/openpyn/files" + list_servers + "*", shell=True)
...
def check_config_files():
    try:
        serverFiles = subprocess.check_output(
            "ls /opt/openpyn/files", shell=True, stderr=subprocess.DEVNULL)
        openvpn_files_str = str(serverFiles)
    except subprocess.CalledProcessError:
        subprocess.run("sudo mkdir -p /opt/openpyn/files".split())
        serverFiles = subprocess.check_output(
            "ls /opt/openpyn/files", shell=True, stderr=subprocess.DEVNULL)
        openvpn_files_str = str(serverFiles)
...
def get_vpn_server_ip(server, port):
    ...
    file_path = "/opt/openpyn/files/" + server + ".nordvpn.com." + port + ".ovpn"
...
def connect(server, port, daemon, test, skip_dns_patch, server_provider="nordvpn"):
    if server_provider == "nordvpn":
        vpn_config_file = "/opt/openpyn/files/" + server + ".nordvpn.com."\
                + port + ".ovpn"
        # print("CONFIG FILE", vpn_config_file)

    elif server_provider == "ipvanish":
        vpn_config_file = "/opt/openpyn/files/ipvanish/" + server
        # print("ipvanish")
    ...
    if resolvconf_exists is True and skip_dns_patch is False:  # Debian Based OS + do DNS patching
        ...
        if daemon:
            subprocess.Popen(
                ["sudo", "openvpn", "--redirect-gateway", "--auth-retry",
                    ...
                    "/opt/openpyn/credentials", "--script-security", "2",
                    "--up", "/opt/openpyn/update-resolv-conf.sh",
                    "--down", "/opt/openpyn/update-resolv-conf.sh", "--daemon",
            ...
            try:
                ...
                subprocess.run(
                    ...
                    /opt/openpyn/credentials --script-security 2 --up \
                    /opt/openpyn/update-resolv-conf.sh --down \
                    /opt/openpyn/update-resolv-conf.sh \
        ...
        if skip_dns_patch is False:
            ...
            apply_dns_patch = subprocess.run(
                ["sudo", "/opt/openpyn/manual-dns-patch.sh"])
        else:
        ...
        if daemon:
            subprocess.Popen(
                ["sudo", "openvpn", "--redirect-gateway", "--auth-retry",
                    ...
                    "--auth-user-pass", "/opt/openpyn/credentials", "--daemon",
                    "--management", "127.0.0.1", "7015", "--management-up-down"])
            ...
            try:
                subprocess.run((
                    "sudo openvpn --redirect-gateway --auth-retry nointeract " +
                    "--config " + vpn_config_file + " --auth-user-pass " +
                    "/opt/openpyn/credentials --management 127.0.0.1 7015 " +
