...
'''
def find_genes_of_interest(studyName, geneCounts, lvl1pct=70, lvl2pct=70, lvl3pct=60, fxnpct=40):
    """
...
def find_organisms_of_interest(studyName, lvl1pct=70, lvl2pct=70, lvl3pct=60, fxnpct=40):
    """
    Summary: uses Boruta machine learning method to roughly determine potential genes of interest. requires tab-separated  matrix from MG-RAST analysis page
    ...
    #geneCounts = pd.read_table(geneCountsName, header=0)#, header=0)#header=0
    mergedCounts = analysis.merge_all_counts('lagoon study').fillna(0)
