...
def renyi_profile_of_level(levelName,countsDF):
    ...
    for i in range(len(countsDF.columns.tolist())):
        ...
        alphasList.insert(i, ['0','.25','.5','1','2','4','8','16','32','64', 'Inf'])
...
def plot_renyi_PCOF(geneList):
    ...
    sns.factorplot(x="alpha", y="H_alpha", col="level", hue="group", data= mergedRenyiDF,
                   col_wrap=4, ci=None, palette="muted", size=15, aspect=.85)
    return
...
def get_diversity_measures(geneName, searchString, studyName):
    ...
    Is the change in gene counts due to a change in diversity? This provides a quantitative answer.

    ''')
    print(geneName)
    plot_renyi_PCOF(geneList)
    print('''
    qualitative measure of diversity: Donut plot by taxonomic level

    A quantitative measure of diversity is useful for ranking diversity very easily, but specific
    changes in species can be obsucred by quantitative measures alone.

    For each taxonomic level, a Donut (hierarchical pie chart) is generated for each group.
    The widget for each table can be adjusted according to the user's needs, and then saved.
    ''')
...
def search_by_taxa(query, level='family', dfCounts):
    """
    Summary: gets the subset of a dataframe by the query of interest
    Args:
        query (str): search term of interest, use [space] for OR & | [pipe] for AND
    ...
    queryList = query.replace(" ", "|").replace("+", " ")
    geneSorted = dfCounts[dfCounts[level].str.contains(queryList)]
    return geneSorted

def subset_by_taxa(studyName, level='family', query, testMerge):
    """
    Summary: subsets counts dataframe by query (gene name/gene family name)
    Args:
        studyName (str): name of study directory
    ...
    geneDF.to_csv(str(level)+'_'+'allcounts.csv')
    os.chdir('..')
    return geneDF
