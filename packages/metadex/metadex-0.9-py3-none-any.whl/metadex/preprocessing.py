...
def get_group_counts_API(studyName):
    ...
    sampleID = [name.split('_')[2] for name in filenames]
...
'''
def find_genes_of_interest(studyName, geneCountsName, lvl1pct=70, lvl2pct=70, lvl3pct=60, fxnpct=40):
    """
    ...
    countsLvl1.to_csv(geneCountsName + '_lvl1.csv')
    countsLvl2.to_csv(geneCountsName +'_lvl2.csv')
    countsLvl3.to_csv(geneCountsName +'_lvl3.csv')
    countsLvl4.to_csv(geneCountsName + '_function.csv')
    Y = countsLvl1.columns.str.split("_").str[0]
    rf = RandomForestClassifier(n_jobs = -1, class_weight = 'auto', max_depth=3)
    feat_selector = BorutaPy(rf, n_estimators='auto', verbose=2, perc=40)

    X = countsLvl1.transpose().values
    feat_selector = BorutaPy(rf, n_estimators='auto', verbose=2, perc=lvl1pct)
    feat_selector.fit(X, Y)
    ...
    feat_selector = BorutaPy(rf, n_estimators='auto', verbose=2, perc=lvl2pct)
    feat_selector.fit(X, Y)
    ...
    feat_selector = BorutaPy(rf, n_estimators='auto', verbose=2, perc=lvl3pct)
    feat_selector.fit(X, Y)
    ...
    feat_selector = BorutaPy(rf, n_estimators='auto', verbose=2, perc=fxnpct)
    feat_selector.fit(X, Y)
