...
try: # try real gi module
    ...
    logging.info("no 'gi' module found: {}".format(e))
    try: # try pgi package
        import pgi
        pgi.install_as_gi()
        import gi
...
class NumericalModelGui(utils.LoggerObject):
    ...
    Args:
        ...
        use_fallback (bool, optional): use the file-based
            ...
            to ``False`` .
    """
    def __init__(self, numericalmodel, use_fallback = False):
        # check for GTK
    ...
    def plot_interfacevalues(self, interfacevalues, figure, times = None,
        ...
        identity_line = False, split_units = True):
        """
        ...
        if split_units:
            ...
            units = {"] [".join([iv.unit for iv in interfacevalues]):
                interfacevalues}
        ...
        for unit,ivlist in sorted(units.items()):
            ...
            if scatter:
                if len(ivlist) == 1:
                    ...
                    plot_kwargs.update(
                        {"label":"{} ({}) {} {} ({})".format(
                            iv2.name,iv2.id,_("vs."),iv1.name,iv1.id)})
                    if consistent_colors:
                        plot_kwargs.update(
                    ...
                    for iv in ivlist:
                        ...
                        try:
                            ...
                            main_iv = iv
                        ax.set_ylabel("["+"] [".join(multiscatter_units)+"]")
                if equal_axes:
    ...
    def update_current_plot(self, *args, **kwargs):
        ...
        for tab,content in self.figures_content.items():
            ...
            if self["plot_{}_box".format(tab)] == current_box:
                ...
                canvas = current_box.get_children()[0]
                if isinstance(canvas,FigureCanvasFile):
                    width = canvas.get_allocation().width
                    height = canvas.get_allocation().height
                    canvas.display(width=width,height=height)
    ...
    def save_current_plot_dialog(self):
        ...
        if response == Gtk.ResponseType.OK: # file selected
            ...
            logging.debug("File '{}' selected".format(filename))
        elif response == Gtk.ResponseType.CANCEL: # cancelled
            filename = False
            logging.debug("File selection cancelled")
        else: # something else
            filename = False
            logging.debug("File selection dialog was closed")
...
class FigureCanvasFile(Gtk.EventBox):
    ...
    def imagefile(self):
        ...
        try:
            ...
            logging.debug("Created new temporary file '{}'".format(path))
    ...
    def imagefile(self, newimagefile):
        try:
            self._imagefile
            logging.debug("Removing old image '{}'".format(self._imagefile))
    ...
    def load(self):
        ...
        """
        logging.debug("Reading image '{}'".format(self.imagefile))
    ...
    def remove_imagefile(self):
        ...
        try:
            self._imagefile
            logging.debug("Removing temporary image '{}'".format(
                self._imagefile))
