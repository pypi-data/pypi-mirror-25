        ...
        if self.CheckStopFileExists():
            ...
            if self.CheckPidFileExists():
                ...
                if psutil.pid_exists(pid):
                    ...
                    if self.bVerbose:
                        PrintTimeMsg("Check(%s)AlreadyQuit.pid=(%s)..." % (sCmd, pid))
            else:
            ...
            if self.CheckPidFileExists():
                ...
                if psutil.pid_exists(pid):
                    self.TouchPidFile()
                    if self.bVerbose:
                        PrintTimeMsg("Check(%s)StillRunning.pid=(%s)..." % (sCmd, pid))
                    return 0
            self.__closeFd()
            if self.bVerbose:
                PrintTimeMsg("CheckAndRunOneCmd.sLogOutFile=(%s)..." % self.sLogOutFile)
            ...
            self.Process = psutil.Popen(sCmd, stdout=self.fdOut,
                                        stderr=self.fdErr, shell=self.bShellPopen)
