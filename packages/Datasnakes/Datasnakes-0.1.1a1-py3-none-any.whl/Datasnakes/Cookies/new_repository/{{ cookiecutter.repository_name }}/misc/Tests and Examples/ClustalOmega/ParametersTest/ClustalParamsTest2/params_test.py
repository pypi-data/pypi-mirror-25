...
a = '/work5/r2295/bin/ClustalParamsTest2/' # Home directory
home = a
...
for name, param in zip(paramnames, parameters):
    ...
    os.system("cp {HTR2A_cds_nucl.fasta,profile.fasta} " + b + "/")

    # Directory change to output directory
    os.chdir(b)

    # Run Clustal Omega Commandline tool
    param(in_file="HTR2A_cds_nucl.fasta", out_file=name + ".fasta", outfmt="fasta", logfile=name + ".log")