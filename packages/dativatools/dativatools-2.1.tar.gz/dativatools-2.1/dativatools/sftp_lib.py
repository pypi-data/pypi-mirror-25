...
class SFTPLib(FileTransfer):
    ...
    def verify_settings(self, settings):
        self.logger_obj.info("Verifying sftp settings")
    ...
    def connect(self, settings):
        try:
            ...
            self.logger_obj.exception("Exception occurred : {0}".format(e.__str__()))
    ...
    def check_source(self, source_path, action, sftp_conn=None):
        self.logger_obj.info("Check if source exists.")
        if action == "get":
            if not sftp_conn.lexists(source_path):
                sftp_conn.close()
                msg = "{0} folder does not exists".format(source_path)
                self.logger_obj.error("Error : {0}".format(msg))
                return False, msg
        else:
    ...
    def _get_delta(self, settings, sftp_conn):
        try:
            ...
            self.logger_obj.info("File download started")
            for each in files:
            ...
            sftp_conn.close()
            self.logger_obj.info("File download completed")
            return True, None
        except Exception as e:
            self.logger_obj.exception("Exception occurred : {0}".format(e.__str__()))
            return False, e.__str__()
    ...
    def get_delta(self, settings):
        try:
            ...
            if not os.path.exists(settings["get"]["destination_path"]):
                os.mkdir(settings["get"]["destination_path"])
    ...
    def _put(self, settings, sftp_conn):
        try:
            self.logger_obj.info("File upload started")
            ...
            sftp_conn.close()
            self.logger_obj.info("File upload completed")
            return True, None
        except Exception as e:
            self.logger_obj.exception("Exception occurred : {0}".format(e.__str__()))
            return False, e.__str__()
