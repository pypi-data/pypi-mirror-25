...
class DatabaseManagement(object):
    ...
    def insert_data(self, insert_table, csv_path, scriptfile, dbconntion, source_table, email_config):
        try:
            self.logger_obj.info(('Start Inserting records in {0} table').format(insert_table))
            ...
            self.logger_obj.info(('Data inserted on table {0} successfully.').format(insert_table))
    ...
    def delete_table(self, tablename, suffix_schema, dbconn):
        try:
            self.logger_obj.info(('Start deleting records from {0} table into Redshift.').format(tablename))
            ...
            self.logger_obj.info(('Records from table {0} is deleted successfully').format(tablename))
