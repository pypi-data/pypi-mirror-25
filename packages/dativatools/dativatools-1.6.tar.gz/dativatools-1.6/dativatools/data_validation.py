...
class DataValidation(object):
    ...
    # -------------------------------------------------------------------------
    #    Name: __init__()
    #    Args: source_name : Dictionary
    #          path : Path for log creation
    #          filename : Name of log files.
    #    Returns: None
    #    Raises: None
    #    Desc: This function is used to initialize the log object, file_list
    #          and source_name dictionary
    # -------------------------------------------------------------------------
    def __init__(self, source_name, log_path, log_filename):
    ...
    def check_name(self):
        ...
        if file_list:
            self.logger_obj.info("Expected filenames found in directory")
            return True
        else:
            self.logger_obj.error("No files of expected filenames found in directory")
            return False
    ...
    def check_extra_files(self):
        ...
        file_list = self.file_list
        if not file_list:
            self.logger_obj.error("No files names with specified names found in the directory.")
        ...
        if add_file and not expected_other_files:
            self.logger_obj.error("The following extra items were found in the directory {0}".format(str(add_file)))
        ...
        if extra_files:
            self.logger_obj.error("The following extra items were found in the directory {0}".format(str(extra_files)))
            return False, extra_files
        else:
            self.logger_obj.info("No extra files were found in directory")
    ...
    def check_date(self):
        ...
        processing_date = datetime.strptime(self.source_name['processing_date'], '%Y%m%d')
        ...
        if out_of_daterange_files:
            self.logger_obj.error("The following files are out of the expected date range {0}".format(str(out_of_daterange_files)))
            return False, out_of_daterange_files
        else:
            self.logger_obj.info("All files are within the expected date range")
    ...
    def check_size(self):
        ...
        if not (len(min_sizes) == len(max_sizes)):
            self.logger_obj.error('Mismatch in the length of arrays for minimum and maximum size thresholds')
            return False, file_list
        ...
        if size_out_of_threshold_files:
            self.logger_obj.error("The following files had sizes outside the threshold {0}".format(str(size_out_of_threshold_files)))
            return False, size_out_of_threshold_files
        else:
            self.logger_obj.info("All files at location were of expected size")
    ...
    def check_file_count(self):
        ...
        if file_count_threshold[0] <= count_of_files <= file_count_threshold[1]:
            self.logger_obj.info('count of files are within the expected threshold.')
            return True, count_of_files
        else:
            self.logger_obj.error("Count of files, {0}, is not within the prescribed threshold, "
    ...
    def check_file_extension(self):
        ...
        if files_unexpected_ext:
            self.logger_obj.error("The following files at specified location"
                               "were with with unexpected extension {0}".format(str(files_unexpected_ext)))
            return False, files_unexpected_ext
        else:
            self.logger_obj.info("All files at specified path are with expected extension.")
